...
class groupbuy_leaderControl extends BaseControl
{
    ...
    public function applyOp()
    {
        ...
        $groupbuy_leader_info = $modelGroupbyLeader->getGroupbuyLeaderInfo(['groupbuy_leader_id'=>$groupbuy_leader_id]);
        ...
        if(!$phone_num){
            ...
            die(json_encode(array('code' => '100', 'message' => '自提点电话不能为空！', 'data' => ''), 320));
        ...
        if(count($groupbuy_leader_ziti_list)>=1){
            ...
            $ziti_address_info['is_current'] = 0;

        }else{

            $ziti_address_info['is_current'] = 1;
    ...
    public function getZitiAddressListByGroupbuyLeaderIDOp(){
        ...
        $groupbuy_leader_ziti_list = $modelGroupbyLeader->getGroupbuyLeaderAndZitiAddressList($condition,'','',0,'is_current desc');

        if(!$groupbuy_leader_ziti_list){

            die(json_encode(array('code' => '100', 'message' => '获取团长自提点列表失败', 'data' => ''), 320));

        }

        foreach($groupbuy_leader_ziti_list as $key=>$value){

            //图片处理
            $groupbuy_leader_ziti_list[$key]['id_photo_front']     = $value['id_photo_front'] ? UPLOAD_SITE_URL.'/'.DIR_UPLOAD_GLID_FRONT.'/'.$value['groupbuy_leader_id'].'/'.$value['id_photo_front'] : '';

            $groupbuy_leader_ziti_list[$key]['id_photo_back']      = $value['id_photo_back'] ? UPLOAD_SITE_URL.'/'.DIR_UPLOAD_GLID_BACK.'/'.$value['groupbuy_leader_id'].'/'.$value['id_photo_back'] : '';

            $groupbuy_leader_ziti_list[$key]['wx_avatar']          = $value['wx_avatar'] ? UPLOAD_SITE_URL.'/'.ATTACH_TZAVATAR.'/'.$value['wx_avatar'] : '';

            $groupbuy_leader_ziti_list[$key]['ziti_photo']         = $value['ziti_photo'] ? UPLOAD_SITE_URL.'/'.DIR_UPLOAD_ZITI.'/'.$value['groupbuy_leader_id'].'/'.$value['ziti_photo'] : '';

        }

        die(json_encode(array('code' => '200', 'message' => '获取团长自提点列表成功', 'data' => $groupbuy_leader_ziti_list), 320));
    ...
    public function changeZitiAddressOp(){
        ...
        //更改当前标识
        $result = $modelGroupbyLeader->editAddressCurrentByAddressId($ziti_address_id, $groupbuy_leader_id);

        if($result == -1){

            die(json_encode(array('code' => '100', 'message' => '更新团长id下所有地址标识失败', 'data' => ''), 320));

        }elseif($result == -2){

            die(json_encode(array('code' => '100', 'message' => '更新当前自提地址标识失败', 'data' => ''), 320));

        }else{
            
            //更改当前数据的标识
            $groupbuy_leader_info['is_current'] = 1;
    ...
    public function zitiApplyOp(){
        ...
        $have_license   =$_POST['have_license'];
        ...
        if(!$add_result){
            ...
            die(json_encode(array('code' => '100', 'message' => '自提点申请失败【未生成自提点信息】', 'data' => ''), 320));
            
        }

        die(json_encode(array('code' => '200', 'message' => '自提点申请成功，请等待审核', 'data' => ''), 320));
     ...
     * 自提点编辑
     */
    public function editZitiApplyOp(){
        ...
        if(!$ziti_address_id){
            ...
            die(json_encode(array('code' => '100', 'message' => '自提点ID不能为空', 'data' => ''), 320));
        ...
        $groupbuy_leader_ziti_info = $modelGroupbyLeader->getGroupbuyLeaderAndZitiAddressInfo($condition);

        $groupbuy_leader_ziti_info['ziti_photo'] = UPLOAD_SITE_URL . '/' . DIR_UPLOAD_ZITI . '/' . $groupbuy_leader_id . '/' . $groupbuy_leader_ziti_info['ziti_photo'];

        $groupbuy_leader_ziti_info['id_photo_front'] = UPLOAD_SITE_URL . '/' . DIR_UPLOAD_GLID_FRONT . '/' . $groupbuy_leader_id . '/' . $groupbuy_leader_ziti_info['id_photo_front'];

        $groupbuy_leader_ziti_info['wx_avatar'] = UPLOAD_SITE_URL.'/'.ATTACH_TZAVATAR.'/'.$groupbuy_leader_ziti_info['wx_avatar'];

        $groupbuy_leader_ziti_info['id_photo_back'] = UPLOAD_SITE_URL . '/' . DIR_UPLOAD_GLID_BACK . '/' . $groupbuy_leader_id . '/' . $groupbuy_leader_ziti_info['id_photo_back'];

        if(!$groupbuy_leader_ziti_info){
            die(json_encode(array('code' => '200', 'message' => '自提地址信息不存在', 'data' => ''), 320));
        }

        die(json_encode(array('code' => '200', 'message' => 'succ', 'data' => $groupbuy_leader_ziti_info), 320));
     ...
     * 自提点编辑保存
     */
    public function editZitiApplySaveOp(){

        $groupbuy_leader_id = $_POST['groupbuy_leader_id'];

        $ziti_address_id = $_POST['ziti_address_id'];
        ...
        $add_result = $model_ziti_address->editAddress($ziti_address_info,['address_id'=>$ziti_address_id]);
     ...
     * 通过团长id和自提点id获取自提点数据
     */
    public function getZitiAddressInfoOp(){
