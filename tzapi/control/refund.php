...
class refundControl  extends BaseControl{
    ...
    */
    ...
    public function add_returnOp(){
        ...
        if(intval($_POST['is_assistant'])==1){
             $refund_array['buyer_message'] = '副团长:'.$_POST['buyer_message'];
        }else{
            $refund_array['buyer_message'] = '团长:'.$_POST['buyer_message'];
        }
    ...
    */
    ...
    public function viewOp(){
        ...
        $refund['admin_time']=date('Y-m-d H:i:s',$refund['admin_time']);
        $refund['reciver_name']=$order['extend_order_common']['reciver_name'];
        $refund['phone'] = $order['extend_order_common']['reciver_info']['phone'];
         $refund_goods=array();
          $yongjin=0;
               foreach ($order['goods_list'] as $k => $v) {
                ...
                if(empty(array_values(unserialize($goods_info['goods_spec']))[0])){
                    ...
                    $refund_goods[$k]['goods_spec'] = array_values(unserialize($goods_info['goods_spec']))[0];
                }
                } else {
               ...
               if(empty(array_values(unserialize($goods_info['goods_spec']))[0])){
                ...
                $refund['yongjin']=ncPriceFormat($yongjin*0.01);
