...
class store_export_wms13Control extends BaseControl
{
    ...
    private function send_goods()
    {
      ...
      $type = isset($_GET['type'])?$_GET['type']:30;
    ...
    private function export_order($address_id,$type)
    {
       $model_goods = Model('goods');
        ...
        $time_start_str = strtotime('-10 days',$time_end_str);//不需要处理
        $time_start_str_1 = strtotime('-5 hours',$time_end_str);//截单6点的
        $condition['order.payment_time'] = array('between', array($time_start_str_1, $time_end_str));
    ...
    {    
        ...
        $time_start_str_1 = strtotime('-5 hours',$time_end_str);
        $condition['order.payment_time'] = array('between', array($time_start_str_1, $time_end_str));
        $data = $model_order->getOrderGoodsExportList($condition,'20000');
        $ordergoods_arr=array();
        $orders_wms = array();
