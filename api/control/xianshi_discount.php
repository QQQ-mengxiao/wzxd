...
class xianshi_discountControl extends BaseControl
{
    ...
    public function goods_listOp()
    {
        ...
        if($sortkey==2){
            ...
            if($postorder=='false'){
                $sortorder = "( CASE WHEN sg.goods_storage = 0 THEN 1 ELSE 0 END ),goods_price DESC,goods_id DESC";
            }else{
                $sortorder = "( CASE WHEN sg.goods_storage = 0 THEN 1 ELSE 0 END ),goods_price ASC,goods_id DESC";
            }
        }

        $sql = "SELECT sg.goods_commonid,spxg.goods_id,min(sgp.price) AS goods_promotion_price,sg.goods_price,sg.goods_name,sg.goods_image,sg.goods_storage,(sg.goods_salenum+sg.goods_presalenum) AS sale_num,spxg.goods_sort,sgc.is_vip_price,ds.by_post FROM 718shop_p_xianshi_goods spxg LEFT JOIN 718shop_goods_promotion sgp ON spxg.goods_id=sgp.goods_id LEFT JOIN 718shop_goods sg ON spxg.goods_id=sg.goods_id LEFT JOIN (SELECT 718shop_daddress.address_id,718shop_storage.by_post FROM 718shop_daddress LEFT JOIN 718shop_storage ON 718shop_daddress.storage_id = 718shop_storage.storage_id) ds ON ds.address_id = sg.deliverer_id LEFT JOIN 718shop_goods_common sgc ON sg.goods_commonid=sgc.goods_commonid LEFT JOIN 718shop_buy_deliver_goods sbdg ON sg.goods_id=sbdg.goods_id LEFT JOIN 718shop_p_xianshi spx ON spxg.xianshi_id = spx.xianshi_id WHERE UNIX_TIMESTAMP() BETWEEN spxg.start_time AND spxg.end_time AND spx.xianshi_type = 2 AND sg.goods_state=1 AND (sbdg.ziti_id=".$ziti_id." OR sbdg.ziti_id IS NULL) AND sg.goods_verify=1 AND sg.is_deleted=0 ".$xinren_str." AND CASE sgp.promotion_type WHEN 30 THEN sgp.member_levels<=".$member_grade." ELSE 1 END GROUP BY spxg.goods_id ORDER BY ".$sortorder." limit 10 offset ".$offset;
        // print_r($sql);die;
