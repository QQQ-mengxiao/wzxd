...
class store_export_wms20Control extends BaseControl
{
    ...
    private function send_goods()
    {
      ...
      $type = isset($_GET['type'])?$_GET['type']:20;
      ...
      //遍历循环数据发货人编号及对应发货信息
      //1.获取需要分拣的仓库id
      $storage_list = Model('storage')->getStorageList(array('is_picked' => 1,'times'=>1),'storage_id');
      $storage_ids = '';
      ...
      file_put_contents('goods/'.$online_sn.".goods", $post_data_json);
    ...
    private function send_orders()
    {
      ...
      $post_data_json = json_encode($post_data,320);
      var_dump($post_data_json);die;
      file_put_contents('orders/'.$online_sn.".order", $post_data_json);
    ...
    private function export_order($address_id,$type)
    {
       $model_goods = Model('goods');
        ...
        //结束时间
        $time_end_str_1 = strtotime('-20 hours',$time_end_str);

        //$time_start_str = strtotime('-15 days',$time_end_str);
        $time_start_str_1 = strtotime('-1 days',$time_end_str);
        $condition['order.payment_time'] = array('between', array($time_start_str, $time_end_str_1));
             ...
             foreach ($vv['extend_order_goods'] as $key => $value) { 
                ...
                if ($type == 10) {
                  ...
                  if ($vv['payment_time'] < $time_start_str_1) {
                    if(!strstr($value['goods_name'],'【团购'.$time_1)){
                            unset($data[$kk]['extend_order_goods'][$key]);
                            continue;
                        }
                    }elseif(strstr($value['goods_name'],'【团购')){
                      if (!strstr($value['goods_name'],'【团购'.$time_1)) {
                        unset($data[$kk]['extend_order_goods'][$key]);
                        continue;
                      }
                  ...
                  //预售
                  //时间格式处理
                  $time_1 = date('n.j',$time_start_str);
                  if(!strstr($value['goods_name'],'【团购'.$time_1)){
                      unset($data[$kk]['extend_order_goods'][$key]);
                      continue;
                    }
