...
//20200415优化搜索以及排序
        ...
        //全部商品对应cate_id为0
        if ($_POST['cate_id'] === 0 || $_POST['keyword'] === '全部商品') {
            unset($_POST['cate_id']);
            unset($_POST['keyword']);
        }
        //显示左侧分类
       ...
       if (isset($_POST['num_page'])) {
        ...
        if (is_array($indexer_ids)) {
            ...
            $goods_list = $model_goods->getGoodsOnlineList($condition, $fields, 0, $order, '', null, false);
...
//                $condition['goods_name|goods_jingle'] = array('like', '%' . $str . '%');
                $condition['goods_name'] = array('like', '%' . $str . '%');
            }
...
//            $goods_list = $model_goods->getGoodsListByColorDistinct($condition, $fields.',min(color_id) as color_id', 'goods_storage desc,goods_id desc', $num);
            //$goods_list = $model_goods->getGoodsListByColorDistinct($condition, $fields.',min(color_id) as color_id', 'goods_storage desc,goods_id desc', '');
           /* $goods_list = $model_goods->getGoodsListByColorDistinct1($condition, $fields, 'goods_storage desc,goods_id desc', '');*/
        //     if ($_POST['zhi']== 'zhi') {
        //     echo 'yu';/*print_r($condition);
        //     echo $fields;die;*/
        //     $i =1;
        ...
        if (isset($_POST['ziti_id'])) {
            $buy_deliver_info = Model()->table('buy_deliver')->where(array('ziti_id' => $_POST['ziti_id']))->field('buy_deliver_id')->select();
            $buy_deliver_info_n = Model()->table('buy_deliver')->where(array('ziti_id' => array('neq',$_POST['ziti_id'])))->field('buy_deliver_id')->select();
            // print_r(array_column($buy_deliver_info,'buy_deliver_id'));
            // print_r(array_column($buy_deliver_info_n,'buy_deliver_id'));die;
            if(count($buy_deliver_info_n)>0){
                $buy_deliver_info_n = implode(',',array_column($buy_deliver_info_n,'buy_deliver_id'));
                // print_r($buy_deliver_info);die;
                $i = " AND (NOT ( sg.is_group_ladder = 5 AND goods_id IN ( SELECT goods_id FROM 718shop_buy_deliver_goods WHERE buy_deliver_id IN (".$buy_deliver_info_n.")))";
                // AND (sg.is_group_ladder = 5 AND goods_id IN ( SELECT goods_id FROM 718shop_buy_deliver_goods WHERE buy_deliver_id = 10 ) OR NOT ( sg.is_group_ladder = 5 AND goods_id IN ( SELECT goods_id FROM 718shop_buy_deliver_goods WHERE buy_deliver_id = 2 )))
            }
            if(count($buy_deliver_info)>0){
                $buy_deliver_info = implode(',',array_column($buy_deliver_info,'buy_deliver_id'));
                $i .= " OR (sg.is_group_ladder = 5 AND goods_id IN ( SELECT goods_id FROM 718shop_buy_deliver_goods WHERE buy_deliver_id = ".$buy_deliver_info." ))";
            }
            if($i){ 
                $i .= ") ";
            }
        ...
        if($_POST['ngoods_sign']==1){
            $ngoods_sign = 1;
        }
        //echo $fields;
            $goods_list = $model_goods->getGoodsListByColorDistinct1($condition, $fields, 'goods_storage desc, goods_price asc, goods_id desc', $num,$i,$sortkey,$postorder,$ngoods_sign);
             if(empty($goods_list) && $num != 0){
                //$num1 = $num-10;
                //$goods_list = $model_goods->getGoodsListByColorDistinct1($condition, $fields, 'goods_storage desc, goods_price asc, goods_id desc', $num1);
                $list['end'] = 1;
                 //$list['num_page'] = $_POST['num_page'] -1;
            }else{
                $list['end'] = 0;
               // $list['num_page'] = $_POST['num_page'];
            }
          //  print_r($goods_list);die;
           /* print_r($goods_list1 );
            ...
            foreach ($goods_list as $key => $value) {
                ...
                $goods_list[$key]['is_vip_price'] =$goods_commond_info['is_vip_price'];
                 if ($value['is_group_ladder'] == 4) {
                        ...
                        $cond=array();
                        $cond['goods_id']=$value['goods_id'];
                        $cond['end_time'] = array('gt', TIMESTAMP);
                        $xianshigoods = $model_xianshi_goods->getXianshiGoodsInfo( $cond);
                        ...
                        $goods_list[$key]['xianshi_type']=$xianshi_info['xianshi_type'];
                }
                //普通商品设置会员卡折扣以及会员等级折扣价格计算
                $member_id = intval($_POST['member_id']);
                if ($value['is_group_ladder'] == 0 && $goods_commond_info['is_vip_price']==1 ){
                    if($value['hui_discount']<1 ){
                        $status = Model()->table('member_uid')->getfby_member_id($member_id,'status');
                        if($status){
                            //原价
                            $goods_list[$key]['or_goods_price'] = $value['goods_price'];
                            //会员卡价格
                            $goods_list[$key]['goods_price'] = $value['goods_price'] *$value['hui_discount'];  
                        }
                    }else{
                        $member_model = Model('member');
                        $discount = $member_model->getDiscount($member_id);
                        $goods_list[$key]['discount'] = $discount;
                        /*$goods_tax_rate = $model_goods->getGoodsCommonList(array('goods_commonid' =>  $goods_info['goods_commonid']))[0]['goods_tax_rate'];*/
                        //原价
                        $goods_list[$key]['or_goods_price'] = $value['goods_price'];
                        //会员等级折扣价
                        $goods_list[$key]['goods_price'] = ncPriceFormat($discount * $value['goods_price']);
        ...
        $n=0;
                ...
                //多规格库存累加
                $goods_storage =  Model('goods')->getGoodsList(array('goods_commonid'=>$value['goods_commonid'],'is_deleted'=>0),'sum(goods_storage) as goods_storage');
                $goods_list[$key]['goods_storage'] = $goods_storage[0]['goods_storage'];
                $goods_common_info = $model_goods->getGoodeCommonInfo(array( 'goods_commonid' => $value['goods_commonid']));
                     $goods_list[$key]['goods_image'] = cthumb($goods_common_info['goods_image'], '360', $value['store_id']);
        ...
        //$goods_list1 = array_slice($goods_list, intval(($_POST['num_page']-1)*self::PAGESIZE), self::PAGESIZE);
        $list['goods_data']=$goods_list;
        //$list['goods_data']=$goods_list1;
