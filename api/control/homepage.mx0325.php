    ...
    public function iconOp()
    {
       ...
       // print_r($iconInfo);die;
        if ($iconInfo) {
            ...
            foreach ($iconInfo[0]['item_data']['item'] as $key => $value) {
                // if($value['name']=="母婴精品"){
                //     unset($iconInfo[0]['item_data']['item'][$key]);
                //     continue;
                // }
                //获取后台设置的ICON 所对应的以及分类ID
                $model_gc = Model('goods_class');
                    ...
                    foreach ($goodsInfo['item_data']['item'] as $key => $value) {
                        ...
                        //var_dump($buy_deliver_info['buy_deliver_id']);die;
                         ...
                         if ($value['is_group_ladder'] == 4) {
                            ...
                            // $xianshi_info = $model_xianshi->getXianshiInfo($xianshigoods['xianshi_id']);
                            $xianshi_info = $model_xianshi->getXianshiInfo(array('xianshi_id' => $xianshigoods['xianshi_id']));
                            $goodsList[$key1]['item_data']['item'][$key]['xianshi_type']=$xianshi_info['xianshi_type'];
                        }
                        
                        $member_id = intval($_POST['member_id']);
                        //普通商品设置会员卡折扣以及会员等级折扣价格计算
                       if ($value['is_group_ladder'] == 0 && $value['is_vip_price']==1 ){
                            if($value['hui_discount']<1 ){
                                $status = Model()->table('member_uid')->getfby_member_id($member_id,'status');
                                if($status){
                                    //原价
                                    $goodsList[$key1]['item_data']['item'][$key]['or_goods_price'] = $value['goods_price'];
                                    //会员卡价格
                                    $goodsList[$key1]['item_data']['item'][$key]['goods_price'] = $value['goods_price'] *$value['hui_discount'];  
                                }
                             }else{
                                ...
                                $goodsList[$key1]['item_data']['item'][$key]['or_goods_price'] = $value['goods_price'];
                                //会员等级折扣价
                                 $goodsList[$key1]['item_data']['item'][$key]['goods_price'] = ncPriceFormat($discount * $value['goods_price']);
