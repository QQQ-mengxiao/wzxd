     ...
     // 
        if(empty($qiandao_info)){
            ...
            $qd_ruler= $model_qd_ruler->getqd_rulerInfo($where);
               //是否同一天，及一天只一次
            if($this->isDiffDays($date2,$date1)){
               if($date1['mday']==1){//是否为每月的一号
                    ...
                    // var_dump( $mpoints_days);die;
                    //日常积分
                    $where=array();
                    $where['id']=1;
                    $qd_ruler1= $model_qd_ruler->getqd_rulerList($where);
                    if($this->isStreakDays($date2,$date1)){
                        $upqiandao_array['num'] = array('exp','num+1');
                        //判断为多送的哪儿一天
                        $qiandao_num=$qiandao_info['num']+1;
                        // var_dump($qiandao_num);
                        if(in_array($qiandao_num, $mpoints_days)){
                            foreach ($qd_rulerlist as $k => $v) {
                              if($v['days']==$qiandao_num){
                                 $ponits=$v['points'];
                                $exp=$v['exp'];
                              }
