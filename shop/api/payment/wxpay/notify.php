...
class PayNotifyCallBack extends WxPayNotify
{
	...
	public function NotifyProcess($data, &$msg)
	{
		...
		if ($order_type == 'r') {
			...
			$pd_result= $logic_payment->getPdOrderInfo($data['out_trade_no']);
			if(!empty($pd_result['data'])){
				if ($pd_result['data']['pdr_payment_state'] == 1) {
					exit($success);
				}
				$order_pay_info = $pd_result['data'];
				$pd_result_ = $logic_payment->getPaymentInfo('wxpay');
				$payment_info = $pd_result_['data'];
				 $pd_result__ = $logic_payment->updatePdOrder($data['out_trade_no'],$data["transaction_id"],$payment_info,$order_pay_info);
				 $xml = "<xml>";
    			$xml.="<return_code><![CDATA[SUCCESS]]></SUCCESS>";
		    ...
		    if ($result['data']['api_pay_state']||$result['data']['pdr_payment_state']) {
		        return true;
		    }
