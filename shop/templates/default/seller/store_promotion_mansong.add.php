  ...
  <form id="add_form" action="index.php?act=store_promotion_mansong&op=mansong_save" method="post">
    ...
    <dl>
      ...
      <dd>
        ...
        <span class="error-message"></span>
 <!--<p class="hint"><?php echo sprintf($lang['mansong_add_start_time_explain'],date('Y-m-d H:i',$output['start_time']));?></p>//gai-->
      </dd>
    </dl>
    <dl>
...
<?php } ?>
	...
	<!-- <dl>
      <dt><i class="required">*</i> 活动商品类别<?php echo $lang['nc_colon'];?></dt>
      <dd>
          <?php if (!empty($output['gc_list'])) {?>
              <select name="mansong_gc_id">
    ...
    <dl>
      ...
      <dd>
        ...
        <span>送数量为&nbsp;<input id="mansong_discount" type="text" class="text w50"><em class="add-on">个</i></em></span>
        <span>礼品:&nbsp;<a href="javascript:void(0);" id="btn_show_search_goods" class="ncsc-btn"><i class="icon-gift"></i>选择礼品</a></span> <div id="mansong_goods_item" class="gift"></div>
        ...
        <span class="error-message"></span>
        <p class="hint">设置当单笔订单满足金额时（必填选项），减免赠送的礼品。<br/>系统最多支持设置五组等级规则。</p>
      </dd>
    </dl>
    <dl>
...
<span>送数量为<strong><%=discount%></strong>个， </span>
<%if(goods_id>0){%>
<span>送礼品 <%==goods%></span>
<%}%>
