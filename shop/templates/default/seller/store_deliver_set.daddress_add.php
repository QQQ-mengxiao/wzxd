...
<div class="eject_con">
  ...
  <form method="post" action="index.php?act=store_deliver_set&op=daddress_add" id="address_form" target="_parent">
    ...
    <dl>
      <dt><i class="required">*</i>wms编码<?php $lang['nc_colon'];?></dt>
      <dd>
        <input type="text" class="text" name="wms_id" value="<?php echo $output['address_info']['wms_id'];?>"/>
        <p class="hint">发货人对应的wms编码，默认000</p>
      </dd>
    </dl>
    <dl>
    ...
    <dl>
      <dt>仓库：</dt>
      <dd>
          <select name="storage_id" id="storage_id" class="querySelect">
              <option value='0'>请选择</option>
              <?php if(is_array($output['storage_info'])){?>
                  <?php foreach ($output['storage_info'] as $k => $v){ ?>
                      <option value='<?php echo $v['storage_id'];?>' <?php echo $v['storage_id'] == $output['address_info']['storage_id']?'selected':''; ?>><?php echo $v['storage_name'];?></option>
                      <?php } ?>
              <?php }?>
          </select>
      </dd>
...
$(document).ready(function(){
    ...
    $('#address_form').validate({
        ...
        messages : {
            ...
            region : {
                checklast : '<i class="icon-exclamation-sign"></i>�뽫����ѡ������'
            },
            address : {
