...
<ul class="goods-list">
  ...
  <li>
    ...
    <a nctype="btn_add_xianshi_goods" data-goods-id="<?php echo $val['goods_id'];?>" data-goods-name="<?php echo $val['goods_name'];?>" data-goods-img="<?php echo thumb($val, 240);?>" data-goods-price="<?php echo $val['goods_price'];?>" data-goods-promotion="<?php echo $val['goods_promotion'];?>" href="javascript:void(0);" class="ncsc-btn-mini">选择商品/修改折扣价</a> </li>
  <?php } ?>
</ul>
...
<script>
    function add() {
        ...
        ajax_form('ajax_plate', '批量添加折扣商品', $('#add').attr('url')+'&goods_id=' + id + '&xianshi_id=' + xianshi_id, '600');
    }
    // 全选
    function checkAll() {
        $('#batchClass').hide();
