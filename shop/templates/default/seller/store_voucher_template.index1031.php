...
<i class="icon-calendar"></i>
          ...
          <th class="w100"><?php echo '在积分中心';?></th>
          <td class="w90">
              <select class="w70" name="voucher_t_display">
                  <option value="0">请选择</option>
                  <?php foreach($output['display'] as $k=>$v) {?>
                      <option value="<?php echo $k;?>"<?php if ($_GET['voucher_t_display']==$k){ echo 'selected';}?>>
                          <?php echo $v;?></option>
                  <?php }?>
              </select>
          </td>
        <th><?php echo $lang['nc_status'];?></th>
        <td class="w80"><select class="w80" name="select_state">
            ...
            <?php }?>
            <?php }?>
          </select></td><th class="w80"><?php echo $lang['voucher_template_title'];?></th>
        <td class="w100"><input type="text" class="text w150"  value="" id="txt_keyword" name="txt_keyword" /></td>
      ...
      <tr class="bd-line">
        ...
        <td class="tl"><?php echo $val['voucher_t_title'];?></td>
          <?php foreach($output['display'] as $key =>$v) {?>
              <?php if($val['voucher_t_display']==$key) {?>
        ...
        <td class="goods-time"><?php echo date("Y-m-d",$val['voucher_t_start_date']).'~'.date("Y-m-d",$val['voucher_t_end_date']-3600*24*1);?></td>
        ...
        <td class="nscs-table-handle">
            <?php if($val['voucher_t_state']==$output['templatestate_arr']['usable'][0]){?>
                <span>
        	    <a class="btn-blue" href="index.php?act=store_voucher&op=templateedit1&tid=<?php echo $val['voucher_t_id'];?>"><i class="icon-edit"></i>
        		      <p>定向</p>
        		  </a>
        	</span>
            <?php }?>
            ...
            <?php }?>
            <?php if ($val['voucher_t_state']==$output['templatestate_arr']['disabled'][0]||!$val['voucher_t_giveout']){//该模板没有发放过代金券时可以删除?>
                <span>
            ...
            <?php }?>
            <span>
            <a class="btn-blue" href="index.php?act=store_voucher&op=template_user_info&tid=<?php echo $val['voucher_t_id'];?>" ><i class="icon-search"></i>
              <p>查询</p>
            </a>
          </span>
        </td>
