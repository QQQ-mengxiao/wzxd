...
<style type="text/css">
    ...
    thead tr th:nth-child(1),tbody tr td:nth-of-type(1) {position:sticky;background: white;height: 30px;padding: 5px 10px;border-top: solid 1px #BCE8F1;left: 0;z-index:2}
    thead tr th:nth-child(2),tbody tr td:nth-of-type(2) {position:sticky;background: white;height: 30px;padding: 5px 10px;border-top: solid 1px #BCE8F1;left: 121px;z-index:2}
    thead tr th:nth-child(3),tbody tr td:nth-of-type(3) {position:sticky;background: white;height: 30px;padding: 5px 10px;border-top: solid 1px #BCE8F1;left: 221px;z-index:2}
    thead tr th:nth-child(4),tbody tr td:nth-of-type(4) {position:sticky;background: white;height: 30px;padding: 5px 10px;border-top: solid 1px #BCE8F1;left: 320px;z-index:2}
    thead tr th:nth-child(-n+4) {background: #D9EDF7}
</style>
...
<div id="fixedNavBar" style="display: none">
    <h3>页面导航</h3>
    <ul>
    ...
    <form method="post" id="goods_form" action="<?php if ($output['edit_goods_sign']) { echo urlShop('store_goods_online', 'edit_save_goods');} else { echo urlShop('store_goods_add', 'save_goods');}?>">
        ...
        <div class="ncsc-form-goods">
            ...
            <dl>
                ...
                <dd id="gcategory"> <?php echo $output['goods_class']['gc_tag_name'];?>  <!-- <a class="ncsc-btn" href="<?php if ($output['edit_goods_sign']) { echo urlShop('store_goods_online', 'edit_class', array('commonid' => $output['goods']['goods_commonid'], 'ref_url' => getReferer())); } else { echo urlShop('store_goods_add', 'add_step_one'); }?>"><?php echo $lang['nc_edit'];?></a> -->
                    <a href="javascript:void(0);" class="btn-blue" nctype="class_edit" data-param="{url:'<?php echo urlShop('store_goods_online', 'class_edit'); ?>', sign:'class_edit',item:'<?php echo $output['goods']['goods_commonid']?>'}"><i class="icon-edit-sign">修改商品分类</i></a>
            ...
            <dl>
                <dt>商品子分类：</dt>
                <dd><span class="new_add"><a href="javascript:void(0)" id="add_sgcategory1" class="ncsc-btn"><?php echo $lang['store_goods_index_new_class'];?></a> </span>
                    <?php if (!empty($output['goods_class_tmp'])) { ?>
                        <?php foreach ($output['goods_class_tmp'] as $v) { ?>
                            <select name="goods_class_id[]" class="sgcategory1">
                                <option value="0"><?php echo $lang['nc_please_choose'];?></option>
                                <?php foreach ($output['goods_class_z'] as $val) { ?>
                                    <option disabled="disabled" value="<?php echo $val['gc_id']; ?>" <?php if ($v==$val['gc_id']) { ?>selected="selected"<?php } ?>><?php echo $val['gc_name']; ?></option>
                                    <?php if (is_array($val['child']) && count($val['child'])>0){?>
                                        <?php foreach ($val['child'] as $child_val){?>
                                            <option value="<?php echo $child_val['gc_id']; ?>" <?php if ($v==$child_val['gc_id']) { ?>selected="selected"<?php } ?>>&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $child_val['gc_name']; ?></option>
                                        <?php }?>
                                    <?php }?>
                                <?php } ?>
                            </select>
                    ...
                    <?php } else { ?>
                        <select name="goods_class_id[]" class="sgcategory1">
                            <option value="0"><?php echo $lang['nc_please_choose'];?></option>
                            <?php if (!empty($output['goods_class_z'])){?>
                                <?php foreach ($output['goods_class_z'] as $val) { ?>
                                    <option value="<?php echo $val['gc_id']; ?>" disabled="disabled"><?php echo $val['gc_name']; ?></option>
                                    <?php if (is_array($val['child']) && count($val['child'])>0){?>
                                        <?php foreach ($val['child'] as $child_val){?>
                                            <option value="<?php echo $child_val['gc_id']; ?>">&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $child_val['gc_name']; ?></option>
                                        <?php }?>
                                    <?php }?>
                                <?php } ?>
                            <?php } ?>
                    ...
                    <?php } ?>
                    <p class="hint">选择“请选择”选项，提交后去掉对应子分类</p>
                </dd>
            </dl>
            <dl>
            ...
            <dl>
                <dt nc_type="no_spec">会员卡折扣：</dt>
                <dd nc_type="no_spec">
                    <input name="hui_discount" value="<?php echo $output['hui_discount']; ?>" type="text"  class="text w60" /><!-- <em class="add-on"></em> --> <span></span>
                    <p class="hint">折扣必须是0.01~1.00之间的数字，不填，默认折扣为1.00，多种规格享受同一会员卡折扣</p>
                </dd>
            </dl>
            <!--app端专享价-->
      ...
      -->
            ...
            <dl>
                ...
                <dd>
                    <input name="commis_rate" value="<?php echo $output['goods']['commis_rate']; ?>" type="text" class="text w60" /><em class="add-on">%</em>
                </dd>
            </dl>
            <!--预设销量-->
            <dl>
            ...
            <dl id="ladder_list">
                <dt>阶梯价活动<?php echo $lang['nc_colon']; ?></dt>
                <dd>
                    <select name="ladder" id="ladder" class="valid">
                        <option name="ladder" value="0">请选择</option>
                        <?php foreach ($output['ladder_list'] as $klader => $vladder) { ?>
                            <option name="ladder" value="<?php echo $vladder['p_ladder_id']?>" <?php echo $output['goods']['ladder']==$vladder['p_ladder_id']?'selected':''?>>
                                <?php echo $vladder['p_name']?>
                            </option>
                        <?php } ?>
                    </select>
                </dd>
            ...
            <dl id="ladder_list1">
                <dt>销售规格<?php echo $lang['nc_colon']; ?></dt>
                <dd>
                <input name="xiao_gui" value="<?php echo $output['goods']['xiao_gui']; ?>" type="text" class="text w80" /> <span></span></dd>
            </dl>
            <dl id="ladder_list2">
                <dt>销售误差<?php echo $lang['nc_colon']; ?></dt>
                <dd>
                <input name="wu_gui" value="<?php echo $output['goods']['wu_gui']; ?>" type="text" class="text w80" /> <span></span></dd>
            </dl>
            <dl id="ladder_list3">
                <dt>最小单位<?php echo $lang['nc_colon']; ?></dt>
                <dd>
                    <select name="ladder1" id="ladder1" class="valid">
                        <option name="ladder1" value="0">请选择</option>
                        <option name="ladder1" value="1" <?php echo $output['goods']['danwei_id']==1?'selected':''?>>件</option>
                        <option name="ladder1" value="2" <?php echo $output['goods']['danwei_id']==2?'selected':''?>>g</option>
                    </select>
                </dd>
            </dl>
            <dl nctype="spec_control">
                <dt><a class="ncsc-btn ncsc-btn-green" id="spec_control">多规格</a></dt>
            </dl>
              ...
              <input type="checkbox" value="<?php echo $v['sp_value_name'];?>" nc_type="<?php echo $v['sp_value_id'];?>" <?php if($k == '1'){?>class="sp_val"<?php }?> name="sp_val[<?php echo $k;?>][<?php echo $v['sp_value_id']?>]">
                                ...
                                <li data-param="{gc_id:<?php echo $output['goods_class']['gc_id'];?>,sp_id:<?php echo $k;?>,url:'<?php echo urlShop('store_goods_add', 'ajax_add_spec');?>'}">
                                    ...
                                    <div nctype="specAdd2" style="display:none;">
                                        <input class="text w60" type="text" placeholder="规格值名称" maxlength="50">
      ...
      <dl nc_type="spec_dl" class="spec-bg" style="display:none; overflow: visible;">
        ...
        <dd class="spec-dd" style="width:810px;overflow: scroll">
          <table border="0" cellpadding="0" cellspacing="0" class="spec_table" style="width:145%;">
            <thead>
              <?php if(is_array($output['spec_list']) && !empty($output['spec_list'])){?>
              <?php foreach ($output['spec_list'] as $k=>$val){?>
            <th style="width: 100px;min-width: 100px;max-width: 100px" nctype="spec_name_<?php echo $k;?>"><?php if (isset($output['goods']['spec_name'][$k])) { echo $output['goods']['spec_name'][$k];} else {echo $val['sp_name'];}?></th>
              <?php }?>
              <?php }?>
<!--              <th class="w100" style="display: none">--><?php //echo 销售规格;?><!--</th>-->
<!--               <th class="w100" style="display: none">--><?php //echo 销售误差;?><!--</th>-->
<!--                <th class="w100" style="display: none">--><?php //echo 最小单位;?><!--</th>-->

              <th style="width: 160px;padding: 0px"><span class="red">*</span>市场价
              ...
              <th style="width: 160px;padding: 0px"><span class="red">*</span>成本价
                <div class="batch"><i class="icon-edit" title="批量操作"></i>
                  <div class="batch-input" style="display:none;">
                    <h6>批量设置价格：</h6>
                    <a href="javascript:void(0)" class="close">X</a>
              ...
              <th style="width: 160px;padding: 0px"><span class="red">*</span><?php echo $lang['store_goods_index_price'];?>
                <div class="batch"><i class="icon-edit" title="批量操作"></i>
                  <div class="batch-input" style="display:none;">
                    <h6>批量设置价格：</h6>
                    <a href="javascript:void(0)" class="close">X</a>
             ...
             <!--jinp170608 多规格添加净重、毛重、件数 配置  S-->
              <th class="w150"><span class="red">*</span><?php echo 净重;?>
                <div class="batch"><i class="icon-edit" title="批量操作"></i>
                  <div class="batch-input" style="display:none;">
                    <h6>批量设置价格：</h6>
                    <a href="javascript:void(0)" class="close">X</a>
              ...
              <th class="w90" style="display: none"><span class="red">*</span><?php echo 毛重 ;?>
                <div class="batch"><i class="icon-edit" title="批量操作"></i>
                  <div class="batch-input" style="display:none;">
                    <h6>批量设置价格：</h6>
                    <a href="javascript:void(0)" class="close">X</a>
              ...
              <th class="w60" style="display: none"><span class="red">*</span><?php echo 件数 ;?>
                <div class="batch"><i class="icon-edit" title="批量操作"></i>
                  <div class="batch-input" style="display:none;">
                    <h6>批量设置价格：</h6>
                    <a href="javascript:void(0)" class="close">X</a>
              ...
              <!--jinp170608 多规格添加净重、毛重、件数 配置  E-->
                ...
                <th class="w90" style="display: none"><span class="red">*</span>App专享价
                <div class="batch"><i class="icon-edit" title="批量操作"></i>
                  <div class="batch-input" style="display:none;">
                    <h6>批量设置价格：</h6>
                    <a href="javascript:void(0)" class="close">X</a>
              ...
              <th class="w70" style="display: none">预警值
                <div class="batch"><i class="icon-edit" title="批量操作"></i>
                  <div class="batch-input" style="display:none;">
                    <h6>批量设置预警值：</h6>
                    <a href="javascript:void(0)" class="close">X</a>
          ...
          <p class="hint">点击<i class="icon-edit"></i>可批量修改所在列的值。</p>
            ...
            <dl>
                <dt nc_type="no_spec">商品条码<?php echo $lang['nc_colon'];?></dt>
                <dd nc_type="no_spec">
                    <p>
                        <input name="g_barcode" value="<?php echo $output['goods']['goods_barcode']; ?>" type="text"  class="text"  />
                    </p>
                    <p class="hint">商品条码是指商家管理商品的编号，买家不可见<br>
        ...
        <dd>
          <ul class="ncsc-form-radio-list">
            ...
            <dl style="display: none;"  style="overflow: visible;">
                ...
                <dd id="ncProductDetails">
                    <div class="tabs">
                        <ul class="ui-tabs-nav" jquery1239647486215="2">
                            <!--               <li class="uselected"><a href="#panel-1" jquery1239647486215="8"><i class="icon-desktop"></i> 电脑端</a></li> -->
                            <li class="ui-tabs-selected"><a href="#panel-2" jquery1239647486215="9"><i class="icon-mobile-phone"></i>手机端</a></li>
                        </ul>
                        <div style="display: none;" id="panel-1" class="ui-tabs-panel" jquery1239647486215="4">
      ...
      <?php }?> -->
            <!-- F码商品专有项 E -->
            <!-- 预售商品 S -->
            <dl class="special-03" style="display: none; nctype="virtual_null" <?php if ($output['goods']['is_virtual'] == 1) {?>style="display:none;"<?php }?>>
            ...
            <!-- 商品发货人开始 -->
            <dl>
                <dt><i class="required">*</i>关联发货人</dt>
                <dd>
                    <?php if (!empty($output['deliverer'])) { ?>
                        <!--<select name="deliverer_id" class="sgcategory changeSelect">
                            <option value="0"><?php echo $lang['nc_please_choose'];?></option>
                            <?php foreach ($output['deliverer'] as $val) { ?>
                                <option value="<?php echo $val['address_id']; ?>" <?php if ($output['goods']['deliverer_id']==$val['address_id']) { ?>selected="selected"<?php } ?>><?php echo $val['seller_name']; ?></option>
                            <?php } ?>
                        </select>-->
                    ...
                    <?php } ?>
                    <p class="hint">关联的发货人是小程序端获取发货人信息所用</p>
                </dd>
            </dl>
            <!-- 商品发货人 -->
            ...
            <dl>
                <dt><?php echo '是否云仓商品';?></dt>
                <dd>
                    <ul class="ncsc-form-radio-list">
                        <li>
                            <label>
                                <input name="is_cw" value="1" <?php if (empty($output['goods']) || $output['goods']['is_cw'] == 1) { ?>checked="checked" <?php } ?> type="radio"/>
                                <?php echo '是';?></label>
                        </li>
                        <li>
                            <label>
                                <input name="is_cw" value="0" <?php if (empty($output['goods']) || $output['goods']['is_cw'] == 0) { ?>checked="checked" <?php } ?> type="radio"/>
                                <?php echo '否';?></label>
                        </li>
                    </ul>
                    <p class="hint"><?php echo '选择云仓商品可同步云仓库存并同步订单相关数据到云仓';?></p>
                </dd>
            </dl>
             <dl>
                <dt><?php echo '是否进销存商品';?></dt>
                <dd>
                    <ul class="ncsc-form-radio-list">
                        <li>
                            <label>
                                <input name="is_jin" value="1" <?php if (empty($output['goods']) || $output['goods']['is_jin'] == 1) { ?>checked="checked" <?php } ?> type="radio"/>
                                <?php echo '是';?></label>
                        </li>
                        <li>
                            <label>
                                <input name="is_jin" value="0" <?php if (empty($output['goods']) || $output['goods']['is_jin'] == 0) { ?>checked="checked" <?php } ?> type="radio"/>
                                <?php echo '否';?></label>
                        </li>
                    </ul>
                    <p class="hint"><?php echo '选择进销存商品可同步进销存系統库存并同步订单相关数据到进销存系統，但进销存与云仓二选择一';?></p>
                </dd>
            </dl>
            <dl>
                <dt><?php echo '是否新品';?></dt>
                <dd>
                    <ul class="ncsc-form-radio-list">
                        <li>
                            <label>
                                <input name="is_new" value="1" <?php if (empty($output['goods']) || $output['goods']['is_new'] == 1) { ?>checked="checked" <?php } ?> type="radio"/>
                                <?php echo '是';?></label>
                        </li>
                        <li>
                            <label>
                                <input name="is_new" value="0" <?php if (empty($output['goods']) || $output['goods']['is_new'] == 0) { ?>checked="checked" <?php } ?> type="radio"/>
                                <?php echo '否';?></label>
                        </li>
                    </ul>
                    <p class="hint"><?php echo '选择新品提交后商品会直接上架成为新品，参与新品活动';?></p>
                </dd>
            </dl>
            <!-- 新增积分额外赠送 -->
            <!--       <?php if($_SESSION['store_id']==2){?>
      <dl>
            <dt><i class="required"></i><?php echo '额外赠送积分'.$lang['nc_colon'];?></dt>
            <dd>
                <input name="points_send" type="text" class="text w210" value="<?php echo $output['goods']['points_send']; ?>" />
                <span></span>
                <p class="hint">大于零则会额外赠送积分，所填值不超过<?php echo C('points_ordermax');?></p>
            </dd>
        </dl>
        <?php }?>     -->
        </div>
...
function getMenuByajax(){
    ...
    var act = <?php echo json_encode($output['current_menu']['act'])?>;
    if(act == 'store_goods_online'){
        $('dl[nctype="spec_group_dl"]').show();
        $('dl[nctype="spec_control"]').hide();
    }else{
        $('dl[nctype="spec_group_dl"]').hide();
    }
    $('#spec_control').click(function () {
        $('dl[nctype="spec_group_dl"]').show();
        $('dl[nctype="spec_control"]').hide();
    });
        ...
        $('#goods_form').validate({
            ...
            messages : {
                ...
                image_path : {
                 ...
                 deliverer_id : {
                    required    : '<i class="icon-exclamation-sign"></i>请设置发货人',
    ...
    var is_cw = $('input[name="is_cw"][checked="checked"]').val();
    $("input[name='is_cw']").change(function(){
        is_cw = $(this).val();
    });

    $('input[name="g_storage"]').change(function(){
        if(is_cw=='1'){
    ...
    <?php }?>
        ...
        $('input[type="submit"]').click(function(){
            ...
            if($('dl[nc_type="spec_dl"]').css('display') == 'none'){
                ...
                $('dl[nc_type="spec_dl"]').find('select').attr('disabled','disabled');
            }
        });

    $(function(){
        //修改分类
        $('a[nctype="class_edit"]').click(function () {
            var data_str = '';
            eval('data_str = ' + $(this).attr('data-param'));
            if (data_str.sign == 'class_edit') {
                ajax_form('ajax_jingle', '选择商品分类', data_str.url + '&commonid=' + data_str.item + '&inajax=1', '962');
            }
        });
    ...
    function recursionSpec($len,$sign) {
        if($len < $sign){
            ...
            $ladder = '<option name="spec[\'+spec_bunch+\'][ladder]" nc_type="\'+spec_bunch+\'|ladder" data_type="ladder" value="0">请选择</option>';
            foreach ($ladder_list as $item){
                $ladder .= '<option name="spec[\'+spec_bunch+\'][ladder]" nc_type="\'+spec_bunch+\'|ladder" data_type="ladder" value="'.$item['p_ladder_id'].'">'.$item['p_name'].'</option>';
            }
            $ladder1 = '<option name="spec[\'+spec_bunch+\'][ladder1]" nc_type="\'+spec_bunch+\'|ladder1" data_type="ladder1" value="0">请选择</option>';
            $ladder1 .= '<option name="spec[\'+spec_bunch+\'][ladder1]" nc_type="\'+spec_bunch+\'|ladder1" data_type="ladder1" value="1">件</option>';
             $ladder1 .= '<option name="spec[\'+spec_bunch+\'][ladder1]" nc_type="\'+spec_bunch+\'|ladder1" data_type="ladder1" value="2">g</option>';
            echo "str +='<td><input class=\"text price\" type=\"text\" name=\"spec['+spec_bunch+'][marketprice]\" data_type=\"marketprice\" nc_type=\"'+spec_bunch+'|marketprice\" value=\"\" /><em class=\"add-on\"><i class=\"icon-renminbi\"></i></em></td><td><input class=\"text price\" type=\"text\" name=\"spec['+spec_bunch+'][costprice]\" data_type=\"costprice\" nc_type=\"'+spec_bunch+'|costprice\" value=\"\" /><em class=\"add-on\"><i class=\"icon-renminbi\"></i></em></td><td><input class=\"text price\" type=\"text\" name=\"spec['+spec_bunch+'][price]\" data_type=\"price\" nc_type=\"'+spec_bunch+'|price\" value=\"\" /><em class=\"add-on\"><i class=\"icon-renminbi\"></i></em></td><td><input class=\"text price\" type=\"text\" name=\"spec['+spec_bunch+'][weight]\" data_type=\"weight\" nc_type=\"'+spec_bunch+'|weight\" value=\"\" /><em class=\"add-on\"><b>kg</b></em></td><td style=\"display:none\"><input class=\"text price\" type=\"text\" name=\"spec['+spec_bunch+'][packages]\" data_type=\"packages\" nc_type=\"'+spec_bunch+'|packages\" value=\"\" /></td><td><input class=\"text stock\" type=\"text\" name=\"spec['+spec_bunch+'][stock]\" data_type=\"stock\" nc_type=\"'+spec_bunch+'|stock\" value=\"\" /></td><td><input class=\"text sku\" style=\"width:150px\" type=\"text\" id=\"spec'+spec_bunch+'\"  name=\"spec['+spec_bunch+'][sku]\" nc_type=\"'+spec_bunch+'|sku\" value=\"\" /></td><td><input style=\"width: 120px\" class=\"text sku\" type=\"text\" id=\"spec'+spec_bunch+'\"  name=\"spec['+spec_bunch+'][barcode]\" nc_type=\"'+spec_bunch+'|barcode\" value=\"\" title=\"\" /></td><td><select name=\"spec['+spec_bunch+'][ladder]\">$ladder</select></td></tr>';\n";
            for($i=0; $i< $len; $i++){
                echo "}\n";
            }
        }
    ...
    $(function(){
        ...
        if(str == '<tr>'){
            ...
            var array = <?php echo $output['sp_value_ladder'];?>;
            for(var ss=0;ss<array.length;ss++){
                var spec = array[ss]['spec'];
                var spec_value = array[ss]['spec_value'];
                $('[nc_type="'+spec+'"][value="'+spec_value+'"]').attr('selected','true');
        }
            $('#ladder_list').hide();
            $('#ladder_list1').hide();
            $('#ladder_list2').hide();
