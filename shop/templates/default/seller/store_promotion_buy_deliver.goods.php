...
<?php if(!empty($output['goods_list']) && is_array($output['goods_list'])){?>
    <ul>
        <a class="ncsc-btn-mini ncsc-btn-green" onclick="add()" id="add" url="<?php echo urlShop('store_buy_deliver', 'buy_deliver_add_all');?>">批量添加</a>
...
<ul class="goods-list">
  ...
  <li>
    ...
    <a nctype="btn_add_buy_deliver_goods" data-goods-id="<?php echo $val['goods_id'];?>" data-goods-name="<?php echo $val['goods_name'];?>" data-goods-img="<?php echo thumb($val, 240);?>" data-goods-price="<?php echo $val['goods_price'];?>" href="javascript:void(0);" class="ncsc-btn-mini">选择商品</a> </li>
  <?php } ?>
</ul>
...
<?php } ?>
<div id="dialog_add_buy_deliver_goods" style="display:none;">
  ...
  <div class="selected-goods-info">
    ...
    <dl class="goods-info" style="width: 350px">
      ...
      <dd>销售价格：￥<span id="dialog_goods_price"></span></dd>
        <dd>商品排序：<input id="dialog_goods_sort" type="text" class="text w70" value="0">
...
<script>
    function add() {
        ...
        var buy_deliver_id = <?php echo $output['buy_deliver_id'];?>;
        ajax_form('ajax_plate', '批量添加即买即送商品', $('#add').attr('url')+'&goods_id=' + id + '&buy_deliver_id=' + buy_deliver_id , '600');
    }
    // 全选
    function checkAll() {
        $('#batchClass').hide();
