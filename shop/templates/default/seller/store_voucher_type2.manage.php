...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('#btn_search_goods').on('click', function() {
            ...
            url = url + '&' + $.param({goods_name: $('#search_goods_name').val()}) + '&' + $.param({goods_serial:$('#search_goods_serial').val()}) + '&' + $.param({tid :<?php echo $_GET['tid'];?>});
        ...
        $('#xianshi_goods_list').on('click', '[nctype="btn_del_xianshi_goods"]', function() {
            ...
            if(confirm('确认删除？')) {
                var xianshi_goods_id = $(this).attr('data-xianshi-goods-id');
                 var xianshi_id =  <?php echo $_GET['tid'];?>;
                $.post('<?php echo urlShop('store_voucher_type', 'vouchertype_goods_delete');?>',
                    {vouchertype_goods_id: xianshi_goods_id,
                        xianshi_id: xianshi_id},
                    function(data) {
                        if(data.result) {
...
<tr class="bd-line">
    ...
    <td><?php echo $lang['currency']; ?><%=goods_price%></td>
      <td class="nscs-table-handle">
        <span><a nctype="btn_del_xianshi_goods" class="btn-red" data-xianshi-goods-id="<%=goods_id%>" href="javascript:void(0);"><i class="icon-trash"></i><p><?php echo $lang['nc_del'];?></p></a></span>
