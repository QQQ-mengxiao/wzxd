...
<form method="get" action="index.php" style="display: none;">
    <table class="search-form">
            ...
            <td class="w160"><select name="stc_id" class="w150">
                    ...
                    <?php if (is_array($output['store_goods_class']) && !empty($output['store_goods_class'])) { ?>
                        <?php foreach ($output['store_goods_class'] as $val) { ?>
                            <option value="<?php echo $val['stc_id']; ?>" <?php if ($_GET['stc_id'] == $val['stc_id']) {
                                                                                echo 'selected=selected';
                                                                            } ?>><?php echo $val['stc_name']; ?></option>
                            <?php if (is_array($val['child']) && count($val['child']) > 0) { ?>
                                <?php foreach ($val['child'] as $child_val) { ?>
                                    <option value="<?php echo $child_val['stc_id']; ?>" <?php if ($_GET['stc_id'] == $child_val['stc_id']) {
                                                                                            echo 'selected=selected';
        ...
        <tr>
            ...
            <td></td>
            <td></td>
            <td class="tc w70" colspan="2">
                <label class="submit-border">
                    <input type="submit" class="submit" value="<?php echo $lang['nc_search']; ?>" />
                </label>
                <!-- <a class="ncsc-btn ncsc-btn-green" target="_blank" href="index.php?<?php echo $_SERVER['QUERY_STRING']; ?>&op=export"><span>导出Excel</span></a>
                <a class="ncsc-btn ncsc-btn-blue" href="index.php?<?php echo $_SERVER['QUERY_STRING']; ?>&op=goods_list1"><span>库存为0</span></a> -->
            </td>
        </tr>
    </table>
</form>
<table class="ncsc-default-table">
    <thead>
        <tr nc_type="table_header">
            ...
            <th class="w100">折扣率</th>
            <th class="w100">上架时间</th>
            <th class="w100">失效时间</th>
            <th class="w100">操作</th>
        </tr>
        <?php if (!empty($output['goods_list'])) { ?>
            <tr>
                ...
                <td colspan="20"><label for="all"><?php echo $lang['nc_select_all']; ?></label>
                    <a href="javascript:void(0);" class="ncsc-btn-mini" nc_type="batchbutton" uri="<?php echo urlShop('store_goods_new', 'goods_unshow'); ?>" name="commonid"><i class="icon-level-down"></i>下架新品</a>
                </td>
        ...
        <?php if (!empty($output['goods_list'])) { ?>
            <?php foreach ($output['goods_list'] as $val) { ?>
                ...
                <tr>
                    ...
                    <td>
                        <div class="pic-thumb"><a href="<?php echo urlShop('goods', 'index', array('goods_id' => $output['storage_array'][$val['goods_commonid']]['goods_id'])); ?>" target="_blank"><img src="<?php echo thumb($val, 60); ?>" /></a></div>
                    </td>
                    <td class="tl">
                        <dl class="goods-name">
                            ...
                            <dd class="serve"> <span class="<?php if ($val['goods_commend'] == 1) {
                                                                echo 'open';
                                                            } ?>" title="店铺推荐商品"><i class="commend">荐</i></span> <span class="<?php if ($val['mobile_body'] != '') {
                                                                                                                                                                                        echo 'open';
                                                                                                                                                                                    } ?>" title="手机端商品详情"><i class="icon-tablet"></i></span> <span class="" title="商品页面二维码"><i class="icon-qrcode"></i>
                    ...
                    <td><span><?php echo $val['goods_new_discount'] . '%'; ?></span></td>
                    <td><span><?php echo date('Y-m-d H:i:s', $val['goods_puton_time']) ?></span></td>
                    <td><span><?php echo date('Y-m-d H:i:s', $val['goods_puton_time'] + $val['goods_show_time']) ?></span></td>
                    <td class="nscs-table-handle"><?php if ($val['goods_lock'] == 0) { ?>
                            <!-- <span><a href="javascript:void(0);" class="btn-blue" nctype="edit" data-param="{url:'<?php echo urlShop('store_goods_online', 'edit_goods_simple'); ?>', sign:'edit',item:'<?php echo $val['goods_commonid'] ?>'}"><i class="icon-plus"></i>
        <p>延时</p>
        </a></span> -->
                            <span><a href="javascript:void(0);" onclick="ajax_get_confirm('是否下架新品', '<?php echo urlShop('store_goods_new', 'goods_unshow', array('commonid' => $val['goods_commonid'])); ?>');" class="btn-red"><i class="icon-level-down"></i>
                                    <p>下架新品</p>
                                </a></span>
                        <?php } else { ?>
                            <span class="tip" title="该商品参加抢购活动期间不能进行编辑及删除等操作,可以编辑赠品和推荐组合"><a href="<?php if ($val['is_virtual'] == 1) {
                                                                                                        echo 'javascript:void(0);';
                                                                                                    } else {
                                                                                                        echo urlShop('store_goods_online', 'add_gift', array('commonid' => $val['goods_commonid']));
                                                                                                    } ?>" class="btn-orange-current"><i class="icon-lock"></i>
                                    <p>锁定</p>
                                </a></span>
                        <?php } ?>
                    </td>
        ...
        <?php if (!empty($output['goods_list'])) { ?>
            <tr>
                ...
                <th colspan="10"><label for="all2"><?php echo $lang['nc_select_all']; ?></label>
                    <a href="javascript:void(0);" nc_type="batchbutton" uri="<?php echo urlShop('store_goods_new', 'goods_unshow'); ?>" name="commonid" class="ncsc-btn-mini"><i class="icon-level-down"></i>下架新品</a>
                </th>
            </tr>
            <tr>
                <td colspan="20">
