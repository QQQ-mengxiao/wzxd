...
<div class="ncsc-oredr-show">
    ...
    <div class="ncsc-order-details">
      ...
      <div class="content">
        <dl>
          ...
          <dd><?php echo $output['order_info']['extend_order_common']['reciver_name'];?>&nbsp; <?php echo @$output['order_info']['extend_order_common']['reciver_info']['phone'];?>&nbsp; <?php echo @$output['order_info']['extend_order_common']['reciver_info']['address']?>&nbsp; <?php echo @$output['order_info']['extend_order_common']['reciver_info']['id_card'];?><?php echo $output['order_info']['extend_order_common']['reciver_info']['dlyp'] ? '[自提服务站]' : '';?></dd>
        </dl>
        <dl>
          <dt>发&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;票：</dt>
          <dd>
  ...
  <div class="ncsc-order-contnet">
              ...
              <?php if(!empty($output['order_info']['extend_order_common']['voucher_code'])){ ?>
                <?php foreach($output['order_info']['extend_order_common']['voucher_code'] as $key =>$v) {?>
                  <?php if($v['voucher_price']<10){?>
                    <dd>使用了面额为 <strong>&nbsp;&nbsp;&nbsp;<?php printf("%3d",$v['voucher_price']);?></strong>元的代金券，编码：<?php echo $v['voucher_code'];?></span></dd><br>
                  <?php }else if($v['voucher_price']>=10&&$v['voucher_price']<100){?>
                    <dd>使用了面额为 <strong>&nbsp;<?php printf("%3d",$v['voucher_price']);?></strong> 元的代金券，编码：<?php echo $v['voucher_code'];?></span></dd><br>
                  <?php }else{?>
                    <dd>使用了面额为 <strong><?php echo $v['voucher_price'];?></strong> 元的代金券，编码：<?php echo $v['voucher_code'];?></span></dd><br>
                  <?php }?>
                <?php } ?>
              <?php }?>
   ...
   <!-- 20181120 新添加商品中心->订单详情中所得分销佣金信息 -->
<?php if (!empty($output['order_commission_list'])) { ?>
  <div class="ncsc-order-contnet">
    <table class="ncsc-default-table order">
      <thead>
        <div>订单:<?php echo $output['order_info']['order_sn']; ?>分销佣金信息</div>
        <tr>
          <th class="w5">&nbsp;</th>
         <!--  <th class="w120"><?php echo 分销商等级;?></th> --> <!-- 181027 隐藏分销商等级 -->
          <th class="w120"><?php echo '分销商ID';?></th>
          <th class="w120"><?php echo '分销商姓名';?></th>
          <th class="w120"><?php echo '分销商手机号';?></th>
          <th class="w100">所得佣金</th>
          <th>佣金状态</th>
          <th class="w120">生成时间</th>
          <th>状态更新时间</th>

        </tr>
      </thead>
      <tbody>
      <?php foreach($output['order_commission_list'] as $k => $v){?>
      <tr class="hover">
        <td class="w12">&nbsp;</td>
        <!-- <td class="align-center"><?php echo str_replace(array('1','2','3'), array('一级分销商','二级分销商','三级分销商'), $v['relation']); ?></td> 181027 隐藏分销等级-->
        <td class="align-center"><?php echo $v['member_id']; ?></td>
        <td class="align-center"><?php echo $v['member_name']; ?></td>
        <td class="align-center"><?php echo empty($v['member_mobile'])?'无':$v['member_mobile'];?></td>
        <td class="align-center"><?php echo $v['commission']; ?></td>
        <td class="align-center"><?php echo str_replace(array('1','2','3'), array('待入账','已入账','已失效'), $v['commission_state']); ?></td>
        <td class="align-center"><?php echo @date('Y-m-d H:i:s',$v['create_time']);?></td>
        <td class="align-center"><?php echo empty($v['update_time'])?'无':date('Y-m-d H:i:s',substr($v['update_time'],0,10));?></td> 
      </tr>
      <?php } ?>
      </tbody>
    </table>
