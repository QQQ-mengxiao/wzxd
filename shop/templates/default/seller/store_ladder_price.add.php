...
<div class="ncsc-form-default">
  ...
  <form id="add_form" action="index.php?act=store_ladder_price&op=mansong_save" method="post">
...
<dl>
      <dt><i class="required">*</i>配送时间.<?php echo $lang['nc_colon'];?></dt>
      <dd>
        <input id="deliver_time" name="deliver_time" type="text" maxlength="25" class="w400 text"/>
        <span class="error-message"></span>
        <p class="hint">填写每天配送的时间点（整点数），并以英文逗号','隔开，例（8,10,12,14,16,18）</p>
      </dd>
    </dl>
    <dl> 
      <dt><i class="required">*</i>阶梯价规则<?php echo $lang['nc_colon'];?></dt>
      <dd>
        ...
        <div class="ncsc-mansong-error"><span id="mansong_price_error" style="display:none;"><i class="icon-exclamation-sign"></i>下单配送时限不能为空且必须为整数</span><span id="mansong_discount_error" style="display:none;"><i class="icon-exclamation-sign"></i>商品折扣填写整数</span></div>
        <div class="ncsc-mansong-rule">
        <span>配送时限&nbsp;<input id="mansong_price" type="text" class="text w50"></span>
        <span>小时内配送,折扣为&nbsp;<input id="mansong_discount" type="text" class="text w50">折</span>
            <div id="mansong_rule_error" style="display:none;">请至少填写一种折扣方式</div>
            <div class="mt10">
            <a href="javascript:void(0);" id="btn_save_rule" class="ncsc-btn ncsc-btn-acidblue"><i class="icon-ok-circle"></i>确定阶梯折扣规则设置</a>
            <a href="javascript:void(0);" id="btn_cancel_add_rule" class="ncsc-btn ncsc-btn-orange"><i class="icon-ban-circle"></i>取消</a></div>
        </div>
        <span class="error-message"></span>
        <p class="hint">配送时限和对应的商品折扣必填<br/>系统最多支持设置10组等级规则。</p>
      </dd>
    </dl>
    <dl>
...
<span>下单后<strong><%=price%></strong>小时内配送</span>
<span>享受商品折扣<strong><%=discount%></strong>折</span>
