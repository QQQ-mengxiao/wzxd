  ...
  <table class="search-form">
    <tr>
      ...
      <td class="w70 tc"><label class="submit-border"><input type="submit" class="submit" value="<?php echo $lang['nc_search'];?>" /></label></td>
        <?php //if($_GET['lock']=='2'){?>
        <?php if($_GET['lock']){?>
        <td class="w70 tc"><a href="index.php?act=store_refund&op=export_refund&add_time_from=<?php echo $_GET['add_time_from']; ?>&add_time_to=<?php echo $_GET['add_time_to']; ?>&state=<?php echo $_GET['state'];?>&type=<?php echo $_GET['type'];?>&key=<?php echo $_GET['key']?>&lock=<?php echo $_GET['lock']; ?>"><span><?php echo '导出报表';?></span></a></td>
        <?php }?>
    </tr>
  </table>
</form>
