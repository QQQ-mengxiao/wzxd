...
<?php if ($output['isOwnShop']) { ?>
  <a style="margin-right: 90px" class="ncsc-btn ncsc-btn-green" href="<?php echo urlShop('store_promotion_xianshi_discount', 'xianshi_add');?>"><i class="icon-plus-sign"></i><?php echo $lang['xianshi_add'];?></a>
<?php } else { ?>
...
<table class="ncsc-default-table">
  ...
  <tbody id="xianshi_list">
    <tr class="bd-line">
      <td></td>
      <td class="tl"><dl class="goods-id">
          <dt><?php echo $val['xianshi_id'];?></dt>
        </dl></td>
      <td class="tl"><dl class="goods-name">
          <dt><?php echo $val['xianshi_name'];?></dt>
        </dl></td>
      <td class="tl"><dl class="goods-discount">
          <dt><?php echo $val['discount']==0?'不打折':$val['discount']*100/10 .'%';?></dt>
        </dl></td>
      ...
      <td class="nscs-table-handle tr">
          <?php //if($val['editable']) { ?>
          <span>
              <a href="index.php?act=store_promotion_xianshi_discount&op=xianshi_edit&xianshi_id=<?php echo $val['xianshi_id'];?>" class="btn-blue">
                  <i class="icon-edit"></i>
          ...
          <?php //} ?>
          <span>
              <a href="index.php?act=store_promotion_xianshi_discount&op=xianshi_manage&xianshi_id=<?php echo $val['xianshi_id'];?>" class="btn-green">
                  <i class="icon-cog"></i>
    ...
    $(document).ready(function(){
        $('[nctype="btn_del_xianshi"]').on('click', function() {
            if(confirm('<?php echo $lang['nc_ensure_del'];?>')) {
                var action = "<?php echo urlShop('store_promotion_xianshi_discount', 'xianshi_del');?>";
