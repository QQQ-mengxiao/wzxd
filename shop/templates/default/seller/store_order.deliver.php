    ...
    <table class="search-form">
            ...
            <td class="w380"><input type="text" class="text w100" name="pay_start_date" id="pay_start_date"
                ...
                <input id="pay_end_date" class="text w100" type="text" name="pay_end_date"
                       value="<?php echo $_GET['pay_end_date']; ?>"/><label class="add-on"><i
                            class="icon-calendar"></i></label></td>
            <th style="width:100px"><?php echo $lang['store_order_order_sn']; ?></th>
      ...
      <td class="w130"><input type="text" class="text w130" name="order_num" value="<?php echo trim($_GET['order_num']) ?>"/></td>-->
        ...
        <tr>
                    ...
                    <select name="peisong">
                        <option value=0 <?php if ($_GET['peisong'] == 0) { ?>selected="selected" <?php } ?>>所有</option>
                            <?php if (is_array($output['pei_list'])) { ?>
                                ...
                                <option value=<?php echo $value['id'];?> <?php if ($_GET['peisong'] == $value['id']) { ?>selected="selected" <?php } ?>><?php echo $value['p_name'];?></option>
                                <?php } ?>
                            <?php } ?>
                    </select>
                </td>
            ...
            <td>
            <a href="javascript:void(0);" class="ncsc-btn-mini" nctype="batch" data-param="{url:'<?php echo urlShop('store_deliver', 'orders_deliver');?>', sign:'orders_deliver'}" style="background-color: #1ab962;color: white;font-weight: 600;width:52px"><i></i>批量发货</a>
                <!-- <input type="submit" class="submit" style="background-color: #1ab962;color: white;font-weight: 600;" value="批量发货"/> -->
            </td>
        </tr>
    </table>
</form>
...
<!--         <a class="ncsc-btn-mini fr" id="yto_print" name="yto_print"><i class="icon-print"></i>打印(申通)</a> -->
                                ...
                                <?php if ($order['order_state'] == ORDER_STATE_PAY) { ?>
                                    <span><a href="index.php?act=store_deliver&op=send&order_id=<?php echo $order['order_id']; ?>"
                                             class="ncsc-btn-mini ncsc-btn-green fr"><i
                                                    class="icon-truck"></i>设置到货</a></span>
                                <?php } elseif ($order['order_state'] == ORDER_STATE_SEND || $order['order_state'] == 25) { ?>
<!--                                    <span>-->
  ...
  $("#belong").attr("value",menuVal);//给隐藏的sysNum字段赋值。
   ...
   if($("#sel_all").attr('checked')) {
    ...
    function YTO_print(ids) {
        ...
        window.open("index.php?act=store_deliver&op=STOPrint&order_id=" + ids);
    }
    ...
    $(document).ready(function () {
        ...
        $('a[nctype="batch"]').click(function(){
            ...
            if ($('input[type="checkbox"]:checked').length != 0) {
                console.log(_items);
                ajax_form('ajax_deliver', '设置发货', data_str.url + '&order_id=' + _items + '&inajax=1', '480');
            }else{
