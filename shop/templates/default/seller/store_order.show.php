...
<div class="ncsc-oredr-show">
  <div class="ncsc-order-info">
    <div class="ncsc-order-details">
      <div class="title"><?php echo $lang['store_show_order_info'];?>
	  <span style="background-color:red;color:white;"><?php echo $output['order_info']['order_type_info'];?></span>
	  </div>
      <div class="content">
        <dl>
          <dt><?php echo $lang['store_show_order_receiver'].$lang['nc_colon'];?></dt>
          <dd style="width:60%"><?php echo $output['order_info']['extend_order_common']['reciver_name'];?>&nbsp; <?php echo @$output['order_info']['extend_order_common']['reciver_info']['phone'];?>&nbsp; <?php echo @$output['order_info']['extend_order_common']['reciver_info']['address']?>&nbsp; <?php echo @$output['order_info']['extend_order_common']['mall_info']?></dd>
		  <dd style="width:20%"><a href="javascript:void(0);" onclick="ajax_form('edit_address', '编辑收货地址', 'index.php?act=store_order&amp;op=edit_address&amp;order_id=<?php echo $output['order_info']['order_id'];?>', 640,0);" class="ncsc-btn-mini fr"><i class="icon-edit">编辑</i></a></dd>
        </dl>
        <!-- <dl>
          <dt>发&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;票：</dt>
          <dd>
        ...
        <dl>
          <dt style="color:#08a0c3"><b>用户ID：</b></dt>
          <dd><?php echo $output['order_info']['buyer_id']; ?></dd>
        </dl>
		<?php if($output['order_info']['order_type']==1){?>
		<dl>
          <dt><?php echo '取货时间：';?></dt>
          <dd><?php echo date('Y-m-d H:i:s',$output['order_info']['extend_order_common']['ziti_ladder_time']); ?></dd>
        </dl>
		<?php }?>
        <dl class="line">
    ...
    <div class="ncsc-order-condition">
      <dl>
        ...
        <?php if($output['order_info']['refund_state'] =='1'){?>
        <dd>部分退款</dd>
        <?php }else{?>
          <dd>已关闭</dd>
        <?php }?>
      ...
      <ul>
        <?php if($output['order_info']['refund_state'] =='1'){?>
        <li><?php echo $output['order_info']['close_info']['log_role'];?> 于 <?php echo date('Y-m-d H:i:s',$output['order_info']['close_info']['log_time']);?> <?php echo '取消了订单（商品部分退款完成取消订单）';?></li>
        <?php }else{?>
    ...
    <div class="ncsc-order-condition">
      ...
      <ul>
        ...
        <li>2. 如果买家没有及时进行收货，系统将于
          ...
          自动完成“确认收货”，完成交易。</li>
          <?php if($output['order_info']['extend_order_common']['deliver_explain'] != ''){?>
        <li>备忘：<?php echo $output['order_info']['extend_order_common']['deliver_explain']?> </li>
        <?php } ?>
      </ul>
    </div>
    <?php } ?>
  ...
  <div id="order-step" class="ncsc-order-step">
    ...
    <?php } ?>
    <dl class="<?php if($output['order_info']['extend_order_common']['shipping_time'] || $output['order_info']['ruku_time'] || $output['order_info']['hexiao_time'] || $output['order_info']['finnshed_time'] ) echo 'current'; ?>">
      <dt>商家发货</dt>
      <dd class="bg"> </dd>
      <dd class="date" style="width: 170px;height: 40px;" title="<?php echo $lang['store_show_order_send_time'];?>">
          <?php if($output['order_info']['extend_order_common']['shipping_time'] ){echo date("Y-m-d H:i:s",$output['order_info']['extend_order_common']['shipping_time']);}elseif($output['order_info']['ruku_time']){echo date("Y-m-d H:i:s",$output['order_info']['ruku_time']); }elseif($output['order_info']['hexiao_time']){echo date("Y-m-d H:i:s",$output['order_info']['hexiao_time']);}elseif($output['order_info']['finnshed_time']){echo date("Y-m-d H:i:s",$output['order_info']['finnshed_time']);}else{ echo '';}?>
           <!--  <a href="javascript:void(0);">更多<i class="icon-angle-down"></i>
                <div class="more"><span class="arrow"></span>
                  <ul>
                    <li>发货方式：<span><?php echo $output['fahuo_info']['fa_state'];?></span></li>
                    <li>发货人id：<span><?php echo $output['fahuo_info']['fa_userid'];?></span></li>
                    <li>发货人名称：<span><?php echo $output['fahuo_info']['fa_username'];?></span></li>
                  </ul>
                </div>
                </a> -->
            <?php echo $output['fahuo_info']; ?>
      </dd>
    </dl>
    <dl class="<?php if(intval($output['order_info']['finnshed_time']) || intval($output['order_info']['hexiao_time'])) { ?>current<?php } ?>">
      <dt>确认收货</dt>
      <dd class="bg"> </dd>
      <dd class="date" style="width: 170px;height: 40px;" title="<?php echo $lang['store_show_order_finish_time'];?>">
        <?php if($output['order_info']['finnshed_time'] ){echo date("Y-m-d H:i:s",$output['order_info']['finnshed_time']);}elseif($output['order_info']['hexiao_time']){echo date("Y-m-d H:i:s",$output['order_info']['hexiao_time']); }else{echo '';}?>
        <?php echo $output['shouhuo_info']; ?>
      </dd>
    </dl>
      ...
      <thead>
        <tr>
          ...
          <th class="w100">优惠活动</th>
                      <th class="w100">代金券优惠金额</th>
          <th class="" style="display:none"><strong>实付 * 佣金比 = 应付佣金(元)</strong></th>
          <th class=""><strong>实付(元)</strong></th>
          <th>交易操作</th>
        </tr>
      </thead>
      <tbody>
        ...
        <tr class="bd-line">
          ...
          <td><?php echo $goods['goods_type_cn']; ?></td>
                     <td><?php echo  $lang['currency'].$goods['voucher_price']; ?></td>
          ...
          <b><?php echo $goods['goods_pay_price']; ?></b>
          <?php } ?>
          </td>

          <!-- S 合并TD -->
          <?php if (($output['order_info']['goods_count'] > 1 && $k ==0) || ($output['order_info']['goods_count'] == 1)){?>
          <td class="bdl bdr" rowspan="<?php echo $output['order_info']['goods_count'];?>"><?php if($output['order_info']['refund_state']=='1'){echo '部分退款';}else if($output['order_info']['refund_state']=='2'){echo '已关闭';}else{echo $output['order_info']['state_desc'];}  ?>
        ...
        <tr>
          <th colspan="20"><dl class="ncm-store-sales">
              ...
              <?php if(!empty($output['order_info']['extend_order_common']['voucher_code'])){ ?>
                <?php foreach($output['order_info']['extend_order_common']['voucher_code'] as $key =>$v) {
                  $p=$v['voucher_code'];
                  $model_voucher= Model('voucher');
                 $voucher_name=Model()->query("SELECT voucher_title FROM 718shop_voucher where voucher_code=\"$p\" ");?>
                 <?php foreach ($voucher_name as $key1 => $voucher) {?>
            
                
                  <?php if($v['voucher_price']<10){?>
                    <dd>使用了面额为 <strong>&nbsp;&nbsp;&nbsp;<?php printf("%3d",$v['voucher_price']);?></strong>元的代金券，编码：<?php echo $v['voucher_code'];?>，名字：<?php echo  $voucher['voucher_title'];?></span></dd><br>
                  <?php }else if($v['voucher_price']>=10&&$v['voucher_price']<100){?>
                    <dd>使用了面额为 <strong>&nbsp;<?php printf("%3d",$v['voucher_price']);?></strong> 元的代金券，编码：<?php echo $v['voucher_code'];?>，名字：<?php echo $voucher['voucher_title'];?></span></dd><br>
                  <?php }else{?>
                    <dd>使用了面额为 <strong><?php echo $v['voucher_price'];?></strong> 元的代金券，编码：<?php echo $v['voucher_code'];?>，名字：<?php echo  $voucher['voucher_title'];?></span></dd><br>
                      <?php }?>
                  <?php }?>
                <?php } ?>
              <?php }?>
              <dd>代金券总价格为<strong><?php echo $output['order_info']['extend_order_common']['voucher_price'];?></strong>元</dd>
            </dl>
          </th>
        </tr>
        <?php } ?>
        <?php if(!empty($output['order_info']['ruku_user'])){ ?>
        <tr>
          <th colspan="20"><dl class="ncm-store-sales">
              <dd>入库人员：<strong><?php echo Model()->table('member')->getfby_member_id($output['order_info']['ruku_user'],'member_name').'('.$output['order_info']['ruku_user'].')';?></strong></dd>
          </th>
        </tr>
        <?php } ?>
        <?php if(!empty($output['order_info']['hexiao_user'])){ ?>
        <tr>
          <th colspan="20"><dl class="ncm-store-sales">
              <dd>核销人员：<strong><?php echo Model()->table('member')->getfby_member_id($output['order_info']['hexiao_user'],'member_name').'('.$output['order_info']['hexiao_user'].')';?></strong></dd>
          </th>
        </tr>
        <?php } ?>
        <tr>
