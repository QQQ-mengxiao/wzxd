...
<div class="ncsc-form-default">
  <form id="add_form" action="<?php echo urlShop('store_account_group', 'group_save');?>" method="post">
    ...
    <dl>
      <dt><i class="required"></i>自提点权限<?php echo $lang['nc_colon'];?></dt>
      <dd>
        <div class="ncsc-account-all">
          <input id="ziti_select_all" class="checkbox" type="checkbox" />
          <label for="ziti_select_all">全选</label>
        </div>
        <div class="ncsc-account-container">
          <?php if (!empty($output['ziti_list'])) {?>
          <ul class="ncsc-account-container-list" style=" width: 99%; padding-left: 1%;">
            <?php foreach ($output['ziti_list'] as $val1) {?>
            <li style=" width: 25%;">
              <input id="<?php echo $val1['address_id'];?>" class="checkbox" name="ziti_limits[]" value="<?php echo $val1['address_id'];?>" <?php if (!empty($output['ziti_limits']) && in_array($val1['address_id'], $output['ziti_limits'])) {?>checked<?php }?> type="checkbox" />
              <label for="<?php echo $val1['address_id'];?>"><?php echo $val1['seller_name'];?></label>
            </li>
            <?php }?>
          </ul>
          <?php }?>
        </div>
        <!-- <p class="hint">如需设置权限，请设置该权限组允许查看“系统消息”。</p> -->
      </dd>
    </dl>
    <div class="bottom">
...
$(document).ready(function(){
    ...
    $('#ziti_select_all').on('click', function() {
        if($(this).prop('checked')) {
            $(this).parents('dl').find('input:checkbox').prop('checked', true);
        } else {
            $(this).parents('dl').find('input:checkbox').prop('checked', false);
