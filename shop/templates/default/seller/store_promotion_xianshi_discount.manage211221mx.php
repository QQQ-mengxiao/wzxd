...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('#btn_search_goods').on('click', function() {
            var url = "<?php echo urlShop('store_promotion_xianshi_discount', 'goods_select');?>";
            url = url + '&' + $.param({goods_name: $('#search_goods_name').val()}) + '&' + $.param({goods_serial:$('#search_goods_serial').val()}) + '&' + $.param({xianshi_id :<?php echo $_GET['xianshi_id'];?>})+ '&' + $.param({stc_id: $('#stc_id').val()})+ '&' + $.param({deliverer_id: $('#deliverer_id').val()});
        ...
        $('#div_goods_search_result').on('click', '#btn_submit', function() {
            ...
            // if(!isNaN(xianshi_price) && xianshi_price > 0 && xianshi_price < goods_price) {
                $.post('<?php echo urlShop('store_promotion_xianshi_discount', 'xianshi_goods_add');?>',
                    {
                        goods_id: goods_id,
        ...
        $('#btn_edit_xianshi_goods_submit').on('click', function() {
			...
			<!--市场价限制-->
            if(!isNaN(xianshi_price) && xianshi_price > 0 && xianshi_price < goods_price ) {
                $.post('<?php echo urlShop('store_promotion_xianshi_discount', 'xianshi_goods_price_edit');?>',
                    {
        ...
        $('#xianshi_goods_list').on('click', '[nctype="btn_del_xianshi_goods"]', function() {
            ...
            if(confirm('确认删除？')) {
                var xianshi_goods_id = $(this).attr('data-xianshi-goods-id');
                $.post('<?php echo urlShop('store_promotion_xianshi_discount', 'xianshi_goods_delete');?>',
                    {xianshi_goods_id: xianshi_goods_id},
                    function(data) {
                        if(data.result) {
...
<div id="div_goods_select" class="div-goods-select" style="display: none;">
    <table class="search-form">
	  ...
	  <tr>
          <td class="w80">商品名称：<input id="search_goods_name" type="text w150" class="text" name="goods_name" value=""/></td>
          <td class="w80">商品货号/条码：<input id="search_goods_serial" type="text w70" class="text" name="goods_serial" value=""/></td>
          <td class="w80">商品分类：
        ...
        <td class="w80">供货商：
			<select name="deliverer_id" id="deliverer_id" class="sgcategory changeSelect">
                <option value="0"><?php echo $lang['nc_please_choose'];?></option>
  ...
  <form method="get">
  <table class="search-form">
    <input type="hidden" name="act" value="store_promotion_xianshi_discount" />
...
<table class="ncsc-default-table">
  <thead>
    <tr>
      <th class="w10" style="display:none"></th>
      <th class="w50"></th>
...
<div class="bottom">
  <label class="submit-border"><input type="submit" class="submit" id="submit_back" value="<?php echo $lang['nc_back'].$lang['xianshi_index'];?>" onclick="window.location='index.php?act=store_promotion_xianshi_discount&op=xianshi_list'"></label>
</div>
...
<tr class="bd-line">
    <td style="display:none"><input id="goods_sort" type="text" class="text w40" value="<%=goods_sort%>" onblur="sort('<%=xianshi_goods_id%>',$(this).val())" style=" margin-right: 10px;"/></td>
    ...
    <?php if($output['xianshi_info']['editable']) { ?>
                <span><a nctype="btn_edit_xianshi_goods" class="btn-blue" data-xianshi-goods-id="<%=xianshi_goods_id%>"
                         data-goods-price="<%=goods_price%>" data_upper_limit="<%=upper_limit%>" href="javascript:void(0);"><i
...
<script>
    function sort(xianshi_goods_id,value) {
        $.post('<?php echo urlShop('store_promotion_xianshi_discount', 'goods_sort');?>',
            {
                goods_sort: value,
                xianshi_goods_id: xianshi_goods_id,
            },
