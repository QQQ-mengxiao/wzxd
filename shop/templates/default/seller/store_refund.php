...
<form method="get" action="index.php">
  ...
  <table class="search-form">
    <tr>
      ...
      <th>退款时间</th>
      <td class="w240"><input name="admin_time_from" id="admin_time_from" type="text" class="text w70" value="<?php echo $_GET['admin_time_from']; ?>" /><label class="add-on"><i class="icon-calendar"></i></label> &#8211; <input name="admin_time_to" id="admin_time_to" type="text" class="text w70" value="<?php echo $_GET['admin_time_to']; ?>" /><label class="add-on"><i class="icon-calendar"></i></label></td>
      ...
      <td class="w70 tc"><label class="submit-border"><input type="submit" class="submit" value="<?php echo $lang['nc_search'];?>" /></label></td>
        <?php //if($_GET['lock']=='2'){?>
        <?php if($_GET['lock']){?>
        <td class="w70 tc"><a href="index.php?act=store_refund&op=export_refund&add_time_from=<?php echo $_GET['add_time_from']; ?>&add_time_to=<?php echo $_GET['add_time_to']; ?>&admin_time_from=<?php echo $_GET['admin_time_from']; ?>&admin_time_to=<?php echo $_GET['admin_time_to']; ?>&state=<?php echo $_GET['state'];?>&type=<?php echo $_GET['type'];?>&key=<?php echo $_GET['key']?>&lock=<?php echo $_GET['lock']; ?>"><span><?php echo '导出报表';?></span></a></td>
        <?php }?>
    </tr>
  </table>
</form>
<table class="ncsc-default-table">
  <thead>
    <tr>
      ...
      <th class="w80">下单时间</th>
      <th class="w80"><?php echo $lang['refund_order_add_time'];?></th>
      <th class="w80">退款时间</th>
  ...
  <tbody>
    ...
    <tr class="bd-line" >
      ...
      <td><?php echo $val['buyer_name']; ?></td>
       <td><?php echo date("Y-m-d H:i:s",$val['order_add_time']);?></td>
       <td><?php echo date("Y-m-d H:i:s",$val['add_time']);?></td>
       <?php if ($val['admin_time'] > 0) { ?>
...
<script>
	$(function(){
      ...
      $('#admin_time_from').datepicker({dateFormat: 'yy-mm-dd'});
      $('#admin_time_to').datepicker({dateFormat: 'yy-mm-dd'});
	});
</script>
