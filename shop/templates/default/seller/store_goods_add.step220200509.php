  ...
  <form method="post" id="goods_form" action="<?php if ($output['edit_goods_sign']) { echo urlShop('store_goods_online', 'edit_save_goods');} else { echo urlShop('store_goods_add', 'save_goods');}?>">
    ...
    <div class="ncsc-form-goods">
      ...
      <dl style="display: none;">
        ...
        <dd>
          <input name="g_marketprice" value="<?php echo $output['goods']['goods_marketprice']; ?>" type="text" class="text w60" /><em class="add-on"><i class="icon-renminbi"></i></em> <span></span>
          <p class="hint"><?php echo $lang['store_goods_index_store_price_help'];?>，此价格仅为市场参考售价，请根据该实际情况认真填写。cjjghgjhgh</p>
        </dd>
      </dl>
      ...
      <dl style="display: none;" nc_type="spec_group_dl_<?php echo $i;?>" nctype="spec_group_dl" class="spec-bg" <?php if($k == '1'){?>spec_img="t"<?php }?>>
        <dt>
       ...
       <dl>
        ...
        <dd>
          ...
          <select name="deliverer_id" class="sgcategory changeSelect">
            ...
            <option value="<?php echo $val['address_id']; ?>" <?php if ($v==$val['store_id']) { ?>selected="selected"<?php } ?>><?php echo $val['seller_name']; ?></option>
            <?php } ?>
          </select>
          <?php } else { ?>
          <select name="deliverer_id" class="sgcategory changeSelect">
