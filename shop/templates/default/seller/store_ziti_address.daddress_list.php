...
<!--       <th class="w70">是否默认</th> -->
  ...
  <tbody>
    ...
    <tr class="bd-line">
      ...
      <td class="tl"><?php echo $address['week'];?></td>
      <td class="tl"><?php echo $address['time'];?></td>
      <!-- <td><span class="tel"><?php echo $address['telphone'];?></span> <br/> -->
      <td class="nscs-table-handle"><span><a href="javascript:void(0);" dialog_id="my_address_edit" dialog_width="640" dialog_title="<?php echo $lang['store_daddress_edit_address'];?>" nc_type="dialog" uri="index.php?act=store_ziti_address&op=daddress_add&address_id=<?php echo $address['address_id'];?>" class="btn-blue"><i class="icon-edit"></i>
        <p><?php echo $lang['nc_edit'];?></p>
        <!-- </a></span><span> <a href="javascript:void(0)" onclick="ajax_get_confirm('<?php echo $lang['nc_ensure_del'];?>', 'index.php?act=store_ziti_address&op=daddress_del&address_id=<?php echo $address['address_id'];?>');" class="btn-red"><i class="icon-trash"></i>
...
$(function (){
	$('input[name="is_default"]').on('click',function(){
		$.get('index.php?act=store_ziti_address&op=daddress_default_set&address_id='+$(this).val(),function(result){})
	});
});
</script> 
