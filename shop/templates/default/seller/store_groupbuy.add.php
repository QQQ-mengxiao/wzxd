...
<?php } ?>
    ...
    <dl>
      <dt>App端抢购价格：</dt>
      <dd>
        <input class="w70 text" id="groupbuy_app_price" name="groupbuy_app_price" type="text" value=""/><em class="add-on"><i class="icon-renminbi"></i></em> <span></span>
      </dd>
    </dl>
    <dl>
...
$(document).ready(function(){
    ...
    $('#div_goods_search_result').on('click', '[nctype="btn_add_groupbuy_goods"]', function() {
        ...
        // $.get('<?php echo urlShop('store_groupbuy', 'groupbuy_goods_info');?>', {goods_commonid: goods_commonid}, function(data) {
          var goods_id = $(this).attr('data-goods-id');//alert(goods_id);
        $.get('<?php echo urlShop('store_groupbuy', 'groupbuy_goods_info');?>', {goods_id: goods_id}, function(data) {
            if(data.result) {
