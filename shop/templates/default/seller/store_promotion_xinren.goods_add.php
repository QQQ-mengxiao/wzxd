<div class="eject_con">
    ...
    <form method="post" action="<?php echo urlShop('store_promotion_xinren', 'xinren_goods_add_all_save'); ?>" id="plate_form">
                    ...
                    <?php if(is_array($output['goods_list']) && !empty($output['goods_list'])){?>
                        <?php foreach ($output['goods_list'] as $k=>$val){?>
                            <tr>
                                ...
                                <td>
                                    <input style="width: 50px" class="text" type="text" name="goods[<?php echo $val['goods_id']; ?>][xinren_price]" nc_type="<?php echo $val['goods_id']; ?>|xinren_price" value="">
                                </td>
                                <td>
...
<script>
    $(function () {
        $('#plate_form').submit(function () {
            ...
            formData.shift();
            var groupedArray = group(formData, 5);
            var a = 1;
            for (var item in groupedArray) {
                if(groupedArray[item][3]['value'] == ''){
                    showDialog(groupedArray[item][2]['value']+'的专享价格不能为空！');
                    a = 0;
                    break;
                }
                if(groupedArray[item][3]['value']>groupedArray[item][1]['value']){
                    showDialog(groupedArray[item][2]['value']+'的专享价格不能高于当前售价！');
                    a = 0;
                    break;
                }
            }
