$(function(){
    ...
    // 添加店铺分类
    $("#add_sgcategory1").unbind().click(function(){
        $(".sgcategory1:last").after($(".sgcategory1:last").clone(true).val(0));
    });
    // 选择店铺分类
    ...
    $('.batch-input > .ncsc-btn-mini').click(function(){
        ...
        if (_type == 'price'||_type == 'marketprice'||_type == 'weight'||_type == 'all_weight'||_type == 'app_price') {
	...
	/* 商品模式选择控制
$('[name="is_mode"]').change(function(){
    if ($('#is_mode_1').prop("checked")) {
       $('[nctype="mode_beihuo"]').show();
	   $('[nctype="mode_jihuo"]').hide();
	} else if ($('#is_mode_2').prop("checked")){
		$('[nctype="mode_beihuo"]').hide();
		$('[nctype="mode_jihuo"]').show();
	} else {
        $('[nctype="mode_beihuo"]').hide();
		$('[nctype="mode_jihuohuo"]').hide();
        $('#g_fccount').val('');
        $('#g_fcprefix').val('');
    }
});
...
$('[name="is_mode"]').change(function(){
    if ($('#is_mode_1').prop("checked")) {
       $('[nctype="mode_beihuo"]').show();
	   $('[nctype="mode_jihuo"]').hide();
	   $('[nctype="mode_kuajing"]').hide();
	} else if ($('#is_mode_2').prop("checked")){
		$('[nctype="mode_beihuo"]').hide();
		$('[nctype="mode_jihuo"]').hide();
		$('[nctype="mode_kuajing"]').show();
	} else {
        $('[nctype="mode_beihuo"]').hide();
		$('[nctype="mode_jihuohuo"]').hide();
		$('[nctype="mode_kuajing"]').hide();
