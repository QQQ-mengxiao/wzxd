   ...
   public function export_orderOp()
    {
        ...
        // var_dump($guoshudong_class);die;
            ...
            // var_dump($condition['order_goods.gc_id']);die;
        //发货人姓名
        ...
        if($_GET['delivery_type_id']>0){
            ...
            if( in_array($_GET['delivery_type_id'],$is_gharr)){
                  $is_gh=1;
            }
        ...
        foreach($data as $kk=>$vv){
            ...
            if($vv['is_zorder']==0 ){
             ...
             foreach ($vv['extend_order_goods'] as $key => $value) { 
                ...
                if($_GET['daddress_id']>0){
                    if($value['order_goods_deliverer_id']!=$_GET['daddress_id']){
                             unset($data[$kk]['extend_order_goods'][$key]);
        ...
        // var_dump($ordergoods_arr);die;
          ...
          if(is_array($data_arr)&&!empty($data_arr)){
              foreach ($data_arr as $ke => $va) {
                ...
                // $data_array[$ke]['gc_name_2']=$goods_class2['gc_name'];
        ...
        if (!$_GET['curpage']) {
            if ($count > self::EXPORT_SIZE) {
                ...
                //直接下载
                if($is_gh==1){
                     $this->excel_order_guohao($data_excel);
                }else{
    ...
    private function excel_order_guohao($data_excel){  
       ...
       $time=date('Y-m-d-H',$data_excel['time1']);
        ...
        // $excel->getActiveSheet()->getColumnDimension('A:G')->setAutoSize(true);
      ...
      $excel->getActiveSheet()->getStyle($w.':'.$z)->applyFromArray($styleThinBlackBorderOutline);
        ...
        //     $a_len = strlen($v['deliverer_name']);
        // $excel->getActiveSheet()->getColumnDimension('A')->setWidth($a_len);
        ...
        $filename =  $time.'点'.$name.'配货商品表.xls';
    ...
    {    
        ...
        if($_GET['delivery_type_id']>0){
            ...
            // var_dump($dir);die;
            set_time_limit(0);
            $daddress_list['p_name'] = str_replace("/"," ",$daddress_list['p_name']);
            $daddress_list['p_name'] = str_replace("amp;","",$daddress_list['p_name']);
            $filezipname = $daddress_list['p_name'].'配货商品';
            $filezipname = iconv('utf-8','gb2312',$filezipname);
            // echo $filezipname;die;
            header("Cache-Control: public");
            header("Content-Description: File Transfer");
            header('Content-disposition: attachment; filename='.date('Y-m-d',time()).$filezipname.'.zip'); //文件名
    ...
    private function excel_orderpl($data_excel,$dir){  
        ...
        $excel->getActiveSheet()->setCellValue('A'.($i-1), "");
    ...
    {    
        ...
        if($_GET['delivery_type_id']>0){
            ...
            $data_list[]= $dir.$filename;
            $names[]=  $name;
            $this->excel_order_subpl($data,$end_unixtime_pay,$daddress_id, $sum,$i,$dir);
        }
            //打包
            $zipname= $dir.'excel.zip';
            $files= $data_list;
            ...
            $daddress_list['p_name'] = str_replace("/"," ",$daddress_list['p_name']);
            $daddress_list['p_name'] = str_replace("amp;","",$daddress_list['p_name']);
            $filezipname = $daddress_list['p_name'].'配货订单';
            $filezipname = iconv('utf-8','gb2312',$filezipname);
            header("Cache-Control: public");
            header("Content-Description: File Transfer");
            header('Content-disposition: attachment; filename='.date('Y-m-d',time()).$filezipname.'.zip'); //文件名
        ...
        foreach ($data_tmp as $key => $order_info) {
            ...
            for ($ii = 0; $ii < count($order_info['extend_order_goods']); $ii++) {
                ...
                $order_data[] = [
                    ...
                    // 'gc_name_1'=>$goods_class3['gc_name'],
                    // 'gc_name_2'=>$goods_class2['gc_name'],
                    ...
                    // 'comp lete_time'=>$order_info['finnshed_time'] != 0 ? date('Y-m-d H:i:s', $order_info['finnshed_time']) : ' ',
    ...
    {    
        ...
        // var_dump($daddress_id);die;
        $this->excel_order_sub($data,$end_unixtime_pay,$daddress_id,$sum,$name);
    }
    ...
    {   
        ...
        foreach ($data_tmp as $key => $order_info) {
            for ($ii = 0; $ii < count($order_info['extend_order_goods']); $ii++) {
                ...
                if (!empty($arr_str)) {
                    ...
                    $qu = $arr_str[2];
                    $mall_info = Model('order')->getOrderCommonInfo(array('order_id' => $order_info['order_id']));
                    // var_dump($order_info['by_post']);die;
                    if ($order_info['by_post'] == 2) {
                        $jie = $reciver_info['address'];
                    } else {
                        $jie = $mall_info['mall_info'];
        ...
        //创建Excel输入对象
        $write = new PHPExcel_Writer_Excel5($excel);
        $filename = $time . '点' . $name . '配货清单表.xls';
        header("Pragma: public");
        header("Expires: 0");
        header("Cache-Control:must-revalidate, post-check=0, pre-check=0");
        header("Content-Type:application/force-download");
     ...
     */
     ...
     public function export_data_mergeOp(){
        /**
         * 1.excel对象
         * 2.创建第一个sheet
         * 3.生成数据，填充sheet
        ...
        $excel->setActiveSheetIndex(0);
        $excel->getActiveSheet()->setTitle("配货清单表");

        $param = $this->param_data_handling($_GET);
        
        list($excel_order_data, $is_gh) = $this->create_excel_order_data($param);

        $this->excel_order_data_filling($excel, $excel_order_data, $is_gh);
        // echo '<pre>';print_r($is_gh);die;
        $excel->createSheet(1);
        $excel->setActiveSheetIndex(1);
        $excel->getActiveSheet()->setTitle("配货商品表");
        
        list($excel_order_goods_data, $sum, $name) = $this->create_excel_order_goods_data($param);

        $this->excel_order_goods_data_filling($excel, $excel_order_goods_data, $param, $sum, $name);
        $param['name'] = $name;
        $param['time'] = date('Y-m-d-H',$param['end_unixtime_pay']);

        $excel->setActiveSheetIndex(0);
        
        $this->download($excel, $param);
    }
    ...
    public function export_data_mergeplOp(){
        ...
        if($_GET['delivery_type_id'] > 0){
            ...
            $daddress_ids = array_values($daddress_ids);
            $count = count($daddress_ids);
            // echo "<pre>";print_r($daddress_ids);die;
            $date = date('Y-m-dhis', time());
            $dir = BASE_ROOT_PATH . '/excel/goods2/' . $date . '/';
            ...
            $daddress_list['p_name'] = str_replace("/"," ",$daddress_list['p_name']);
            $daddress_list['p_name'] = str_replace("amp;","",$daddress_list['p_name']);
            $filezipname = $daddress_list['p_name'].'配货订单+商品';
            $filezipname = iconv('utf-8','gb2312',$filezipname);
            header("Cache-Control: public");
            header("Content-Description: File Transfer");
            header('Content-disposition: attachment; filename='.date('Y-m-d',time()).$filezipname.'.zip'); //文件名
    ...
    private function excel_order_sub_mergepl($excel,$data_tmp,$time=0,$address_id=0, $sum,$i,$dir){
        // echo "<pre>";print_r($data_tmp);echo '--';
        $type = 1;
        if($address_id > 0){
            $model_daddress = Model('daddress');
            $address = $model_daddress->getAddressInfo(array('address_id' => $address_id));
        ...
        $letter = array('A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K', 'L', 'M', 'N', 'O', 'P', 'Q', 'R');
        $tableheader = array('订单号', '收货人', '发货人', '商品数量', '商品名称', '商品货号', '收货人电话', '自提点','详细地址', '买家','支付时间','送货时间','买家留言','发货备注','订单状态','备注(退款信息)','促销信息','商品总成本');
        for ($ii = 0; $ii < count($tableheader); $ii++) {
            $excel->getActiveSheet()->setCellValue("$letter[$ii]1", "$tableheader[$ii]");
            $excel->getActiveSheet()->getStyle("$letter[$ii]1", "$tableheader[$ii]")->getFont()->setBold(true);
        }
        $styleThinBlackBorderOutline = array(
            'borders' => array(
                'allborders' => array( //设置全部边框
        ...
        $excel->getActiveSheet()->getStyle($w . ':' . $z)->applyFromArray($styleThinBlackBorderOutline);
        $model_order_log = Model('order_log');
        if(is_array($data_tmp) && $data_tmp != null){
            foreach ($data_tmp as $key => $order_info) {
                // echo "<pre>";print_r($data_tmp);echo '==';
                for ($ii = 0; $ii < count($order_info['extend_order_goods']); $ii++) {
                    // echo "<pre>";print_r(count($order_info['extend_order_goods']));echo '++';die;
                    $reciver_info = unserialize($order_info['reciver_info']);
                    $address = $reciver_info['area'];
                    $street = $reciver_info['street'];
                    $arr_str = explode(" ", preg_replace('#\s+#', ' ', trim($address)));
