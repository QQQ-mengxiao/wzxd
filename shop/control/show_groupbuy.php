...
class show_groupbuyControl extends BaseHomeControl {
    ...
    public function indexOp()
    {
        ...
        //即将开始
        $groupbuy_soon = $model_groupbuy->getGroupbuySoonList(array(
            'recommended' => 1,
            'is_vr' => 0,
        ), 9);
        Tpl::output('groupbuy_soon', $groupbuy_soon);
    ...
    private function _show_groupbuy_list($function_name) {
        ...
        $groupbuy_list = $model_groupbuy->$function_name($condition, 21, $order);
        //20181120 隐藏团购活动中商品会员折扣价
        foreach ($groupbuy_list as $key => $value){
            $is_vip_price = Model()->table('goods_common')->getfby_goods_commonid($value['goods_commonid'],'is_vip_price');
             if($is_vip_price == 1){
                 $groupbuy_list[$key]['goods_price'] = ncPriceFormat($value['goods_price'] * $discount);
                 $groupbuy_list[$key]['groupbuy_price'] = ncPriceFormat($value['groupbuy_price'] * $discount);
             }
         }
        Tpl::output('groupbuy_list', $groupbuy_list);
        Tpl::output('show_page', $model_groupbuy->showpage(5));
    ...
    public function groupbuy_detailOp() {
        ...
        //20181120 隐藏抢购活动，团购商品中的会员折扣价
         $is_vip_price = Model()->table('goods_common')->getfby_goods_commonid($groupbuy_info['goods_commonid'],'is_vip_price');
        if($is_vip_price == 1){
            $groupbuy_info['goods_price'] = ncPriceFormat($groupbuy_info['goods_price'] * $discount);
            $groupbuy_info['groupbuy_price'] = ncPriceFormat($groupbuy_info['groupbuy_price'] * $discount);
         }
