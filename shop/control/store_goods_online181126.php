...
class store_goods_onlineControl extends BaseSellerControl {
    ...
    public function edit_goodsOp() {
        ...
        $goodscommon_info['spec_name'] = unserialize($goodscommon_info['spec_name']);
         $goodscommon_info['goods_fx_percent'] = unserialize($goodscommon_info['goods_fx_percent']);//一级比例反序列化
        ...
        $goods_array = $model_goods->getGoodsList($where, 'goods_id,goods_marketprice,goods_price,goods_weight,goods_all_weight,goods_packages,goods_fx_percent,goods_fx_percent2,goods_app_price,goods_storage,goods_serial,goods_storage_alarm,goods_spec,is_mode,goods_hs,goods_tax,goods_shipper_id');
        $sp_value = array();
        if (is_array($goods_array) && !empty($goods_array)) {
            ...
            foreach ( $goods_array as $k => $v ) {
                ...
                if (!empty($a)) {
                    ...
                    //一级分销比例 181020
                    $sp_value ['i_' . $id . '|fx_percent[]'] = $v['goods_fx_percent'];
                    //二级分销比例 181020
                    $sp_value ['i_' . $id . '|g_fx_percent2'] = $v['fx_percent2'];
        ...
        Tpl::output('kuajing_shipper', $kuajing_shipper);
         ...
         //20181120  新添加 获取一级分销比例四板块 
        $fenxiao_info = Model('fenxiao')->dataQuery();
        $fenxiao_info['mode_trans'] = ['一','二','三','四'];
 ...
 /**
    ...
    public function edit_save_goodsOp() {
        ...
        $model_hs = Model('ctax_hs');
          ...
          // //一级分销比例
          //   if(!empty($_POST['fenxiao_percent'])){
               
          //        $update_common['goods_fx_percent']    = serialize($_POST['fenxiao_percent']);
          //   }
          // //二级分销比例
          //  $update_common['goods_fx_percent2']    = $_POST['fenxiao_percent2'];

    // 20181120  新添加 编辑过程中的一级比例与二级比例
            $update_common['is_fenxiao']         = intval($_POST['is_fenxiao']); 
            $update_common['goods_fx_percent']         = $update_common['is_fenxiao'] == 1&&!empty($_POST['fenxiao_percent']) ? serialize($_POST['fenxiao_percent']) : 0;      
            $update_common['goods_fx_percent2']         = $update_common['is_fenxiao'] == 1 ? intval($_POST['fenxiao_percent2']) : 0;    
        ...
        // 20181120 新添加 商品是否参与分销及其对应一级、二级分销比例
                    $update['is_fenxiao']        = $update_common['is_fenxiao'];
                    $update['goods_fx_percent']       = $value['fx_percent[]'];
                    $update['goods_fx_percent2']       = $value['fx_percent2'];
            ...
            // 生成商品二维码
                   ...
                   //  20181120 新添加 商品是否参与分销
                    $insert['is_fenxiao']        = $update_common['is_fenxiao']; 
                    //20181120 新添加 一级分销比例
                     $insert['goods_fx_percent']       = $value['fx_percent[]'];
                     //20181120 新添加 二级分销比例
                      $insert['goods_fx_percent2']       = $value['fx_percent2'];
            ...
            if (!empty($goods_info)) {
                ...
                $update['is_fenxiao']        = $update_common['is_fenxiao']; 
                //20181120 新添加 一级分销比例
                $update['goods_fx_percent']       = $update_common['goods_fx_percent'];
                //20181120 新添加 二级分销比例
                $update['goods_fx_percent2']        = $update_common['goods_fx_percent2'];
         ...
         // 生成商品二维码
                ...
                //20181120 新添加商品是否参与分销 
                $insert['is_fenxiao']        = $update_common['is_fenxiao'];
                //20181120 新添加 一级分销比例
                $insert['goods_fx_percent']       = $update_common['goods_fx_percent'];
                //20181120 新添加 二级分销比例
                $insert['goods_fx_percent2']        = $update_common['goods_fx_percent2'];
