    ...
    <td >
      ...
      <td >
        <select name="storage_id" class="w160">
          <option value="0" <?php if($_GET['storage_id']==0 || !isset($_GET['storage_id'])){?>selected<?php }?>>--请选择--</option>
          <?php foreach($output['storage_list'] as $val) { ?>
              <option <?php if($_GET['storage_id'] == $val['address_id']){?>selected<?php }?> value="<?php echo $val['storage_id']; ?>"><?php echo $val['storage_name']; ?></option>
          <?php } ?>
        </select>
      </td>  
      <th>是否包邮</th>
      <td>
        <select name="by_post">
          <option value=0 <?php if ($_GET['by_post'] == 0) {?>selected="selected"<?php }?>>所有</option>
