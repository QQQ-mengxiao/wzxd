...
class store_voucherControl extends BaseSellerControl{
    ...
    public function templateaddOp(){
        ...
        if(chksubmit()){
	        ...
	        $limit =  doubleval($_POST['txt_template_limit'])>0?doubleval($_POST['txt_template_limit']):0.00;
	        ...
	        if ($error){
	        	...
	        	//代金卷开始生效时间
	        	if ($_POST['txt_template_adddate']){
					$adddate = strtotime($_POST['txt_template_adddate']);
					// if (!$isOwnShop && $enddate > $quotainfo['quota_endtime']){
					// 	$enddate = $quotainfo['quota_endtime']+3600*24*1;
					// }
	        		$insert_arr['voucher_t_start_date'] = $adddate;
	        ...
	        //默认代金券模板的有效期为当前时间
	        	...
	        	if(!empty($_POST['voucher_order_type'])){
			      $insert_arr['voucher_t_ordertype'] =  implode(",",$_POST['voucher_order_type']);
				}else{
					$insert_arr['voucher_t_ordertype'] =  0;
				}
				...
				$insert_arr['voucher_t_gc_id'] = 1; //代金券分类
				...
				$insert_arr['voucher_t_is_xinren'] = intval($_POST['voucher_t_is_xinren']);
				$insert_arr['voucher_t_type'] = intval($_POST['voucher_t_type']);
				$insert_arr['voucher_t_is_lg'] = intval($_POST['voucher_t_is_lg']);
	        	//自定义图片
		  ...
		  //       	$upload->set('thumb_width','160');
	            ...
	            if($rs){
	            	if($_POST['voucher_t_type']>0){
					$insert_type=array();
					$insert_type['voucher_tid']=$rs;
					$insert_type['type']=$_POST['voucher_t_type'];
					$insert_type['store_id']=$_SESSION['store_id'];
					$voucher_type = Model('voucher_type');
	                $info=$voucher_type->addvouchertype($insert_type);
				}
    ...
    public function templateeditOp(){
        ...
        $t_info['voucher_order_type'] = explode(",", $t_info['voucher_t_ordertype']);
        if (empty($t_info)){
        	showMessage(Language::get('wrong_argument'),'index.php?act=store_voucher&op=templatelist','html','error');
        }
        ...
        if(chksubmit()){
	        ...
	        if ($error){
	        	...
	        	if ($_POST['txt_template_adddate']){
					...
					// 	$enddate = $quotainfo['quota_endtime']+3600*24*1;
					// }
	        		$update_arr['voucher_t_start_date'] = $adddate;
	        	}else {//如果没有添加有效期则默认为套餐的结束时间
                 
						$update_arr['voucher_t_start_date'] = time() ; // 
	        	}
	        //默认代金券模板的有效期为当前时间
	        	if ($_POST['txt_template_enddate']){
					$enddate = strtotime($_POST['txt_template_enddate']);
	        	...
	        	if(!empty($_POST['voucher_order_type'])){
			      $update_arr['voucher_t_ordertype'] =  implode(",",$_POST['voucher_order_type']);
				}else{
					$update_arr['voucher_t_ordertype'] =  0;
				}
	        	$update_arr['voucher_t_price'] = $price;
	        	$update_arr['voucher_t_limit'] = $limit;
	        	$update_arr['voucher_t_sc_id'] = intval($_POST['sc_id']);
				$update_arr['voucher_t_gc_id'] = 1;
				...
				$update_arr['voucher_t_is_xinren'] = intval($_POST['voucher_t_is_xinren']);
				$update_arr['voucher_t_type'] = intval($_POST['voucher_t_type']);
				$update_arr['voucher_t_is_lg'] = intval($_POST['voucher_t_is_lg']);
	        	//自定义图片
		        if (!empty($_FILES['customimg']['name'])){
		        	$upload = new UploadFile();
		        	$upload->set('default_dir',	ATTACH_VOUCHER.DS.$_SESSION['store_id']);
	            ...
	            if($rs){
	            	$voucher_type = Model('voucher_type');
	            	$condition=array();
	                $condition['voucher_tid']= $t_info['voucher_t_id'];
	                $info=$voucher_type->getvouchertypeInfo($condition);
	                if(empty($info)){
                        $insert_type=array();
						$insert_type['voucher_tid']=$t_info['voucher_t_id'];
						$insert_type['type']=$_POST['voucher_t_type'];
						$insert_type['store_id']=$_SESSION['store_id'];
		                $info=$voucher_type->addvouchertype($insert_type);
	                }else{
	                	$update=array();
	                	$update['type']=$_POST['voucher_t_type'];
	                	$vouchertype = $voucher_type->editvouchertype($update,array('voucher_tid'=>$t_info['voucher_t_id']));
	                }
	...
	public function templateedit1Op(){
		...
		if ($_GET['search_field_value'] != '') {
			if($_GET['search_field_name']=='member_name'){
				$condition['member_name'] = array('like', '%' . trim($_GET['search_field_value']) . '%');

			}else{
				$condition['member_id'] = array('like', '%' . trim($_GET['search_field_value']) . '%');
		...
		// 			$condition['member_name'] = array('like', '%' . trim($_GET['search_field_value']) . '%');
		// 			// case 'member_id':
		// 			// $condition['member_id'] = $_GET['search_field_value'];
		// 			break;
		// 	}
		}
			...
			if($error){
				...
				if($t_id&&$tmp&&$_POST['txt_voucher_number']){
					if($tmp){
						...
						// }
						$data = Model('voucher')->batch_sendVoucher($t_id,$tmp,$_POST['txt_voucher_number']);
						//echo 1;
    ...
    public function templateinfoOp(){
        ...
        if($t_info['voucher_t_is_xinren']==1){
               $t_info['voucher_t_is_xinren']='是';
        }else{
              $t_info['voucher_t_is_xinren']='否';
        }
        TPL::output('t_info',$t_info);
	...
	public function template_user_infoOp(){
		...
		//新增20190516
		if(!empty($_GET['huiyuan'])){
		$member_name=$_GET['huiyuan'];
		$where=array();
		$where['member_name'] = $member_name;
		$where['t_id'] = $t_id;
		$user_info=$model_voucher->get_user_info($where);
        }else{
        $where=array();
		$where['t_id'] = $t_id;
        	$user_info=$model_voucher->get_user_info($where);
	...
	private function profile_menu($menu_type,$menu_key='') {
		...
		switch ($menu_type) {
			case 'voucher':
				...
				2=>array('menu_key'=>'template_user_search','menu_name'=>'用户ID查询','menu_url'=>'index.php?act=store_voucher&op=template_user_search'),
				);
				break;
			case 'quota_add':
				$menu_array = array(
			...
			case 'template_user_info':
			    ...
			    break;
			case 'template_user_search':
				$menu_array = array(
			    	1=>array('menu_key'=>'templatelist','menu_name'=>Language::get('nc_member_path_store_voucher'),	'menu_url'=>'index.php?act=store_voucher&op=templatelist'),
                    2=>array('menu_key'=>'template_user_search','menu_name'=>'用户ID查询','menu_url'=>''),
			    	);
			    break;
		}
	 ...
	 * 查询用户id
	 */
	public function template_user_searchOp(){
		...
		if(chksubmit()){
			...
			if($_GET['phone']){
				$condition['order_common.reciver_info'] = array('like','%'.$_GET['phone'].'%');
			}
			// print_r($condition);die;
			
			$user_info = $model_order->table('order,order_common')->join('left')->on('order.order_id=order_common.order_id')->where($condition)->field('order.buyer_id,order.buyer_name')->group('order.buyer_id')->select();
			Tpl::output('order_sn',$_GET['order_sn']);
			Tpl::output('phone',$_GET['phone']);
		}
		Tpl::output('user_info',$user_info);
		Tpl::output('show_page',$model_order->showpage(2));
        $this->profile_menu('template_user_search','template_user_search');
		Tpl::showpage('store_voucher_template.user.search');
	}
}
