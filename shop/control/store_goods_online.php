...
class store_goods_onlineControl extends BaseSellerControl {
    ...
    public function goods_listOp() {
        ...
        if(trim($_GET['keyword'])!='' && $_GET['search_type']=='3'){
            $where['goods_serial']=array('like','%'.trim($_GET['keyword']).'%');
            $goods_list = $model_goods->getGoodsCommonOnlineList2($where);
        }
        if(trim($_GET['keyword'])!='' && $_GET['search_type']=='4'){
            $where['goods_barcode']=array('like','%'.trim($_GET['keyword']).'%');
            $goods_list = $model_goods->getGoodsCommonOnlineList2($where);
        }
        ...
        if(is_array($goods_list)){
            foreach($goods_list as $k=>$v){
                ...
                //增加判断是否已生成分享图片
                $shareimgs = $model_goods->getGoodsList(array('goods_commonid'=>$v['goods_commonid']),'goods_id');
                if (is_array($shareimgs)) {
                    foreach ($shareimgs as $key => $goods_info) {
                        if (is_file('/data/default/wzxd/data/upload/shop/shareimgs/'.$goods_info['goods_id'].'.jpg')) {
                            $goods_list[$k]['shareimgs'] = 1;
                            break;
                        }
                    }
                }
        ...
        if($seller_group_id == 46){
            $address_id = Model()->table('seller')->getfby_member_id($_SESSION['member_id'],"address_id");
            if($address_id){
                $daddress_list = Model('daddress')->getAddressList(array('store_id' => $_SESSION['store_id'],"address_id"=>$address_id),'address_id,seller_name');
            }
        }
...
//                $goods_spec_list = $model_goods->getGoodsList(array('goods_commonid'=>$value['goods_commonid'],'is_deleted'=>0),'gc_id_1,gc_id_2,gc_id_3,goods_name,goods_jingle,goods_price,goods_costprice,goods_weight,goods_presalenum,goods_serial,goods_barcode,goods_storage,goods_storage_alarm,goods_spec,goods_freight,goods_packages,deliverer_id,goods_marketprice,is_cw');
                $goods_spec_list = $model_goods->getGoodsExportList($value['goods_commonid']);
                if($goods_spec_list){
...
//         echo '<pre>';print_r($goods_list_all);die;
...
//            $data = $model_goods->getGoodsCommonOnlineList($where,'*',0);
    ...
    private function createExcel($data = array()){
        ...
        $tableheader = array('商品分类1','商品分类2','商品分类3', '商品名称','商品规格', '商品卖点','成本价', '商品价格', '市场价', '预设销量', '商品库存', '库存预警值', '商家货号', '商品编码', '运费','发货人','云仓商品','当前活动','活动价格');
        for ($i = 0; $i < count($tableheader); $i++) {
         ...
         foreach ((array)$data as $k=>$v){
            ...
            $order_data[] = [
                $goods_class1,$goods_class2,$goods_class3,$v['goods_name'],$goods_spec_info,$v['goods_jingle'],$v['goods_costprice'],$v['goods_price'],$v['goods_marketprice'],$v['goods_presalenum'],intval($v['goods_storage']), $v['goods_storage_alarm'], $v['goods_serial'], $v['goods_barcode'], $v['goods_freight'],$deliverer_name,$v['is_cw']==0?'否':'是',$v['promotion_type'],$v['goods_promotion_price']
                ];
        }
    ...
    public function edit_goodsOp() {
        ...
        $where = array('goods_commonid' => $common_id, 'store_id' => $_SESSION['store_id'],'is_deleted'=>0);
        //$where['is_deleted'] = 0;
        ...
        // 商品分类
        $goods_class_z = Model('goods_class')->getClassTree(array('gc_show' => '1'));
        Tpl::output('goods_class_z', $goods_class_z);
        $goods_class_tmp = array();
        if (!empty($goods_class_z)){
            foreach ($goods_class_z as $k=>$v) {
                $goods_class_tmp[$v['gc_id']] = $v;
                if (is_array($v['child'])) {
                    foreach ($v['child'] as $son_v){
                        $goods_class_tmp[$son_v['stc_id']] = $son_v;
                    }
                }
            }
        }
        $goodscommon_info['goods_class_id'] = trim($goodscommon_info['goods_class_id'], ',');
        $goods_gcids = empty($goodscommon_info['goods_class_id'])?array():explode(',', $goodscommon_info['goods_class_id']);
        $goods_gcids_tmp = $goods_gcids_new = array();
        if (!empty($goods_gcids)){
            foreach ($goods_gcids as $k=>$v){
                $gc_parent_id = $goods_class_tmp[$v]['gc_parent_id'];
                //分类进行分组，构造为array('1'=>array(5,6,8));
                if ($gc_parent_id > 0){//如果为二级分类，则分组到父级分类下
                    $goods_gcids_tmp[$gc_parent_id][] = $v;
                } elseif (empty($goods_gcids_tmp[$v])) {//如果为一级分类而且分组不存在，则建立一个空分组数组
                    $goods_gcids_tmp[$v] = array();
                }
            }
            foreach ($goods_gcids_tmp as $k=>$v){
                if (!empty($v) && count($v) > 0){
                    $goods_gcids_new = array_merge($goods_gcids_new,$v);
                } else {
                    $goods_gcids_new[] = $k;
                }
            }
        }
        Tpl::output('goods_class_tmp', $goods_gcids_new);
        // 实例化店铺商品分类模型
        $store_goods_class = Model('store_goods_class')->getClassTree(array('store_id' => $_SESSION ['store_id'], 'stc_state' => '1'));
        Tpl::output('store_goods_class', $store_goods_class);
        //处理商品所属分类
 ...
 /**
    ...
    public function edit_save_goodsOp() {
        ...
        if (!chksubmit() || $common_id <= 0) {
         ...
         if(!$_POST ['deliverer_id']){ 
                showDialog('请设置关联发货人，此项为必填项！', 'reload');
        }
...
//        echo '<pre>';var_dump($order_spec);die;
        ...
        if (is_array($_POST['spec'])) {
            foreach ($_POST['spec'] as $value) {
                if ($value['sku'] == '') {
                    showMessage(L('error') . '货号不能为空', urlShop('seller_center'), 'html', 'error');
                }
            }
        }else{
            if ($_POST['g_serial'] == '') {
        ...
        //商品子分类
        $goods_class_arr = array();
        if (!empty($_POST['goods_class_id'])){
            $goods_class_arr = array();
            foreach ($_POST['goods_class_id'] as $v){
                $goods_class_arr[] = intval($v);
            }
            $goods_class_arr = array_unique($goods_class_arr);
            $store_goods_class = Model('goods_class')->getGoodsClassList(array('gc_id' => array('in', $goods_class_arr), 'gc_show' => '1'),'gc_id,gc_parent_id');
            if (!empty($store_goods_class)){
                foreach ($store_goods_class as $v){
                    if ($v['gc_id'] > 0){
                        $goods_class_arr[] = $v['gc_id'];
                        $goods_class_name_arr[] = Model()->table('goods_class')->getfby_gc_id($v['gc_id'],'gc_name');
                        $goods_class_id_1 = Model('goods_class')->getParentIdById($v['gc_id']);
                        $goods_class_id_1_arr[] = Model()->table('goods_class')->getfby_gc_id($goods_class_id_1,'gc_id');
                        $goods_class_name_1_arr[] = Model()->table('goods_class')->getfby_gc_id($goods_class_id_1,'gc_name');
                    }
                    // if ($v['gc_parent_id'] > 0){
                    //     $goods_class_arr[] = $v['gc_parent_id'];
                    // }
                }
                $goods_class_arr = array_unique($goods_class_arr);
                $goods_class_name_arr = array_unique($goods_class_name_arr);
                $goods_class_id_1_arr = array_unique($goods_class_id_1_arr);
                $goods_class_name_1_arr = array_unique($goods_class_name_1_arr);
                sort($goods_class_arr);
                sort($goods_class_name_arr);
                sort($goods_class_id_1_arr);
                sort($goods_class_name_1_arr);
            }
        }
        if (empty($goods_class_arr)){
            $update_common['goods_class_id'] = '';
            $update_common['goods_class_name'] = '';
            $update_common['goods_class_id_1'] = '';
            $update_common['goods_class_name_1'] = '';
        } else {
            $update_common['goods_class_id'] = ','.implode(',',$goods_class_arr).',';
            $update_common['goods_class_name'] = ','.implode(',',$goods_class_name_arr).',';
            $update_common['goods_class_id_1'] = ','.implode(',',$goods_class_id_1_arr).',';
            $update_common['goods_class_name_1'] = ','.implode(',',$goods_class_name_1_arr).',';
        }
...
//        $model_type->delGoodsAttr(array('goods_commonid' => $common_id));
        ...
        if (is_array ( $_POST ['spec'] )) {
            ...
            $goods_info = $model_goods->getGoodsInfo(array('goods_commonid' => $common_id, 'store_id' => $_SESSION['store_id'],'color_id'=>0),'goods_id,is_group_ladder');
            ...
            foreach ($_POST['spec'] as $value) {
                    $goods_info = $model_goods->getGoodsInfo(array('color_id' => $value['color'], 'goods_commonid' => $common_id, 'store_id' => $_SESSION['store_id']), 'goods_id,is_group_ladder');
                if (!empty($goods_info)) {
                    ...
                    $update['hui_discount']    = floatval($_POST['hui_discount']);
					...
					if($p_ladder_id >0){
						...
						if($goods_group_ladder <= 1){
                          ...
                          $update['is_group_ladder'] = $goods_info['is_group_ladder'];
                        }
					}
                    //活动价格更新
                    $goodsprice_info = $model_goods->getGoodsInfo(array('goods_id' => $goods_id ),'goods_price');
                    if($goodsprice_info['goods_price']!=$update['goods_price']){
                       Model('goods_promotion')->update_changeprice($goods_id ,$update['goods_price']);
                    }
                    if($update['is_group_ladder'] ==1){
                        //多活动商品活动
                            $model_goodspromotion=Model('goods_promotion'); 
                            $array_goodspromotion=array();
                            $array_goodspromotion['goods_id']=$goods_id;
                            $array_goodspromotion['promotion_type']=60;
                            $info=$model_goodspromotion->getgoods_promotionInfo($array_goodspromotion);
                            if(!empty($info)){
                                $update_goodspromotion=array();
                                $update_goodspromotion['promotion_type_id']=$p_ladder_id;
                                $update_goodspromotion['price']=$update['goods_price'];
                                $model_goodspromotion->editgoods_promotion($update_goodspromotion,$array_goodspromotion);
                            }else{
                                $array_goodspromotion['price']=$update['goods_price'];
                                 $array_goodspromotion['promotion_type_id']=$p_ladder_id;
                    ...
                    $goods_new_sign = Model()->table('goods_common')->getfby_goods_commonid($common_id,'is_new');
                    if($goods_new_sign == 1){
                        $update['is_group_ladder'] = 7;
                    }
					//}
                      //会员价所涉及的商品活动表存贮
                    $model_goodspromotion=Model('goods_promotion'); 
                    $vipgoods=array();
                    $vipgoods['goods_id']=$goods_id;
                    $vipgoods['promotion_type']=30;
                    $vipgoods_info=$model_goodspromotion->getgoods_promotionInfo($vipgoods);
                    if($update_common['is_vip_price']==1){
                        //如果存在次商品的会员价数据则更新，没有则新增
                        if(!empty($vipgoods_info)){
                            // var_dump($update['goods_price']);die;
                          $model_goodspromotion->updategoods_promotion_vip($goods_id,$update['goods_price'],$update['hui_discount']);
                        }else{
                             $model_goodspromotion->addgoods_promotion_vip($goods_id,$update['goods_price'],$update['hui_discount']);
                        }
                    }else{ //会员表示等于0，如果存在次商品的会员价数据则删除
                        if(!empty($vipgoods_info)){
                          $model_goodspromotion->delgoods_promotion($vipgoods);
                        }
                    }
		    ...
		    // 生成商品二维码
                    ...
                    $insert['hui_discount']    = floatval($_POST['hui_discount']);
            ...
            if($goods_info['is_group_ladder'] < 1){
              $insert['is_group_ladder'] = 0;
            }else{
              $insert['is_group_ladder'] = $goods_info['is_group_ladder'];
            }
...
//                    $insert['goods_all_weight']       = $value['all_weight'];
                    ...
                    //多活动商品活动阶梯价
                    if($insert['is_group_ladder'] == 1){
                        $model_goodspromotion=Model('goods_promotion'); 
                        $array_goodspromotion=array();
                        $array_goodspromotion['goods_id']=$goods_id;
                        $array_goodspromotion['promotion_type']=60;
                        $array_goodspromotion['price']=$insert['goods_price'] ;
                        $array_goodspromotion['promotion_type_id']=$p_ladder_id;
                        $model_goodspromotion->addgoods_promotion($array_goodspromotion);
                    }
                     //slkadd多活动商品表存储之会员价
                        if($update_common['is_vip_price']==1){
                            $model_goodspromotion=Model('goods_promotion'); 
                            $model_goodspromotion->addgoods_promotion_vip($goods_id,$insert['goods_price'],$insert['hui_discount']);
                        }
                        // 生成商品二维码
                        //$PhpQRCode->set('date',WAP_SITE_URL . '/tmpl/product_detail.html?goods_id='.$goods_id);
                        $PhpQRCode->set('date',WAP_SITE_URL . '/goods/goodsDetail.shtml?goodsId='.$goods_id);
...
//                $model_type->addGoodsType($goods_id, $common_id, array('cate_id' => $_POST['cate_id'], 'type_id' => $_POST['type_id'], 'attr' => $_POST['attr']));
            ...
            $goods_info = $model_goods->getGoodsInfo(array('goods_spec' => serialize(null), 'goods_commonid' => $common_id, 'store_id' => $_SESSION['store_id']), 'goods_id,is_group_ladder');
            if (!empty($goods_info)) {
                $goods_id = $goods_info['goods_id'];
...
//        $model_goods->delGoods(array('goods_id' => array('not in', $goodsid_array), 'goods_commonid' => $common_id, 'store_id' => $_SESSION['store_id']));
        ...
        //处理规格取消后公共表中商品价格未同步改为最小价格的问题
        $lowest_price = Model()->table('goods')->where(['goods_commonid'=>$common_id,'is_deleted'=>0])->order('goods_price asc')->find();
        $update_common['goods_price'] = $lowest_price['goods_price'];
...
//        }else{
        $model_goods->editGoods(array('is_cw'=>$_POST['is_cw']),array('goods_commonid'=>$common_id));
            $return = $model_goods->editGoodsCommon($update_common, array('goods_commonid' => $common_id, 'store_id' => $_SESSION['store_id']));

        /**********新品**********/
        $goods_is_new_sign = Model()->table('goods_common')->getfby_goods_commonid($common_id,'is_new');
        if($_POST['is_new'] == 1){
            if($goods_is_new_sign != 1){
                //添加新品活动
                $model_goods->addNewPromotion($common_id);
            }
        }else{
            if($goods_is_new_sign == 1){
                //移除新品活动
                $model_goods->removeNewPromotion($common_id);
            }
        }
        /**********新品**********/
    ...
    public function class_editOp() {
        ...
        // $this->profile_menu('edit_class', 'edit_class');
        Tpl::output('edit_goods_sign', true);
        Tpl::showpage('store_goods_online.class_edit', 'null_layout');
    }
        /**
     * 编辑分类提交
     */
    public function class_edit_submitOp() {
        // echo '<pre>';var_dump($_GET);die;
        header("Content-type:application/json;charset=UTF-8");
        $common_id = $_GET['commonid'];
        if(!$common_id){
            showDialog(L('nc_common_op_succ'), urlShop('store_goods_add','add_step_two',array('class_id'=>$_GET['class_id'],'t_id'=>$_GET['t_id'])));
        }
        //通过commonid获取所有goods信息
        $old_goods_list = Model()->table('goods')->field('goods_id,goods_spec,color_id')->where(array('goods_commonid'=>$common_id,'store_id'=>4))->select();
        if($old_goods_list){
            foreach ($old_goods_list as $old_goods_list_key => $old_goods_list_value) {
                if(unserialize($old_goods_list_value['goods_spec'])!=null){//存在规格
                    $old_goods_info[$old_goods_list_key]['goods_id'] = $old_goods_list_value['goods_id'];
                    $old_goods_info[$old_goods_list_key]['goods_spec'] = array_values(unserialize($old_goods_list_value['goods_spec']))[0];
                    $old_goods_info[$old_goods_list_key]['color_id'] = $old_goods_list_value['color_id'];
                }else{
                    $old_goods_info[$old_goods_list_key]['goods_id'] = $old_goods_list_value['goods_id'];
                    $old_goods_info[$old_goods_list_key]['goods_spec'] = '';
                    $old_goods_info[$old_goods_list_key]['color_id'] = $old_goods_list_value['color_id'];
                }
            }
        }
        ...
        //新分类信息
        $new_goods_class = Model('goods_class')->getGoodsClassLineForTag($_GET['class_id']);
        $commis_rate = Model()->table('commission')->getfby_gc_id($new_goods_class['gc_id_1'],'commis_rate');
        //获取新分类下所对应的所有规格名称
        $model_spec = Model('spec');//var_dump(unserialize($goodscommon_info['spec_value']));die;null
        $model_goods = Model('goods');
        $spec_value_list = $model_spec->getSpecValueListByClassId($_GET['class_id']);
        $new_spec_value_array = [];
        // print_r($old_goods_info);
        // echo '-----'."\n";
        // print_r($spec_value_list);die;
        // die;
        $common_update_sign = 0;
        if($spec_value_list){//新分类下有规格
            if(is_array($old_goods_info) && $old_goods_info[0]['goods_spec']!=null){//旧分类下有规格
                foreach ($old_goods_info as $oldSpec => $oldSpecValue) {//遍历旧分类下所有规格
                    foreach($spec_value_list as $newSpec => $newSpecValue){//遍历新分类下所有规格
                        //如果旧分类中规格名称等于新分类下的某一个分类名称，修改goods表、goodsimages表、goodscommon表里关于分类以及规格的数据
                        if($oldSpecValue['goods_spec'] == $newSpecValue['sp_value_name']){

                            /*---------goods表---------*/
                            $goods_update_info['gc_id'] = $new_goods_class['gc_id'];
                            $goods_update_info['gc_id_1'] = $new_goods_class['gc_id_1'];
                            $goods_update_info['gc_id_2'] = $new_goods_class['gc_id_2'];
                            $goods_update_info['gc_id_3'] = $new_goods_class['gc_id_3'];
                            $goods_update_info['goods_spec'] = serialize([$newSpecValue['sp_value_id']=>$newSpecValue['sp_value_name']]);
                            $goods_update_info['color_id'] = $newSpecValue['sp_value_id'];
                            $goods_update_info['commis_rate'] = $commis_rate;
                            $result = $model_goods->editGoods($goods_update_info,array('goods_id'=>$oldSpecValue['goods_id']));
                            /*---------goods表---------*/

                            /*---------goods_images表---------*/
                            $goods_images_update_info['color_id'] = $newSpecValue['sp_value_id'];
                            $result = Model()->table('goods_images')->where(array('goods_commonid'=>$common_id,'color_id'=>$oldSpecValue['color_id']))->update($goods_images_update_info);
                            /*---------goods_images表---------*/

                            $new_spec_value_array[1][$newSpecValue['sp_value_id']] = $newSpecValue['sp_value_name'];
                            // echo $newSpecValue['sp_value_id'].'+';
                            // echo $newSpecValue['sp_value_name'];
                            // echo '-----'."\n";
                            continue 2;
                            // print_r($goods_update_info);die;
                        }else{//如果不等于，在当前分类下新建名称为$oldSpecValue的规格，并修改goods表、goodsimages表、goodscommon表里关于分类及规格的数据
                            if($newSpec == (count($spec_value_list)-1)){//直到最后一个才新增，否则继续遍历
                                // echo $newSpec;echo '===';echo (count($spec_value_list)-1);
                                $new_spec_value_info = [];
                                $new_spec_value_info['sp_value_name'] = $oldSpecValue['goods_spec'];
                                $new_spec_value_info['sp_id'] = 1;
                                $new_spec_value_info['gc_id'] = $new_goods_class['gc_id'];
                                $new_spec_value_info['store_id'] = 4;
                                $new_spec_value_info['sp_value_sort'] = 0;
                                $new_spec_id = $model_spec->specValueAdd($new_spec_value_info);
                                ...
                                $goods_update_info['gc_id_2'] = $new_goods_class['gc_id_2'];
                                $goods_update_info['gc_id_3'] = $new_goods_class['gc_id_3'];
                                $goods_update_info['goods_spec'] = serialize([$new_spec_id=>$oldSpecValue['goods_spec']]);
                                $goods_update_info['color_id'] = $new_spec_id;
                                $goods_update_info['commis_rate'] = $commis_rate;
                                $result = $model_goods->editGoods($goods_update_info,array('goods_id'=>$oldSpecValue['goods_id']));
                                /*---------goods表---------*/

                                /*---------goods_images表---------*/
                                $goods_images_update_info['color_id'] = $new_spec_id;
                                $result = Model()->table('goods_images')->where(array('goods_commonid'=>$common_id,'color_id'=>$oldSpecValue['color_id']))->update($goods_images_update_info);
                                /*---------goods_images表---------*/

                                $new_spec_value_array[1][$new_spec_id] = $oldSpecValue['goods_spec'];
                                // echo $oldSpecValue['goods_spec'].'======<br>';
                            }else{
                                continue;
                            }
                        }
            ...
            }else{//旧分类下无规格，直接修改goods表、goodsimages表、goodscommon表里关于分类及规格的数据
                /*---------goods表---------*/
                $goods_update_info['gc_id'] = $new_goods_class['gc_id'];
                $goods_update_info['gc_id_1'] = $new_goods_class['gc_id_1'];
                $goods_update_info['gc_id_2'] = $new_goods_class['gc_id_2'];
                $goods_update_info['gc_id_3'] = $new_goods_class['gc_id_3'];
                $goods_update_info['commis_rate'] = $commis_rate;
                $result = $model_goods->editGoods($goods_update_info,array('goods_id'=>$old_goods_info[0]['goods_id']));
                /*---------goods表---------*/

                /*---------goods_images表---------*/
                // $goods_images_update_info['color_id'] = $new_spec_id;//0
                /*---------goods_images表---------*/

                /*---------goods_common表---------*/
                $goods_common_update_info['gc_id'] = $new_goods_class['gc_id'];
                $goods_common_update_info['gc_id_1'] = $new_goods_class['gc_id_1'];
                $goods_common_update_info['gc_id_2'] = $new_goods_class['gc_id_2'];
                $goods_common_update_info['gc_id_3'] = $new_goods_class['gc_id_3'];
                $goods_common_update_info['gc_name'] = $new_goods_class['gc_tag_name'];
                $goods_common_update_info['type_id'] = $_GET['t_id'];
                $goods_common_update_info['commis_rate'] = $commis_rate;
                $result = $model_goods->editGoodsCommon($goods_common_update_info,array('goods_commonid'=>$common_id));
                /*---------goods_common表---------*/
                $common_update_sign = 1;
            }
        }else{//新分类下无规格
            // if(is_array(unserialize($goodscommon_info['spec_value']))){//旧分类下有规格
            if(is_array($old_goods_info) && $old_goods_info[0]['goods_spec']!=null){//旧分类下有规格
                // foreach (unserialize($goodscommon_info['spec_value']) as $oldSpec => $oldSpecValue) {//遍历旧分类下所有规格
                foreach ($old_goods_info as $oldSpec => $oldSpecValue) {//遍历旧分类下所有规格
                    //在当前分类下新建名称为$oldSpecValue的规格，并修改goods表、goodsimages表、goodscommon表里关于分类及规格的数据
                    $new_spec_value_info = [];
                    $new_spec_value_info['sp_value_name'] = $oldSpecValue['goods_spec'];
                    $new_spec_value_info['sp_id'] = 1;
                    $new_spec_value_info['gc_id'] = $new_goods_class['gc_id'];
                    ...
                    $goods_update_info['gc_id_2'] = $new_goods_class['gc_id_2'];
                    $goods_update_info['gc_id_3'] = $new_goods_class['gc_id_3'];
                    $goods_update_info['goods_spec'] = serialize([$new_spec_id=>$new_spec_value_info['sp_value_name']]);
                    $goods_update_info['color_id'] = $new_spec_id;
                    $goods_update_info['commis_rate'] = $commis_rate;
                    $result = $model_goods->editGoods($goods_update_info,array('goods_id'=>$oldSpecValue['goods_id']));
                    /*---------goods表---------*/
                    ...
                    $new_spec_value_array[1][$new_spec_id] = $new_spec_value_info['sp_value_name'];
                    // echo $newSpecValue['sp_value_name'].'++++++<br>';
                }
            }else{//旧分类下无规格，直接修改goods表、goodsimages表、goodscommon表里关于分类及规格的数据
                /*---------goods表---------*/
                $goods_update_info['gc_id'] = $new_goods_class['gc_id'];
...
// print_r($new_spec_value_array);
        if($common_update_sign == 0){
            if(is_array($new_spec_value_array) && $new_spec_value_array!=null){//多规格
                $goods_common_update_info['gc_id'] = $new_goods_class['gc_id'];
                $goods_common_update_info['gc_id_1'] = $new_goods_class['gc_id_1'];
                $goods_common_update_info['gc_id_2'] = $new_goods_class['gc_id_2'];
                $goods_common_update_info['gc_id_3'] = $new_goods_class['gc_id_3'];
                $goods_common_update_info['gc_name'] = $new_goods_class['gc_tag_name'];
                $goods_common_update_info['spec_name'] = 'a:1:{i:1;s:6:"规格";}';
                $goods_common_update_info['spec_value'] = serialize($new_spec_value_array);
                $goods_common_update_info['type_id'] = $_GET['t_id'];
            }else{//无规格
                $goods_common_update_info['gc_id'] = $new_goods_class['gc_id'];
                $goods_common_update_info['gc_id_1'] = $new_goods_class['gc_id_1'];
                ...
                $goods_common_update_info['gc_id_3'] = $new_goods_class['gc_id_3'];
                $goods_common_update_info['gc_name'] = $new_goods_class['gc_tag_name'];
                $goods_common_update_info['spec_name'] = serialize([]);
                $goods_common_update_info['spec_value'] = serialize($new_spec_value_array);
                $goods_common_update_info['type_id'] = $_GET['t_id'];
                $goods_common_update_info['commis_rate'] = $commis_rate;
            }
        ...
        if ($result) {
            // 添加操作日志
            $this->recordSellerLog('修改商品分类，平台货号：'.$common_id);
            showDialog(L('nc_common_op_succ'), urlShop('store_goods_online','edit_goods',array('commonid'=>$common_id)));
        } else {
            showDialog(L('nc_common_save_fail'), urlShop('store_goods_online', 'index'));
        }
    }
    ...
    public function goods_unshowOp() {
        ...
        if ($return) {
            ...
            $condition=array();
            $condition['goods_id']=array('in',array_column($goods_list,'goods_id'));
            $condition['promotion_type']=40;
            $model_goodspromotion->delgoods_promotion($condition);
            // 更新优惠套餐状态关闭
            $goods_list = $model_goods->getGoodsList($where, 'goods_id');
            if (!empty($goods_list)) {
                $goodsid_array = array();
            ...
            //更新新品活动标记及价格
            Model()->execute("UPDATE 718shop_goods SET is_group_ladder=0 WHERE goods_commonid IN (".$common_id.") AND is_group_ladder=7");
            //修改商品表中商品的促销价格，促销类型0，is_group_ladder
            Model()->execute("UPDATE 718shop_goods SET goods_promotion_price = NULL,goods_promotion_type = 0,is_group_ladder = 0 WHERE is_group_ladder IN (3,4) AND goods_commonid IN (".$common_id.")");
            //删除该commond下的所有活动标签是3新人的活动商品表里的数据
            Model()->execute("DELETE FROM 718shop_p_xinren_goods WHERE goods_id IN (SELECT goods_id FROM 718shop_goods WHERE is_group_ladder=3 AND goods_commonid IN (".$common_id."))");
            //删除该commond下的所有活动标签是4秒杀折扣的活动商品表里的数据
            Model()->execute("DELETE FROM 718shop_p_xianshi_goods WHERE goods_id IN (SELECT goods_id FROM 718shop_goods WHERE is_group_ladder=4 AND goods_commonid IN (".$common_id."))");
            // 添加操作日志
    ...
    public function get_goods_list_ajaxOp() {
        ...
        foreach ($goods_list as $key => $val) {
            ...
            //slk0812edit
             $model_goodspromotion=Model('goods_promotion'); 
            $array_goodspromotion=array();
            $array_goodspromotion['goods_id']=$val['goods_id'];
            $info=$model_goodspromotion->getgoods_promotionList($array_goodspromotion);
            if(empty($info)){
                $allp_name= '无活动';
                ...
                // $allp_name='';
                 foreach ($info as $k => $v) {
                    $p_name=promotion_typeName($v['promotion_type']);
                        if($k==0){
                           $allp_name= $p_name;
                        }else{
                             $allp_name= $allp_name.','.$p_name;
                        }
                     
                 }
            }
            $arrp_name=explode(",",$allp_name);
            $arrp_name = array_unique($arrp_name);
            $allp_name=implode(",",$arrp_name);
             $goods_list[$key]['p_name'] = $allp_name;
    ...
    public function edit_goods_simpleOp() {
        ...
        $ladder_list = Model('p_ladder')->getMansongList(['store_id'=>$_SESSION['store_id']],'','','p_ladder_id,p_name');
        Tpl::output('is_cw',Model()->table('goods')->getfby_goods_commonid($common_id,'is_cw'));
        Tpl::output('ladder_list', $ladder_list);
        Tpl::output('goods_list', $goods_list);
            ...
            foreach ($goods_commonid_arr as $key => $value) {
                ...
                if ($spec_value) {
                    foreach ($spec_value as $k => $v) {
                        foreach ($v as $kk => $vv) {
                            //再通过goods_commonid和color_id在goods_images表里获取对应的图片名称和store_id，拼接成url
                            $goods_image_list = $model_goods->getGoodsImageList(array('goods_commonid' => $value, 'store_id' => $_SESSION['store_id'], 'color_id' => array('in',$kk.',0')));
                            if ($goods_image_list) {
                                foreach ($goods_image_list as $vl) {
                                    $file_name = $goods_name . iconv('utf-8', 'gbk//IGNORE', $vv);
                                    $file_name_tmp = str_replace('/',' ',$file_name);
...
//        $goods_serial_list = $model_goods->getGoodsList(array('goods_commonid'=>array('in',$goods_commonid_array)),'goods_serial as goodsCode');
        $goods_serial_list = $model_goods->getGoodsList(array('goods_commonid'=>array('in',$goods_commonid_array),'is_deleted'=>0),'goods_id,goods_serial as goodsCode');
     ...
     */
    public function create_image_fromOp()
    {
        //获取背景默认背景
        $bg_image_info = Model()->table('sharepic')->field('share_pic,sharepic_id')->where(array('sharepic_recommend' => 1))->find();
        if (empty($bg_image_info)) {
           echo '<br/><br/><br/>请设置背景图片<br/><br/><br/>';
           die;
        }
        Tpl::output('bg_image_id', $bg_image_info['sharepic_id']);
        Tpl::showpage('store_goods_list.create_image', 'null_layout');
    }

    /**
     * 查看生成的图片
     */
    public function view_shareimgsOp($value='')
    {
      $common_id = $_GET['commonid'];
      $model_goods = Model('goods');
      //获取goods_id,遍历显示图片
      $goods_list = $model_goods->getGoodsList(array('goods_commonid'=>$common_id),'goods_id');
      foreach ($goods_list as $goods_info) {
        echo '<img src="http://117.159.3.227:8088/wzxd/data/upload/shop/shareimgs/'.$goods_info["goods_id"].'.jpg">';
      }      
    }
    /**
     * 全部商品图片下载
...
//                            $file_name = $goods_name . iconv('utf-8', 'gbk//IGNORE', $vv);
	...
	**/
    ...
    public function goods_unshow_deliverOp(){
        $daddress_id = $_GET['daddress_id'];
        if($daddress_id){
            $goods_commonid_array = Model()->table("goods_common")->field("goods_commonid")->where(["deliverer_id"=>$daddress_id,"goods_state"=>1,"goods_verify"=>1])->select();
            $commonid_array = [];
            foreach($goods_commonid_array as $item){
                $commonid_array[] = $item["goods_commonid"];
            }
            $common_id = implode("," ,$commonid_array);
            $where = array();
            $where['goods_commonid'] = array('in', $commonid_array);
            $where['store_id'] = $_SESSION['store_id'];
            $return = Model('goods')->editProducesOffline($where);
            ...
            if ($return) {
                ...
                // 添加操作日志
                $this->recordSellerLog('商品下架，平台货号：'.$common_id);
                showDialog("商品下架成功", getReferer() ? getReferer() : 'index.php?act=store_goods_online&op=goods_list', 'succ', '', 2);
            }else{
                showDialog("商品下架失败", '', 'error');
            }
        }else{
            showDialog("请选择发货人", '', 'error');
        }
    }
