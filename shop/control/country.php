...
class countryControl extends BaseHomeControl {
    const PAGESIZE = 16;
    public function index0Op() {
        ...
        if (!empty($_GET['curpage'])) {
            Tpl::showpage('country.item','null_layout');
        } else {
    ...
    public function countryOp() {
        //板块信息
		$model_web_config = Model('country');
		$web_html = $model_web_config->getWebHtml('index');
		Tpl::output('web_html',$web_html);
        //print_r($web_html);
        //break;

        $web_name= $model_web_config->getWebName('index');
		  foreach($web_name as $key =>$value){
            if($value['name']=="白俄罗斯"){
                $web_name[$key]['name']="白俄";
            }
        }
        Tpl::output('web_name',$web_name);
    ...
    public function countryDetailOp($web_id = 1) {
        $web_id = $_GET['web_id'];
        $model_web_config = Model('country');

        $country_html = $model_web_config->getWebCountryHtml($web_id,'index');
        Tpl::output('country_html',$country_html);

        $country_name = $model_web_config->getfby_web_id($web_id,'web_name');
        ...
        $nav_link = array(
                    ...
                    1=>array(
                            'title'=>'国家馆',
                            'link'=>SHOP_SITE_URL.'/index.php?act=country&op=country',
                    ),
                    2=>array(
                            'title'=>$country_name.'馆',
                    )

            );
        Tpl::output('nav_link_list',$nav_link);
        Tpl::showpage('country.detail');
    }
