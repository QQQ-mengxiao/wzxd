...
class seller_loginControl extends BaseSellerControl {
    ...
    public function loginOp() {
        ...
        $seller_info = $model_seller->getSellerInfo(array('seller_name' => $_POST['seller_name']));
         if($seller_info['is_use']!=1){
             showDialog('此账号未允许登录','','error');
        }
        if($seller_info) {

            $model_member = Model('member');
            $jiami = Logic('login');
            $abc = $jiami->password_verify($_POST['password'],$model_member->getfby_member_id($seller_info['member_id'],'member_passwd'));
            if ($abc==1) {
