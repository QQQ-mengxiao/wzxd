...
class store_export_warehouse_excel extends BaseSellerControl
{
     ...
     * 批量导出仓库报表
     */
    public function batchExportWarehouseOp($start_time_pay, $end_time_pay, $shipperCategory, $zitiAddress, $type)
    {
        // 发货人类别名称
    ...
    private function createLocalExcel($start_time_pay, $end_time_pay, $shipperCategory, $zitiAddress, $zitiName, $categoryName, $type)
    {
        ...
        // 发货人类别对应的发货人
        $shippers = $this->getShippersByCategoryId($shipperCategory);

        // 非团购，创建查询条件
        $condition1 = $this->createCondition($payStartTime, $payEndTime, $shippers, $zitiAddress, $type, 0);
        // 查询数据
        $data = $this->getWarehouseOrderGoodsExportList($condition1);

        // // 团购，创建查询条件
        // $condition2 = $this->createCondition($payStartTimeYushou, $payEndTime, null, $zitiAddress, $type, 1);
        // // 查询数据
        // $data2 = $this->getWarehouseOrderGoodsExportList($condition2);
    ...
    private function createWarehouseExcel($data, $zitiName, $categoryName)
    {
        ...
        try {
            ...
            // 文件名
            $filename = date('m.d', time()) . $zitiName . '待发货订单' . '.xlsx';
    ...
    private function exportSingleExcel($start_time_pay, $end_time_pay, $shipperCategory, $zitiAddress, $zitiName, $categoryName, $type)
    {
        ...
        $condition1 = $this->createCondition($payStartTime, $payEndTime, $shippers, $zitiAddress, $type, 0);
        // 查询数据
        $data1 = $this->getWarehouseOrderGoodsExportList($condition1);
