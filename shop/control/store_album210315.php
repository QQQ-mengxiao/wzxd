...
//		Tpl::output('show_page',$page->show());
    ...
    {
        $apic_id = $_GET['apic_id'];
        $model_album = Model('album');
        $model_zipfile = Model('zipfile');
        $apic_info = $model_album->getOne(array('apic_id' => $apic_id));
        $pic_cover = $apic_info['apic_cover'];//Model()->table('album_pic')->getfby_apic_id($apic_id,'apic_cover');
        $pic_url = UPLOAD_SITE_URL . '/' . ATTACH_GOODS . '/' . $apic_info['store_id'] . '/' . $pic_cover;//echo'<pre>';echo $pic_url;die;
        $pic_name = $apic_info['apic_name'];//Model()->table('album_pic')->getfby_apic_id($apic_id,'apic_name');
        $img = array(array('url' => $pic_url, 'name' => $pic_name));
        $model_zipfile->download($img);
		return true;
    }

    public function img_downloadOp()
    {
        $apic_id = $_GET['apic_id'];
        if ($apic_id) {
            $model_album = Model('album');
            ...
            $apic_id_arr = explode(',', $apic_id);
            array_pop($apic_id_arr);
            $img = array();
            foreach ($apic_id_arr as $key => $value) {
                $apic_info = $model_album->getOne(array('apic_id' => $value));
                $pic_cover = $apic_info['apic_cover'];
                $pic_url = UPLOAD_SITE_URL . '/' . ATTACH_GOODS . '/' . $apic_info['store_id'] . '/' . $pic_cover;
                $pic_name = $apic_info['apic_name'];
                $img[] = array('url' => $pic_url, 'name' => $pic_name);
		}
            $model_zipfile->download($img);
			return true;
        } else {
            showDialog('请选择需要下载的图片！', '', 'notice');
        }
    }

    public function img_download_allOp()
    {
        $aclass_id = $_GET['aclass_id'];
        $model_album = Model('album');
        $model_zipfile = Model('zipfile');
        $pic_list = $model_album->getPicList(array('aclass_id' => $aclass_id), '', 'apic_name,apic_cover,store_id');
        if ($pic_list) {
        	$img = [];
            foreach ($pic_list as $key => $value) {
				$img[$key]['url'] = UPLOAD_SITE_URL . '/' . ATTACH_GOODS . '/' . $value['store_id'] . '/' . $value['apic_cover'];
				$img[$key]['name'] = $value['apic_name'];
			}
			$model_zipfile->download($img);
			return true;
        } else {
