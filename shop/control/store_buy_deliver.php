...
class store_buy_deliverControl extends BaseSellerControl {
    ...
    public function buy_deliver_delOp() {
        ...
        if($result) {
            ...
            $array_goodspromotion['promotion_type_id']=$buy_deliver_id;
    ...
    public function buy_deliver_manageOp() {
        ...
        $buy_deliver_goods_list = $model_buy_deliver_goods->getBuyDeliverGoodsExtendListShop($condition,5,'goods_sort asc','','',$_GET['goods_serial']);
    ...
    public function goods_selectOp() {
        ...
        Tpl::output('buy_deliver_id', $_GET['buy_deliver_id']);
    ...
    public function buy_deliver_goods_addOp() {
        ...
        $buy_deliver_id = intval($_POST['buy_deliver_id']);
        $goods_sort = intval($_POST['goods_sort']);
       ...
       /* if ($goods_info['goods_storage'] == 0) {
        ...
        // if ($is_group_ladder != 5 && $is_group_ladder != 0) {
        //     $data['result'] = false;
        //     $data['message'] = "商品已参与其他活动";
        //     echo json_encode($data);die;
        // }
        ...
        $condition['buy_deliver_id'] = $buy_deliver_id;
        $buy_deliver_goods = $model_buy_deliver_goods->getBuyDeliverGoodsExtendList($condition);
        if(!empty($buy_deliver_goods)) {
            $data['result'] = false;
            $data['message'] = '商品已添加该自提点活动';
            echo json_encode($data);die;
        }
        ...
        $param['ziti_id'] = $buy_deliver_info['ziti_id'];
  ...
  //       $param['xianshi_price'] = $xianshi_price;
        ...
        if($buy_deliver_goods_info) {
              //add商品活动表
                    ...
                    $array_goodspromotion['promotion_type']=80;
                    $array_goodspromotion['price']=$goods_info['goods_price'] ;
                    $array_goodspromotion['promotion_type_id']=$buy_deliver_info['buy_deliver_id'];
   ...
   //          $this->recordSellerLog('添加即买即送商品，活动名称：'.$buy_deliver_info['xianshi_name'].'，商品名称：'.$goods_info['goods_name']);
    ...
    public function buy_deliver_goods_deleteOp() {
        ...
        //del商品活动表
            $model_goodspromotion=Model('goods_promotion'); 
            $array_goodspromotion=array();
            $array_goodspromotion['goods_id']=$buy_deliver_goods_info['goods_id'];
            $array_goodspromotion['promotion_type']=80;
            $array_goodspromotion['promotion_type_id']=$buy_deliver_goods_info['buy_deliver_id'];
            $model_goodspromotion->delgoods_promotion($array_goodspromotion);

            //adel商品活动表
        //判断即买即送活动商品表里是否还有该商品
        $buy_deliver_goods = Model()->table('buy_deliver_goods')->where(array('goods_id'=>$buy_deliver_goods_info['goods_id'],'store_id'=>$_SESSION['store_id']))->select();
        if(!$buy_deliver_goods){
            //更改商品状态
    ...
    public function goods_sortOp(){
        $buy_deliver_goods_id = $_POST['buy_deliver_goods_id'];
        if(!$buy_deliver_goods_id){
            showDialog('参数错误！');
        }
        $model_buy_deliver_goods = Model('buy_deliver_goods');
        $goods_sort = $model_buy_deliver_goods->getBuyDeliverGoodsInfoByID($buy_deliver_goods_id,$_SESSION['store_id']);
        $update = $model_buy_deliver_goods->editBuyDeliverGoods(array('goods_sort' => $_POST['goods_sort']), array('buy_deliver_goods_id' => $buy_deliver_goods_id));
        $this->recordSellerLog('修改即买即送活动商品排序，id：' . $buy_deliver_goods_id . '修改之前为' . $goods_sort['goods_sort'] . '修改之后为' . $_POST['goods_sort']);
        if ($update) {
    ...
    public function buy_deliver_add_allOp(){
        ...
        Tpl::output('buy_deliver_id', $_GET['buy_deliver_id']);
        Tpl::output('goods_list', $goods_list);
        Tpl::showpage('store_promotion_buy_deliver.goods_add','null_layout');
    }

    public function buy_deliver_add_all_saveOp(){
        ...
        if($goods_list){
            if(is_array($goods_list)){
                foreach ($goods_list as $key=>$value) {
                    $buy_deliver_id = intval($_POST['buy_deliver_id']);
                    $goods_sort = intval($value['goods_sort']);
                    ...
                    /*if ($goods_info['goods_storage'] == 0) {
                        showDialog($goods_info['goods_name'].'，库存不足');
                        break;
                    }*/
                    //基于id，更改goods_type
                    $buy_deliver_info = $model_buy_deliver->getBuyDeliverInfoByID($buy_deliver_id, $_SESSION['store_id']);
                    ...
                    $buy_deliver_goods_id = Model('buy_deliver_goods')->insert($param);
            ...
            //add商品活动表
                        $update = array('is_group_ladder' => 5);
                        $goods_condition = array('goods_id' => $key);
                        $model_goods->editGoods($update, $goods_condition);
