...
class store_groupbuyControl extends BaseSellerControl {
    ...
    public function groupbuy_listOp() {
        ...
        //lxs
        $str=$_GET['groupbuy_name'];
        if(!empty($str)){
            $str = Model('search')->decorateSearch_pre($str);
            $condition['groupbuy_name'] = array('like', '%' . $str . '%');
        }
    ...
    public function groupbuy_saveOp() {
		...
		$param['groupbuy_app_price'] = floatval($_POST['groupbuy_app_price']);
		...
		$param['groupbuy_rebate'] = number_format($param['groupbuy_price']/$param['goods_price'],3)*10;////////插入数据表MX
        ...
        if($result) {
			...
			$data_array['groupbuy_app_price']	= $param['groupbuy_app_price'];
    ...
    public function search_goodsOp() {
        ...
        $goods_list = $model_goods->getGoodsListForPromotion($condition, '*', 8, 'groupbuy');
