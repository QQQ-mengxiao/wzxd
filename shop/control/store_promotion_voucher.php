...
class store_promotion_voucherControl extends BaseSellerControl {
    ...
    public function mansong_listOp() {
        ...
        $model_mansong = Model('p_voucher');
        $model_mansong_rule = Model('p_voucher_rule');
        $condition = array();
        ...
        //     $rule_list = $model_mansong_rule->getMansongRuleListByID($value['p_voucher_id']);
        ...
        Tpl::showpage('store_promotion_voucher.list');
    }
    ...
    public function mansong_addOp() {
        $model_mansong_quota = Model('p_voucher_quota');
        $model_mansong = Model('p_voucher');

        //输出导航
        self::profile_menu('mansong_add');
        Tpl::showpage('store_promotion_voucher.add');
    }
    ...
    public function mansong_saveOp() {
        ...
        $param['order_limit'] = $_POST['order_limit'];
        $param['add_time'] = time();
        ...
        if($mansong_id) {
            ...
            foreach ($_POST['mansong_rule'] as $value) {
                ...
                $mansong_rule = array();
                $mansong_rule['p_voucher_id'] = $mansong_id;
 		...
 		$flag=1;
            ...
            showDialog('活动添加成功', urlShop('store_promotion_voucher', 'mansong_list'), 'succ');
        } else {
            showDialog('活动添加失败');
        }
    }
...
// var_dump($rule_list);die;
        ...
        Tpl::showpage('store_promotion_voucher.detail');
    }
    ...
    public function mansong_delOp() {
        ...
        $condition = array();
        $condition['p_voucher_id'] = $mansong_id;
        $result = $model_mansong->delMansong($condition);

        if($result) {
            $this->recordSellerLog('删除代金券活动，活动名称：'.$mansong_info['p_name']);
            showDialog(L('nc_common_op_succ'), urlShop('store_promotion_voucher', 'mansong_list'), 'succ');
        } else {
    ...
    public function mansong_quota_addOp() {
        ...
        Tpl::showpage('store_promotion_voucher_quota.add');
    }
    ...
    public function mansong_quota_add_saveOp() {
        ...
        showDialog(Language::get('mansong_quota_add_success'), urlShop('store_promotion_voucher', 'mansong_list'), 'succ');
    }
    ...
    public function search_goodsOp() {
        ...
        Tpl::showpage('store_promotion_voucher.goods', 'null_layout');
    }
    ...
    private function profile_menu($menu_key='') {
        $menu_array = array(
            1=>array('menu_key'=>'mansong_list','menu_name'=>Language::get('promotion_active_list'),'menu_url'=>urlShop('store_promotion_voucher', 'mansong_list')),
        );
        switch ($menu_key){
        	case 'mansong_add':
                $menu_array[] = array('menu_key'=>'mansong_add','menu_name'=>Language::get('promotion_join_active'),'menu_url'=>urlShop('store_promotion_voucher', 'mansong_add'));
        		break;
        	case 'mansong_quota_add':
                $menu_array[] = array('menu_key'=>'mansong_quota_add','menu_name'=>Language::get('promotion_buy_product'),'menu_url'=>urlShop('store_promotion_voucher', 'mansong_quota_add'));
        		break;
        	case 'mansong_detail':
                $menu_array[] = array('menu_key'=>'mansong_detail','menu_name'=>Language::get('mansong_active_content'),'menu_url'=>urlShop('store_promotion_voucher', 'mansong_detail', array('mansong_id' => $_GET['mansong_id'])));
        		break;
        }
