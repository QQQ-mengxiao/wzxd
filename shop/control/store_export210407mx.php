...
ini_set('max_execution_time', '0');
    ...
    public function export_order_subOp()
    {
        ...
        if (!$_GET['curpage']) {
            if ($count > self::EXPORT_SIZE) {
                ...
                //直接下载
                $this->excel_order_sub(array_values($data));
            }
        } else {  //下载
            $limit1 = ($_GET['curpage'] - 1) * self::EXPORT_SIZE;
            $limit2 = self::EXPORT_SIZE;
    ...
    private function excel_order_sub($data_tmp)
    {
        ...
        $tableheader = array('订单号', '商品名称', '规格型号', '商品净重', '商品分类', '商品数量', '发货人姓名', '收货人姓名', '收货人地址', '', '', '收货地址', '收货人电话', '子订单号', '店铺', '买家', '订单来源', '下单时间', '	支付时间', '	完成时间	', '商品货号	', '商品单价', '	商品成本	', '单价税金', '	商品总价', '	总税金', '运费', '预存款支付金额', '充值卡支付金额	', '优惠券优惠', '实际支付金额', '订单总额', '支付方式	', '发货人姓名', '身份证号', '发货时间', '买家留言', '发货备注	', '商品模式	', '交易流水号', '订单状态', '退款金额', '备注', '运单号', '促销信息', '代金券');
        for ($i = 0; $i < count($tableheader); $i++) {
            $excel->getActiveSheet()->setCellValue("$letter[$i]1", "$tableheader[$i]");
            $excel->getActiveSheet()->getStyle("$letter[$i]1", "$tableheader[$i]")->getFont()->setBold(true);
        }
        ...
        foreach ($data_tmp as $key => $order_info) {
            unset($order_info['extend_order_goods']['kuajing_info']);
            for ($ii = 0; $ii < count($order_info['extend_order_goods']); $ii++) {
                $goods_id = $order_info['extend_order_goods'][$ii]['goods_id'];
                $goods_info = $model_goods->getGoodsInfo(array('goods_id' => $goods_id));
                $address = $order_info['extend_order_common']['reciver_info']['area'];
                ...
                $shipping_time = $model_common->getfby_order_id($order_info['order_id'], 'shipping_time');
                $order_message = $model_common->getfby_order_id($order_info['order_id'], 'order_message');
                $deliver_explain = $model_common->getfby_order_id($order_info['order_id'], 'deliver_explain');
                if ($ii == 0) {
                    if ($order_info['extend_order_goods'][$ii]['voucher_price'] > 0) {
                        $yhqzf = $order_info['extend_order_goods'][$ii]['voucher_price'];
                    } else {
                        $yhqzf = $order_info['extend_order_common']['voucher_price'];
                    }
                    if ($order_info['extend_order_goods'][$ii]['voucher_price'] > 0) {
                ...
                if ($order_info['refund_state'] == '1') {
                    $goodsid = Model('refund_return')->table('refund_return')->where(array('order_sn' => $order_info['order_sn']))->select();
                    $state = '部分退款';
                ...
                $waybill_info = unserialize($order_info['extend_order_common']['waybill_info']);
                if ($order_info['is_mode'] == 0) {
                    $waybill = $order_info['shipping_code'];
                } else {
                    if ($waybill_info) {
                        $waybill = $waybill_info['logisticsNo'];
                    } else {
                        $waybill = ' ';
                    }
                }
                ...
                if (!empty($voucher_code[0]['voucher_code'])) {
                    ...
                    foreach ($voucher as $key1 => $value1) {
                        if (!empty($value1['voucher_code'])) {
                            ...
                            $vou = $voucher_name[0]['voucher_title'];
                        } else {
                            $vou = ' ';
                        }
                    }
                ...
                $order_data[] = [
                    $ii == 0 ? $order_info['order_sn'] : ' ',
                    $order_info['extend_order_goods'][$ii]['goods_name'],
                    unserialize($goods_info['goods_spec']) ? array_values(unserialize($goods_info['goods_spec']))[0] : ' ',
                    $goods_info['goods_weight'] ? $goods_info['goods_weight'] . 'kg' : ' ',
                    Model('goods_class')->getGoodsClassInfoById($order_info['extend_order_goods'][$ii]['gc_id'])['gc_name'] ? Model('goods_class')->getGoodsClassInfoById($order_info['extend_order_goods'][$ii]['gc_id'])['gc_name'] : ' ',
                    $order_info['extend_order_goods'][$ii]['goods_num'],
                    Model('daddress')->getAddressInfo(array('address_id' => $order_info['extend_order_common']['daddress_id']))['seller_name'],
                    $order_info['extend_order_common']['reciver_name'],
                    $sheng, $shi, $qu, $jie,//省，市，区，街
                    $order_info['extend_order_common']['reciver_info']['phone'],
                    $order_info['order_sn'] . $goods_id,
                    $ii == 0 ? $order_info['store_name'] : ' ',
                    $ii == 0 ? $order_info['buyer_name'] : ' ',
                    '微信小程序',
                    date('Y-m-d H:i:s', $order_info['add_time']),
                    $order_info['payment_time'] != 0 ? date('Y-m-d H:i:s', $order_info['payment_time']) : ' ',
                    $order_info['finnshed_time'] != 0 ? date('Y-m-d H:i:s', $order_info['finnshed_time']) : ' ',
                    $goods_info['goods_serial'],
                    $order_info['extend_order_goods'][$ii]['goods_price'],
                    $order_info['extend_order_goods'][$ii]['goods_costprice'] ? $order_info['extend_order_goods'][$ii]['goods_costprice'] : Model()->table('goods_common')->getfby_goods_commonid($goods_info['goods_commonid'], 'goods_costprice'),
                    '0.00',//税金暂定0
                    number_format($order_info['extend_order_goods'][$ii]['goods_price'] * $order_info['extend_order_goods'][$ii]['goods_num'], 2),
                    '0.00',//总税金暂定0
                    $order_info['shipping_fee'],
                    $order_info['pd_amount'],
                    $order_info['rcb_amount'],
                    $yhqzf, $sjzf,
                    $ii == 0 ? $order_info['order_amount'] : ' ',
                    orderPaymentName($order_info['payment_code']),
                    ' ',//发货人姓名暂定空
                    str_replace(" ", "1", $order_info['extend_order_common']['reciver_info']['id_card']),
                    $shipping_time != 0 ? date('Y-m-d H:i:s', $shipping_time) : ' ',
                    $order_message != '' ? $order_message : ' ',
                    $deliver_explain != '' ? $deliver_explain : ' ',
                    $is_mode,
                    $ii == 0 ? explode(' ', $model_order_log->where(array('order_id' => $order_info['order_id'], 'log_msg' => array('like', '%支付平台交易号%')))->select()[0]['log_msg'])[4] : ' ',
                    $state, $refund_amount, $beizhu,
                    $ii == 0 ? $waybill : ' ',
                    $order_type,
                    $ii == 0 ? $vou : ' ',
                ];
