...
class store_promotion_mansongControl extends BaseSellerControl {
    ...
    public function mansong_saveOp() {
        ...
        $_POST['mansong_gc_id']=1;
	$mansong_gc_id = trim($_POST['mansong_gc_id']);
        ...
        if(empty($mansong_name)) {
			...
			$gc_count= $model_mansong->getMansonggcCount($_SESSION['store_id'],$mansong_gc_id);
        if ($gc_count >= 1) {
			// showDialog('此类商品已添加满即送活动，不能重复添加,请重新选择活动商品类别');
            showDialog('已添加正在进行的满即送活动，不能重复添加');
         }   //xinzeng
	...
	//xinzeng
        $param['mansong_gc_id'] = $mansong_gc_id;
        ...
        if($mansong_id) {
 		...
 		$flag=1;
                foreach($mansong_rule_array as $mansong_rule_1){
                    if($mansong_rule['price']==$mansong_rule_1['price']){
                        showDialog('同样的规则只能加一个');
                        $flag=0;
                    }
                }
                if($flag==1){
