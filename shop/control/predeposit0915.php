...
class predepositControl extends BaseMemberControl {
	...
	public function pd_log_listOp(){
		...
		// $model=Model();
        // $flag = $model->table('member')->field('cardNo')->where(array('member_id'=>$_SESSION['member_id']))->find();
     ...
     * 一卡通余额
     */
    public function card_log_listOp(){
        $model_member=Model('member');
    	$model_card=Model('card');
        // $member_info = $model_member->table('member')->where(array('member_id'=>$_SESSION['member_id']))->limit(1)->find();
        $member_info = Model()->table('member_card')->where(array('member_id'=>$_SESSION['member_id']))->limit(1)->find();
        $card_info=$model_card->getMemberCardInfo($member_info['cardno']);
        
        $condition=array();
        $condition['member_id']=$_SESSION['member_id'];
        $list=Model()->table('card_log')->where($condition)->page(20)->order('log_id desc')->select();
        Tpl::output('card_info',$card_info);
        Tpl::output('show_page',Model()->showpage());
        Tpl::output('list',$list);
        self::profile_menu('log','cardlist');
    	Tpl::showpage('member_card_log.list');
    }
	...
	private function profile_menu($menu_type,$menu_key=''){
		$menu_array	= array(
            ...
            array('menu_key' =>'cardlist',
            	'menu_name'=>'一卡通余额',
            	'menu_url' =>'index.php?act=predeposit&op=card_log_list')
		);		
		switch ($menu_type) {
			case 'rechargeinfo':
		...
		//这里要加一个判断,根据判断结果显示这个选项
		$flag=Model()->table('member_card')->field('cardno')->where(array('member_id'=>$_SESSION['member_id'],'status'=>1))->find();
		if($flag['cardno']==null){
