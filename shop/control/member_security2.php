...
class member_securityControl extends BaseMemberControl {
    ...
    public function indexOp() {
        ...
        $card_info=Model()->table('member_card')->where(array('member_id'=>$member_id))->find();
        if(!empty($card_info)){
            Tpl::output('flag',1);
        }
    ...
    public function modify_pwdOp() {
        ...
        if ($error != ''){
         ...
         $jiami = Logic('login');
		 $passwd       = $jiami->password_hash(trim($_POST['password']),PASSWORD_BCRYPT,array());
    ...
    public function modify_paypwdOp() {
        ...
        $update1 = $model_member->editMember(array('member_id'=>$_SESSION['member_id']),array('member_paypwd'=>md5($_POST['password'])));
        $result=$update1;
        //新增一卡通密码修改
        $member_id=$_SESSION['member_id'];
        
        $card=Model()->table('member_card')->where(array('member_id'=>$member_id))->find();
        if(!empty($card)){
            $data_card=array();
            $data_card['Password']=$_POST['password'];   
            $update2 = Model('card')->updateCardBase($card['cardno'],$data_card,'pwd');
            $result=($update1&&$update2);
        }       

        $message = $result ? '密码设置成功' : '密码设置失败';
        unset($_SESSION['auth_modify_paypwd']);
        showDialog($message,'index.php?act=member_security&op=index',$result ? 'succ' : 'error');
    }
    ...
    public function send_modify_mobileOp() {
        ...
        $param['verify_code'] = $verify_code;
        $message	= ncReplaceText($tpl_info['content'],$param);
        //$message = "【郑欧商城】你的店铺即将到期，请及时续期。";
		//exit(json_encode(array('state'=>'false','msg'=>$message)));
        $sms = new Sms();
        $result = $sms->send($_GET["mobile"],$message);
        if ($result) {
            exit(json_encode(array('state'=>'true','msg'=>'发送成功')));
        } else {
            exit(json_encode(array('state'=>'false','msg'=>$result)));
        }
    }
