...
class searchControl extends BaseHomeControl {
    ...
    public function indexOp() {
        ...
        $discount = $member_model->getDiscount($_SESSION['member_id']);
        //显示左侧分类
        //默认分类，从而显示相应的属性和品牌
        ...
        //$order = 'is_own_shop asc,brand_id asc,goods_id desc,goods_salenum desc';
        //$order = 'is_own_shop asc,gc_id_3 asc,brand_id asc,goods_id desc,goods_salenum desc';
        $order = 'goods_click desc,goods_salenum desc,goods_id desc';
        //$order = 'gc_id_3 asc,is_own_shop asc,brand_id asc,goods_salenum desc';
        if (in_array($_GET['key'],array('1','2','3'))) {
            $sequence = $_GET['order'] == '1' ? 'asc' : 'desc';
            $order = str_replace(array('1','2','3'), array('goods_salenum+goods_presalenum','goods_click','goods_promotion_price'), $_GET['key']);
            $order .= ' '.$sequence;
        }
		...
		$n=0;
                ...
                //20181120 隐藏商品搜索页面的会员折扣价
                $is_vip_price = Model()->table('goods_common')->getfby_goods_commonid($value['goods_commonid'],'is_vip_price');
                 if($is_vip_price ==1){
                    $goods_list[$key]['goods_vip_price'] = ncPriceFormat($value['goods_price'] * $discount);
                    $goods_list[$key]['goods_promotion_price'] = ncPriceFormat($value['goods_promotion_price']);//0502去掉*discount
                }
                $model_groupbuy=Model('groupbuy');
                $groupbuy = $model_groupbuy->getGroupbuyOnlineInfo(array('goods_id' =>$value['goods_id']));
               if(!empty($groupbuy) && is_array($groupbuy)){
            
                    $goods_list[$key]['goods_promotion_type'] = 1;
                    $goods_list[$key]['goods_promotion_price'] = ncPriceFormat($groupbuy['groupbuy_price']);//20181101 
                }
            }
        }
        ...
        foreach ($viewed_goods as $key => $value) {
            $goods_commonid = Model()->table('goods')->getfby_goods_id($value['goods_id'],'goods_commonid');
            //20181120 隐藏会员折扣价
            $is_vip_price = Model()->table('goods_common')->getfby_goods_commonid($goods_commonid,'is_vip_price');
             if($is_vip_price ==1){
                 $viewed_goods[$key]['goods_marketprice'] = ncPriceFormat($value['goods_marketprice'] * $discount);
                 $viewed_goods[$key]['goods_promotion_price'] = ncPriceFormat($value['goods_promotion_price'] * $discount);
         }
        }
    ...
    public function get_hot_goodsOp() {
        ...
        $fieldstr = "goods_id,goods_commonid,goods_name,goods_jingle,store_id,store_name,goods_price,goods_promotion_price,goods_promotion_type,goods_marketprice,goods_storage,goods_image,goods_freight,goods_salenum,color_id,evaluation_count,is_mode";
