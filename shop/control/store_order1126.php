...
class store_orderControl extends BaseSellerControl {
	...
	public function indexOp() {
        ...
        $order_list = $model_order->getOrderList2($consignee_name,$condition, 20, '*', 'order_id desc','', array('order_goods','order_common','member'));
	    //print_r($order_list);
		//break;
        //页面中显示那些操作
...
//			$order_info['order_confirm_day'] = $order_info['delay_time'] + ORDER_AUTO_RECEIVE_DAY + 15 * 24 * 3600;
     ...
     // 20181120 新增商家中心->交易订单详情中显示分销佣金信息
        $order_id = intval($_GET['order_id']);
        $order_list = Model('order')->getOrderInfo(array('order_id'=>$order_id));
        $order_sn = $order_list['order_sn'];
        $order_commission_list = Model('commission')->getCommissionList(array('order_sn'=>$order_sn));

        $sql = "SELECT a.member_id,a.member_name,a.member_truename,a.member_mobile,b.id,b.member_id,b.order_sn,b.commission,b.commission_state,b.relation,b.create_time,b.update_time from 718shop_member as a inner join 718shop_distribute_commission as b on a.member_id=b.member_id where b.order_sn=".$order_list['order_sn']." order by b.relation asc";
        $order_commission_list = Model()->query($sql);
        //var_dump($order_commission_list);die;

/*        foreach ($order_commission_list as $key => $value) {
        $member_id[] = $value['member_id'];
        }
*/
        Tpl::output('order_commission_list',$order_commission_list);
