...
class store_goods_onlineControl extends BaseSellerControl {
    ...
    public function edit_goodsOp() {
        ...
        $goods_array = $model_goods->getGoodsList($where, 'goods_id,goods_marketprice,goods_costprice,goods_price,goods_weight,goods_all_weight,goods_packages,goods_app_price,goods_storage,goods_serial,goods_storage_alarm,goods_spec,is_mode,goods_hs,goods_tax,goods_shipper_id');
        $sp_value = array();
...
//        }
    ...
    public function edit_goods_simpleOp() {
        ...
        if (chksubmit()) {
            ...
            $goods_serial = $_POST['goods_serial'];

            $goods_info = $model_goods->getGoodsInfo(array('goods_commonid'=>$common_id));
            if($goods_info){
                $update1 = array('goods_serial'=>$goods_serial);//更新goods_common
                $update2 = array('goods_serial'=>$goods_serial,'goods_storage'=>$goods_storage);//更新goods
                $result = $model_goods->editProduces(array('goods_commonid'=>$common_id),$update1,$update2);
                if($result){
                    //$this->recordSellerLog('修改商品货号、库存，平台货号：'.$common_id);
					$this->recordSellerLog('修改商品货号、库存，平台货号：'.$common_id.'。库存修改前为'.$goods_info['goods_storage'].'，修改后为'.$goods_storage.'。');
                    showDialog(L('nc_common_op_succ'), 'reload', 'succ');
                }else{
        ...
        //查询商品库存和货号信息
        $goods_info = $model_goods->getGoodsInfo(array('goods_commonid'=>$common_id),'goods_storage,goods_serial');
...
//                            $file_name = $goods_name . iconv('utf-8', 'gbk//IGNORE', $vv);
            ...
            return $model_zipfile->download($img);
        }
    }
