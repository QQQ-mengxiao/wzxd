...
class buyControl extends BaseBuyControl {
    ...
    public function buy_step1Op() {
        ...
        //新增抢购的运费使其为零
         if(is_array($result['store_cart_list'][$result['store_id']] ) && !empty($result['store_cart_list'][$result['store_id']])){
            $result['freight_list']='';
            foreach ($result['store_cart_list'][$result['store_id']] as $k => $v) {
               if(!empty($v['groupbuy_info'])){
                   $result['store_cart_list'][$result['store_id']][$k]['transport_id']=0;
                   $result['store_cart_list'][$result['store_id']][$k]['goods_freight']=0;
                }
            }
        }
        ...
        //显示一卡通支付
           if(!empty($result['cardInfo'])){
            if($result['store_id']==31|| $result['store_id']==2){
        ...
        if($pay_info) {
            ...
            foreach ($info as $k => $value) {
               ...
               // }
                ...
                // var_dump($DeliverAddr1);
                     $return = preg_replace('#\s+#', ' ',trim($DeliverAddr1));
               $arr_str=explode(" ",$return);
                 ...
                 // var_dump($str_reverse);
                   //发货人所在省
                     $prov =$str_reverse[0];
                  //发货人所在市
                $scity = trim($str_reverse[2]);
               }
                  // var_dump($scity);die;
                    $query = "select * from subshop where OrgName='$scity'";
        ...
        $model_inv = Model('invoice');
     	if (chksubmit()){
            ...
            if ($_POST['invoice_type'] == 1) {
                ...
                $data['inv_title'] = $_POST['inv_title_select'] == 'person' ? '个人' : '单位';
                $data['inv_content'] = $_POST['inv_content'];
                $data['inv_company'] = $_POST['inv_title'];
                $data['inv_code'] = $_POST['inv_title2'];
            } else {
                $data['inv_state'] = 2;
