...
class store_export_member_zitiControl extends BaseSellerControl
{
    ...
    {
        //支付时间
        $if_start_time = preg_match('/^20\d{2}-\d{2}-\d{2}$/', $_GET['query_start_date_pay']);
        $if_end_time = preg_match('/^20\d{2}-\d{2}-\d{2}$/', $_GET['query_end_date_pay']);
        $start_unixtime = $if_start_time ? strtotime($_GET['query_start_date_pay']) : null;
        $end_unixtime = $if_end_time ? strtotime($_GET['query_end_date_pay']) + 86399 : null;
        $stime = date('Ymd', $start_unixtime);
        $ltime = ($end_unixtime - $start_unixtime + 1) / 86400;
        $model_daddress = Model('ziti_address');
        ...
        $sql = "
        SELECT
        A.DAY AS days,A.count,A.amount,A.cost,A.profit,A.profit_rate,A.remark,B.sqamount,'".$seller_name."' as seller_name
    FROM
        (
            SELECT
                A.days AS days,
                A.count AS count,
                A.goods_pay_price AS amount,
                A.goods_cost_price AS cost,
                (
                    A.goods_pay_price - A.goods_cost_price
                ) AS profit,
                concat(
                    format(
                        (
                            A.goods_pay_price - A.goods_cost_price
                        ) / A.goods_pay_price * 100,
                        2
                    ),
                    '%'
                ) AS profit_rate,
                A.voucher_price AS remark,day_list.DAY as DAY
            FROM
                (
                    SELECT
                        FROM_UNIXTIME(o.payment_time, '%Y-%m-%d') AS days,
                        count(DISTINCT o.order_id) AS count,
                        sum(og2.goods_pay_price) AS goods_pay_price,
                        sum(
                            og2.goods_cost_price * og2.goods_num
                        ) AS goods_cost_price,
                        sum(og2.voucher_price) AS voucher_price
                    FROM
                        718shop_order_goods og2
                    LEFT JOIN 718shop_order o ON o.order_id = og2.order_id
                    WHERE
                        o.payment_time BETWEEN " . $start_unixtime . "
                    AND " . $end_unixtime . "
                    AND o.order_state > 0
                    AND o.is_zorder <> 0
                    AND o.refund_state <> 2
                    AND og2.deliverer_id != 90
                    AND og2.rec_id NOT IN (
                        SELECT
                            og3.rec_id
                        FROM
                            718shop_order_goods og3
                        LEFT JOIN 718shop_refund_return rr1 ON og3.order_id = rr1.order_id
                        AND og3.goods_id = rr1.goods_id
                        WHERE
                            rr1.refund_state = 3
                        AND rr1.seller_state = 2
                    )
                    GROUP BY
                        days
                ) A
            RIGHT JOIN (
                ...
                FROM
                    (
                        SELECT
                            @date := DATE_ADD('" . $stime . "', INTERVAL - 1 DAY)
                        FROM
                            718shop_order
                    ) days
                LIMIT " . $ltime . "
            ) day_list ON day_list. DAY = A.days
        ) A
    LEFT JOIN (
        SELECT
            A.days AS days,
            A.goods_pay_price AS sqamount
        FROM
            (
                SELECT
                    FROM_UNIXTIME(o.payment_time, '%Y-%m-%d') AS days,
        ...
        if (!$condition) {
            foreach ($address_list as $k => $v) {
                ...
                A.DAY AS days,A.count,A.amount,A.cost,A.profit,A.profit_rate,A.remark,B.sqamount,'".$v['seller_name']."' as seller_name
            FROM
                (
                    SELECT
                        A.days AS days,
                    ...
                    FROM
                        (
                            ...
                            FROM
                                718shop_order_goods og2
                            LEFT JOIN 718shop_order o ON o.order_id = og2.order_id
                            LEFT JOIN 718shop_order_common oc ON oc.order_id = og2.order_id
                            WHERE
                                o.payment_time BETWEEN " . $start_unixtime . "
                            AND " . $end_unixtime . "
                            AND o.order_state > 0
                            ...
                            AND o.refund_state <> 2
                            AND og2.deliverer_id != 90
                            AND oc.reciver_ziti_id = ".$v['address_id']."
                            AND og2.rec_id NOT IN (
                                SELECT
                                    og3.rec_id
                                FROM
