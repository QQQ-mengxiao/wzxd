...
//        $xianshi_goods_list = $model_xianshi_goods->getXianshiGoodsExtendList($condition,5);
        $xianshi_goods_list = $model_xianshi_goods->getXianshiGoodsExtendList($condition,5,'goods_sort,xianshi_goods_id','','',$_GET['goods_serial']);
    ...
    public function xianshi_goods_addOp() {
        ...
        $condition['xianshi_id'] = array('neq', $xianshi_id);
        $condition['goods_id'] = $goods_id;
        $sql = "SELECT t.* FROM (SELECT spxg.xianshi_id,spxg.start_time,spxg.end_time,spxg.xianshi_name FROM 718shop_p_xianshi_goods spxg LEFT JOIN 718shop_p_xianshi spx ON spxg.xianshi_id=spx.xianshi_id WHERE (spxg.end_time> ".TIMESTAMP." AND spxg.goods_id=".$goods_id." AND spx.xianshi_type=1) or spxg.xianshi_id=".$xianshi_id.") t GROUP BY t.xianshi_id ORDER BY t.start_time";
        // die(json_encode(array('result'=>false,'message'=>$sql),320));
        ...
        if($xianshi_goods){
            foreach ($xianshi_goods as $k => $v){
                ...
                if ($k > 0 && $xianshi_goods[$k]['start_time'] < $xianshi_goods[$k-1]['end_time']){
                    die(json_encode(array('result'=>false,'message'=>'该商品已经参加了<'.$xianshi_goods[$k]['xianshi_name'].'>活动，活动时间重叠，不可添加！'),320));
                }
            }
        }
        //添加到活动商品表
