...
class store_goods_addControl extends BaseSellerControl {
    ...
    public function add_step_twoOp() {
        ...
        $commis_rate = Model()->table('commission')->getfby_gc_id($goods_class['gc_id_1'],'commis_rate');
        Tpl::output('goods', ['commis_rate'=>$commis_rate]);
        ...
        if($address_id >0){
            $deliverer=Model('daddress')->where(array('store_id'=>$_SESSION ['store_id'],'address_id'=>$address_id))->select();
        }else{
    ...
    public function save_goodsOp() {
        ...
        if (chksubmit()) {
           ...
           if(!$_POST ['deliverer_id']){ 
            ...
            if($_POST['points_send']){
            $common_array['points_send']         = $_POST['points_send'];
            }else{
                 $common_array['points_send']         = '0';
            }
            ...
            $common_array['is_vip_price']       = intval($_POST['is_vip_price']);//MX   可以取到值的 没有问题
            ...
            //佣金比例
            $common_array['commis_rate']        = $_POST['commis_rate'];
            ...
            $common_array['goods_weight']        = floatval($_POST['g_weight']);
            // $common_array['goods_all_weight']    = floatval($_POST['g_all_weight']);
            $common_array['goods_packages']        = intval($_POST['g_packages']);
            $common_array['deliverer_id']= intval($_POST["deliverer_id"]);
 		...
 		require_once(BASE_RESOURCE_PATH.DS.'phpqrcode'.DS.'index.php');
                ...
                // 商品规格
                if (is_array($_POST['spec'])) {
                    $_POST['spec'] = $this->multi_array_sort($_POST['spec'] , 'color');
                    foreach ($_POST['spec'] as $value) {
                        $goods = array();
                        $goods['goods_commonid']    = $common_id;
                        //佣金比例
                        $goods['commis_rate']        = $common_array['commis_rate'];
                        $goods['goods_kuajingD_id'] = $kuajing_id;            
                        $goods['goods_name']        = $common_array['goods_name'] . ' ' . implode(' ', $value['sp_value']);
                         $goods['points_send']        = $common_array['points_send'];
                        $goods['goods_jingle']      = $common_array['goods_jingle'];
                        $goods['is_cw']       = intval($_POST['is_cw']);
                        $goods['is_jin']       = intval($_POST['is_jin']);
                        ...
                        $goods['goods_costprice']=$value['costprice'] == 0 ? $common_array['goods_costprice'] : $value['costprice'];
                        ...
                        $goods['deliverer_id']     = $common_array['deliverer_id'];
                         //销售规格、选择单位
                        $goods['xiao_gui']     = intval($value['xiao_gui']);
                        $goods['danwei_id']     = intval($value['ladder1']);
                        $goods['wu_gui'] =  intval($value['wu_gui']);

                        //商品阶梯价格
                        $p_ladder_id = $value['ladder'];
                        $goods['p_ladder_id'] = $p_ladder_id;
						if($p_ladder_id>0){
							$goods['is_group_ladder'] = 1;
						}
						if(!empty($_POST['hui_discount'])){
							$goods['hui_discount']       = $_POST['hui_discount'];
						}else{
							$goods['hui_discount'] =1;
						}
                        ...
                        // jinp170608 商品净重、毛重、件数
                        $goods['goods_weight']       = $value['weight'];
                        // $goods['goods_all_weight']       = $value['all_weight'];
                        $goods['goods_packages']       = $value['packages'];
                        $goods['is_deleted']       = 0;

                        $goods_id = $model_goods->addGoods($goods);
                          //slkadd多活动商品表存储之会员价
                        if($common_array['is_vip_price']==1){
                            $model_goodspromotion=Model('goods_promotion'); 
                            $model_goodspromotion->addgoods_promotion_vip($goods_id,$goods['goods_price'],$goods['hui_discount']);
                        }
                        if($goods['is_group_ladder'] == 1){
                             $model_goodspromotion=Model('goods_promotion'); 
                            $array_goodspromotion=array();
                            $array_goodspromotion['goods_id']=$goods_id;
                            $array_goodspromotion['promotion_type']=60;
                            $array_goodspromotion['price']=$goods['goods_price'] ;
			 ...
			 // 生成商品二维码
                    ...
                    $goods['goods_barcode']      = $_POST['g_barcode'];
                    ...
                    $goods['deliverer_id']     = $common_array['deliverer_id'];
                    //销售规格、最小单位
                    $goods['xiao_gui'] =  intval($_POST['xiao_gui']);
                    $goods['wu_gui'] =  intval($_POST['wu_gui']);
                    $goods['danwei_id'] =  intval($_POST['ladder1']);
                    //商品阶梯价格
                    ...
                    // $goods['goods_all_weight']       = $common_array['goods_all_weight'];
                    $goods['goods_packages']        = $common_array['goods_packages'];
                        $goods['is_deleted']       = 0;
