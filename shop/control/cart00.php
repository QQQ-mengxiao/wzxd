...
class cartControl extends BaseBuyControl {
	...
	public function indexOp() {
        ...
        foreach ($cart_list as $cart) {
            ...
            PC::debug($cart['goods_num']);
            //组合套装读取税金，一般商品直接算税率
            if($cart['bl_id'] == 0) {
            	$goods_tax_rate = $model_goods->getfby_goods_id($cart['goods_id'],'goods_tax_rate');
	...
	public function updateOp() {
		...
		if ($update) {
			...
			$return['subtotal_tax'] = $goods_info['goods_tax_rate'] * $return['subtotal'];//* $quantity;
