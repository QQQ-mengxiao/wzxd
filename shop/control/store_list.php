...
class store_listControl extends BaseHomeControl {
	...
	public function indexOp(){
        ...
        $discount = $model_member->getDiscount($_SESSION['member_id']);
        ...
        foreach ($store_list as $key => $value) {
            foreach ($value['search_list_goods'] as $k=>$v) {
                $goods_id = $v['goods_id'];
                $goods_commonid = Model()->table('goods')->getfby_goods_id($goods_id,'goods_commonid');
               // 20181120 隐藏店铺列表中商品的会员折扣价
                $is_vip_price = Model()->table('goods_common')->getfby_goods_commonid($goods_commonid,'is_vip_price');
                if($is_vip_price ==1 ){
                     $store_list[$key]['search_list_goods'][$k]['goods_price'] = ncPriceFormat($v['goods_price'] * $discount);
                 }
            }
        }
        //信用度排序
