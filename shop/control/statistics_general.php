...
class statistics_generalControl extends BaseSellerControl {
	 ...
	 * 数据分析
	 */
     public function general1Op(){
       header("location:http://117.159.3.238:8030/");
    }
	/**
	 * 促销分析
	 */
	public function generalOp(){
		...
		if($j==0){
		$where['order_from']= array(in,array(1,2,3,4));
		}else{$where['order_from']= $j;}
		...
		if($j!=0){
		Tpl::output('statnew_arr'.$j,$statnew_arr);
		}else{
	    ...
	    //Tpl::output('statnew_arr0',$statnew_arr);
