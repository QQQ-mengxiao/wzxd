...
/*        public function commissionOp() {
        $model_commission = Model('bill');
        $condition = array();
        $condition['store_id'] = $_SESSION['store_id'];
        if (preg_match('/^20\d{5,12}$/',$_GET['member_id'])) {
            $condition['member_id'] = $_GET['member_id'];
        }
        $bill_list = $model_bill->get_ComBillList($condition,'*',12,'ob_state asc,ob_no asc');
        Tpl::output('bill_list',$bill_list);
        Tpl::output('show_page',$model_bill->showpage());
        $this->profile_menu('list','list');
        Tpl::showpage('store_bill.index');
	...
	public function show_billOp(){
		...
		if ($_GET['type'] =='refund'){
		    ...
		    $model_order = Model('order');
		    $model_bill = Model('bill');
		    $model_commission = Model('commission');
		    $order_list = $model_order->getOrderList($order_condition,20);

			$order_condition1 = array();
			$order_condition1['order_sn'] = $order_list['order_sn'];
		    $order_info = $model_commission->getPdRechargeList($order_condition1,array());
		    ...
		    foreach ($order_list as $k=>$order_info) {
                ...
                $distribute_commission_condition['order_sn'] = $order_sn;
                ...
                $order_comminssion_list = $model_commission->getPdRechargeList($distribute_commission_condition);
                if($order_comminssion_list){
                foreach ($order_comminssion_list as $value) {
                	 $order_commission += $value['commission'];
                }
            }else{
            	$order_commission = 0;
            }
                $order_list[$k]['order_commission'] = $order_commission;
                // $order_list[$k]['commission_totals'] += $order_goods_info['commission'];
            }
      ...
      //           $order_info['commission_totals'] += $order_goods_info['commission'];
      //       }else{
      //           // $commission_totals = 0;
      //           $order_info['commission_totals'] = 0;
      //       }
		    ...
		    Tpl::output('order_commission',$order_commission);
	...
	public function export_orderOp(){
		...
		$export_data = array();
		$export_data[0] = array('订单编号','订单金额','运费','佣金','分销佣金','下单日期','成交日期','买家','买家编号','商品');
		$order_totals = 0;
		...
		foreach ($data as $v) {
		    ...
		    $order_commission += $export_data[$k+1][] = $order_list['order_commission'];
		...
		$export_data[$count][] = $order_commission;
		$csv = new Csv();
