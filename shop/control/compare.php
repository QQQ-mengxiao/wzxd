...
class compareControl extends BaseHomeControl{
    ...
    public function indexOp() {
        ...
        if($gid_arr){
            ...
            for ($j=0; $j<self::MAXNUM; $j++){
               // 20181120 隐藏商品价格对比中的会员折扣价
                 $is_vip_price = Model()->table('goods_common')->getfby_goods_commonid($compare_listtmp[$j]['goods_commonid'],'is_vip_price');
                 if($is_vip_price == 1){
                     $compare_list[$i][] = ncPriceFormat(($compare_listtmp[$j]?$compare_listtmp[$j]['goods_promotion_price']:'') * $discount);
                 }else{
    ...
    public function showcompareOp(){
        ...
        if($gid_arr){
            ...
            if ($goods_listtmp){
                foreach ($goods_listtmp as $k=>$v){
                    $goods_list[$v['goods_id']] = $v;
                    //20181120 隐藏商品对比中商品的会员折扣价
                     $is_vip_price = Model()->table('goods_common')->getfby_goods_commonid($v['goods_commonid'],'is_vip_price');
                     if($is_vip_price == 1){
                         $goods_list[$v['goods_id']]['goods_promotion_price'] = ncPriceFormat($v['goods_promotion_price'] * $discount);
                     }
                }
