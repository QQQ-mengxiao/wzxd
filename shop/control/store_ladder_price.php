...
// var_dump($rule_list);die;
    ...
    public function mansong_delOp() {
        ...
        if($result) {
            ...
            $model_goods->editGoods( array('is_group_ladder' =>0,'p_ladder_id'=>0), array('p_ladder_id' => $mansong_id,'is_group_ladder'=>1));
   ...
   public function default_setOp() {
    ...
    public function ladder_manageOp() {
        $model = Model('goods');
        $p_ladder_id = $_GET['p_ladder_id'];
        $condition = array();
        $condition['is_group_ladder'] = 1;
        $condition['store_id'] = $_SESSION['store_id'];
        $condition['p_ladder_id'] = $p_ladder_id;
        ...
        $goods_list = $model->getGoodsOnlineList($condition, '*', 5, '-ladder_sort desc', 0, '');
        foreach ($goods_list as $key => $value) {
        if (is_null($value['ladder_sort'])) {
            $goods_list[$key]['ladder_sort'] = 9999;
        }
    ...
    public function ladder_goods_addOp() {
        ...
        $ladder_sort = intval($_POST['ladder_sort']);
        $p_ladder_id = intval($_POST['p_ladder_id']);
        $model_goods = Model('goods');

        $data = array();
        $data['result'] = true;

        $ladder_info = $model_goods->editGoods(array('is_group_ladder' => 1, 'ladder_sort' => $ladder_sort, 'p_ladder_id' => $p_ladder_id), array('goods_id' => $goods_id));
        if (!$ladder_info) {
            $data['result'] = false;
         ...
         $data['message'] = '添加成功';
        $data['xianshi_goods'] = $model_goods->getGoodsInfoByID($goods_id);;
    ...
    public function ladder_goods_deleteOp() {
       $goods_id = intval($_POST['xianshi_goods_id']);
        ...
        $xianshi_info = $model_goods->editGoods(array('is_group_ladder' => 0, 'p_ladder_id' => 0, 'ladder_sort' => 0), array('goods_id' => $goods_id));
        if(!$xianshi_info) {
            $data['result'] = false;
            $data['message'] = L('param_error');
            echo json_encode($data);die;
    ...
    public function goods_sortOp(){
        ...
        $model_goods = Model('goods');
        $goods_sort = $model_goods->getGoodsInfo(array('goods_id'=>$goods_id,'is_group_ladder'=>1),'ladder_sort');
        $update = $model_goods->editGoods(array('ladder_sort' => $_POST['goods_sort']), array('goods_id' => $goods_id,'is_group_ladder'=>1));
        $this->recordSellerLog('修改阶梯价活动商品排序，id：' . $goods_id . '修改之前为' . $goods_sort['ladder_sort'] . '修改之后为' . $_POST['goods_sort']);
        if ($update) {
    ...
    private function profile_menu($menu_key='') {
        ...
        switch ($menu_key){
            ...
            case 'mansong_detail':
                ...
                break;
            case 'ladder_manage':
                $menu_array[] = array('menu_key' => 'ladder_manage', 'menu_name' => '商品管理', 'menu_url' => urlShop('store_ladder_price', 'ladder_manage', array('mansong_id' => $_GET['mansong_id'])));
                break;
        }
