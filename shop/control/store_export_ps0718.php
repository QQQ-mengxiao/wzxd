...
class store_export_psControl extends BaseSellerControl
{
        ...
        foreach($data as $kk=>$vv){
            ...
            if($vv['is_zorder']==0 ){
             ...
             foreach ($vv['extend_order_goods'] as $key => $value) { 
                ...
                if($_GET['class_type']>0){
                     if($_GET['class_type']==1){
                       if(!in_array($value['gc_id'], $guoshu_class)){
                                 unset($data[$kk]['extend_order_goods'][$key]);
                            }  
                      }else{
                        if(in_array($value['gc_id'], $guoshu_class)){
    ...
    private function excel_order($data_excel){  
       ...
       $time=date('Y-m-d',$data_excel['time1']);
        ...
        $excel->setActiveSheetIndex(0)
                ...
                ->setCellValue('D2', '一级分类')
                ->setCellValue('E2', '二级分类')
                 ->setCellValue('F2', '商品条码')
                  ->setCellValue('G2', '商品货号')
                      ->setCellValue('H2', '商品总成本价')
    ...
    {   $type=1;
       // var_dump($data_tmp);die;
        ...
        $tableheader = array('订单号', '收货人', '发货人', '商品数量', '商品名称', '一级分类', '二级分类', '收货人电话', '自提点', '买家','支付时间','完成时间','送货时间','买家留言','发货备注','订单状态','备注(退款信息)','促销信息','商品总成本');
        for ($i = 0; $i < count($tableheader); $i++) {
            $excel->getActiveSheet()->setCellValue("$letter[$i]1", "$tableheader[$i]");
            $excel->getActiveSheet()->getStyle("$letter[$i]1", "$tableheader[$i]")->getFont()->setBold(true);
        }
