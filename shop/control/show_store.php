...
class show_storeControl extends BaseStoreControl {
	...
	public function indexOp(){
        if(!$this->store_decoration_only) {
            ...
            $fieldstr = "goods_id,goods_commonid,goods_name,goods_jingle,store_id,store_name,goods_price,goods_promotion_price,goods_marketprice,goods_storage,goods_image,goods_freight,goods_salenum,goods_presalenum,color_id,evaluation_good_star,evaluation_count,goods_promotion_type";
            //得到最新12个商品列表
    ...
    private function getGoodsMore($goods_list1, $goods_list2 = array()) {
        ...
        if (!empty($goods_list)) {
            ...
            foreach ($goods_list1 as $key => $value) {
                ...
                //20181120 隐藏店铺商品中的会员折扣价
                $is_vip_price = Model()->table('goods_common')->getfby_goods_commonid($value['goods_commonid'], 'is_vip_price');
                 if($is_vip_price == 1) {
                     $goods_list1[$key]['goods_price'] = ncPriceFormat($value['goods_price'] * $discount);
                     $goods_list1[$key]['goods_promotion_price'] = ncPriceFormat($value['goods_promotion_price'] * $discount);
                 }
            }
            ...
            if (!empty($goods_list2)) {
                foreach ($goods_list2 as $key => $value) {
                    ...
                    $is_vip_price = Model()->table('goods_common')->getfby_goods_commonid($value['goods_commonid'], 'is_vip_price');
                    if($is_vip_price == 1) {
                         $goods_list2[$key]['goods_price'] = ncPriceFormat($value['goods_price'] * $discount);
                         $goods_list2[$key]['goods_promotion_price'] = ncPriceFormat($value['goods_promotion_price'] * $discount);
                     }
                }
            }
        }
