	...
	public function indexOp() {
		...
		if (!empty($order_info['extend_order_goods'])){
			...
			foreach ($order_info['extend_order_goods'] as $k => $v){
				...
				$v['goods_serial'] = Model('goods')->getGoodsInfo(array('goods_id'=>$v['goods_id']),'goods_serial')['goods_serial'];
		...
		//运单号
        if($order_info['is_mode']==0){//一般贸易
            Tpl::output('logisticsNo',$order_info['shipping_code']);
        }elseif($order_info['is_mode']==2) {//集货
            Tpl::output('logisticsNo',$order_info['extend_order_common']['waybill_info']['logisticsNo']);
        }
