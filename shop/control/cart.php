...
class cartControl extends BaseBuyControl {
	...
	public function indexOp() {
        ...
        foreach ($cart_list as $cart) {
            ...
            //组合套装读取税金，一般商品直接算税率
            if($cart['bl_id'] == 0) {
            	$goods_tax_rate = Model()->table('goods')->getfby_goods_id($cart['goods_id'],'goods_tax_rate');
            	$cart['goods_tax_total'] = ncPriceFormat($goods_tax_rate * $cart['goods_total']);
            } else {
            	$cart['goods_tax_total'] = ncPriceFormat($cart['goods_tax'] * $cart['goods_num']);
            }
            //$cart['goods_tax_total'] = ncPriceFormat($cart['goods_tax'] * $cart['goods_num']);
        ...
        //取得店铺级活动 - 可用的满即送活动 //xingai
	    list($store_agc_total) = $logic_buy_1->calcCartGCList($store_cart_list);
		list($store_premiums_list,$store_mansong_rule_list) = $logic_buy_1->getMansongRuleCartgcListByTotal($store_agc_total);
	    Tpl::output('mansong_rule_list',$store_mansong_rule_list);
		//商品分类id
		$gc_list=rkcache('gc_list1',true);
        Tpl::output('gc_list', $gc_list);
	   //xinzeng
		//$mansong_rule_list = $logic_buy_1->getMansonggcRuleList($store_cart_list);	    
	...
	public function addOp() {
        ...
        if (is_numeric($_GET['goods_id'])) {
            ...
            $logic_buy_1->getDiscount($goods_info);
            ...
            $bl_amount = 0;
            $bl_tax_amount = 0; //组合套装税金总计
            $bl_is_mode = 0;
            $bl_goods_shipper_id = 0;
            foreach ($bl_goods_list as $goods) {
            	$goods_id_array[] = $goods['goods_id'];
            	$bl_amount += $goods['bl_goods_price'];
            	$bl_tax_rate = $model_goods->getfby_goods_id($goods['goods_id'],'goods_tax_rate');
            	$bl_tax_amount += $goods['bl_goods_price'] * $bl_tax_rate;
            	//$bl_tax_amount += $model_goods->getfby_goods_id($goods['goods_id'],'goods_tax');
            	$bl_is_mode =  $model_goods->getfby_goods_id($goods['goods_id'],'is_mode');
            	$bl_goods_shipper_id =  $model_goods->getfby_goods_id($goods['goods_id'],'goods_shipper_id');
            }
            ...
            //跨境商品支持组合套装
            $goods_info['is_mode'] = $bl_is_mode;
            $goods_info['goods_tax'] = $bl_tax_amount;
            $goods_info['goods_shipper_id'] = $bl_goods_shipper_id;
            $quantity = 1;
        }
	...
	public function add_combOp() {
        ...
        foreach ($goods_list as $goods_info) {
            ...
            $cart_info['is_mode'] = $goods_info['is_mode'];
            $cart_info['goods_all_weight'] = $goods_info['goods_all_weight'];
            $cart_info['goods_shipper_id'] = $goods_info['goods_shipper_id'];
            $quantity = 1;
    	    //已登录状态，存入数据库,未登录时，存入COOKIE
    	    if($_SESSION['member_id']) {
    	        $save_type = 'db';
	...
	public function updateOp() {
		...
		if ($cart_info['bl_id'] == '0') {
		    ...
		    if($goods_info['ifgroupbuy'] || $goods_info['ifxianshi']) {
                //折扣
                $logic_buy_1->getDiscount($goods_info);
            }
		    ...
		    $goods_info['goods_tax'] = $cart_info['goods_tax'];  //修正组合套装购物车税金显示（js显示）
		}

		$data = array();
        $data['goods_num'] = $quantity;
        $data['goods_price'] = $goods_info['goods_price'];//最终商品价格
		...
		if ($update) {
			...
			$return['subtotal_tax'] = ncPriceFormat($goods_info['goods_tax_rate'] * $return['subtotal']);//* $quantity;
			//$return['subtotal_tax'] = $goods_info['goods_tax'] * $quantity;
			$return['goods_price'] = $goods_info['goods_price'];
