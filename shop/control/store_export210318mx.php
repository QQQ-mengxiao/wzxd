    ...
    private function createExcel_sub($data = array(),$goods_name,$goods_serial){
        ...
        foreach ((array)$data as $k=>$v){
            ...
            foreach ((array)$v['extend_order_goods'] as $k1=>$v1){
                ...
                $tmp[] = array('data'=>$goods_weight[0]['goods_weight']?$goods_weight[0]['goods_weight'].'kg':'');
                //商品分类
                $gcid=$v1['gc_id'];
                ...
                $goods_costprice = Model()->table('goods')->getfby_goods_id($v1['goods_id'],'goods_costprice');
                $goods_costprice_c = Model()->table('goods_common')->getfby_goods_commonid($goods_common_id,'goods_costprice');
                //$tmp[] = array('data'=>$goods_costprice);
				$tmp[] = array('data'=>$goods_costprice?$goods_costprice:$goods_costprice_c);
                //单价税金
                ...
                if($ii==1){
                    ...
                    // else {$voucher_tmp = '0.00';}
                    // $tmp[] = array('data'=>$voucher_tmp);
                    if($v1['voucher_price']>0){
                        //优惠券
                         $tmp[] = array('data'=>$v1['voucher_price']);
                    }else{
                         $tmp[] = array('data'=>$v['extend_order_common']['voucher_price']);
                    }

                    //实际支付金额
                    if($v1['voucher_price']>0){
                         $tmp[] = array('data'=>number_format($v1['goods_price']*$v1['goods_num']-$v1['voucher_price'],2));
                    }else{
                         $tmp[] = array('data'=>number_format($v1['goods_price']*$v1['goods_num'],2));
                    }
                     //实际支付金额
