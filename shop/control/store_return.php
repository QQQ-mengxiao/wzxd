...
class store_returnControl extends BaseSellerControl {
	...
	private function profile_menu($menu_type,$menu_key='') {
		...
		switch ($menu_type) {
			case 'return':
				$menu_array = array(
					//array('menu_key'=>'2','menu_name'=>'PC端退货',	'menu_url'=>'index.php?act=store_return&lock=2'),
					array('menu_key'=>'2','menu_name'=>'退货记录','menu_url'=>'index.php?act=store_return&lock=1')
				);
				break;
		}
    ...
    //退货记录导出
    public function export_returnOp(){
        $lang   = Language::getLangContent();
        $model_return = Model('refund_return');
        $condition = array();
        $condition['store_name'] = $_SESSION['store_name'];
        ...
        if (!is_numeric($_GET['curpage'])){
            $count = $model_return->getRefundReturn($condition);
            $array = array();
            /* echo "<pre>";
             var_dump($count);
             echo "</pre>";
            ...
            $data = $model_return->getReturnList($condition,$count);
    ...
    private function createExcel($data = array()){
        ...
        $excel_data[0][] = array('styleid'=>'s_title','data'=>'退货编号');
        ...
        foreach ((array)$data as $k=>$v){
            ...
            $tmp[] = array('data'=>$v['goods_num']);//退货数量
        ...
        $excel_obj->addWorksheet($excel_obj->charset('退货记录单',CHARSET));
        $excel_obj->generateXML($excel_obj->charset('退货记录单',CHARSET).$_GET['curpage'].'-'.date('Y-m-d-H',time()));
    }
}
