...
ini_set('max_execution_time', '0');
    ...
    {
		Model()->table('order_sub_export')->where(array('id'=>array('gt',0)))->delete();//var_dump($a);die;
        $model_order = Model('order');
        $condition = array();
        if ($_GET['order_sn']) {
            $condition['order.order_sn'] = $_GET['order_sn'];
...
//        QueueClient::push('excel_order_sub_insert', $data);die;
        $userCount = count($data);
        for($i = 0; $i < ceil($userCount/1000); $i++) {
            $begin    = $i * 1000;
//            $end      = ($i + 1) * 500;
//            $userList = D('model')->field('key_name')->limit($begin, $end)->select();
            $userList = array_slice($data,$begin,1000);//var_dump($userList);
            $this->excel_order_sub_insert($userList);
//            QueueClient::push('excel_order_sub_insert', $userList);//die;
        }
//        Tpl::showpage('store_export_sub');
        ...
        $order_data = Model()->table('order_sub_export')->where(array('id'=>array('gt',0)))->field('order_sn,z_order_sn,goods_name,goods_spec,goods_weight,goods_class1,goods_class,goods_num,storage,reciver_name,reciver_address1,reciver_address2,reciver_address3,recive_address,reciver_phone,order_sub_id,store,buyer,order_from,add_time,pay_time,complete_time,ziti_time,goods_serial,goods_barcode,goods_price,goods_costprice,goods_costall,goods_tax,goods_priceall,goods_taxall,deliver_fee,pd_amount,rcb_amount,voucher_price,pay_amount,order_amount,pay_type,deliver_name,id_card,deliver_time,order_message,deliver_explain,is_mode,pay_sn,order_state,refund_amount,refund_time,seller_state,admin_state,seller_message,admin_message,buyer_message,beizhu,waybill,order_type,voucher,share_name,company_name,commis_rate')->order('id asc')->limit('20000')->select();
//        echo '<pre>';var_dump($order_data);die;
        for ($i = 2; $i <= count($order_data) + 1; $i++) {
            $j = 0;
            foreach ($order_data[$i - 2] as $key => $value) {
                $excel->getActiveSheet()->setCellValue("$letter[$j]$i", "$value");
    ...
    private function excel_order_sub_insert($data){
        $model_order_log = Model('order_log');
        set_time_limit(0); //执行时间无限
        ini_set('memory_limit', '-1'); //内存无限
        $num = 0;
...
//                $order_data[] = [
                ...
                Model()->table('order_sub_export')->insert($order_datas);
                unset($order_datas);
            }
            unset($data[$key]);
        }
