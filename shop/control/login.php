...
class loginControl extends BaseHomeControl {
	...
	public function indexOp(){
		...
		if ($result !== false){
           ...
           // $abc = $model_member->l_login($password,$condition);
            $jiami = Logic('login');
            $abc = $jiami->password_verify($password,$model_member->getfby_member_name($user_name,'member_passwd'));
            if ($abc==1) {
            ...
            if(empty($member_info) && preg_match('/^0?(13|15|17|18|14)[0-9]{9}$/i', $user_name)) {//根据会员名没找到时查手机号
                ...
                //$condition['member_passwd'] = md5($password);
                $abc = $jiami->password_verify($password,$model_member->getfby_member_mobile($user_name,'member_passwd'));
            	if ($abc==1) {
            ...
            if(empty($member_info) && (strpos($user_name, '@') > 0)) {//按邮箱和密码查询会员
                ...
                //$condition['member_passwd'] = md5($password);
                $abc = $jiami->password_verify($password,$model_member->getfby_member_email($user_name,'member_passwd'));
            	if ($abc==1) {
                $member_info = $model_member->getMemberInfo($condition);
            	}
            }
			...
			if ($_pic[0] != ''){
				...
				Tpl::output('lpic',UPLOAD_SITE_URL.'/'.ATTACH_LOGIN.'/'.'1.jpg');
			}else{
	 ...
	 * @return
	 */
	public function register_mailOp() {
		Language::read("home_login_register");
		$lang	= Language::getLangContent();
		$model_member	= Model('member');
		$model_member->checkloginMember();
		Tpl::output('html_title',C('site_name').' - '.$lang['login_register_join_us']);
		Tpl::showpage('register_mail');
	}
	...
	public function find_passwordOp(){
		...
		$jiami = Logic('login');
		 $passwd       = $jiami->password_hash(trim($new_password),PASSWORD_BCRYPT,array());
		
		if(!($member_model->editMember(array('member_id'=>$member['member_id']),array('member_passwd'=>$passwd)))){
