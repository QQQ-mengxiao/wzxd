...
class store_deliverControl extends BaseSellerControl {
	...
	public function indexOp() {
		...
		if (!in_array($_GET['state'],array('deliverno','delivering','delivered','posting'))){
		    $_GET['state'] = 'deliverno';
        }
		
        $pei_list =Model()->table('peisong')->field('id,p_name')->select();
		...
		$order_state = str_replace(array('deliverno','delivering','delivered','posting'),
		        array(ORDER_STATE_PAY,ORDER_STATE_SEND,ORDER_STATE_SUCCESS,25),$_GET['state']);
		$condition = array();
		...
		if ($_GET['deliverer_id'] != '') {
			...
			$condition['order_id'] = array('in',$deliever_order);
		}
        ...
        if ($_GET['peisong'] >0) {//配送方式
			...
			$sql="SELECT order_id from `718shop_order_goods` where deliverer_id IN (".$deliever_ids.")";
			$order_ps_list=Model()->query($sql);
			$order_id=array();
			for($i=0;$i<count($order_ps_list);$i++){
				$order_id[$i]=$order_ps_list[$i]['order_id'];
			}
            $condition['order_id']=array('in',$order_id);
            // $deliever_ids = Model()->table('peisong')->getfby_id($_GET['peisong'],'deliever_id');
            // $condition .= " AND order_goods.deliverer_id in (".$deliever_ids.") ";
        }
		// echo $_GET['peisong'];die;
		// Tpl::output('peisong',$_GET['peisong']); 
	...
	public function sendOp(){
		...
		if (chksubmit()){
			...
			if($_POST['shipping_code']){//快递
				$result = $logic_order->changeOrderPost($order_info, $_POST);
			}else{
		    	$result = $logic_order->changeOrderSend($order_info, 'seller', $_SESSION['member_name'], $_POST);
			}

			if (!$result['state']) {
			    showDialog($result['msg'],'','html','error');
			} else {
	...
	private function profile_menu($menu_type,$menu_key='') {
		...
		switch ($menu_type) {
			case 'deliver':
                ...
                array('menu_key'=>'posting',			'menu_name'=>'运输中',	'menu_url'=>'index.php?act=store_deliver&op=index&state=posting'),
...
/**
        ...
        // var_dump($order_goods_list);die;
            foreach ($order_goods_list as $value) {
              ...
              $kuajing_info['specifications'] = $_POST['reciver_totalLogisticsNo'];
                ...
                $kuajing_info['qty1'] = $_POST['reciver_transTool'];//法定数量
                $kuajing_info['qty2'] = $_POST['reciver_transType'];//法定第二数量
                $kuajing_info['unit'] = $_POST['reciver_transType1'];//计量单位
                $kuajing_info['unit1'] = $_POST['reciver_transType2'];//法定计量单位
                $kuajing_info['unit2'] = $_POST['reciver_transType3'];//法定第二计量单位
                $kuajing_info['itemNo'] = $_POST['reciver_transType4'];//货号
                $kuajing_info['goodidinsp'] = $_POST['reciver_transType5'];//备案号
                $kuajing_info['ciqbarcode'] = $_POST['reciver_transType6'];//HS编码
                $kuajing_info['unitinsp'] = $_POST['reciver_transType7'];
    ...
    public function orders_deliverOp() {
		$model_order = Model('order');
		$logic_order = Logic('order');
        $order_id = $_GET['order_id'];
		$order_id_arr = explode(',',$order_id);
		$order_list = $model_order->field('order_id,order_sn')->where(array('order_id'=>array('in',$order_id_arr)))->select();
		
		// echo '<pre>';print_r($order_list[0]['order_id']);die;
        if (chksubmit()) {
			...
			foreach($order_list as $value){
				...
				// $reciver_info['reciver_info'] = unserialize($order_info['reciver_info']);
	
				$result = $logic_order->changeOrderSend($order_info, 'seller', $_SESSION['member_name'], $order_info);
				if($result['state']){
					$order_id_result .= ' '.$value['order_id'].' ';
				}
			}
			if (!$result['state']) {
				showDialog($result['msg'],'','html','error');
			} else {
				$this->recordSellerLog('设置发货，订单id：'.$order_id_result);
				showDialog($result['msg'],'reload','succ');
			}
        }
        Tpl::output('order_list', $order_list);

        Tpl::showpage('store_goods_list.orders_deliver', 'null_layout');
    }
