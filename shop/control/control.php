...
/*                        'member_commission'      => array('name' => '我的佣金信息', 'url'=>urlShop('member_commission', 'member_commission')),*/
                ...
                'info' => array('name' => '资料管理', 'child' => array(
                        ...
                        'member_activate'   => array('name' => '账户激活', 'url'=>urlShop('member_activate', 'index')),
    ...
    private function filter_menu($menu_list){
        ...
        if($_SESSION['member_id']!='13736') unset($menu_list['info']['child']['member_connect']);
    	return $menu_list;
    }
}
...
class BaseStoreControl extends Control {
    ...
    protected function outputStoreInfo($store_info){
        if(!$this->store_decoration_only) {
            ...
            //20181120 隐藏热销排行中商品的会员折扣价
             foreach ($hot_sales as $key=>$value) {
                 $is_vip_price = Model()->table('goods_common')->getfby_goods_commonid($value['goods_commonid'], 'is_vip_price');
                 if ($is_vip_price == 1) {
                     $hot_sales[$key]['goods_price'] =  ncPriceFormat($value['goods_price'] * $discount);
                     $hot_sales[$key]['goods_promotion_price'] =  ncPriceFormat($value['goods_promotion_price'] * $discount);
                 }
             }
           ...
           // 20181120 隐藏收藏排行中商品的会员折扣价
             foreach ($hot_collect as $key=>$value) {
                 $is_vip_price = Model()->table('goods_common')->getfby_goods_commonid($value['goods_commonid'], 'is_vip_price');
                 if ($is_vip_price == 1) {
                     $hot_collect[$key]['goods_price'] =  ncPriceFormat($value['goods_price'] * $discount);
                     $hot_collect[$key]['goods_promotion_price'] =  ncPriceFormat($value['goods_promotion_price'] * $discount);
                 }
             }
...
class BaseSellerControl extends Control {
    ...
    public function __construct(){
        ...
        if ($_GET['act'] !== 'seller_login') {
            ...
            if ($_SESSION['seller_is_admin'] !== 1 && $_GET['act'] !== 'seller_center' && $_GET['act'] !== 'seller_logout' && $_GET['act'] !== 'store_order_print') {
    ...
    private function _getMenuList() {
        $menu_list = array(
            'goods' => array('name' => '商品', 'child' => array(
                ...
                //array('name' => '在线预约商品', 'act'=>'store_goods_reservation', 'op'=>'index'),
                ...
                array('name' => '图片空间', 'act'=>'store_album', 'op'=>'album_cate'),
                 array('name' => '分享背景图', 'act'=>'store_sharepic', 'op'=>'index'),
                array('name' => '仓库管理', 'act'=>'storage', 'op'=>'storage_list'),
                array('name' => '佣金管理', 'act'=>'commission', 'op'=>'setting'),
            )),
            'order' => array('name' => '订单物流', 'child' => array(
                array('name' => '实物交易订单', 'act'=>'store_order', 'op'=>'index'),
                array('name' => '团购订单', 'act'=>'store_company_order', 'op'=>'index'),
                // array('name' => '打印异常订单', 'act'=>'store_error_order', 'op'=>'index'),
                // array('name' => '虚拟兑码订单', 'act'=>'store_vr_order', 'op'=>'index'),
                // array('name' => '订单价格修改', 'act'=>'store_order_price', 'op'=>'index'),
                ...
                array('name' => '报表导出', 'act'=>'store_export', 'op'=>'index'),
                array('name' => '销售额导出', 'act'=>'store_export_amount', 'op'=>'index'),
                array('name' => '活动类型销售额', 'act'=>'store_export_type', 'op'=>'index'),
                array('name' => '结算明细', 'act'=>'store_export_goods', 'op'=>'index'),
                array('name' => '仓库报表', 'act'=>'store_export_order_list', 'op'=>'index'),
                array('name' => '运营报表', 'act'=>'store_export_ps', 'op'=>'index'),
                array('name' => '会员-销售报表', 'act'=>'store_export_member_ziti', 'op'=>'index'),
            )),
            'promotion' => array('name' => '促销', 'child' => array(
                 //array('name' => '抢购管理', 'act'=>'store_groupbuy', 'op'=>'index'),
                array('name' => '限时秒杀', 'act'=>'store_promotion_xianshi', 'op'=>'xianshi_list'),
                array('name' => '限时折扣', 'act'=>'store_promotion_xianshi_discount', 'op'=>'xianshi_list'),
                array('name' => '新人专享', 'act'=>'store_promotion_xinren', 'op'=>'xinrengoods_list'),
                array('name' => '周边美食', 'act'=>'store_buy_deliver', 'op'=>'buy_deliver_list'),
                ...
                array('name' => '代金券类型管理', 'act'=>'store_voucher_type', 'op'=>'templatelist'),
                array('name' => '代金券满送活动', 'act'=>'store_promotion_voucher', 'op'=>'mansong_list'),
                 array('name' => '分享活动管理', 'act'=>'store_promotion_fenxiang', 'op'=>'mansong_list'),
                // array('name' => '活动管理', 'act'=>'store_activity', 'op'=>'store_activity'),
                  array('name' => '进口商品', 'act'=>'store_ladder_price', 'op'=>'mansong_list'),
                   array('name' => '团购管理', 'act'=>'store_tuangou', 'op'=>'ladder_manage'),
                   array('name' => '前缀词条', 'act'=>'store_prefix', 'op'=>'prefix_list'),
                   array('name' => '新品管理', 'act'=>'store_goods_new', 'op'=>'setting'),
            )),
            'cw' => array('name' => '云仓管理', 'child' => array(
               array('name' => '下单补推', 'act'=>'store_cw_submit', 'op'=>'order_submit'),
               array('name' => '收货补推', 'act'=>'store_cw_over', 'op'=>'order_over'),
            //    array('name' => '退款补推', 'act'=>'store_cw_refund', 'op'=>'order_refund'),
           )),
            'store' => array('name' => '店铺', 'child' => array(
                ...
                // array('name' => '电子口岸参数', 'act'=>'store_kuajing', 'op'=>'D_canshu'),
                array('name' => '团购自提地址', 'act'=>'store_ziti_address', 'op'=>'daddress_list'),
                array('name' => '打印机设置', 'act'=>'dayin_setting', 'op'=>'dayin_list'),
                array('name' => '发货人类别', 'act'=>'pei_setting', 'op'=>'pei_list'),
            )),
            ...
            'statistics' => array('name' => '统计结算', 'child' => array(
                array('name' => '数据分析', 'act'=>'statistics_general', 'op'=>'general1'),
            ...
            'account' => array('name' => '账号', 'child' => array(
                ...
                array('name' => '修改密码', 'act'=>'seller_security', 'op'=>'index'),
            ))
        );
        return $menu_list;
    }
