...
//        if(empty($xianshi_info) || !$xianshi_info['editable']) {
        ...
        if($xianshi_goods_list){
            foreach ($xianshi_goods_list as $key=>$value){
                $condition['end_time'] = array('gt', strtotime($_POST['end_time']));
...
//                var_dump($value);
//                $result = ob_get_clean();
//                file_put_contents('C:\Users\Administrator\Desktop\abcd.txt', $result);
                if(!empty($xianshi_goods)) {
                    showDialog('该活动中'.$value['goods_name'].'已经参加了同时段活动，不可修改活动结束时间');
                }
            }
        }
...
//        $xianshi_goods_list = $model_xianshi_goods->getXianshiGoodsExtendList($condition,5);
    ...
    public function xianshi_goods_addOp() {
        ...
        $param['xianshi_price'] = $xianshi_info['discount']==0?$goods_info['goods_price']:number_format($goods_info['goods_price'] * $xianshi_info['discount'] / 10,2);
