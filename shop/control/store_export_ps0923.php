...
class store_export_psControl extends BaseSellerControl
{
    ...
    public function export_orderOp()
    {
       $model_goods = Model('goods');
        ...
        //       $order_list = $model_order->getOrderList( array('payment_time'=>$payment_time,'order_state'=>20));
        ...
        $data = $model_order->getOrderGoodsExportList($condition,'20000');
          ...
          //   $order_info = $model_order->getOrderInfo(array('order_id' => $value['order_id']));
          //    if($order_info['is_zorder']==0 || $order_info['order_state']!=20 ){
          //       unset($data[$key]);
          //   }
          ...
          //   $refund_list = $model_refund_return->getRefundReturnList(array('order_id' => $value['order_id']));
        ...
        foreach($data as $kk=>$vv){
            ...
            if(!empty( $refund_list)&&is_array( $refund_list)){
                ...
                if($value['deliverer_id']!=$_GET['daddress_id']){
                    // if(count($vv['extend_order_goods'])==1){
                    //     unset($data[$kk]);
                    // }else{
                         unset($data[$kk]['extend_order_goods'][$key]);
                    // }
