...
class store_goods_onlineControl extends BaseSellerControl {
    ...
    public function edit_goodsOp() {
        ...
        // 取得商品规格的输入值
        $goods_array = $model_goods->getGoodsList($where, 'goods_id,goods_marketprice,goods_price,goods_storage,goods_serial,goods_storage_alarm,goods_spec,is_mode,goods_hs,goods_tax,goods_shipper_id');
        $sp_value = array();
 ...
 /**
    ...
    public function edit_save_goodsOp() {
        ...
        $model_hs = Model('ctax_hs');
            $xiaofei_rate = $model_hs->getfby_hs(intval($_POST['goods_hs']),'xiaofei_rate');
            $zengzhi_rate = $model_hs->getfby_hs(intval($_POST['goods_hs']),'zengzhi_rate');          
            $tax_all = array();
