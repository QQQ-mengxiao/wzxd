       ...
       if($str!=''){
        ...
        if($goods_list){
            foreach ($goods_list as $key=>$value){
                $goods_spec_list = $model_goods->getGoodsList(array('goods_commonid'=>$value['goods_commonid']),'gc_id_1,gc_id_2,gc_id_3,goods_name,goods_jingle,goods_price,goods_costprice,goods_weight,goods_presalenum,goods_serial,goods_storage,goods_storage_alarm,goods_spec,goods_freight,goods_packages');
                if($goods_spec_list){
...
//            $data = $model_goods->getGoodsCommonOnlineList($where,'*',0);
    ...
    private function createExcel($data = array()){
        ...
        $tableheader = array('商品分类1','商品分类2','商品分类3', '商品名称','商品规格', '商品卖点', '商品价格','成本价', '商品净重', '商品件数', '预设销量', '商品库存', '库存预警值', '商家货号', '运费');
        for ($i = 0; $i < count($tableheader); $i++) {
         ...
         foreach ((array)$data as $k=>$v){
            ...
            $order_data[] = [
                $goods_class1,$goods_class2,$goods_class3,$v['goods_name'],$goods_spec_info,$v['goods_jingle'],$v['goods_price'],$v['goods_costprice'],$v['goods_weight'],$v['goods_packages'],$v['goods_presalenum'],intval($v['goods_storage']), $v['goods_storage_alarm'], $v['goods_serial'], $v['goods_freight'],
                ];
        }
    ...
    public function edit_goodsOp() {
        ...
        $goods_array = $model_goods->getGoodsList($where, 'goods_id,goods_marketprice,goods_costprice,goods_price,goods_weight,goods_all_weight,goods_packages,goods_app_price,goods_storage,goods_serial,goods_storage_alarm,goods_spec,is_mode,goods_hs,goods_tax,goods_shipper_id,p_ladder_id,xiao_gui,danwei_id,wu_gui');
        $sp_value = array();
...
//        }
    ...
    public function get_goods_list_ajaxOp() {
        ...
        $goods_list = $model_goods->getGoodsList(array('store_id' => $_SESSION['store_id'], 'goods_commonid' => $common_id,'is_deleted'=>0), 'goods_id,goods_spec,store_id,goods_price,goods_serial,goods_storage_alarm,goods_storage,goods_image,p_ladder_id');
        if (empty($goods_list)) {
            echo 'false';exit();
        }
        ...
        foreach ($goods_list as $key => $val) {
            ...
            $goods_list[$key]['p_name'] = Model()->table('p_ladder')->getfby_p_ladder_id($val['p_ladder_id'],'p_name')?Model()->table('p_ladder')->getfby_p_ladder_id($val['p_ladder_id'],'p_name'):'无';
		}
    ...
    public function edit_goods_simpleOp() {
        ...
        if (chksubmit()) {
            ...
            if($goods_info){
                        $update = array('goods_serial' => $value['serial'], 'goods_storage' => $value['storage'],'p_ladder_id'=>$value['ladder']);
						if($value['ladder'] >0){
							$update['is_group_ladder'] = 1;
						}
        ...
        $goods_list = $model_goods->getGoodsList(array('goods_commonid' => $common_id,'is_deleted'=>0), 'goods_spec,goods_storage,goods_serial,goods_id,goods_name,p_ladder_id');
