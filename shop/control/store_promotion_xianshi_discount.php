...
class store_promotion_xianshi_discountControl extends BaseSellerControl {

    const LINK_XIANSHI_LIST = 'index.php?act=store_promotion_xianshi_discount&op=xianshi_list';
    const LINK_XIANSHI_MANAGE = 'index.php?act=store_promotion_xianshi_discount&op=xianshi_manage&xianshi_id=';
    ...
    public function xianshi_listOp() {
        ...
        Tpl::showpage('store_promotion_xianshi_discount.list');
    }
    ...
    public function xianshi_addOp() {
        ...
        Tpl::showpage('store_promotion_xianshi_discount.add');
...
//        if(empty($xianshi_info) || !$xianshi_info['editable']) {
        ...
        $xianshi_goods_info = $model_xianshi_goods->getXianshiGoodsList(array('xianshi_id'=>$xianshi_id));
        if($xianshi_goods_info){
            foreach ($xianshi_goods_info as $key=>$value){
                $param['xianshi_price'] = $_POST['discount'] == 0 ? $value['goods_price'] : number_format($value['goods_price'] * $_POST['discount'] / 10, 2,'.','');
                unset($param['discount']);
                $model_xianshi_goods->editXianshiGoods($param, array('xianshi_id'=>$xianshi_id,'xianshi_goods_id'=>$value['xianshi_goods_id']));
            }
        }
        if($result) {
    ...
    public function xianshi_delOp() {
        ...
        if($result) {
            ...
            showDialog(L('nc_common_op_succ'), urlShop('store_promotion_xianshi_discount', 'xianshi_list'), 'succ');
        } else {
...
//        $xianshi_goods_list = $model_xianshi_goods->getXianshiGoodsExtendList($condition,5);
        $xianshi_goods_list = $model_xianshi_goods->getXianshiGoodsExtendList($condition,5,'','','',$_GET['goods_serial']);
        ...
        Tpl::showpage('store_promotion_xianshi_discount.manage');
    }
    ...
    public function goods_selectOp() {
        ...
        if($_GET['goods_serial']) {
            $condition['goods_serial|goods_barcode'] = array('like', '%' . $_GET['goods_serial'] . '%');
        }
        if ($_GET['stc_id']) {
            $condition['gc_id|gc_id_1|gc_id_2|gc_id_3'] = $_GET['stc_id'];
        }
        if ($_GET['deliverer_id']) {
            $condition['deliverer_id'] = $_GET['deliverer_id'];
        }
        //$condition['goods_name'] = array('like', '%'.$_GET['goods_name'].'%');
        ...
        Tpl::showpage('store_promotion_xianshi_discount.goods', 'null_layout');
    }
    ...
    public function xianshi_goods_addOp() {
        ...
        //检查商品是否已经参加同时段活动
        $sql = "SELECT t.* FROM (SELECT spxg.start_time,spxg.end_time,spxg.xianshi_name FROM 718shop_p_xianshi_goods spxg LEFT JOIN 718shop_p_xianshi spx ON spxg.xianshi_id=spx.xianshi_id WHERE spxg.end_time> ".TIMESTAMP." AND spxg.goods_id=".$goods_id." AND spxg.xianshi_id !=".$xianshi_id." AND spx.xianshi_type=2 UNION SELECT start_time,end_time,xianshi_name FROM 718shop_p_xianshi WHERE xianshi_id=".$xianshi_id.") t ORDER BY t.start_time";
        $xianshi_goods = Model()->query($sql);
        array_multisort(array_column($xianshi_goods, 'start_time'), SORT_ASC, $xianshi_goods);
        if($xianshi_goods){
            foreach ($xianshi_goods as $k => $v){
                if ($v['start_time'] >= $v['end_time']){
                    die(json_encode(array('result'=>false,'message'=>'该商品已经参加了<'.$v['xianshi_name'].'>活动，活动时间重叠，不可添加！'),320));
                }
                if ($k > 0 && $xianshi_goods[$k]['start_time'] < $xianshi_goods[$k-1]['end_time']){
                    die(json_encode(array('result'=>false,'message'=>'该商品已经参加了<'.$xianshi_goods[$k-1]['xianshi_name'].'>活动，活动时间重叠，不可添加！'),320));
                }
            }
        }
        ...
        $param['xianshi_price'] = str_replace(",","",$xianshi_info['discount']==0?$goods_info['goods_price']:number_format($goods_info['goods_price'] * $xianshi_info['discount'] / 10,2,'.',''));
    ...
    public function xianshi_goods_price_editOp() {
        ...
        if($result) {
            ...
            $this->recordSellerLog('折扣率修改为：'.$xianshi_goods_info['xianshi_price'].'，限时折扣购买上限修改为：'.$xianshi_goods_info['upper_limit'].'，商品名称：'.$xianshi_goods_info['goods_name'].'，佣金比例修改为：'.$commis_rate);
        } else {
            $data['result'] = false;
    ...
    private function profile_menu($menu_key='') {
        $menu_array = array(
            1=>array('menu_key'=>'xianshi_list','menu_name'=>Language::get('promotion_active_list'),'menu_url'=>'index.php?act=store_promotion_xianshi_discount&op=xianshi_list'),
        );
        switch ($menu_key){
        	case 'xianshi_add':
                $menu_array[] = array('menu_key'=>'xianshi_add','menu_name'=>Language::get('promotion_join_active'),'menu_url'=>'index.php?act=store_promotion_xianshi_discount&op=xianshi_add');
        		break;
        	case 'xianshi_edit':
                $menu_array[] = array('menu_key'=>'xianshi_edit','menu_name'=>'编辑活动','menu_url'=>'javascript:;');
        		break;
        	case 'xianshi_quota_add':
                $menu_array[] = array('menu_key'=>'xianshi_quota_add','menu_name'=>Language::get('promotion_buy_product'),'menu_url'=>'index.php?act=store_promotion_xianshi_discount&op=xianshi_quota_add');
        		break;
        	case 'xianshi_manage':
                $menu_array[] = array('menu_key'=>'xianshi_manage','menu_name'=>Language::get('promotion_goods_manage'),'menu_url'=>'index.php?act=store_promotion_xianshi_discount&op=xianshi_manage&xianshi_id='.$_GET['xianshi_id']);
        		break;
			case 'xianshi_goods_import':
                $menu_array[] = array('menu_key' => 'xianshi_goods_import', 'menu_name' => '商品导入', 'menu_url' => 'index.php?act=store_promotion_xianshi_discount&op=xianshi_goods_import&xianshi_id=' . $_GET['xianshi_id']);
                break;
        }
    ...
    public function xianshi_goods_importOp()
    {
        if ($_POST) {
			...
			unlink($path);
            if($dataArray['result']){
                showDialog('批量导入限时折扣商品成功！', 'reload', 'succ');
            }else{
...
//                活动存在且未开始，插入数据
    ...
    private function xianshi_goods_add($value)
    {
        ...
        $param['start_time'] = $xianshi_info['start_time'];
        $param['end_time'] = $xianshi_info['end_time'];
        $param['upper_limit'] = $xianshi_info['upper_limit'];
...
//        die;
    ...
    public function goods_sortOp(){
        ...
        $this->recordSellerLog('修改限时折扣商品排序，id：' . $xianshi_goods_id . '修改之前为' . $goods_sort['goods_sort'] . '修改之后为' . $_POST['goods_sort']);
        if ($update) {
...
//        echo '<pre>';var_dump($goods_list);die;
        ...
        Tpl::output('goods_list', $goods_list);
        Tpl::output('commis_rate', $commis_rate);
        Tpl::showpage('store_promotion_xianshi_discount.goods_add','null_layout');
    }
...
//                            showDialog($item['goods_name'] . '的折扣价格不能为空！');
//                        }else{
//                            if ($item['xianshi_price'] > $item['goods_price']) {
//                                showDialog($item['goods_name'] . '的折扣价格不能高于当前售价！');
//                                $tmp = 0;
//                                break;
//                            }
//                        }
...
//                    }
                    ...
                    $param['xianshi_price'] = $xianshi_info['discount']==0?$value['goods_price']:number_format($value['goods_price'] * $xianshi_info['discount'] / 10,2,'.','');
                ...
                $msg = '添加限时折扣商品，活动名称：' . $xianshi_info['xianshi_name'] . $succ . $fail;
