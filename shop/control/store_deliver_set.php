...
class store_deliver_setControl extends BaseSellerControl {
    ...
    //    $model_goods = Model('goods');
       ...
       $address_list = $model_daddress->getAddressList($condition,'*','',0,10);
       if(is_array($address_list)){
           foreach ($address_list as $key=>$value){
               if($value['storage_id'] == 0){
                   $address_list[$key]['storage_name'] = '';
               }else{
                   $address_list[$key]['storage_name'] = $model_storage->getfby_storage_id($value['storage_id'],'storage_name');
               }
               $address_list[$key]['goods_online'] = Model()->table("goods_common")->where(["deliverer_id"=>$value['address_id'],"goods_state"=>1])->count();
               $address_list[$key]['goods_offline'] = Model()->table("goods_common")->where(["deliverer_id"=>$value['address_id'],"goods_state"=>0])->count();
           }
       }
       $fahuo_list =Model()->table('daddress')->where(array('store_id'=>$_SESSION ['store_id']))->select();
    ...
    public function daddress_addOp() {
        ...
        $model_daddress = Model('daddress');
        $model_storage = Model('storage');
        $storage_info = $model_storage->getStorageList(array('store_id'=>$_SESSION['store_id']),'storage_id,storage_name');
        if (chksubmit()) {
            //保存 新增/编辑 表单
            ...
            $obj_validate->validateparam = array(
                ...
                array("input"=>$_POST["wms_id"],"require"=>"true","message"=>'wms编码不能为空')
            );
