...
class complainControl extends SystemControl {
   ...
   public function all_complain_listOp() {
        $page = new Page();
        $page->setEachNum(20) ;
        $page->setStyle('admin') ;
        $model_complain = Model('complain') ;
        ...
        $condition['complain_datetime_end'] = strtotime($_GET['input_complain_datetime_end']);
            $condition['order'] = 'complain_id desc';
            // var_dump( $condition['complain_subject_content']);die;
            // $condition['order'] = 'complain_id asc';
        // $condition['complain_state'] = $complain_state;
        $complain_list = $model_complain->getComplain($condition,$page) ;
        $this->show_menu('all_complain_list');
        // Tpl::output('op',$op);
        Tpl::output('list', $complain_list) ;
        Tpl::output('show_page',$page->show()) ;
        Tpl::showpage('all_complain.list') ;
    }
    public function exportOp() {
        ...
        $model_complain = Model('complain') ;
        // echo '1';die;
          // var_dump($_GET['input_complain_accuser']);die;
        //搜索条件
        $condition = array();
        if (!empty($_GET['input_complain_accuser'])) { 
        $condition['accuser_name'] = array('like', '%'.$_GET['input_complain_accuser'].'%');
        }
        // var_dump($condition['complain_accuser']);die;
        if (!empty($_GET['input_complain_accused'])) {
        $condition['complain_accused'] =  array('like', '%'.$_GET['input_complain_accused'].'%');
        }
        if (!empty($_GET['input_complain_subject_content'])) {
        $condition['complain_subject_content'] =  array('like', '%'.$_GET['input_complain_subject_content'].'%');
        }
        $condition['complain_datetime'] =array('time', array(strtotime($_GET['input_complain_datetime_start']), strtotime($_GET['input_complain_datetime_end'])));
        if (!is_numeric($_GET['curpage'])){
            $array = array();
            $count = $model_complain->getComplainCount1($condition);  
            // var_dump($count);die;
                $data = $model_complain->getComplainList($condition,'complain_id desc','*',$count);
                $this->createExcel($data);
            //}
        }else{  //下载
            $limit1 = ($_GET['curpage']-1) * self::EXPORT_SIZE;
            $limit2 = self::EXPORT_SIZE;
            $data = $model_complain->getComplainList($condition,'complain_id desc','*',self::EXPORT_SIZE);
            $this->createExcel($data);
        }
    }
    ...
    private function createExcel($data = array()){
        ...
        $excel_data[0][] = array('styleid'=>'s_title','data'=>'投诉人');
        $excel_data[0][] = array('styleid'=>'s_title','data'=>'被投诉店铺');
        $excel_data[0][] = array('styleid'=>'s_title','data'=>'投诉主题');
        $excel_data[0][] = array('styleid'=>'s_title','data'=>'投诉时间');
        //data
        foreach ((array)$data as $k=>$v){
            $tmp = array();
            $tmp[] = array('data'=>$v['accuser_name']);
            $tmp[] = array('data'=>$v['accused_name']);
            $tmp[] = array('data'=>$v['complain_subject_content']);
            $tmp[] = array('data'=>date('Y-m-d H:i:s',$v['complain_datetime']));
        ...
        $excel_obj->generateXML($excel_obj->charset('投诉',CHARSET).$_GET['curpage'].'-'.date('Y-m-d-H',time()));
    }
    /*
     * 进行中的投诉
     */
	...
	private function show_menu($menu_key) {
		$menu_array = array(
            ...
            'all_complain_list'=>array('menu_type'=>'link','menu_name'=>'所有投诉','menu_url'=>'index.php?act=complain&op=all_complain_list'),
