...
class memberControl extends SystemControl{
		...
		if ($_GET['search_field_value'] != '') {
    		switch ($_GET['search_field_name']){
    			...
    			case 'member_truename':
    				...
    				break;
    			case 'is_store':
    				if ($_GET['search_field_value'] == '是'){
    					$condition['is_store'] = 2;
    				}else{
	 ...
	 * 会员修改为可扫码核销的商户
	 */
	public function member_storeOp(){
		$update_array = array();
		$update_array['is_store']	= intval($_GET['is_store']);
		$model_member = Model('member');
		...
		if ($result){
			...
			if(intval($_GET['is_store']) == 2){
				$url[0]['url'] = 'index.php?act=member&op=member&search_field_name=is_store&search_field_value=否';
				showMessage('设为商户，可在小程序核销扫码',$url);
			}else{
				$url[0]['url'] = 'index.php?act=member&op=member&search_field_name=is_store&search_field_value=是';
				showMessage('设为普通用户',$url);
			}
			
		}else {
			if(intval($_GET['is_store']) == 2){
				showMessage('商户状态设置失败');
			}else{
				showMessage('普通用户状态设置失败');
			}
		}
	...
	public function member_tuanOp(){
		...
		if(!empty($is_tuan)){
			if(!empty($member_info['share_id'])){
        			$update_array['is_tuan']	= intval($_GET['is_tuan']);
        	}else{
		        $update_array['is_tuan']	= intval($_GET['is_tuan']);
		        $update_array['share_id']	= $member_id;
        	}
		}else{
			if($member_info['share_id'] == $member_id ){
				$update_array['share_id']	= 0;
				$update_array['is_tuan']	= $is_tuan;
			}else{
				$update_array['is_tuan']	= $is_tuan;
			}
		...
		$result = $model_member->editMember(array('member_id'=>$member_id),$update_array);
		if ($result){
			$url = array(
				array(
					'url'=>'index.php?act=member&op=member',
			...
			if(intval($_GET['is_tuan']) == 1){
				$url[0]['url'] = 'index.php?act=member&op=member&search_field_name=is_tuan&search_field_value=否';
				showMessage( $member_info['member_name'].'已设为团长!',$url);
			}else{
				$url[0]['url'] = 'index.php?act=member&op=member&search_field_name=is_tuan&search_field_value=是';
				showMessage($member_info['member_name'].'设为普通用户',$url);
			}
			
		}else {
			if(intval($_GET['is_tuan']) == 1){
				showMessage('团长状态设置失败');
			}else{
				showMessage('普通用户状态设置失败');
			}
		}
	...
	public function member_editOp(){
		...
		if (chksubmit()){
			...
			if ($error != ''){
				...
				$update_array['company_id']		= $_POST['company_id'];
		...
		$c['company_id'] = array('gt',0);
		$company_info = $model_company->table('company')->where($c)->select();
	...
	public function member_addOp(){
		...
		if (chksubmit()){
			...
			if ($error != ''){
				...
				$insert_array['company_id']	= $_POST['company_id'];
	...
	public function auditOp(){
		...
		if (chksubmit()){
			...
			if ($error != ''){
				...
				$update_array['member_verify'] 			= $_POST['member_verify'];
				if ($_POST['member_verify']=1) {
					$update_array['is_buy']       = 1;
				...
				if ($result){
				    $url="index.php?act=member&op=member";
					// $this->log(L('nc_edit,member_index_name').'[ID:'.$_POST['member_id'].']',1);
					showMessage('用户信息审核成功',$url);
				}else {
					showMessage('用户信息审核失败');
				}
			}
		}
		$condition['member_id'] = intval($_GET['member_id']);
	...
	private function createExcel($data = array()){
        ...
        $excel_data[0][] = array('styleid' => 's_title', 'data' => '会员身份证号');
		...
		$excel_data[0][] = array('styleid'=>'s_title','data'=>'是否为商户');
		$excel_data[0][] = array('styleid'=>'s_title','data'=>'是否为团长');
		foreach ((array)$data as $k=>$v){
            $member_id = $v['member_id'];
            $addresslist = $address_model->getAddressList(array('member_id' => $member_id));
            $addressCount = empty($addresslist)?0:count($addresslist);
			...
			$tmp[] = array('data'=>$v['member_qq']);
                    $tmp[] = array('data' => $addresslist[$i]['id_card']);//身份证号
			...
			if ($v['is_tuan'] == 1){
				...
				$tmp[] = array('data'=>'否');
                    ...
                    if (empty($addresslist[$i]['id_card'])) continue;
                    $tmp[] = array('data' => '');
                    $tmp[] = array('data' => '');
                    $tmp[] = array('data' => '');
                    $tmp[] = array('data' => '');
