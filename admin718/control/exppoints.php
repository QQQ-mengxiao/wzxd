    ...
    public function addOp(){
        if (chksubmit()){
            ...
            $obj_validate->validateparam = array(
                array("input"=>$_POST["member_id"], "require"=>"true", "message"=>"会员信息错误，请重新填写会员ID"),
                array("input"=>$_POST["exppointsnum"], "require"=>"true",'validator'=>'Compare','operator'=>' >= ','to'=>1,"message"=>"积分值必须大于0")
            );
            ...
            if (!is_array($member_info) || count($member_info)<=0){
                showMessage("会员信息错误",'index.php?act=exppoints&op=add','','error');
            }

            $exppointsnum = intval($_POST['exppointsnum']);
            if ($_POST['operatetype'] == 2 && $exppointsnum > intval($member_info['member_exppoints'])){
                showMessage("经验值不足，会员当前经验值为".$member_info['member_exppoints'],'index.php?act=exppoints&op=add','','error');
            }

            $obj_exppoints = Model('exppoints');
...
//            } else {
                $insert_arr['exp_desc'] = '修改经验值'.$member_info['member_name'].'['.(($_POST['operatetype'] == 2)?'':'+').strval($insert_arr['exp_points']).']';
//            }
            $result = $obj_exppoints->saveExppointsLog('system',$insert_arr);
            if ($result){
                $this->log('修改经验值'.$member_info['member_name'].'['.(($_POST['operatetype'] == 2)?'':'+').strval($insert_arr['exp_points']).']',null);
                showMessage(Language::get('nc_common_save_succ'),'index.php?act=exppoints&op=add');
            }else {
                showMessage(Language::get('nc_common_save_fail'),'index.php?act=exppoints&op=add','','error');
            }
        }else {
    ...
    public function checkmemberOp(){
        ...
        $obj_member = Model('member');
        $member_info = $obj_member->getMemberInfo(array('member_id'=>$id));
        if (is_array($member_info) && count($member_info)>0){
