...
//			$tmp[] = array('data'=>orderState($v));
          //订单状态
            ...
            if ($result) {
                if ($result[$key]['refund_type'] == 1) {
                    if ($result[$key]['refund_state'] == 1 || $result[$key]['refund_state'] == 2) {
                        $tmp[] = array('data' => '退款中'); //退款中

                    } else if ($result[$key]['refund_state'] == 3) {
                    	if ($result[$key]['seller_state']==2) {
                    		 $tmp[] = array('data' => '退款完成'); //退款完成
                    	}else if ($result[$key]['seller_state']==3) {
                ...
                } elseif ($result[$key]['refund_type'] == 2) {
                    if ($result[$key]['refund_state'] == 1 || $result[$key]['refund_state'] == 2) {
            ...
            //商家处理状态
			if($list[$key]['seller_state']=='1'){
                $tmp[] = array('data'=>'待审核');
            }else if($list[$key]['seller_state']=='2'){
                $tmp[] = array('data'=>'同意');
            }else if($list[$key]['seller_state']=='3'){
                $tmp[] = array('data'=>'不同意');
            }else{
                $tmp[] = array('data'=>'');
            }
            //平台确认
			if($list[$key]['seller_state']=='2'){
                if($list[$key]['refund_state']=='1'){
                    $tmp[] = array('data'=>'处理中');
                }else if($list[$key]['refund_state']=='2'){
                    $tmp[] = array('data'=>'待管理员处理');
                }else if($list[$key]['refund_state']=='3'){
                    $tmp[] = array('data'=>'已完成');
                }else{
                    $tmp[] = array('data'=>'无');
                }
            ...
            //商家意见
			$tmp[]=array('data'=>$list[$key]['seller_message']);
			//管理员意见
			$tmp[]=array('data'=>$list[$key]['admin_message']);
            //商品分类
            $goodsid=Model()->query("SELECT goods_id  FROM `718shop_order_goods` where order_id=\"$order_id\" ");
            $goods_id=$goodsid[0]['goods_id'];
            $gc_id=Model()->query("SELECT gc_id  FROM `718shop_goods` where goods_id=\"$goods_id\" ");
            $gcid=$gc_id[0]['gc_id'];
