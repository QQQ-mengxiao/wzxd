...
class bhcs_transportControl extends SystemControl{
	 ...
	 * 店铺等级
	 */
	public function bhcs_transportOp(){
		/**
		 * 读取语言包
		 */
		//$query="select package_name from ctax_emallyipackage";
		//$result=Model()->query('select * from v4_ctax_emallyipackage');
		$model = Model('ctax_emallyitransportationway');
		$result=$model->select();


		//Tpl::output('like_sg_name',trim($_POST['like_sg_name']));
		Tpl::output('grade_list',$result);
		Tpl::showpage('bhcs_transport.index');
	}

	/**
	 * 新增等级
	 */
	public function bhcs_packing_addOp(){
	 ...
	 * 等级编辑
	 */
	public function bhcs_packing_editOp(){
		$lang	= Language::getLangContent();
