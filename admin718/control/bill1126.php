...
class billControl extends SystemControl{
	...
	public function show_billOp(){
		...
		if ($_GET['query_type'] == 'refund') {
		   ...
		   // 20181119新添加 取订单商品分销佣金
		    ...
		    //181119 新添加 订单商品的分销佣金$order_commission
	...
	public function export_billOp(){
		...
		$export_data = array();
		$export_data[0] = array('账单编号','开始日期','结束日期','订单金额','运费','佣金金额','退款金额','退还佣金','分销商佣金','店铺费用','本期应结','出账日期','账单状态','店铺','店铺ID');//181119新添加导出表格中分销商佣金标题
		$ob_order_totals = 0;
		$ob_shipping_totals = 0;
		$ob_commis_totals = 0;
		$ob_order_return_totals = 0;
		$ob_commis_return_totals = 0;
		$ob_commission_totals = 0;//181119新添加商品的分销佣金
		...
		foreach ($data as $k => $v) {
		    ...
		    //181119新添加订单商品的分销佣金$ob_commission_totals
		    $ob_commission_totals += $export_data[$k+1][] = $v['ob_commission_totals'];
	...
	public function export_orderOp(){
		...
		$export_data = array();
		$export_data[0] = array('订单编号','订单金额','运费','佣金','分销佣金','下单日期','成交日期','商家','商家编号','买家','买家编号','商品');
		$order_totals = 0;
		$shipping_totals = 0;
		$commis_totals = 0;
		$k = 0;
