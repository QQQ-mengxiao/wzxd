...
class fx_becomeControl extends SystemControl{
	 ...
	 * 分销轮播图片上传
	 */
	public function fx_becomeOp() {
	    $size = 1;//上传显示图片总数
	    $i = 1;
	    $info['pic'] = array();
	    $model_setting = Model('setting');
	    $code_info = $model_setting->getRowSetting('fx_become_pic');
	    ...
	    if (chksubmit()) {
    	    ...
    	    $update_array = array();
    	    $update_array['fx_become_pic'] = $code_info;
    	    $result = $model_setting->updateSetting($update_array);
    	    showMessage(Language::get('nc_common_save_succ'),'index.php?act=fx_become&op=fx_become');
	    }
		Tpl::output('size',$size);
		Tpl::output('pic',$info['pic']);
		Tpl::showpage('fx_become');
	}
	/**
	 * 上传图片
	 */
