...
class voucherControl extends SystemControl{
	...
	public function donateOp(){
		...
		if($_GET['sdate']&&$_GET['edate']){
			$data['add_time']=array(array('egt',strtotime($_GET['sdate'])),array('lt',strtotime($_GET['edate'])),'and');
		}
		if ($_GET['search_field_value'] != '') {
			switch ($_GET['search_field_name']){
				case 'donate':
					$data['donate_name'] = trim($_GET['search_field_value']);
					break;
				case 'accept':
					$data['accept_name'] = trim($_GET['search_field_value']);
					break;
			}
		}
		$model=Model('voucher');
		$donate_list=$model->table('donate_log')->where($data)->page(10)->select();
		foreach($donate_list as $key=>$voucher){
			$donate_list[$key]['voucher_price']=Model('voucher')->table('voucher')->getfby_voucher_id($voucher['voucher_id'],'voucher_price');
		}
		Tpl::output('show_page',$model->showpage(2));
		$this->show_menu('voucher','donate');
		Tpl::output('list',$donate_list);
		Tpl::showpage('voucher_donatelist');
	}
	/**
     * 套餐管理
     **/
    ...
    public function templatedelOp(){
        $voucher_t_id = trim($_POST['voucher_t_id']);//var_dump($voucher_t_id);die;测试是否能够获取值
//        if(empty($voucher_t_id)) {
//            showMessage(Language::get('param_error'),'index.php?act=voucher&op=templatelist');
//        }
        $model = Model();
        $rs = $model->table('voucher_template')->where(array('voucher_t_id'=>array('in',$voucher_t_id)))->delete();
        if ($rs){
            $this->log(L('nc_del').'[ID:'.$voucher_t_id.']');
            showMessage(Language::get('nc_common_del_succ'),'index.php?act=voucher&op=templatelist');
        }else{
            showMessage(Language::get('nc_common_del_fail'),'index.php?act=voucher&op=templatelist');
        }
    }
		...
		switch ($menu_type) {
			case 'voucher':
				...
				4=>array('menu_key'=>'setting','menu_name'=>Language::get('admin_voucher_setting'),	'menu_url'=>'index.php?act=voucher&op=setting'),
					7=>array('menu_key'=>'donate','menu_name'=>'代金券券转赠与接收记录',	'menu_url'=>'index.php?act=voucher&op=donate'),
				);
				break;
			case 'priceedit':
				$menu_array = array(
