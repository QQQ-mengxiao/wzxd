...
//		$refund = $refund_list[0];
	...
	private function createCashExcel($data = array()){
		...
		$excel_data[0][] = array('styleid'=>'s_title','data'=>'所在公司');
		...
		$excel_data[0][] = array('styleid'=>'s_title','data'=>L('exp_tx_ctime'));
				$excel_data[0][] = array('styleid'=>'s_title','data'=>'审核退款时间');
		...
		foreach ((array)$data as $k=>$v){
			...
			$cardno=Model()->table('member_card')->where(array('member_id'=>$v['pdc_member_id']))->limit(1)->find();
			if($cardno){
            $card_info=Model('card')->getMemberCardInfo($cardno['cardno']);
            // var_dump($card_info);die;
            $tmp[] = array('data'=>$card_info['Department1']);
             $tmp[] = array('data'=>$card_info['PersonalID']);//PersonalID
               }else{
            	$tmp[] = array('data'=>'');
            	$tmp[] = array('data'=>'');
               }
		...
		$excel_data = $excel_obj->charset($excel_data,CHARSET);
		$excel_obj->addArray($excel_data);
		$excel_obj->addWorksheet($excel_obj->charset('退款提现',CHARSET));
		$excel_obj->generateXML($excel_obj->charset('退款提现',CHARSET).$_GET['curpage'].'-'.date('Y-m-d-H',time()));
	}
	...
	public function pd_tixianOp(){
        ...
        if (chksubmit()) {
            ...
            // $serial_sign_rule['serial_points_reg'] = $points;
             $pd_tixian = $_POST['pd_tixian'];
            // $list_setting=serialize($points);
            $result = $model_setting->updateSetting($pd_tixian);
            if ($result) {
                $this->log('余额提现限制金额编辑', 1);
                showDialog(L('nc_common_save_succ'), 'reload', 'succ');
            } else {
                $this->log('余额提现限制金额编辑', 0);
                showDialog(L('nc_common_save_fail'));
            }
        }
        Tpl::output('pd_tixian',$pd_tixian);
        Tpl::output('list_setting',$list_setting);
        Tpl::showpage('pd.xianzhi');
    }
	/**
     * 设置
     **/
