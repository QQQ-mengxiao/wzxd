...
class signingControl extends SystemControl{
	...
	public function auditOp() {
		$model_signing = Model('signing');
		$condition = array();
		$condition['id'] = intval($_GET['signing_id']);
		$state = $model_signing->editSigning(array('status' =>20),$condition);
	    if ($state) {
				showMessage(Language::get('nc_common_save_succ'),'index.php?act=signing&op=signing_all');
			} else {
