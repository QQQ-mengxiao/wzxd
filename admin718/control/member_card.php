...
class member_cardControl extends SystemControl
{
    ...
    public function indexOp()
    {
        ...
        if ($_GET['search_field_value'] != '') {
            switch ($_GET['search_field_name']) {
                ...
                case 'cardno':
                    ...
                    break;
                case 'gonghao':
                    $card_info = $model_card_new->getMemberCardInfobygh($_GET['search_field_value']);
                    $con1['cardno'] = $card_info['cardNo'];
                    $member_id = Model()->table('member_card')->where($con1)->find();
                    $con['member_card.member_id'] = $member_id['member_id'];
                    break;
            }
        ...
        foreach ($member_list as $k => $v) {
            ...
            $member_list[$k]['personalId'] = $v['gonghao'];
    ...
    public function editOp()
    {
        ...
        $model_member = Model('member');
        $insert_array['exp_memberid'] = trim($_POST['member_id']);
        $insert_array['exp_membername'] = $model_member->getfby_member_id(trim($_POST['member_id']), 'member_name');
        $model_setting = Model('setting');
        $list_setting = $model_setting->getListSetting();
        $member_grade = $list_setting['member_grade'] ? unserialize($list_setting['member_grade']) : array();
        $insert_array['exp_points'] = -$member_grade[count($member_grade) - 1]['exppoints'];
        $result1 = $model_exppoints->saveExppointsLog('card', $insert_array);
        //解绑时自动减经验值E
    ...
    public function member_addOp()
    {
        ...
        if (chksubmit()) {
            ...
            $obj_validate->validateparam = array(
                ...
                array("input" => $_POST["search_field_name"], "require" => "true", "message" => '一卡通卡号或工号必须填一个'),
            );
            ...
            if ($error != '') {
                ...
                if (strlen($_POST['search_field_name']) < 7) { //穿的值为一卡通卡号
                    $sno = trim($_POST['search_field_value']);
                } else { //chhuan的值为工号
                    $model_card_new = Model('card_new');
                    $gonghao = $_POST['search_field_value'];
                    $card_info = $model_card_new->getMemberCardInfobygh($gonghao);
                    $sno = $card_info['cardNo'];
                }
                $model = Model();
    ...
    public function ajaxOp()
    {
        switch ($_GET['branch']) {
            ...
            case 'check_member_id':
                ...
                if (empty($list)) {echo 'true';exit;} else {echo 'false';exit;}
                break;
            //验证一卡通卡号
            case 'check_cardno':
        ...
        while (!feof($doc_open)) {
            ...
            if ($i != 0) {
                if (!empty($data)) {
                    ...
                    $member_grade = $list_setting['member_grade'] ? unserialize($list_setting['member_grade']) : array();
                    $insert_array['exp_points'] = $member_grade[count($member_grade) - 1]['exppoints'];
                    $insert_res = $model_exppoints->saveExppointsLog('card', $insert_array);
                    //绑定时自动添加经验值E
