...
class goodsControl extends SystemControl{
    ...
    public function goods_delOp() {
      $common_id = trim($_POST['goods_commonid']);
         if(empty($common_id)) {
            showMessage(Language::get('param_error'),'');
        }    
         $ids_array = explode(',', $common_id);
        //数组值转为整数型
        ...
        $condition = array();
        $condition['goods_commonid'] = array('in', $ids_array);
    ...
    public function get_goods_list_ajaxOp() {
        ...
        $goods_list = $model_goods->getGoodsList(array('goods_commonid' => $commonid,'is_deleted'=>0), 'goods_id,goods_spec,store_id,goods_price,goods_serial,goods_storage,goods_image');
        if (empty($goods_list)) {
            echo 'false';exit();
        }
