	...
	public function indexOp(){
        ...
        //add
         if($_GET['address_id']) {
             ...
             $model_ordergood= Model('order_common');
             $array=$model_ordergood->where(array('reciver_ziti_id'=>$_GET['address_id']))->select();
                 foreach ($array as $k => $v) {
                 }
         $arr3 = array_reduce($array, create_function('$result, $v', '$result[] = $v["order_id"];return $result;'));
              $condition['order_id'] = array('in',$arr3);
        }
        if($arr2&&$arr3){
            $condition['order_id'] = array('in',array_intersect($arr2,$arr3));//收货人和地址同时存在筛选相同的order_id
        }
        ...
        // }
        //部分退款导出
         if($_GET['order_state']=='1') {
            $condition['refund_state'] = $_GET['order_state'];
        }
...
//xinzeng订单完成时间 0331
        ...
        // $order_list	= $model_order->getOrderList($condition,30);
          $order_list	= $model_order->getOrderList($condition,30,'*','order_id desc','',array('order_goods','order_common'));
        ...
        foreach ($order_list as $order_id => $order_info) {
            ...
            $order_list[$order_id]['if_system_receive_pay'] = $model_order->getOrderOperateState('system_receive_pay',$order_info);
             $ziti_id=$order_info['extend_order_common']['reciver_ziti_id'];
             $ziti_name = Model('ziti_address')->getAddressInfo(array('address_id'=>$ziti_id));
            $order_list[$order_id]['ziti_name'] =$ziti_name['seller_name'];
...
/**jinp 1222
	...
	public function show_orderOp(){
        ...
        foreach ($order_info['extend_order_goods'] as $key =>$value) {
            ...
            // if($key==$count){
            //      if($key==0){
            //         $order_info['extend_order_goods'][$key]['voucher_price']= $voucher_price;
            //      }else{
            //        $order_info['extend_order_goods'][$key]['voucher_price']= $voucher_price-$all_voucher_price;
            //      }
            //  }else{
            //      $order_info['extend_order_goods'][$key]['voucher_price']= $voucher_price*$rate;
            //  }
            //  $all_voucher_price+= $order_info['extend_order_goods'][$key]['voucher_price'];
             $order_info['extend_order_goods'][$key]['ordergoods_pay_price']= $goods_price- $value['voucher_price'];
        }
        //退款信息
		...
		switch ($order_info['order_type']){
            case 0:
                $order_info['order_type_info'] = '无活动';
                break;
            case 1:
...
//xinzeng支付时间 11.2
	...
	private function createExcel($data = array()){
		...
		$excel_data[0][] = array('styleid'=>'s_title','data'=>'活动类型');
		...
		$excel_data[0][] = array('styleid'=>'s_title','data'=>'买家公司');
        ...
        $excel_data[0][] = array('styleid'=>'s_title','data'=>'买家留言');
         $excel_data[0][] = array('styleid'=>'s_title','data'=>'自提点');
		...
		foreach ((array)$data as $k=>$v){
			...
			$tmp[] = array('data'=>orderPaymentName($v['payment_code']));
						            //促销类型
            $tmp[]=array('data'=>orderTypeName($v['order_type']));
//			$tmp[] = array('data'=>orderState($v));
          //订单状态
			...
			if($cardno){
              $card_info=Model('card_new')->getMemberCardInfobygh($cardno['gonghao']);
              $tmp[] = array('data'=>$card_info['yuliu1']);
               }else{
                $tmp[] = array('data'=>'');
               }
            }else{
            ...
            $tmp[]=array('data'=>$order_message[0]['order_message']);
                        //自提点
            $ziti_id=Model()->query("SELECT reciver_ziti_id FROM `718shop_order_common` WHERE  order_id=\"$order_id\"");
             $ziti_name = Model('ziti_address')->getAddressInfo(array('address_id'=>$ziti_id[0]['reciver_ziti_id']));
            $tmp[]=array('data'=>$ziti_name['seller_name']);
                //代金券名称
          ...
          //   }
            //商品名称
            $goods_name=Model()->query("SELECT goods_name  FROM `718shop_order_goods` WHERE order_id=\"$order_id\"");
