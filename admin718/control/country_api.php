...
class country_apiControl extends SystemControl {
	...
	public function focus_editOp() {
		...
		$screen_adv_list = $model_country->getAdvList("screen");//焦点大图广告数据
		Tpl::output('screen_adv_list',$screen_adv_list);
		$focus_adv_list = $model_country->getAdvList("focus");//三张联动区广告数据
		Tpl::output('focus_adv_list',$focus_adv_list);

		Tpl::showpage('country_focus.edit');
	}
		/**
	 * 底部图设置
	 */
	public function focus2_editOp() {
	    $model_country = Model('country');
	    $web_id = '102';
	    $code_list = $model_country->getCodeList(array('web_id'=> $web_id));
	    if(is_array($code_list) && !empty($code_list)) {
		...
		Tpl::showpage('country_focus2.edit');
	}
	...
	public function html_updateOp() {
		...
		if(!empty($web_array) && is_array($web_array)) {
			$model_country->updateWebHtml($web_id);
	...
	public function sale_editOp() {
		...
		Tpl::showpage('country_sale.edit');
	}
	...
	public function category_listOp() {
		...
		Tpl::output('goods_class',$goods_class);
		Tpl::showpage('country_goods_class','null_layout');
	}
	...
	public function recommend_listOp() {
		...
		$goods_list = $model_country->getGoodsList($condition,'goods_id desc',6);
       // echo "why";
		Tpl::output('show_page',$model_country->showpage(2));
		Tpl::output('goods_list',$goods_list);
		Tpl::showpage('country_goods.list','null_layout');
	}
	...
	public function recommends_listOp() {
		...
		Tpl::output('show_page',$model_country->showpage(2));
		Tpl::output('goods_list',$goods_list);
		Tpl::showpage('country_goods_list','null_layout');
	}
	...
	public function goods_listOp() {
		...
		$goods_list = $model_country->getGoodsList($condition,$order,6);
		Tpl::output('show_page',$model_country->showpage(2));
		Tpl::output('goods_list',$goods_list);
		Tpl::showpage('country_goods_order','null_layout');
	}
	...
	public function brand_listOp() {
		...
		Tpl::showpage('country_brand.list','null_layout');
	}
	...
	public function code_updateOp() {
		...
		$model_country = Model('country');
		$code = $model_country->getCodeRow($code_id,$web_id);
		if (!empty($code)) {
			$code_type = $code['code_type'];
			$var_name = $code['var_name'];
			$code_info = $_POST[$var_name];
			$code_info = $model_country->get_str($code_info,$code_type);
			$state = $model_country->updateCode(array('code_id'=> $code_id),array('code_info'=> $code_info));
		}
		if($state) {
			echo '1';exit;
		} else {
	 ...
	 * 详情页保存设置
	 */
	public function code_country_updateOp() {
		$code_id = intval($_POST['code_id']);
		$web_id = intval($_POST['web_id']);
		$model_country = Model('country');
		$code = $model_country->getCodeRow($code_id,$web_id);
		if (!empty($code)) {
			...
			$code_info = $_POST[$var_name];
			$code_info = $model_country->get_str($code_info,$code_type);
			$state = $model_country->updateCode(array('code_id'=> $code_id),array('country_info'=> $code_info));
		}
		if($state) {
			echo '1';exit;
		} else {
	...
	public function upload_picOp() {
		...
		if (!empty($code)) {
			...
			$code_info = $model_country->get_str($code_info,$code_type);
			$state = $model_country->updateCode(array('code_id'=> $code_id),array('code_info'=> $code_info));
			Tpl::showpage('country_upload_pic','null_layout');
		}
	}
	 ...
	 * 保存详情页图片
	 */
	public function upload_country_picOp() {
		$code_id = intval($_POST['code_id']);
		$web_id = intval($_POST['web_id']);
		$model_country = Model('country');
		$code = $model_country->getCodeRow($code_id,$web_id);
		if (!empty($code)) {
			...
			$file_name = 'country-'.$web_id.'-'.$code_id;
			$pic_name = $this->_upload_pic($file_name);//上传图片
			if (!empty($pic_name)) {
				$code_info['pic'] = $pic_name;
			}
	...
	public function recommend_picOp() {
		...
		if (!empty($code) && $key_id > 0 && $pic_id > 1) {
			...
			$code_type = $code['code_type'];
			$code_info = $model_country->get_array($code_info,$code_type);//原数组
			...
			$code_info = $model_country->get_str($code_info,$code_type);
			$state = $model_country->updateCode(array('code_id'=> $code_id),array('code_info'=> $code_info));
			Tpl::showpage('country_recommend_pic','null_layout');
		}
	}
	...
	public function slide_advOp() {
		...
		if (!empty($code)) {
			...
			$code_info = $model_country->get_str($code_info,$code_type);
			$model_country->updateCode(array('code_id'=> $code_id),array('code_info'=> $code_info));

    		Tpl::showpage('country_upload_slide','null_layout');
		}
	}
	...
	public function screen_picOp() {
		...
		if (!empty($code)) {
			...
			if ($pic_id > 0 && $pic_id == $key) {
    			...
    			$file_name = 'country-'.$web_id.'-'.$code_id.'-'.$pic_id;
    			$pic_name = $this->_upload_pic($file_name);//上传图片
    			if (!empty($pic_name)) {
    				$pic_info['pic_img'] = $pic_name;
    			}
    		...
    		Tpl::showpage('country_upload_screen','null_layout');
		}
	}
	...
	public function focus_picOp() {
		...
		if (!empty($code)) {
    		...
    		Tpl::showpage('country_upload_focus','null_layout');
		}
	}
	...
	private function _upload_pic($file_name) {
	    ...
	    if (!empty($file_name)) {
			if (!empty($_FILES['pic']['name'])) {//上传图片
    			...
    			$upload->set('default_dir','/country');
    			$upload->set('file_name',$file_name.".".$ext);
				$result = $upload->upfile('pic');
				if ($result) {
					$pic_name = '/country'."/".$upload->file_name.'?'.mt_rand(100,999);//加随机数防止浏览器缓存图片
				}
			}
	    }
	    return $pic_name;
