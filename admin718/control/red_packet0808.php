...
//        }
    ...
    public function red_packet_settingOp(){
        ...
        //Tpl::output('red_packet_price_region',$red_packet_price_region);
...
/**
    ...
    private function createExcel($data = array())
    {
        ...
        foreach ((array)$data as $k => $v) {
            ...
            $tmp[] = array('data' => empty($val['open_time'])?'无':date('Y-m-d H:i:s',$val['open_time']));
        ...
        $excel_obj->addWorksheet($excel_obj->charset('红包信息', CHARSET));
        $excel_obj->generateXML($excel_obj->charset('红包信息', CHARSET) . $_GET['curpage'] . '-' . date('Y-m-d-H', time()));
    }
...
//导出提现信息
    public function exportOp(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            $limit1 = ($_GET['curpage']-1) * self::EXPORT_SIZE;
            $limit2 = self::EXPORT_SIZE;
            $data = $model_withdraw_records->getWithdrawrecordsList($condition,'','*','withdraw_recordid asc',"{$limit1},{$limit2}");
            $this->createExcel1($data);
        }
    }
...
*/
    private function createExcel1($data = array()){
        ...
        foreach ((array)$data as $k=>$v){
            ...
            $tmp[] = array('data'=>empty($v['withdraw_time'])?'无':date('Y-m-d H:i:s',$v['withdraw_time']));
            $tmp[] = array('data'=>$v['withdraw_amount']);
            $excel_data[] = $tmp;
        }
        $excel_data = $excel_obj->charset($excel_data,CHARSET);
        $excel_obj->addArray($excel_data);
        $excel_obj->addWorksheet($excel_obj->charset('提现信息',CHARSET));
        $excel_obj->generateXML($excel_obj->charset('提现信息',CHARSET).$_GET['curpage'].'-'.date('Y-m-d-H',time()));
    }
}
