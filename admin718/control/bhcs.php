...
class bhcsControl extends SystemControl{
	private $links = array(
	...
	public function baseOp(){
		...
		//备货参数
		$model_bhcs = Model ( 'ctax_emallyibase' );
        //Tpl::output('ecpcodeciq',1123);
        //Tpl::output('companycode',$model_bhcs -> where(array('shop_id'=>5)) ->select());
        Tpl::output('ecpcodeciq',$model_bhcs->getfby_emallyi_id(1,'ecpcodeciq'));
        Tpl::output('ecpcodecus',$model_bhcs->getfby_emallyi_id(1,'ecpcodecus'));
        Tpl::output('ecpnameciq',$model_bhcs->getfby_emallyi_id(1,'ecpnameciq'));
        Tpl::output('ecpnamecus',$model_bhcs->getfby_emallyi_id(1,'ecpnamecus'));
        Tpl::output('emallyi_version',$model_bhcs->getfby_emallyi_id(1,'emallyi_version'));
        Tpl::output('companycode',$model_bhcs->getfby_emallyi_id(1,'companycode'));



		Tpl::showpage('bhcs.base');
	}
    ...
    public function edit_save_bhcsOp() {
     $model_bhcs = Model ( 'ctax_emallyibase' );
        ...
        $data = array();
        $data['ecpcodeciq']         = $_POST['bh_ecpcodeciq'];
        $data['ecpcodecus']       = $_POST['bh_ecpcodecus'];
        $data['ecpnameciq']       = $_POST['bh_ecpnameciq'];
        $data['ecpnamecus']       = $_POST['bh_ecpnamecus'];
        $data['emallyi_version']       = $_POST['bh_emallyi_version'];
        $data['companycode']       = $_POST['bh_companycode'];
        //$data['']                  =  系统当前时间
        //echo $data['ecpcodeciq'];
        $model_bhcs -> where(array('emallyi_id'=>1)) -> update($data);
        //echo "<script type=\"text/javascript\">alert (".$data['ecpcodeciq'].");</script>";
        header('Location: index.php?act=bhcs&op=base');
}
