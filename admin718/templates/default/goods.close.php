...
<div class="page">
  ...
  <form method='post' id="form_goods" action="<?php echo urlAdmin('goods', 'goods_del');?>">
    <input type="hidden" id="goods_commonid"  name="goods_commonid" value="ok" />
      ...
      <tbody>
        ...
        <tr class="hover edit">
          ...
          <td class="w48 align-center"><a href="<?php echo urlShop('goods', 'index', array('goods_id' => $output['storage_array'][$v['goods_commonid']]['goods_id']));?>" target="_blank"><?php echo $lang['nc_view'];?></a>&nbsp;|&nbsp;
            <!-- <a href="javascript:void(0);" nctype="del_batch" uri="<?php echo urlAdmin('goods', 'goods_del', array('goods_id' => $v['goods_commonid']));?>"><?php echo $lang['nc_del'];?></a> -->
            <a href="JavaScript:void(0);" onclick="submit_delete('<?php echo $v['goods_commonid'];?>');"><?php echo $lang['nc_del'];?></a>
          </td>
        </tr>
...
function submit_delete(id){
    if(confirm('<?php echo $lang['nc_ensure_del'];?>')) {
        $('#form_goods').attr('method','post');
        $('#form_goods').attr('action','index.php?act=goods&op=goods_del');
        $('#goods_commonid').val(id);
        $('#form_goods').submit();
    }
}
</script>
