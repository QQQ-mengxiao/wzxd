            ...
            <tbody>
              ...
              <tr>
                ...
                <td class="w96 align-center"><span class="red_common">
                  <?php
                  $tax_rate=unserialize($goods['kuajing_info']);
                   if ($tax_rate['goods_tax_rate']=='') {
                    $goid=$goods['goods_id'];
                    $goods_tax_rate = Model()->query("SELECT goods_tax_rate FROM `718shop_goods` where goods_id=\"$goid\" LIMIT 10");
                    $goods_tax_rate=$goods_tax_rate[0]['goods_tax_rate'];
                    $tax_rate1=round($goods_tax_rate * $goods['goods_price'],2);
                  }else
                   {
                    $tax_rate1=round($tax_rate['goods_tax_rate'] * $goods['goods_price'],2);
                  }

                  echo $lang['currency']. $tax_rate1;?></span></td>
