...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base">
        ...
        <li><a href="index.php?act=kuajing_platform&op=add" ><span><?php echo $lang['nc_new'];?></span></a></li>
      </ul>
    </div>
  </div>
      ...
      <tbody>
        <tr>
          ...
          <td><a href="javascript:document.formSearch.submit();" class="btn-search " title="<?php echo $lang['nc_query']; ?>">&nbsp;</a>
            ...
            <a class="btns " href="index.php?act=kuajing_platform&op=index" title="<?php echo $lang['cancel_search'];?>"><span>取消检索<?php echo $lang['cancel_search'];?></span></a>
            <?php }?></td>
        </tr>
      </tbody>
    </table>
  ...
  <form id="form_grade" method='post' name="">
    <input type="hidden" name="platform_id" id="platform_id" value="ok" />
    <table class="table tb-type2">
      ...
      <tbody>
        ...
        <?php $no++;?>
        <tr class="hover">
        <td><input type="checkbox" name='check_id[]' value="<?php echo $v['platform_id'];?>" class="checkitem"></td>
          <td class="align-center"><?php echo $no;?></td>
          <td class="align-center"><?php echo $v['platform_name'];?></td>
          <td class="align-center"><?php echo $v['code_guan'];?></td>
          <td class="align-center"><?php echo $v['code_jian'];?></td>
          <td class="w270 align-center">
		    <a href="index.php?act=kuajing_platform&op=edit&platform_id=<?php echo $v['platform_id'];?> "><?php echo $lang['nc_edit'];?></a> |<a href="javascript:submit_delete(<?php echo $v['platform_id'];?>)">
...
function submit_delete(platform_id){
    if(confirm('<?php echo $lang['nc_ensure_del'];?>')) {
        $('#form_grade').attr('method','post');
        $('#form_grade').attr('action','index.php?act=kuajing_platform&op=del');
