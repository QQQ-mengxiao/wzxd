      ...
      <tbody>
        <tr>
          <td><select name="search_field_name" >
              ...
              <option <?php if($output['search_field_name'] == 'is_store'){ ?>selected='selected'<?php } ?> value="is_store">是否商户</option>     
              <option <?php if($output['search_field_name'] == 'is_tuan'){ ?>selected='selected'<?php } ?> value="is_tuan">是否团长</option>       
            </select></td>
    ...
    <tbody>
      ...
      <tr>
        <td><ul>
            ...
            <li>筛选条件选择是否为团长/商户，后面文本框输入是/否进行检索</li>
          </ul></td>
      </tr>
    </tbody>
  </table>
    ...
    <table class="table tb-type2 nobdb">
      ...
      <tbody>
        ...
        <tr class="hover member">
          ...
          <?php if($v['is_store']==2){ ?>
          <a href="index.php?act=member&op=member_store&  is_store=1&member_id=<?php echo $v['member_id']; ?>">取消商户</a>
          <?php }else {?>
            <a href="index.php?act=member&op=member_store&is_store=2&member_id=<?php echo $v['member_id']; ?>">设置商户</a> 
             <?php } ?>
             <?php if($v['is_tuan']==1){ ?>
                 | <a href="index.php?act=member&op=member_tuan&is_tuan=0&member_id=<?php echo $v['member_id']; ?>">取消团长</a> 
              <?php }else {?>
                 | <a href="index.php?act=member&op=member_tuan&is_tuan=1&member_id=<?php echo $v['member_id']; ?>">设置团长</a> 
             <?php } ?>
             | <a href="index.php?act=member&op=member_edit&member_id=<?php echo $v['member_id']; ?>"><?php echo $lang['nc_edit']?></a></td>
...
<!--          <?php if($v['member_verify']==0){ ?>
           <td class="align-center">认证不通过</td>
         <?php }elseif ($v['member_verify']==1) {?>
             <td class="align-center">认证通过</td>
          <?php }elseif ($v['member_verify']==10) {?>
            <td class="align-center">待认证</td>
          <?php }else {?>
             <td class="align-center"><a href="index.php?act=member&op=audit&member_id=<?php echo $v['member_id']; ?>">认证审核</a> </td>
             <?php } ?> -->
