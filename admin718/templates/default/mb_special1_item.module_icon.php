...
<div class="index_block home3">
  ...
  <div nctype="item_content" class="content">
    ...
    <div nctype="item_image" class="item"> <img nctype="image" style="width:55px;padding-left: 8px;padding-top: 5px; " src="<?php echo getMbSpecial1ImageUrl($item_value['image']);?>" alt="">
      <?php if($item_edit_flag) { ?>
      <input nctype="image_name" name="item_data[item][<?php echo $item_key;?>][image]" type="hidden" value="<?php echo $item_value['image'];?>">
      <input nctype="image_names" name="item_data[item][<?php echo $item_key;?>][name]" type="hidden" value="<?php echo $item_value['name'];?>">
      ...
      <a nctype="btn_del_item_image" href="javascript:;"><i class="icon-trash"></i>删除</a>
          <a nctype="btn_edit_item_image" data-desc="88*88" href="javascript:;" style="right: 62px;"><i class="icon-edit"></i>编辑</a>
      <?php } ?>
    </div>
    <?php } ?>
    <?php } ?>
  ...
  <?php if($item_edit_flag) { ?>
  <a nctype="btn_add_item_image" class="btn-add" data-desc="320*85" href="javascript:;">添加新的内容</a>
  <?php } ?>
</div>
