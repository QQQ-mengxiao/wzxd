...
<div class="page">
  ...
  <div class="fixed-empty"></div>
    <table class="table tb-type2 nobdb">
      <tbody>
        <tr class="noborder">
          <td colspan="2" class="required"><label>提现单号:</label></td>
        </tr>
        <tr class="noborder">
      ...
      <?php if (!intval($output['info']['pdc_payment_state'])) {?>
        ...
        <tr class="tfoot">
        <td colspan="2">
        <a class="btn" href="javascript:if (confirm('<?php echo $lang['admin_predeposit_cash_confirm'];?>')){window.location.href='index.php?act=brokerage&op=pd_cash_pay&id=<?php echo $output['info']['pdc_id']; ?>';}else{}"><span>确认支付</span></a>
        </td>
        </tr>
        </tfoot>
     <?php } ?>
