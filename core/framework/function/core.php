...
function write_file($filepath, $data, $mode = null)
{
    ...
    $data = "<?php defined('In718Shop') or exit('Access Invalid!'); return ".$data.";";
 ...
 * 获取运单图片地址
 */
function getMbSpecial1ImageUrl($image_name = '') {
    $name_array = explode('_', $image_name);
    if(count($name_array) == 2) {
        $image_path = DS . ATTACH_MOBILE . DS . 'special1' . DS . $name_array[0] . DS . $image_name;
    } else {
        $image_path = DS . ATTACH_MOBILE . DS . 'special1' . DS . $image_name;
    }
    if(is_file(BASE_UPLOAD_PATH . $image_path)) {
        return UPLOAD_SITE_URL . $image_path;
    } else {
