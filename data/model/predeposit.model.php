...
class predepositModel extends Model {
    ...
    public function changeCard($type,$data=array()){
        ...
        switch($type){
            case 'order_pay': 
              ...
              $pay_data = array(
                  "consumerMoney" => strval($data['amount']),
                  "orderSn" => strval($data['order_sn']),
                  "personalID" => strval($data['gonghao']),
              );
            break;
            case 'refund':
              $card_type='refund';
        ...
        //支付成功为true
        $cardPayResult = Model('card_new')->updateCardBaseAndConsume($pay_data);
     ...
     *变更新零售一余额
     */
    public function exchangeCard($type,$data=array()){
        //更新本地日志表
        $data_log=array();
        //更新一卡通会员信息表
        $data_card=array();
        ...
        $data_msg=array();
        $data_log['member_id']=$data['member_id'];
        $data_log['cardno']=$data['gonghao'];//改为工号
        $data_log['log_time']=TIMESTAMP;
        $data_card['LastDate']=date('Y/m/d H:i:s');
        $data_consume['ConsumeDate']=date('Y-m-d');
        $data_consume['ConsumeTime']=date('H:i:s');
        //ADD
         $data_consume['OrderSno']=$data['order_sn'];
        $data_card['gonghao']=$data['gonghao'];
        $data_msg['time']=date('Y-m-d H:i:s');
        $data_msg['url']=urlShop('predeposit','card_log_list');
        ...
        switch($type){
            case 'order_pay': 
              ...
              $data_log['action']=1;
              $data_log['avai_amount']=$data['amount'];
              $data_log['content']='下单，新零售余额支付，订单号：'.$data['order_sn'];
            ...
            case 'refund':
              $card_type='refund';
               $data_log['action']=2;
              $data_log['order_sn']=$data['order_sn'];
              $data_log['content']='确认退款，订单号：'.$data['order_sn'];
               $data_log['avai_amount']=$data['amount'];
              $data_card['amount']=$data['amount'];
        ...
        //支付成功state0
        $cardPayResult = Model('card_new')->updateHCConsumeExchange($pay_data);
        if($cardPayResult['state']==0){
            $data_log['result']=1;
            $insert = Model()->table('member_exchange_log')->insert($data_log);
        }else{
             $data_log['result']=2;
            $insert = Model()->table('member_exchange_log')->insert($data_log);
        }
