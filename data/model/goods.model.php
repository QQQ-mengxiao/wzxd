...
class goodsModel extends Model{
    ...
    */
    ...
    //shang商品信息导出用
     public function getGoodsCommonList2($condition, $field = '*', $page = 10, $order = 'goods_commonid desc') {
        // sleep(50);
        $condition = $this->_getRecursiveClass($condition);
    ...
    public function getGoodsCommonOnlineList($condition, $field = '*', $page = 10, $order = "goods_commonid desc") {
     ...
     //商品导出
    public function getGoodsCommonOnlineList2($condition, $field = '*', $page = 10, $order = "goods_commonid desc") {
        $condition['goods_state']   = self::STATE1;
        $condition['goods_verify']  = self::VERIFY1;
        // sleep(50);
    ...
    public function editProducesNewOnline($condition,$commonid){
        ...
        $sql = "UPDATE 718shop_goods SET is_group_ladder=7,goods_state=".self::STATE1." WHERE goods_commonid IN (".$commonid.")";
        //adddslk
         $insert_arr=array();
         $model_goodspromotion=Model('goods_promotion'); 
        $goods_list =$this->getGoodsList($condition, 'goods_id,goods_price');
        ...
        //批量增加
        $model_goodspromotion->addallgoods_promotion($insert_arr);
        // ob_start();
        // var_dump($commonid);
        // $result = ob_get_clean();
...
/**
	...
	public function getMobileBodyByCommonID($goods_commonid, $fields = 'mobile_body') {
     ...
     public function getGoodsAllList($condition, $field = '*', $group = '',$order = '', $limit = 0, $page = 0, $lock = false, $count = 0){
         ...
         return $this->table('goods_common')->field($field)->where($condition)->order($order)->group($group)->limit($limit)->page($page, $count)->lock($lock)->select();
          ...
          public function getSearchGoodsList($keyword, $cate_id, $ziti_id,  $offset=0, $sortkey=2, $postorder, $ngoods_sign, $allgoods,$member_grade,$xinren_str,$by_post) {
        $sortorder = "( CASE WHEN a.goods_storage = 0 THEN 1 ELSE 0 END ),a.tempsort,a.goods_storage DESC,a.goods_id DESC";
        if($sortkey==2){
            if($postorder == 'false'){
                $sortorder = "( CASE WHEN a.goods_storage = 0 THEN 1 ELSE 0 END ),a.tempsort,a.goods_storage DESC,a.goods_id DESC";
            }else{
                $sortorder = "( CASE WHEN a.goods_storage = 0 THEN 1 ELSE 0 END ),a.tempsort,a.goods_storage ASC,a.goods_id DESC";
            }
        }elseif($sortkey==1){
            if($postorder =='false'){
                $sortorder = "( CASE WHEN a.goods_storage = 0 THEN 1 ELSE 0 END ),a.tempsort,a.sale_num DESC,a.goods_id DESC";
            }else{
                $sortorder = "( CASE WHEN a.goods_storage = 0 THEN 1 ELSE 0 END ),a.tempsort,a.sale_num ASC,a.goods_id DESC";
            }
        }elseif($sortkey==3){
            if($postorder=='false'){
                $sortorder = "( CASE WHEN a.goods_storage = 0 THEN 1 ELSE 0 END ),a.tempsort,goods_price DESC,a.goods_id DESC";
            }else{
                $sortorder = "( CASE WHEN a.goods_storage = 0 THEN 1 ELSE 0 END ),a.tempsort,goods_price ASC,a.goods_id DESC";
            }
        }
        if($ngoods_sign==1){//新品
            if($sortkey==4){
                if($postorder == 'false'){
                    $sortorder = "( CASE WHEN a.goods_storage = 0 THEN 1 ELSE 0 END ),a.tempsort,a.goods_puton_time DESC,a.goods_id DESC";
                }else{
                    $sortorder = "( CASE WHEN a.goods_storage = 0 THEN 1 ELSE 0 END ),a.tempsort,a.goods_puton_time ASC,a.goods_id DESC";
                }
            }
            $sql = "SELECT * FROM (SELECT e.goods_commonid,e.goods_id,e.goods_name,MIN(e.goods_promotion_price) AS goods_promotion_price,e.goods_price AS goods_price,sum(e.goods_storage) AS goods_storage,e.goods_puton_time AS goods_puton_time,sum(e.goods_salenum+e.goods_presalenum) AS sale_num,e.is_group_ladder,e.goods_marketprice,e.goods_image,2 AS tempsort,e.is_vip_price,e.by_post FROM (SELECT sg.goods_commonid,sg.goods_name,sg.is_group_ladder,sg.goods_marketprice,sg.goods_storage,sgc.goods_puton_time,sgc.goods_image,sg.goods_salenum,sg.goods_presalenum,sgc.is_vip_price,sg.goods_id,sbdg.buy_deliver_goods_id,sg.goods_price AS goods_price,ifnull(d.price,sg.goods_price) AS goods_promotion_price,ds.by_post FROM 718shop_goods sg INNER JOIN (SELECT DISTINCT sg.goods_commonid FROM 718shop_goods sg) g ON g.goods_commonid=sg.goods_commonid LEFT JOIN (SELECT 718shop_daddress.address_id,718shop_storage.by_post FROM 718shop_daddress LEFT JOIN 718shop_storage ON 718shop_daddress.storage_id = 718shop_storage.storage_id) ds ON ds.address_id = sg.deliverer_id LEFT JOIN (SELECT sgp.goods_id,min(price) AS price FROM 718shop_goods_promotion sgp WHERE CASE sgp.promotion_type WHEN 30 THEN sgp.member_levels<=".$member_grade." ELSE 1 END ".$xinren_str." GROUP BY sgp.goods_id) d ON d.goods_id=sg.goods_id LEFT JOIN 718shop_buy_deliver_goods sbdg ON sg.goods_id=sbdg.goods_id LEFT JOIN 718shop_goods_common sgc ON sg.goods_commonid=sgc.goods_commonid WHERE sg.goods_state=1 AND (sbdg.ziti_id=".$ziti_id." OR sbdg.ziti_id IS NULL) AND sg.goods_verify=1 AND sgc.is_new=1 AND sg.is_deleted=0) e GROUP BY goods_commonid ORDER BY goods_promotion_price LIMIT 99999) a ORDER BY".$sortorder." limit 10 offset ".$offset;
            $goods_list = Model()->query($sql);
            return $goods_list;
        }
        if($allgoods){//全部商品
            $sql = "SELECT * FROM (SELECT e.goods_commonid,e.goods_id,e.goods_name,MIN(e.goods_promotion_price) AS goods_promotion_price,e.goods_price AS goods_price,sum(e.goods_storage) AS goods_storage,sum(e.goods_salenum+e.goods_presalenum) AS sale_num,e.is_group_ladder,e.goods_marketprice,e.goods_image,2 AS tempsort,e.is_vip_price,e.by_post FROM (SELECT sg.goods_commonid,sg.goods_name,sg.is_group_ladder,sg.goods_marketprice,sg.goods_storage,sgc.goods_image,sg.goods_salenum,sg.goods_presalenum,sgc.is_vip_price,sg.goods_id,sbdg.buy_deliver_goods_id,sg.goods_price AS goods_price,ifnull(d.price,sg.goods_price) AS goods_promotion_price,ds.by_post FROM 718shop_goods sg INNER JOIN (SELECT DISTINCT sg.goods_commonid FROM 718shop_goods sg) g ON g.goods_commonid=sg.goods_commonid LEFT JOIN (SELECT 718shop_daddress.address_id,718shop_storage.by_post FROM 718shop_daddress LEFT JOIN 718shop_storage ON 718shop_daddress.storage_id = 718shop_storage.storage_id) ds ON ds.address_id = sg.deliverer_id LEFT JOIN (SELECT sgp.goods_id,min(price) AS price FROM 718shop_goods_promotion sgp WHERE CASE sgp.promotion_type WHEN 30 THEN sgp.member_levels<=".$member_grade." ELSE 1 END ".$xinren_str." GROUP BY sgp.goods_id) d ON d.goods_id=sg.goods_id LEFT JOIN 718shop_buy_deliver_goods sbdg ON sg.goods_id=sbdg.goods_id LEFT JOIN 718shop_goods_common sgc ON sg.goods_commonid=sgc.goods_commonid WHERE sg.goods_state=1 AND (sbdg.ziti_id=".$ziti_id." OR sbdg.ziti_id IS NULL) AND sg.goods_verify=1 AND sg.is_deleted=0 ORDER BY goods_promotion_price LIMIT 99999) e GROUP BY goods_commonid) a ORDER BY".$sortorder." limit 10 offset ".$offset;
            $goods_list = Model()->query($sql);
            return $goods_list;
        }
        if($by_post){//包邮商品
            $sql = "SELECT * FROM (SELECT e.goods_commonid,e.goods_id,e.goods_name,MIN(e.goods_promotion_price) AS goods_promotion_price,e.goods_price AS goods_price,sum(e.goods_storage) AS goods_storage,sum(e.goods_salenum+e.goods_presalenum) AS sale_num,e.is_group_ladder,e.goods_marketprice,e.goods_image,2 AS tempsort,e.is_vip_price,e.by_post FROM (SELECT sg.goods_commonid,sg.goods_name,sg.is_group_ladder,sg.goods_marketprice,sg.goods_storage,sg.deliverer_id,ds.by_post,sgc.goods_image,sg.goods_salenum,sg.goods_presalenum,sgc.is_vip_price,sg.goods_id,sbdg.buy_deliver_goods_id,sg.goods_price AS goods_price,ifnull(d.price,sg.goods_price) AS goods_promotion_price FROM 718shop_goods sg INNER JOIN (SELECT DISTINCT sg.goods_commonid FROM 718shop_goods sg) g ON g.goods_commonid=sg.goods_commonid LEFT JOIN (SELECT 718shop_daddress.address_id,718shop_storage.by_post FROM 718shop_daddress LEFT JOIN 718shop_storage ON 718shop_daddress.storage_id = 718shop_storage.storage_id) ds ON ds.address_id = sg.deliverer_id LEFT JOIN (SELECT sgp.goods_id,min(price) AS price FROM 718shop_goods_promotion sgp WHERE CASE sgp.promotion_type WHEN 30 THEN sgp.member_levels<=".$member_grade." ELSE 1 END ".$xinren_str." GROUP BY sgp.goods_id) d ON d.goods_id=sg.goods_id LEFT JOIN 718shop_buy_deliver_goods sbdg ON sg.goods_id=sbdg.goods_id LEFT JOIN 718shop_goods_common sgc ON sg.goods_commonid=sgc.goods_commonid WHERE sg.goods_state=1 AND (sbdg.ziti_id=".$ziti_id." OR sbdg.ziti_id IS NULL) AND sg.goods_verify=1 AND sg.is_deleted=0 AND ds.by_post=1 ORDER BY goods_promotion_price LIMIT 99999) e GROUP BY goods_commonid) a ORDER BY".$sortorder." limit 10 offset ".$offset;
            $goods_list = Model()->query($sql);
            return $goods_list;
        }
        if($keyword){//关键字搜索
            $sql = "SELECT goods_commonid,goods_id,goods_name,goods_price,goods_promotion_price,goods_storage,sale_num,goods_marketprice,goods_image,tempsort,is_vip_price,by_post FROM ((SELECT e.goods_commonid,e.goods_id,e.goods_name,MIN(e.goods_promotion_price) AS goods_promotion_price,e.goods_price AS goods_price,sum(e.goods_storage) AS goods_storage,sum(e.goods_salenum+e.goods_presalenum) AS sale_num,e.goods_marketprice,e.goods_image,1 AS tempsort,e.is_vip_price,e.by_post FROM (SELECT sg.goods_commonid,sg.goods_name,sg.goods_marketprice,sg.goods_storage,sgc.goods_image,sg.goods_salenum,sg.goods_presalenum,sgc.is_vip_price,sg.goods_id,ifnull(d.price,sg.goods_price) AS goods_promotion_price,sg.goods_price AS goods_price,ds.by_post FROM 718shop_goods sg LEFT JOIN (SELECT 718shop_daddress.address_id,718shop_storage.by_post FROM 718shop_daddress LEFT JOIN 718shop_storage ON 718shop_daddress.storage_id = 718shop_storage.storage_id) ds ON ds.address_id = sg.deliverer_id INNER JOIN (SELECT sgc.gc_id FROM 718shop_goods_class sgc WHERE sgc.gc_name LIKE '%".$keyword."%' AND sgc.gc_parent_id> 0) c ON c.gc_id=sg.gc_id LEFT JOIN (SELECT sgp.goods_id,min(price) AS price FROM 718shop_goods_promotion sgp WHERE CASE sgp.promotion_type WHEN 30 THEN sgp.member_levels<=".$member_grade." ELSE 1 END ".$xinren_str." GROUP BY sgp.goods_id) d ON d.goods_id=sg.goods_id LEFT JOIN 718shop_buy_deliver_goods sbdg ON sg.goods_id=sbdg.goods_id LEFT JOIN 718shop_goods_common sgc ON sg.goods_commonid=sgc.goods_commonid WHERE sg.goods_state=1 AND (sbdg.ziti_id=".$ziti_id." OR sbdg.ziti_id IS NULL) AND sg.goods_verify=1 AND sg.is_deleted=0 ORDER BY goods_promotion_price LIMIT 99999) e GROUP BY goods_commonid) UNION (SELECT e.goods_commonid,e.goods_id,e.goods_name,MIN(e.goods_promotion_price) AS goods_promotion_price,e.goods_price as goods_price,sum(e.goods_storage) AS goods_storage,sum(e.goods_salenum+e.goods_presalenum) AS sale_num,e.goods_marketprice,e.goods_image,2 AS tempsort,e.is_vip_price,e.by_post FROM (SELECT sg.goods_commonid,sg.goods_name,sg.goods_marketprice,sg.goods_storage,sgc.goods_image,sg.goods_salenum,sg.goods_presalenum,sgc.is_vip_price,sg.goods_id,ifnull(d.price,sg.goods_price) AS goods_promotion_price,sg.goods_price,ds.by_post FROM 718shop_goods sg LEFT JOIN (SELECT 718shop_daddress.address_id,718shop_storage.by_post FROM 718shop_daddress LEFT JOIN 718shop_storage ON 718shop_daddress.storage_id = 718shop_storage.storage_id) ds ON ds.address_id = sg.deliverer_id INNER JOIN (SELECT DISTINCT sg.goods_commonid FROM 718shop_goods sg WHERE sg.goods_name LIKE '%".$keyword."%' AND sg.is_deleted=0) g ON g.goods_commonid=sg.goods_commonid LEFT JOIN (SELECT sgp.goods_id,min(price) AS price FROM 718shop_goods_promotion sgp WHERE CASE sgp.promotion_type WHEN 30 THEN sgp.member_levels<=".$member_grade." ELSE 1 END ".$xinren_str." GROUP BY sgp.goods_id) d ON d.goods_id=sg.goods_id LEFT JOIN 718shop_buy_deliver_goods sbdg ON sg.goods_id=sbdg.goods_id LEFT JOIN 718shop_goods_common sgc ON sg.goods_commonid=sgc.goods_commonid WHERE sg.goods_state=1 AND (sbdg.ziti_id=".$ziti_id." OR sbdg.ziti_id IS NULL) AND sg.goods_verify=1 AND sg.is_deleted=0 ORDER BY goods_promotion_price LIMIT 99999) e GROUP BY goods_commonid) UNION (SELECT e.goods_commonid,e.goods_id,e.goods_name,MIN(e.goods_promotion_price) AS goods_promotion_price,e.goods_price as goods_price,sum(e.goods_storage) AS goods_storage,sum(e.goods_salenum+e.goods_presalenum) AS sale_num,e.goods_marketprice,e.goods_image,2 AS tempsort,e.is_vip_price,e.by_post FROM (SELECT sg.goods_commonid,sg.goods_name,sg.goods_marketprice,sg.goods_storage,sgc.goods_image,sg.goods_salenum,sg.goods_presalenum,sgc.is_vip_price,sg.goods_id,ifnull(d.price,sg.goods_price) AS goods_promotion_price,sg.goods_price,ds.by_post FROM 718shop_goods sg LEFT JOIN (SELECT 718shop_daddress.address_id,718shop_storage.by_post FROM 718shop_daddress LEFT JOIN 718shop_storage ON 718shop_daddress.storage_id = 718shop_storage.storage_id) ds ON ds.address_id = sg.deliverer_id INNER JOIN (SELECT DISTINCT sgc.goods_commonid FROM 718shop_goods_common sgc WHERE sgc.goods_class_name_1 LIKE '%".$keyword."%' OR sgc.goods_class_name LIKE '%".$keyword."%') g ON g.goods_commonid=sg.goods_commonid LEFT JOIN (SELECT sgp.goods_id,min(price) AS price FROM 718shop_goods_promotion sgp WHERE CASE sgp.promotion_type WHEN 30 THEN sgp.member_levels<=".$member_grade." ELSE 1 END ".$xinren_str." GROUP BY sgp.goods_id) d ON d.goods_id=sg.goods_id LEFT JOIN 718shop_buy_deliver_goods sbdg ON sg.goods_id=sbdg.goods_id LEFT JOIN 718shop_goods_common sgc ON sg.goods_commonid=sgc.goods_commonid WHERE sg.goods_state=1 AND (sbdg.ziti_id=".$ziti_id." OR sbdg.ziti_id IS NULL) AND sg.goods_verify=1 AND sg.is_deleted=0 ORDER BY goods_promotion_price LIMIT 99999) e GROUP BY goods_commonid) UNION (SELECT e.goods_commonid,e.goods_id,e.goods_name,MIN(e.goods_promotion_price) AS goods_promotion_price,e.goods_price as goods_price,sum(e.goods_storage) AS goods_storage,sum(e.goods_salenum+e.goods_presalenum) AS sale_num,e.goods_marketprice,e.goods_image,3 AS tempsort,e.is_vip_price,e.by_post FROM (SELECT sg.goods_commonid,sg.goods_name,sg.goods_marketprice,sg.goods_storage,sgc.goods_image,sg.goods_salenum,sg.goods_presalenum,sgc.is_vip_price,sg.goods_id,ifnull(d.price,sg.goods_price) AS goods_promotion_price,sg.goods_price,ds.by_post FROM 718shop_goods sg LEFT JOIN (SELECT 718shop_daddress.address_id,718shop_storage.by_post FROM 718shop_daddress LEFT JOIN 718shop_storage ON 718shop_daddress.storage_id = 718shop_storage.storage_id) ds ON ds.address_id = sg.deliverer_id INNER JOIN (SELECT sgc.gc_id FROM 718shop_goods_class sgc WHERE sgc.gc_name LIKE '%".$keyword."%' AND sgc.gc_parent_id=0) c ON c.gc_id=sg.gc_id_1 LEFT JOIN (SELECT sgp.goods_id,min(price) AS price FROM 718shop_goods_promotion sgp WHERE CASE sgp.promotion_type WHEN 30 THEN sgp.member_levels<=".$member_grade." ELSE 1 END ".$xinren_str." GROUP BY sgp.goods_id) d ON d.goods_id=sg.goods_id LEFT JOIN 718shop_buy_deliver_goods sbdg ON sg.goods_id=sbdg.goods_id LEFT JOIN 718shop_goods_common sgc ON sg.goods_commonid=sgc.goods_commonid WHERE sg.goods_state=1 AND (sbdg.ziti_id=".$ziti_id." OR sbdg.ziti_id IS NULL) AND sg.goods_verify=1 AND sg.is_deleted=0 ORDER BY goods_promotion_price LIMIT 99999) e GROUP BY goods_commonid)) a GROUP BY a.goods_commonid ORDER BY ".$sortorder." limit 10 offset ".$offset;
            $goods_list = Model()->query($sql);
            return $goods_list;
        }
        if($cate_id){//分类搜索
            $sql="SELECT*FROM (SELECT e.goods_commonid,e.goods_id,e.goods_name,MIN(e.goods_promotion_price) AS goods_promotion_price,e.goods_price AS goods_price,sum(e.goods_storage) AS goods_storage,sum(e.goods_salenum+e.goods_presalenum) AS sale_num,e.is_group_ladder,e.goods_marketprice,e.goods_image,2 AS tempsort,e.is_vip_price,e.by_post FROM (SELECT sg.goods_commonid,sg.goods_name,sg.is_group_ladder,sg.goods_marketprice,sg.goods_storage,sgc.goods_image,sg.goods_salenum,sg.goods_presalenum,sgc.is_vip_price,sg.goods_id,sbdg.buy_deliver_goods_id,ifnull(d.price,sg.goods_price) AS goods_promotion_price,sg.goods_price,ds.by_post FROM 718shop_goods sg INNER JOIN (SELECT DISTINCT sg.goods_commonid FROM 718shop_goods sg) g ON g.goods_commonid=sg.goods_commonid LEFT JOIN (SELECT 718shop_daddress.address_id,718shop_storage.by_post FROM 718shop_daddress LEFT JOIN 718shop_storage ON 718shop_daddress.storage_id = 718shop_storage.storage_id) ds ON ds.address_id = sg.deliverer_id LEFT JOIN (SELECT sgp.goods_id,min(price) AS price FROM 718shop_goods_promotion sgp WHERE CASE sgp.promotion_type WHEN 30 THEN sgp.member_levels<=".$member_grade." ELSE 1 END ".$xinren_str." GROUP BY sgp.goods_id) d ON d.goods_id=sg.goods_id LEFT JOIN 718shop_buy_deliver_goods sbdg ON sg.goods_id=sbdg.goods_id LEFT JOIN 718shop_goods_common sgc ON sg.goods_commonid=sgc.goods_commonid WHERE sg.goods_state=1 AND (sbdg.ziti_id=".$ziti_id." OR sbdg.ziti_id IS NULL) AND sg.goods_verify=1 AND sg.is_deleted=0 AND (sgc.gc_id_1=".$cate_id." or sgc.gc_id_3=".$cate_id." OR sgc.goods_class_id LIKE '%,". $cate_id .",%' OR sgc.goods_class_id_1 LIKE '%,". $cate_id .",%') ORDER BY goods_promotion_price LIMIT 99999) e GROUP BY goods_commonid) a ORDER BY ".$sortorder." limit 10 offset ".$offset;
            $goods_list = Model()->query($sql);
            return $goods_list;
        }
     }
    ...
    public function addNewPromotion($common_id)
    {
        ...
        $goods_list = Model()->table('goods')->where(['goods_commonid' => $common_id])->field('goods_id,goods_price')->select();
        foreach ($goods_list as $goods) {
            $temp['goods_id'] = $goods['goods_id'];
            $temp['promotion_type'] = 40;
            $temp['price'] = $goods_show_discount['value'] * $goods['goods_price'] / 100;
            $insert[] = $temp;
        }
        //添加新商品活动表中数据
        $model_goodspromotion->addallgoods_promotion($insert);

        //更新商品表新品活动标记
        Model()->table('goods')->where(['goods_commonid' => $common_id])->update(['is_group_ladder' => 7]);

        //更新公共表商品的新品标记
        $update['goods_puton_time'] = TIMESTAMP;
        $update['goods_show_time'] = $goods_show_time['value'] * 3600;
        $update['goods_new_discount'] = $goods_show_discount['value'];
        $update['is_new'] = 1;
        Model()->table('goods_common')->where(['goods_commonid' => $common_id])->update($update);

    }

    public function removeNewPromotion($common_id)
    {

        //删除商品活动表中数据
        Model()->execute("DELETE FROM 718shop_goods_promotion WHERE goods_id IN (SELECT goods_id FROM 718shop_goods WHERE is_group_ladder = 7 AND goods_commonid = " . $common_id . " ) AND promotion_type = 40");
           file_put_contents('/data/default/wzxd/qlog/delgoods_p.log', date("Y-m-d H:i:s",time()).'common_id'.$common_id.'22222'.$_SERVER['HTTP_REFERER']." \n", FILE_APPEND);
        //更新新品活动标记
        Model()->execute("UPDATE 718shop_goods SET is_group_ladder = 0 WHERE goods_commonid = " . $common_id . " AND is_group_ladder = 7");

        //更新公共表商品的新品标记
        Model()->execute("UPDATE 718shop_goods_common SET goods_puton_time = 0,goods_show_time = 0,goods_new_discount = 0,is_new = 0 WHERE goods_commonid = " . $common_id);

    }

    public function getGoodsExportList($goods_commonid){
        $sql = "SELECT e.goods_id,MIN(e.goods_promotion_price) AS goods_promotion_price,e.gc_id_1,e.gc_id_2,e.gc_id_3,e.goods_name,e.goods_jingle,e.goods_costprice,e.goods_spec,e.goods_freight,e.goods_serial,e.goods_barcode,e.goods_storage_alarm,e.deliverer_id,e.goods_marketprice,e.goods_storage,e.goods_presalenum,e.goods_price,e.promotion_type,e.is_cw FROM (SELECT sg.gc_id_1,sg.gc_id_2,sg.gc_id_3,sg.goods_name,sg.goods_jingle,sg.goods_costprice,sg.goods_spec,sg.goods_freight,sg.goods_serial,sg.goods_barcode,sg.goods_storage_alarm,sg.deliverer_id,sg.goods_marketprice,sg.goods_storage,sg.goods_presalenum,sg.goods_id,ifnull(d.price,sg.goods_price) AS goods_promotion_price,sg.goods_price,d.promotion_type AS promotion_type,sg.is_cw FROM 718shop_goods sg INNER JOIN (SELECT DISTINCT sg.goods_commonid FROM 718shop_goods sg) g ON g.goods_commonid=sg.goods_commonid LEFT JOIN (SELECT sgp.goods_id,min(sgp.price) AS price,GROUP_CONCAT(DISTINCT (CASE sgp.promotion_type WHEN 10 THEN '秒杀' WHEN 20 THEN '折扣' WHEN 30 THEN '会员' WHEN 40 THEN '新品' WHEN 50 THEN '新人' WHEN 60 THEN '进口' WHEN 70 THEN '团购' WHEN 80 THEN '周边' WHEN 90 THEN '进口' ELSE '' END) ORDER BY sgp.price ASC SEPARATOR '/') AS promotion_type FROM 718shop_goods_promotion sgp GROUP BY sgp.goods_id) d ON d.goods_id=sg.goods_id LEFT JOIN 718shop_goods_common sgc ON sg.goods_commonid=sgc.goods_commonid WHERE sg.goods_state=1 AND sg.goods_verify=1 AND sg.is_deleted=0 AND sg.goods_commonid=".$goods_commonid." ORDER BY goods_promotion_price LIMIT 99999) e GROUP BY goods_id";
