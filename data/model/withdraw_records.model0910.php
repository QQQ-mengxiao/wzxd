...
class withdraw_recordsModel extends Model{
     ...
     * 获取记录列表
     */
    public function getWithdrawrecordsList($condition, $page = 0, $field = '*', $order = '', $limit = 0, $count = 0) {
        return $this->table('withdraw_records')->field($field)->where($condition)->order($order)->limit($limit)->page($page, $count)->select();
    }
