	...
	function saveExppointsLog($stage,$insertarr){
		...
		switch ($stage){
			...
			case 'order':
				...
				break;
            case 'card':
                if($insertarr['exp_points'] >= 0){
                    $insertarr['exp_desc'] = '绑定一卡通';
                }else{
                    $insertarr['exp_desc'] = '解绑一卡通';
                }
				break;
            case 'system':
				break;
		...
		if ($result){
			...
			$member_exppoints = $obj_member->getfby_member_id($insertarr['exp_memberid'],'member_exppoints');
			if($member_exppoints + $insertarr['exp_points'] >= 0) {
			$upmember_array['member_exppoints'] = array('exp','member_exppoints + '.$insertarr['exp_points']);
            }else{
                $upmember_array['member_exppoints'] = 0;
            }
