...
class mb_special1Model extends Model{
    ...
    public function __construct() {
        parent::__construct('mb_special1');
    }
    ...
    public function getMbSpecialList($condition, $page='', $order='special_id desc', $field='*') {
        $list = $this->table('mb_special1')->field($field)->where($condition)->page($page)->order($order)->select();
        return $list;
    }
    public function getMbSpecialItemList($condition = array())
    {
    ...
    public function addMbSpecial($param){
        return $this->table('mb_special1')->insert($param);
    }
    ...
    public function editMbSpecial($update, $special_id) {
        ...
        $result = $this->table('mb_special1')->where($condition)->update($update);
        if($result) {
            //删除缓存
    ...
    public function delMbSpecialByID($special_id) {
        ...
        return $this->table('mb_special1')->where($condition)->delete();
    }
    ...
    private function _getMbSpecialItemList($condition, $order = 'item_sort asc') {
        $item_list = $this->table('mb_special_item1')->where($condition)->order($order)->select();
    ...
    public function isMbSpecialItemExist($condition) {
        $item_list = $this->table('mb_special_item1')->where($condition)->select();
        if($item_list) {
            return true;
        } else {
            return false;
    ...
    public function getMbSpecialItemInfoByID($item_id) {
        ...
        $item_info = $this->table('mb_special_item1')->where($condition)->find();
    ...
    private function _initMbSpecialItemGoodsData($item_data, $item_type) {
        ...
        if(!empty($item_data['item'])) {
            ...
            $goods_list = $model_goods->getGoodsList($condition, 'goods_id, goods_commonid, hui_discount,goods_name,goods_promotion_price,goods_price,goods_image');
    ...
    public function addMbSpecialItem($param) {
        ...
        $result = $this->table('mb_special_item1')->insert($param);
        //删除缓存
        if($result) {
            //删除缓存
    ...
    public function editMbSpecialItemByID($update, $item_id, $special_id) {
        if(isset($update['item_data'])) {
            // var_dump($update);die;
            //$update['item_data']['data'] = stripcslashes($update['item_data']['data']);
           if(!empty($update['item_data']['data'])){
            $update['item_data']['data']='/pages/pageGood/goodlist/goodlist?search='.$update['item_data']['data'];
            }
        ...
        return $this->table('mb_special_item1')->where($condition)->update($update);
    }
    ...
    public function delMbSpecialItem($condition, $special_id) {
        ...
        return $this->table('mb_special_item1')->where($condition)->delete();
    }
