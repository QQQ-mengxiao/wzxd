...
/**
     ...
     */
    public function getComplainCount1($condition = array()) {
        return $this->table('complain')->where($condition)->count();
    }

    public function getComplainList($condition,$order = 'complain_id desc', $field = '*',$limit='') {
        // $condition = $this->getCondition($condition);
        // $order=$condition['order']?;
        $list = $this->table('complain')->where($condition)->field($field)->order($order)->limit($limit)->select();
        return $list;
    }
}
