...
class cartModel extends Model {
	...
	private function _addCartDb($goods_info = array(),$quantity) {
		...
		$array['is_mode'] = $goods_info['is_mode'];
		$array['goods_shipper_id'] = $goods_info['goods_shipper_id'];
//echo $array['is_mode'];
//break;
		if ($goods_info['goods_promotion_type'] == 2) {$array['goods_tax'] = ncPriceFormat($goods_info['goods_tax']*$goods_info['xianshi_info']['xianshi_price']/$goods_info['xianshi_info']['goods_price']);
		}	else{
				$array['goods_tax'] = ncPriceFormat($goods_info['goods_tax']);}
		
		//$goods_info['goods_tax'];
		//jinp170603
		$array['goods_all_weight'] = $goods_info['goods_all_weight'];
  ...
  //   	if ($goods_info['goods_promotion_type'] == 2) {$goods_tax = ncPriceFormat($goods_info['goods_tax']*$goods_info['xianshi_info']['xianshi_price']/$goods_info['xianshi_info']['goods_price']);
		// }	else{
		// 		$goods_tax = ncPriceFormat($goods_info['goods_tax']);}


		//$goods_info['goods_tax']*$goods_info['xianshi_info']['xianshi_price']/$goods_info['xianshi_info']['goods_price']
				//print_r($goods_info);
     // 	if ($goods_info['goods_tax'] == ''){
				 // break;}
		$cart_array[$goods_info['goods_id']] = array(
		  ...
		  'goods_num' => $quantity,
           		   // 'is_mode' => $goods_info['is_mode'],
		   // 'goods_tax' => $goods_info['goods_tax'],
		   // 'xianshi_info' => $goods_info['xianshi_info']
	...
	public function listCart($type, $condition = array(), $limit = '') {
        if ($type == 'db') {
    		$cart_list = $this->where($condition)->order('cart_id desc')->limit($limit)->select();
        } elseif ($type == 'cookie') {
        	//去除斜杠
        	$cart_str = get_magic_quotes_gpc() ? stripslashes(cookie('cart')) : cookie('cart');
        	$cart_str = base64_decode(decrypt($cart_str));
	 ...
	 * @param int $limit
	 */
	public function listCartD($member_id) {
    	$cart_list = Model()->query("select c.cart_id,c.buyer_id,c.store_id,c.store_name,c.goods_id,c.goods_name,c.goods_price,c.goods_num,c.goods_image,c.bl_id,c.is_mode,c.goods_tax,c.goods_shipper_id,c.goods_all_weight from 718shop_cart c left join 718shop_goods g on c.goods_id=g.goods_id where c.buyer_id=".$member_id." and g.is_deleted=0 ");
        $cart_list = is_array($cart_list) ? $cart_list : array();
        //顺便设置购物车商品数和总金额
		$this->cart_goods_num =  count($cart_list);
	    $cart_all_price = 0;
