...
class yundayinModel extends Model {
     ...
     * 构建云打印
    */
    public function _yundayin($order_sn,$is_refund = 0,$refund_goodsid=0) {
        ...
        $order_info = $model_order->getFpOrderInfo($condition,array('order_common','order_goods','member'));
         $order_info['refund_goodsid']=$refund_goodsid;
        //print_r($order_info);
        if (empty($order_info)) {
            $order_info = false;
        }else{
            ...
            if($is_dayin){
                ...
                $address_id = $order_info['extend_order_common']['reciver_ziti_id'] ;
                 ...
                 $yundayin1 = Model()->table('yundayin')->where(array('storage_id'=>$order_info['storage_id'],'is_wai'=>'1'))->find();
                if($yundayin1['dayin_sn'] == '221503365111'){
                    $yundayin = Model()->table('yundayin')->where(array('address_id'=>$order_info['extend_order_common']['reciver_ziti_id'],'is_wai'=>'2'))->find();
                }else{
                    $yundayin =  $yundayin1;
                }
                //设置打印机接口参数
    ...
    */
     ...
     * 构建云打印
    */
    public function _yundayin1($order_sn,$is_refund = 0,$refund_goodsid=0) {
        $model_order = Model('order');
        $condition = array();
        $condition['order_sn'] = $order_sn;
    ...
    private function _printorder1($SN, $order_info ,$is_refund = 0,$times = 1 ) {
        //标签说明：
        //单标签:
        //"<BR>"为换行,"<CUT>"为切刀指令(主动切纸,仅限切刀打印机使用才有效果)
        //"<LOGO>"为打印LOGO指令(前提是预先在机器内置LOGO图片),"<PLUGIN>"为钱箱或者外置音响指令
        ...
        $content .= '订单号：<W>'.$order_info['order_sn'].'</W><BR>';
        $content .= '商品名称/货号/条码<BR>';//.'　　　　　 单价'.'  数量'.' 金额<BR>';
        $content .= '    单价'.'     数量'.'     金额<BR>';
        $content .= '--------------------------------<BR>';
        foreach ($order_info['extend_order_goods'] as $key=>$value){
            ...
            if($order_info['refund_goodsid']==0){
               if($value['goods_type'] == 6){
                    $content .= '【赠品】'.$value['goods_name'].'/'.Model()->table('goods')->getfby_goods_id($value['goods_id'],'goods_serial').'/'.Model()->table('goods')->getfby_goods_id($value['goods_id'],'goods_barcode').'<BR>'.'    '.$value['goods_price'].'       '.$value['goods_num'].'       '.$value['goods_pay_price'].'<BR>';
                }else{
                    $content .= $value['goods_name'].'/'.Model()->table('goods')->getfby_goods_id($value['goods_id'],'goods_serial').'/'.Model()->table('goods')->getfby_goods_id($value['goods_id'],'goods_barcode').'<BR>'.'    '.$value['goods_price'].'       '.$value['goods_num'].'       '.$value['goods_pay_price'].'<BR>';
                }
           }else{
            if($order_info['refund_goodsid']==$value['goods_id']){
               if($value['goods_type'] == 6){
                    $content .= '【赠品】'.$value['goods_name'].'/'.Model()->table('goods')->getfby_goods_id($value['goods_id'],'goods_serial').'/'.Model()->table('goods')->getfby_goods_id($value['goods_id'],'goods_barcode').'<BR>'.'    '.$value['goods_price'].'       '.$value['goods_num'].'       '.$value['goods_pay_price'].'<BR>';
                }else{
                    $content .= $value['goods_name'].'/'.Model()->table('goods')->getfby_goods_id($value['goods_id'],'goods_serial').'/'.Model()->table('goods')->getfby_goods_id($value['goods_id'],'goods_barcode').'<BR>'.'    '.$value['goods_price'].'       '.$value['goods_num'].'       '.$value['goods_pay_price'].'<BR>';
        ...
        $content .= '<BR>';
        $content .= '收货人：<L><W><B>'.$order_info['extend_order_common']['reciver_name'].'</B></L></W><BR>';
        
        $content .= '<BR>';
        $content .= '联系电话：<W>'.$order_info['extend_order_common']['reciver_info']['mob_phone'].'</W><BR>';
        $content .= '买家留言：'.$order_info['extend_order_common']['order_message'].'<BR>';
        $content .= '打单时间：'.date('Y-m-d H:i:s',time()).'<BR>';
        ...
        $wms_sn = Model()->table('order')->getfby_order_id($order_info['order_id'],'wms_sn');
         if(!empty($wms_sn)){
        	$content .= '<BR><CB><L><W>'.$wms_sn.'</L></W></CB>';
        }
       ...
       if($SN == '221505963'){
        ...
        //打开注释可测试
        $result = $this->printMsg($SN,$content,$times);
    ...
    private function _printorder($SN, $order_info ,$is_refund = 0,$times = 1 ) {
        ...
        foreach ($order_info['extend_order_goods'] as $key=>$value){
            ...
            if($order_info['refund_goodsid']==0){
               if($value['goods_type'] == 6){
                    //$content .= '【赠品】'.$value['goods_name'].'/'.Model()->table('goods')->getfby_goods_id($value['goods_id'],'goods_serial').'/'.Model()->table('goods')->getfby_goods_id($value['goods_id'],'goods_barcode').'<BR>'.'    '.$value['goods_price'].'       '.$value['goods_num'].'       '.$value['goods_pay_price'].'<BR>';
                    $content .= '【赠品】'.$value['goods_name'].'/'.Model()->table('goods')->getfby_goods_id($value['goods_id'],'goods_serial').'/'.Model()->table('goods')->getfby_goods_id($value['goods_id'],'goods_barcode').'<BR>'.'                         X '.$value['goods_num'].'<BR>';
                }else{
                    //$content .= $value['goods_name'].'/'.Model()->table('goods')->getfby_goods_id($value['goods_id'],'goods_serial').'/'.Model()->table('goods')->getfby_goods_id($value['goods_id'],'goods_barcode').'<BR>'.'    '.$value['goods_price'].'       '.$value['goods_num'].'       '.$value['goods_pay_price'].'<BR>';
                    $content .= $value['goods_name'].'/'.Model()->table('goods')->getfby_goods_id($value['goods_id'],'goods_serial').'/'.Model()->table('goods')->getfby_goods_id($value['goods_id'],'goods_barcode').'<BR>'.'                         X '.$value['goods_num'].'<BR>';
                }
           }else{
            if($order_info['refund_goodsid']==$value['goods_id']){
               if($value['goods_type'] == 6){
        ...
        $content .= '联系电话：<W>'.$order_info['extend_order_common']['reciver_info']['mob_phone'].'</W><BR>';
        $content .= '买家留言：'.$order_info['extend_order_common']['order_message'].'<BR>';
        $content .= '打单时间X：'.date('Y-m-d H:i:s',time()).'<BR>';
        // $content .= '商家备注：'.$order_info['extend_order_common']['deliver_explain'].'<BR><BR>';
        //$content .= '<QR>https://france.banliego.cn/wzxd/applet?id=1</QR>';
        //把二维码字符串用标签套上即可自动生成二维码
        $content .= '<QR>act=saoma&op=ruku&order_sn='.$order_info['order_sn'].'&order_id='.$order_info['order_id'].'</QR>';
  ...
  /**
    ...
    private function _insert_dayinlog($arr, $json, $logdata) {
       ...
       $data['json'] = $json;
       $data['from_dayin'] = $logdata['from_dayin'];
       $data['send_time'] = time();
