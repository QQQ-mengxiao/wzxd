...
class p_mansongModel extends Model{
	...
	//20170214商城修改
	const MANSONG_STATE_NORMAL = 2;
    const MANSONG_STATE_CLOSE = 1;
...
* 获取店铺某类商品满即送活动个数
     *
     */ //xinzeng
    public function getMansonggcCount($store_id,$mansong_gc_id) {
        if(empty($store_id)) {
            return null;
        }
        $condition = array();
        $condition['store_id'] = $store_id;
        $condition['state'] = self::MANSONG_STATE_NORMAL;
		$condition['mansong_gc_id'] = $mansong_gc_id;
    ...
    public function getMansongInfoByStoreID($store_id) {
        ...
        if (empty($info)) {
            ...
            foreach($mansong_list as $key=>$mansong_info){
				...
				$mansong_lists[$key]=$mansong_info;
			}
        }
		if (empty($mansong_lists)) {
	       return array();
		}

        return $mansong_lists;
    }
	 ...
	 *
	 */ //xinzeng
    public function getMansongInfoBygcStoreID($store_id) {
        if(intval($store_id) <= 0) {
            return array();
        }
        $info = $this->_rGoodsMansongCache($store_id);
        if (empty($info)) {
			...
			foreach($mansong_list as $key=>$mansong_info){
                ...
                $mansong_list[$key]['rule']=$mansong_info;
			}
			$mansong_rule_list[$store_id]=$mansong_list;
		 }
    ...
    public function getMansongRuleByStoreID($store_id, $order_price) {
        ...
        foreach ($mansong_info['rules'] as $value) {
            if($order_price >= $value['price']) {
                ...
                $rule_info['mansong_gc_id'] = $mansong_info['mansong_gc_id'];
                break;
            }
        }
 ...
 /**
	 * 获取订单可用满即送规则(goods_class)
	 * @param array $store_id 店铺编号 
	 * @param array $order_price 订单金额
     * @return array 满即送规则
	 *
	 */  //xinzeng
    public function getMansongRuleByStoregcID($store_id,$gc_id_1,$order_price) {
        
        $mansong_info = $this->getMansongInfoByStoregcID($store_id,$gc_id_1);
if(!empty($mansong_info)) {
            for($i =0, $j = count($mansong_info); $i < $j; $i++) {
                $mansong_info[$i] = $this->getMansongExtend($mansong_info[$i]);
            }
        }
        ...
        foreach ($mansong_info['rules'] as $value) {
            if($order_price >= $value['price']) {
                ...
                $rule_info['end_time'] = $mansong_info['end_time'];
			    $rule_info['mansong_gc_id'] = $mansong_info['mansong_gc_id'];
			    $rule_info['mansong_gc_name'] = $mansong_info['mansong_gc_name'];//xin
                break;
            }
        }
...
/**
     * 获取订单可用满即送规则(订单级_class)
     * @param array $store_id 店铺编号 
     * @param array $order_price 订单金额
     * @return array 满即送规则
     *
     */  //xinzeng
    public function getMansongRuleByStoregID($store_id,$gc_id_1,$order_price) {          
        $mansong_info = $this->getMansongInfoByStoregcID($store_id,$gc_id_1);
        // if(!empty($mansong_info)) {
        //     for($i =0, $j = count($mansong_info); $i < $j; $i++) {
        //         $mansong_info[$i] = $this->getMansongExtend($mansong_info[$i]);
        ...
        $rule_info = null;
        //print_r( $mansong_info['rules']);die;
        foreach ($mansong_info['rules'] as $key => $value) {
            if($order_price >= $value['price']) {
                $rule_info = $value;
                $rule_info['mansong_name'] = $mansong_info['mansong_name'];
                $rule_info['start_time'] = $mansong_info['start_time'];
	...
	/**
	 * 获取店铺当前可用满即送活动(goods_class)
	 * @param array $store_id 店铺编号 
     * @return array 满即送活动
	 *
	 */  //xinzeng
    public function getMansongInfoByStoregcID($store_id,$gc_id_1) {
        if(intval($store_id) <= 0) {
            return array();
        }
        $info = $this->_rGoodsMansongCache($store_id);
        if (empty($info)) {
            ...
            $condition['state'] = self::MANSONG_STATE_NORMAL;
            $condition['store_id'] = $store_id;
            $condition['mansong_gc_id'] = $gc_id_1;
            $condition['end_time'] = array('gt', TIMESTAMP);
            $mansong_list = $this->getMansongList($condition, null, 'start_time asc', '*', 1);
            //$mansong_list = $this->getMansongList($condition, null, 'price desc', '*', 1);
     ...
     *
     */  //xin
    public function getMansongExtend($mansong_info) {
	    $mansong_gc_id=$mansong_info['mansong_gc_id'];
		$goods_class_name = Model()->query("SELECT gc_name  FROM `718shop_goods_class` where gc_id=\"$mansong_gc_id\" LIMIT 1");
		$mansong_info['mansong_gc_name']=$goods_class_name[0]['gc_name'];
        return $mansong_info;
    }
