...
class refund_returnModel extends Model
{
    ...
    public function editOrderRefund($refund)
    {
        ...
        if ($refund_id > 0) {
            ...
            $field = 'order_id,buyer_id,buyer_name,store_id,order_sn,order_amount,payment_code,order_state,refund_amount,rcb_amount,card_amount';
            $model_order = Model('order');
            ...
            try {
                ...
                if ($card_amount > 0) {
                    ...
                    //20181121 新添加订单金额与商品金额的判断条件
                    if (($card_state && $order_state >= $order_paid) || ($order['order_amount'] == $all_refund_amount||$goods_amount['goods_amount'] == $all_refund_amount)) {
                        //order_log表
    ...
    public function editOrderRefund_jp($refund) 
    {
        ...
        if ($refund_id > 0) {
            Language::read('model_lang_index');
            $order_id = $refund['order_id'];//订单编号
            $field = 'order_id,buyer_id,buyer_name,store_id,order_sn,pay_sn,order_amount,payment_code,order_state,refund_amount,refund_state,rcb_amount,pd_amount,card_amount';//mx 多获取预存款支付金额一列
            $model_order = Model('order');
            $order = $model_order->getOrderInfo(array('order_id' => $order_id), array('order_common'), $field);
            ...
            try {
                ...
                //var_dump($refund_last);die;
                //181119 新添加以下的全部退款与部分退款
                // 全部退款
                $model_refund = Model('refund_return');
                $condition = array();
                $condition['order_sn'] = $order['order_sn'];
                $condition['seller_state'] = 2;
                $condition['refund_state'] = 2;

                //var_dump($condition);die;
                $refund_info1 = $model_refund->getRefundReturnListamount($condition);
                //var_dump($refund_info1);die;
                //$all_refund_amount = array_sum(array_column($refund_info1,'refund_amount'));
                $all_refund_amount = 0;
                foreach($refund_info1 as $v){
                    $all_refund_amount += $v['refund_amount'];
                }
                $goods_amount = Model('order');
                $goods_amount = $goods_amount->getOrderGoodsAmount(array('order_id' => $order_id));

                if($goods_amount['goods_amount'] == $all_refund_amount || $order['order_amount'] == $all_refund_amount){
                    $sql = "SELECT a.id,a.member_id,a.commission,a.commission_state,a.real_pay,a.relation,b.fx_before_account,b.fx_invalid_account from 718shop_distribute_commission as a inner join 718shop_member as b on a.member_id=b.member_id where a.order_sn=".$order['order_sn']." order by a.relation asc";
                    $tree_info = Model()->query($sql);
                    //var_dump($tree_info);die;
                    if($tree_info){
                        foreach($tree_info as $v){
                        if ($v['commission_state']==1) {
                              $update = [];
                            $update['fx_before_account'] = $v['fx_before_account'] - $v['commission'];
                            $update['fx_invalid_account'] = $v['fx_invalid_account'] + $v['commission'];
                            //var_dump($update);die;
                            Model()->table('member')->where(['member_id' => $v['member_id']])->update($update);
                            Model()->table('distribute_commission')->where(['id' => $v['id']])->update(['commission_state' => 3, 'update_time' => time()]);
                            }    
                        }
                    }
                }
                //部分退款
                    ...
                    if($refund_last == 0){
                        ...
                        $goods_info = Model()->query("SELECT b.goods_fx_percent,b.is_fenxiao FROM 718shop_goods AS a INNER JOIN 718shop_goods_common AS b ON a.goods_commonid=b.goods_commonid WHERE a.goods_id=$goods_id LINIT 1");
                        if($goods_info && $goods_info[0]['is_fenxiao']){
                            $fx_percent = unserialize($goods_info[0]['goods_fx_percent']);
                            $order_info = Model()->table('order_goods')->where(['order_id' => $order['order_id'], 'goods_id' => $goods_id])->field('goods_price')->find();
                            $num = 0.00;
                            foreach($fx_percent as $v){
                                //根据实际方法计算佣金总额（商品价或实际支付价）
                                $num += $order_info['goods_price'] * $v;
                            }
                            if($rcb_amount > $num){
                                $rcb_amount -= $num;
                            }
                            else if(($rcb_amount + $pd_amount) > $num){
                                $pd_amount = $rcb_amount + $pd_amount - $num;
                                $rcb_amount = 0.00;
                            }
                            else{
                                $last = $num - $pd_amount - $rcb_amount;
                                $pd_amount = 0.00;
                                $rcb_amount = 0.00;
                            }
                            $card_amount -= $num;
...
//                      //先退充值卡剩余金额
                        ...
                        if ($refund_now - ($rcb_amount - $refund_last) <= $pd_amount) {//本次退款金额-充值卡剩余金额的剩余金额  如果小于或等于预存款支付金额
                            ...
                            // 181119 新添加 减去剩余佣金
                            $last = isset($last) ? $last : 0.00;
                            $data['pdc_amount'] = $refund_now - ($rcb_amount - $refund_last) - $pd_amount - $last;
                ...
                $order_state = $order['order_state'];
                //var_dump($order_state);die;
                $model_trade = Model('trade');
                $order_paid = $model_trade->getOrderState('order_paid');//订单状态20:已付款
                //181119 以下为新添加或修改 判断是否为全部退款
                $model_refund = Model('refund_return');
                $condition = array();
                $condition['order_sn'] = $order['order_sn'];
                $condition['seller_state'] = 2;
    ...
    public function getRefundReturnList($condition = array(), $page = '', $fields = '*', $limit = '')
    {
        ...
        /**
     * 取得退款列表 测试 181119 新添加
     * @param unknown $condition
     * @param string $pagesize
     * @param string $fields
     * @param string $order
     */
    public function getRefundReturnListamount($condition, $pagesize = '', $fields = 'refund_amount', $order = '', $limit = '') {
         $a= $this->table('refund_return')->where($condition)->field($fields)->order($order)->limit($limit)->page($pagesize)->select();
        return $a;
    }
