...
class messageModel {
	...
	public function countNewMessage($member_id){
		...
		$condition_arr['no_message_type']='3';
	...
	private function getCondition($condition_array){
		...
		//排除掉手机发送的通知消息
		if($condition_array['no_message_type'] != ''){
			$condition_sql	.= " and message.message_type != '{$condition_array['no_message_type']}'";
		}
		//站内信类型
		...
		if($condition_array['to_member_id'] != ''){
			$condition_sql	.= " and (message.to_member_id ='all' or (message.to_member_id ='V0' and '{$condition_array['member_grade']}' = 'V0' )or (message.to_member_id ='V1' and '{$condition_array['member_grade']}' = 'V1' ) or (message.to_member_id ='V2' and '{$condition_array['member_grade']}' = 'V2' ) or (message.to_member_id ='V3' and '{$condition_array['member_grade']}' = 'V3' )  or (message.message_ismore=0 and message.to_member_id='{$condition_array['to_member_id']}') or (message.message_ismore=1 and message.to_member_id like '%,{$condition_array['to_member_id']},%'))";
		}
