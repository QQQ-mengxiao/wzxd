...
class red_packetModel extends Model{
     ...
     * 获取红包列表
     */
    public function getRedpacketList($condition, $page = 0, $field = '*', $order = '', $limit = 0, $count = 0) {
        return $this->table('red_packet')->field($field)->where($condition)->order($order)->limit($limit)->page($page, $count)->select();
    }
