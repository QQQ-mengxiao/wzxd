...
class yundayin1Model extends Model {
    ...
    */
        ...
        foreach ($order_info['extend_order_goods'] as $key=>$value){
            ...
            if($order_info['refund_goodsid']==0){
               if($value['goods_type'] == 6){
                    $content .= '【赠品】'.$value['goods_name'].'/'.Model()->table('goods')->getfby_goods_id($value['goods_id'],'goods_serial').'<BR>'.'    '.$value['goods_price'].'       '.$value['goods_num'].'       '.$value['goods_pay_price'].'<BR>';
                }else{
    ...
    private function _printorder($SN, $order_info ,$is_refund = 0,$times = 1 ) {
        ...
        $content .= '收货人：'.$order_info['extend_order_common']['reciver_name'].'<BR>';
