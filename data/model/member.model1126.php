...
public function l_login($passwd,$condition) {
	...
	public function getDiscount($member_id=''){
		...
		/**
	 * 181119 新添加 更新是否成分销商的状态
	 *
	 * @param array $data
	 */
    	public function is_fenxiao($member_id=''){
	    $model_com = Model('member');
	    $condition = array();
	    $condition['id'] = $member_id;
	    $condition['is_fenxiao'] = 0;
	    $info = $model_com->getMemberCommonInfo($condition);
	    //查询用户信息
	    $model_member = Model('member');
	    $member_info = $model_member->getMemberCommonInfo(array('member_id'=>$info['member_id']));
        $update = array();
        $update['is_fenxiao'] = 1;
        }
