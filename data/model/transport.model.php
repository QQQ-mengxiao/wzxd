...
class transportModel extends Model {
   ...
   /**jinp170606
     ...
     * @return number/boolean
     */
    public function calc_transport_weight($transport_id, $area_id) {
        if (empty($transport_id) || empty($area_id)) return 0;
        $extend_list = $this->getExtendList(array('transport_id'=>$transport_id));
        if (empty($extend_list)) {
            return false;
        } else {
            return $this->_calc_unit_weight($area_id,$extend_list);
        }
    }
     ...
     * @return number/false 总运费
     */
    private function _calc_unit_weight($area_id, $extend){
        if (!empty($extend) && is_array($extend)){
            foreach ($extend as $v) {
                if (strpos($v['area_id'],",".$area_id.",") !== false){
                    $calc_total = $v['weight'];
...
/**jinp170606
     ...
     * @return number/boolean
     */
    public function calc_transport_overweight($transport_id, $area_id) {
        if (empty($transport_id) || empty($area_id)) return 0;
        $extend_list = $this->getExtendList(array('transport_id'=>$transport_id));
        if (empty($extend_list)) {
            return false;
        } else {
            return $this->_calc_unit_overweight($area_id,$extend_list);
        }
    }
     ...
     * @return number/false 总运费
     */
    private function _calc_unit_overweight($area_id, $extend){
        if (!empty($extend) && is_array($extend)){
            foreach ($extend as $v) {
                if (strpos($v['area_id'],",".$area_id.",") !== false){
                    $calc_total = $v['money_overweight'];
                }
            }
        }
        return isset($calc_total) ? ncPriceFormat($calc_total) : false;
