...
class p_xinren_goodsModel extends Model{                        
	...
	public function getXinRenGoodsOnList($condition, $page=null, $order='', $field='*', $limit = 0,$goods_serial) {
        $where = array();
        if($condition['goods_name']){
            $where['p_xinren_goods.goods_name'] = $condition['goods_name'];
        }
        $where['p_xinren_goods.store_id'] = $condition['store_id'];
        $where['goods.goods_serial'] = $goods_serial;
        $on = 'goods.goods_id = p_xinren_goods.goods_id';
        return $xinren_goods_list = $this->table('p_xinren_goods,goods')->join('left')->field($field)->where($where)->on($on)->page($page)->order($order)->limit($limit)->select();
//        return $xinren_goods_list = $this->field($field)->where($condition)->page($page)->order($order)->limit($limit)->select();
	}
	 ...
	 *
	 */
	public function getXinRenGoodsExtendList($condition, $page=null, $order='', $field='*', $limit = 0,$goods_serial='') {
	    if($goods_serial){
            $xinren_goods_list = $this->getXinRenGoodsOnList($condition, $page, $order, $field, $limit,$goods_serial);
        }else {
