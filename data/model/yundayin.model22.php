...
class yundayinModel extends Model {
     ...
     * 构建云打印
     */
    public function _yundayin11($order_sn,$is_refund = 0) {
        ...
        if (empty($order_info)) {
            ...
            $insert_data = array('order_id' =>$order_info['order_id'] );
            $is_dayin = Model()->table('isdayin')->insert($insert_data);
            print_r($insert_data );
            var_dump($is_dayin);die;
            if($is_dayin){
                if($order_info['order_type'] != 5){
                    //echo '<br/>'.$order_info['order_type'].'zhi';die;
                    $array_SN['8'] = '221505964'; //物资集团中心仓打印机编号
                    $array_SN['9'] = '221505963'; //中陆仓打印机编号
                    foreach ($array_SN as $key => $csn) {
                        //调用小票机打印订单接口
                        $dy_result = $this->_printorder($csn,$order_info,$is_refund);
                        $dy_logdata['order_id'] =  $order_info['order_id'];
                        $dy_logdata['dayin_id'] = $key;
                        $dy_logdata['address_id'] = $order_info['extend_order_common']['reciver_ziti_id'];
                        $dy_logdata['is_refund'] = $is_refund;
                        //判断接口是否成功，不成功重复请求
                        if($dy_result['code'] == 1){
                            ...
                            if($dy_result['arr']['ret'] == 0){
                                //更新订单发送状态
                                $dy_condition['order_id'] = $order_info['order_id'];
                                $dy_updata['dayin_state'] = 1;
                                $a = $model_order->table('order')->where($dy_condition)->update($dy_updata);
                                // var_dump($a);die;
                           
                                 //添加订单日志
                                $this->_insert_dayinlog($dy_result['arr'], $dy_result['json'], $dy_logdata);
                            }else{
                                //添加订单日志
                                $this->_insert_dayinlog($dy_result['arr'], $dy_result['json'], $dy_logdata);
                                $dy_result1 = $this->_printorder($csn,$order_info,$is_refund);
                                if($dy_result1['code'] == 1){
                                    $dy_result1['arr'] = json_decode($dy_result1['json'] ,true);
                                    //正确例子
                                    if($dy_result1['arr']['ret'] == 0){
                                        //更新订单发送状态
                                        $dy_condition2['order_id'] = $order_info['order_id'];
                                        $dy_updata1['dayin_state'] = 1;
                                        $model_order->table('order')->where($dy_condition2)->update($dy_updata1);

                                        //添加订单日志
                                        $this->_insert_dayinlog($dy_result1['arr'], $dy_result1['json'], $dy_logdata);
                                    }else{
                                        //两次请求都失败，插入到异常打印单表
                                        $this->_insert_errordayin($dy_result1['arr'], $dy_result1['json'], $dy_logdata);
                                        $this->_insert_dayinlog($dy_result1['arr'], $dy_result1['json'], $dy_logdata);
                                    }
                                }else{
                                    $dy_result1['arr']['ret'] = 'fail';
                                    $dy_result1['arr']['msg'] = 'fail';
                                    //两次请求都失败，插入到异常打印单表
                                    $this->_insert_errordayin($dy_result1['arr'], $dy_result1['json'], $dy_logdata);
                                    $this->_insert_dayinlog($dy_result1['arr'], $dy_result1['json'], $dy_logdata);
                                }
                            ...
                            $dy_result['arr']['ret'] = 'fail';
                            $dy_result['arr']['msg'] = 'fail';
                            $this->_insert_dayinlog($dy_result['arr'], $dy_result['json'], $dy_logdata);
                            $dy_result2 = $this->_printorder($csn,$order_info,$is_refund);

                            if($dy_result2['code'] == 1){
                                $dy_result2['arr'] = json_decode($dy_result2['json'] ,true);
                                //正确例子
                                if($dy_result2['arr']['ret'] == 0){
                                    //更新订单发送状态
                                    $dy_condition3['order_id'] = $order_info['order_id'];
                                    $dy_updata2['dayin_state'] = 1;

                                    $model_order->table('order')->where($dy_condition3)->update($dy_updata2);

                                    //添加订单日志
                                    $this->_insert_dayinlog($dy_result2['arr'], $dy_result2['json'], $dy_logdata);
                                }else{
                                    //两次请求都失败，插入到异常打印单表
                                    $this->_insert_errordayin($dy_result2['arr'], $dy_result2['json'], $dy_logdata);
                                    $this->_insert_dayinlog($dy_result2['arr'], $dy_result2['json'], $dy_logdata);
                                }
                            }else{
                                $dy_result2['arr']['ret'] = 'fail';
                                $dy_result2['arr']['msg'] = 'fail';
                                //两次请求都失败，插入到异常打印单表
                                $this->_insert_errordayin($dy_result2['arr'], $dy_result2['json'], $dy_logdata);
                                $this->_insert_dayinlog($dy_result2['arr'], $dy_result2['json'], $dy_logdata);
                            }
