...
class orderModel extends Model {
 ...
 /**
    ...
    public function getOrderList2($consignee_name = '',$condition, $pagesize = '', $field = '*', $order = 'order.order_id desc', $limit = '', $extend = array(), $master = false){
        ...
        //181119 新添加 追加订单分销佣金信息
        if (in_array('distribute_commission',$extend)) {
            $order_list_key=array_keys($order_list);
            foreach ($order_list_key as $key => $value) {
                $order_list = Model('order')->getOrderInfo(array('order_id'=>$value));
                $order_sn = $order_list['order_sn'];
                $array[] = $order_sn;
            }
            //取佣金列表
            $order_commission_list = Model('commission')->getCommissionList(array('order_sn'=>array('in',$array)));
            //var_dump($order_commission_list);die;
            if (!empty($order_commission_list)) {
                foreach ($order_commission_list as $value) {
                    $order_list[$value['order_id']]['extend_distribute_commission'][] = $value;
                }
            } else {
                $order_list[$value['order_id']]['extend_distribute_commission'] = array();
            }
        }
 ...
 public function getOrderList3($consignee_name = '',$condition, $pagesize = '', $field = '*', $order = 'order.order_id desc', $limit = '', $extend = array(), $master = false){
    ...
    public function getOrderAndOrderGoodsSalesRecordList($condition, $field="*", $page = 0, $order = 'rec_id desc') {
     ...
     // 20181121 新添加 获取订单商品总金额
        public function getOrderGoodsAmount($condition = array(),$fields = 'goods_amount') {
         $a= $this->table('order')->where($condition)->field($fields)->find();
        return $a;
    }
