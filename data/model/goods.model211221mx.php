...
class goodsModel extends Model{
     ...
     * @return array 二维数组
     */
    public function getGoodsList3($condition, $field = '*', $group = '',$order = '', $limit = 0, $page = 0, $lock = false,  $offset = 0,$i=0) {
        ...
        if($keyword){
            $sql = "SELECT * FROM ((SELECT sg.goods_commonid,sg.goods_id,sg.goods_name,sg.goods_jingle,sg.gc_id,sg.store_id,sg.store_name,min(sg.goods_price) as goods_price,sg.goods_promotion_price,sg.goods_promotion_type,sg.goods_marketprice,sum(sg.goods_storage) as goods_storage,sg.goods_image,sg.goods_freight,sg.goods_salenum,sg.goods_presalenum,sg.color_id,sg.evaluation_good_star,sg.evaluation_count,sg.is_virtual,sg.is_fcode,sg.is_appoint,sg.is_presell,sg.have_gift,sg.is_mode,sg.is_group_ladder,1 AS tempsort FROM 718shop_goods sg INNER JOIN (SELECT sgc.gc_id FROM 718shop_goods_class sgc WHERE sgc.gc_name LIKE '".$keyword."' AND sgc.gc_parent_id> 0) c ON c.gc_id=sg.gc_id WHERE sg.goods_state=1 AND sg.goods_verify=1 AND sg.is_deleted=0 GROUP BY sg.goods_commonid) UNION (SELECT sg.goods_commonid,sg.goods_id,sg.goods_name,sg.goods_jingle,sg.gc_id,sg.store_id,sg.store_name,min(sg.goods_price) as goods_price,sg.goods_promotion_price,sg.goods_promotion_type,sg.goods_marketprice,sum(sg.goods_storage) as goods_storage,sg.goods_image,sg.goods_freight,sg.goods_salenum,sg.goods_presalenum,sg.color_id,sg.evaluation_good_star,sg.evaluation_count,sg.is_virtual,sg.is_fcode,sg.is_appoint,sg.is_presell,sg.have_gift,sg.is_mode,sg.is_group_ladder,2 AS tempsort FROM 718shop_goods sg INNER JOIN (SELECT DISTINCT sg.goods_commonid FROM 718shop_goods sg WHERE sg.goods_name LIKE '".$keyword."') g ON g.goods_commonid=sg.goods_commonid WHERE sg.goods_state=1 AND sg.goods_verify=1 AND sg.is_deleted=0 GROUP BY sg.goods_commonid) UNION (SELECT sg.goods_commonid,sg.goods_id,sg.goods_name,sg.goods_jingle,sg.gc_id,sg.store_id,sg.store_name,min(sg.goods_price) as goods_price,sg.goods_promotion_price,sg.goods_promotion_type,sg.goods_marketprice,sum(sg.goods_storage) as goods_storage,sg.goods_image,sg.goods_freight,sg.goods_salenum,sg.goods_presalenum,sg.color_id,sg.evaluation_good_star,sg.evaluation_count,sg.is_virtual,sg.is_fcode,sg.is_appoint,sg.is_presell,sg.have_gift,sg.is_mode,sg.is_group_ladder,3 AS tempsort FROM 718shop_goods sg INNER JOIN (SELECT sgc.gc_id FROM 718shop_goods_class sgc WHERE sgc.gc_name LIKE '".$keyword."' AND sgc.gc_parent_id=0) c ON c.gc_id=sg.gc_id_1 WHERE sg.goods_state=1 AND sg.goods_verify=1 AND sg.is_deleted=0 GROUP BY sg.goods_commonid)) a GROUP BY a.goods_commonid ORDER BY (CASE WHEN a.goods_storage=0 THEN 1 ELSE 0 END),a.tempsort,a.goods_storage DESC limit  10 offset ".$offset;
        }else{
    ...
    */
     ...
     * @return array
     */
    public function getGoodsListByColorDistinct1($condition, $field = '*', $order = 'goods_id asc', $offset= 0,$i=0) {
        $condition['goods_state']   = self::STATE1;
        $condition['goods_verify']  = self::VERIFY1;
        $condition['is_deleted']  = 0;
        $condition = $this->_getRecursiveClass($condition);
    ...
    $count = $this->getGoodsOnlineCount($condition,"distinct CONCAT(goods_commonid)");
        $goods_list = array();
        if ($count != 0) {
           $goods_list = $this->getGoodsOnlineList1($condition, $field, $page, $order, 0, 'nc_distinct', false, $offset,$i);
        }
        return $goods_list;
    }
     ...
     * @return array
     */
    public function getGoodsOnlineList1($condition, $field = '*', $page = 0, $order = 'goods_id desc', $limit = 0, $group = '', $lock = false, $offset = 0,$i=0) {
        $condition['goods_state']   = self::STATE1;
        $condition['goods_verify']  = self::VERIFY1;
        $condition['is_deleted']  = 0;
         return $this->getGoodsList3($condition, $field, $group, $order, $limit, $page, $lock, $offset,$i);
    }
        ...
        foreach ($goods_array as $key => $val) {
            ...
            foreach ($val as $k => $v) {
                ...
                if (!empty($xianshigoods)) {
                    // 更新价格
                    $this->editGoodsById(array('goods_promotion_price' => $xianshigoods['xianshi_price'], 'goods_promotion_type' => 2,'is_group_ladder'=>4), $k);
                    continue;
                }
             ...
             if( $is_group_ladder==4){
               $is_group_ladder=0;
                ...
                // 没有促销使用原价
              // var_dump($k);die;
                $this->editGoodsById(array('goods_promotion_price' => array('exp', 'goods_price'), 'goods_promotion_type' => 0,'is_group_ladder'=>$is_group_ladder), $k);
            }
        }
        return true;
    }
