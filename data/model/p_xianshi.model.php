...
class p_xianshiModel extends Model{
    ...
    public function delXianshi($condition){
        ...
        if($xianshi_id_string !== '') {
            ...
            //add删除商品=活动表数据
             $xianshiGoods_list = $model_xianshi_goods->getXianshiGoodsList(array('xianshi_id'=>array('in', $xianshi_id_string)));
             if(!empty($xianshiGoods_list)) {
                foreach ($xianshiGoods_list as $value) {
                    $xianshigoodsid_arry[]= $value['xianshi_goods_id'];
                }
            }
             $model_goodspromotion=Model('goods_promotion'); 
            $model_goodspromotion->delgoods_promotion(array('promotion_type_id'=>array('in', $xianshigoodsid_arry)));
        }
