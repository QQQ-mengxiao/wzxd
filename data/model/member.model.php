...
public function l_login($passwd,$condition) {
	//$jiami = Logic('login');
	//return $jiami->password_verify($passwd,$this->table('member')->field('member_passwd')->where($condition)->master(false)->find());
	return 1;
}
		...
		if(!empty($member_info['member_login_time'])) {
			if(!empty($member_info['member_login_time'])) {
				...
				$update_info = array(
						'member_login_num' => ($member_info['member_login_num'] + 1),
						'member_login_time' => TIMESTAMP,
						'member_old_login_time' => TIMESTAMP,
						'member_login_ip' => getIp(),
						'member_old_login_ip' => getIp()
				);
			}
	...
	public function addMember($param) {
		...
		try {
		    ...
		    $member_info['share_id']			= $param['share_id'];
		    $member_info['is_liebian']			= intval($param['is_liebian']);
		    $member_info['company_id']			= intval($param['company_id']);
		    // $member_info['is_liebian']			= $param['is_liebian'];
		    //$member_info['member_passwd']		= md5(trim($param['member_passwd']));
		    $jiami = Logic('login');
		    $member_info['member_passwd']       = $jiami->password_hash(trim($param['member_passwd']),PASSWORD_BCRYPT,array());
			...
			if	($param['groupbuy_leader_id']) {
				$member_info['groupbuy_leader_id'] = $param['groupbuy_leader_id'];
			}
            ...
            //添加会员积分
            if (C('points_isuse')){
                Model('points')->savePointsLog('regist',array('pl_memberid'=>$insert_id,'pl_membername'=>$param['member_name'],'pl_inviterid'=>$param['inviter_id']),false);
                //添加邀请会员积分
                $inviter_name = Model('member')->table('member')->getfby_member_id($member_info['inviter_id'],'member_name');
				Model('points')->savePointsLog('inviter',array('pl_memberid'=>$param['inviter_id'],'pl_membername'=>$inviter_name,'invited'=>$insert_id));
            }
		    $this->commit();
		    return $insert_id;
		} catch (Exception $e) {
	...
	public function getDiscount($member_id=''){
		//获取等级和折扣值------MX
		...
		$member_detail = $model_member->getMemberInfoByID($member_id);
		$member_exppoints = $member_detail['member_exppoints'];//可以取到当前买家的经验值

		//判断当前买家的经验值可以享受什么折扣
		if($member_exppoints >= $list_setting['member_grade'][0]['exppoints'] && $member_exppoints < $list_setting['member_grade'][1]['exppoints']){
			$discount = $list_setting['member_grade'][0]['discount'];
		}elseif($member_exppoints < $list_setting['member_grade'][2]['exppoints']){
			$discount = $list_setting['member_grade'][1]['discount'];
		}elseif($member_exppoints < $list_setting['member_grade'][3]['exppoints']) {
			$discount = $list_setting['member_grade'][2]['discount'];
		}else{
			$discount = $list_setting['member_grade'][3]['discount'];
		}
		//$discount = number_format($discount/100,2);//将对应的折扣返回
		$discount = $discount/100;
        return $discount;
	}
		public function getpoints_grade($member_id=''){
		//获取等级和折扣值------MX
		$list_setting = Model('setting')->getListSetting();
		$list_setting['member_grade'] = $list_setting['member_grade']?unserialize($list_setting['member_grade']):array();
		...
		//判断当前买家的经验值可以享受什么折扣
		if($member_exppoints >= $list_setting['member_grade'][0]['exppoints'] && $member_exppoints < $list_setting['member_grade'][1]['exppoints']){
			$discount = $list_setting['member_grade'][0]['points_grade'];
		}elseif($member_exppoints < $list_setting['member_grade'][2]['exppoints']){
			$discount = $list_setting['member_grade'][1]['points_grade'];
		}elseif($member_exppoints < $list_setting['member_grade'][3]['exppoints']) {
			$discount = $list_setting['member_grade'][2]['points_grade'];
		}else{
			$discount = $list_setting['member_grade'][3]['points_grade'];
		}
		//$discount = number_format($discount/100,2);//将对应的折扣返回
		$points_grade = $discount;
        return $points_grade;
	}
	public function addmemberlog($type='',$phone=''){
        $data=gmdate('Y-m-d H:i:s', time() + 3600 * 8)."\t".$_SESSION['member_id']."\t".$_SESSION['member_name']."\t".$type."\t".getIp()."\t".$phone;
        file_put_contents("../log/".gmdate('Ymd',time() + 3600 * 8).".txt",$data.PHP_EOL,FILE_APPEND);
    }

	public function getGrade($member_id=''){
		//获取等级和折扣值------MX
		$list_setting = Model('setting')->getListSetting();
		$list_setting['member_grade'] = $list_setting['member_grade']?unserialize($list_setting['member_grade']):array();
		...
		//判断当前买家等级
		if($member_exppoints >= $list_setting['member_grade'][0]['exppoints'] && $member_exppoints < $list_setting['member_grade'][1]['exppoints']){
			$grade = $list_setting['member_grade'][0]['level'];
		}elseif($member_exppoints < $list_setting['member_grade'][2]['exppoints']){
			$grade = $list_setting['member_grade'][1]['level'];
		}elseif($member_exppoints < $list_setting['member_grade'][3]['exppoints']) {
			$grade = $list_setting['member_grade'][2]['level'];
		}else{
			$grade= $list_setting['member_grade'][3]['level'];
		}
        return $grade;
	}

	//获取会员等级接口
	public function getmember_level($member_id=''){
		//获取等级和折扣值------MX
		$list_setting = Model('setting')->getListSetting();
		$list_setting['member_grade'] = $list_setting['member_grade']?unserialize($list_setting['member_grade']):array();
		...
		//判断当前买家等级
		if($member_exppoints >= $list_setting['member_grade'][0]['exppoints'] && $member_exppoints < $list_setting['member_grade'][1]['exppoints']){
			$grade['level'] = $list_setting['member_grade'][0]['level'];
			$grade['level_name'] = $list_setting['member_grade'][0]['level_name'];
		}elseif($member_exppoints < $list_setting['member_grade'][2]['exppoints']){
			$grade['level'] = $list_setting['member_grade'][1]['level'];
			$grade['level_name'] = $list_setting['member_grade'][1]['level_name'];
		}elseif($member_exppoints < $list_setting['member_grade'][3]['exppoints']) {
			$grade['level'] = $list_setting['member_grade'][2]['level'];
			$grade['level_name'] = $list_setting['member_grade'][2]['level_name'];
		}else{
			$grade['level'] = $list_setting['member_grade'][3]['level'];
			$grade['level_name'] = $list_setting['member_grade'][3]['level_name'];
		}
        return $grade;
	}
