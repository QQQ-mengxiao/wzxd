...
class voucherModel extends Model {
    ...
    /**
     * 返回当前可用的代金券列表,每种类型(模板)的代金券里取出一个代金券码(同一个模板所有码面额和到期时间都一样) ,增加分类、单品券
     * @param array $condition 条件
     * @param array $goods_total 商品总金额
     * @return string
     */
    public function getCurrentAvailableVoucherPro($condition = array(), $goods_total = array()) {
        if (isset($_GET['test']) && $_GET['test']==12) {
       ...
       foreach ($voucher_list as $key => $voucher) {
        $voucher_t_info = Model()->table('voucher_template')->field('voucher_t_state,voucher_t_type')->where(array('voucher_t_id'=>$voucher['voucher_t_id']))->find();
        $voucher_type_info = Model()->table('voucher_type')->field('is_use,goods_id,goodsClass_id')->where(array('voucher_tid'=>$voucher['voucher_t_id']))->find();
        ...
        if ($voucher_t_info['voucher_t_state']!=1) {
            unset($voucher_list[$key]);
            continue;
        }
        if ($voucher_t_info['voucher_t_type'] == '0' && $goods_total[1] < $voucher['voucher_limit']) {//是否判断全场不可用或可用？
            unset($voucher_list[$key]);
            continue;  
        }
        if ($voucher_t_info['voucher_t_type'] == '1') {//品类
            $class_total = 0;
            //字符集转数组
            $class_ids = explode(",",$voucher_t_info['goodsClass_id']);
            //1.可用
            if ($voucher_t_info['is_use'] == '1') {
                foreach ($goods_total['gc_id'] as $k => $value) {
            ...
            if ($class_total < $voucher['voucher_limit']) {
                unset($voucher_list[$key]);
                continue;
            }
        }
        if ($voucher_t_info['voucher_t_type'] == '2') {
            $goods_total1 = 0;
            //字符集转数组
            ...
            //1.可用
            if ($voucher_t_info['is_use'] == 1) {
                foreach ($goods_total['goods_id'] as $k => $value) {
            ...
            if ($goods_total1 < $voucher['voucher_limit']) {
                unset($voucher_list[$key]);
                continue;
            }
        }
    ...
    public function getCurrentAvailableVoucher($condition = array(), $goods_total = 0) {
       ...
       $voucher_list = $this->table('voucher')->where($condition)->key('voucher_t_id')->select();
       foreach ($voucher_list as $key => $voucher) {
        $voucher_t_state=Model('voucher_template')->getfby_voucher_t_id($voucher['voucher_t_id'], 'voucher_t_state'); 
           if ($goods_total < $voucher['voucher_limit']|| $voucher_t_state!=1) {
...
/**
     * 查询可兑换代金券模板详细信息，包括店铺信息
     */
    public function getCanChangeTemplateInfo2($vid,$member_id){
        if ($vid <= 0 || $member_id <= 0 ){
            return array('state'=>false,'msg'=>'参数错误');
        }
        //查询可用代金券模板
        ...
        if (!empty($voucher_list)){
            ...
            // var_dump(C('promotion_voucher_buyertimes_limit'));die;
            //买家最多只能拥有同一个店铺尚未消费抵用的店铺代金券最大数量的验证
            if ($voucher_count >= intval(C('promotion_voucher_buyertimes_limit'))){
                $message = sprintf('您的可用代金券已有%s张,不可再兑换了',C('promotion_voucher_buyertimes_limit'));
                return array('state'=>false,'msg'=>$message);
     ...
     * 查询可兑换代金券模板详细信息，包括店铺信息
     */
    public function getCanChangeTemplateInfo3($vid,$member_id){
        if ($vid <= 0 || $member_id <= 0 ){
            return array('state'=>false,'msg'=>'参数错误');
        }
        //查询可用代金券模板
    ...
    public function getMemberVoucherList($member_id, $voucher_state, $page = null) {
        ...
        $field = 'voucher_id,voucher_code,voucher_title,voucher_desc,voucher_start_date,voucher_end_date,voucher_price,voucher_limit,voucher_state,voucher_order_id,voucher_store_id,voucher_order_type,store_name,store_id,store_domain,voucher_t_type';
    ...
    public function getVoucherTemplateList($where, $field = '*', $limit = 0, $page = 0, $order = '', $group = '') {
        ...
        if (!empty($voucher_list) && is_array($voucher_list)){
            foreach ($voucher_list as $k=>$v){
                ...
                $v['voucher_t_gc_name'] = $gc_list[$v['voucher_t_gc_id']]['gc_name'];
                //领取方式
     ...
     * @param int $num 查询条数
     */
    public function getRecommendTemplate2(){
        //查询推荐的热门代金券列表
        $where = array();
        $where['voucher_t_state'] = $this->templatestate_arr['usable'][0];
        $where['voucher_t_is_xinren'] = 1;
        //领取方式为积分兑换
        //$where['voucher_t_gettype'] = $this->voucher_gettype_array['points']['sign'];
        $where['voucher_t_end_date'] = array('gt',time());
        $recommend_voucher = $this->getVoucherTemplateList($where, $field = '*', $num, 0, 'voucher_t_recommend desc,voucher_t_id desc');
    ...
    public function exchangeVoucher($template_info, $member_id, $member_name = ''){
        ...
        // $k=intval($template_info['voucher_t_validity']);
        // $d = strtotime(date("Y-m-d ",strtotime("+$k day")));
...
//        if ($template_info['voucher_t_points'] > 0 && $template_info['voucher_t_gettype'] == $this->voucher_gettype_array['points']['sign']){
    ...
    public function exchangeVoucher1($rs,$member_id,$number){
        $member_info = Model('member')->where(array('member_id' =>trim($member_id)))->find();
        $template_info=$this->table('voucher_template')->where(array('voucher_t_id' =>$rs))->find();
        for($i=0;$i<=$number-1;$i++){
         ...
         $d =$insert_arr['voucher_start_date']+$template_info['voucher_t_validity']*24*3600;
            ...
            $insert_arr['voucher_store_id'] = $template_info['voucher_t_store_id'];
            $insert_arr['voucher_state'] = 1;
              $insert_arr['voucher_order_type'] = $template_info['voucher_order_type'];
            $insert_arr['voucher_active_date'] = time();
            $insert_arr['voucher_owner_id'] = $member_info['member_id'];
            $insert_arr['voucher_owner_name'] = $member_info['member_name'];;
            $insert_arr['voucher_gc_id'] = $template_info['voucher_t_gc_id'];
            $result[$i] = $this->table('voucher')->insert($insert_arr);
                        // var_dump($result[$i]);die;
            if ($result[$i]){
                $result[$i] = $this->editVoucherTemplate(array('voucher_t_id'=>$template_info['voucher_t_id']), array('voucher_t_giveout'=>array('exp','voucher_t_giveout+1')));
                if (!$result[$i]){
                    return array('state'=>false,'msg'=>'发放失败');
                    break;
                }
            } else {
                return array('state'=>false,'msg'=>'发放失败');
     ...
     ///批量发放代金券slk20200731
        public function batch_sendVoucher($rs,$member_id,$number){
            $model_member = Model('member');
            $condition=array();
        ...
        for($i=0;$i<=$number-1;$i++){
            $num=ceil($count/1000);
          for($t = 0; $t < $num; $t++){
            $a=array_slice($member_id,1000*$t,1000);
            $condition=array();
            $condition['member_id']  = array('in',$member_id);
            $member_list = $model_member->getMemberList($condition);
            $addall_arr=array();
              foreach ($member_list as $key=>$val){
                $insert_arr = array();
                $insert_arr['voucher_code'] = $this->get_voucher_code($val['member_id']);
                $insert_arr['voucher_t_id'] = $template_info['voucher_t_id'];
                $insert_arr['voucher_title'] = $template_info['voucher_t_title'];
                $insert_arr['voucher_desc'] = $template_info['voucher_t_desc'];
                //所发放代金券时用户得到的代金券有效起止日期
                ...
                $insert_arr['voucher_order_type'] = $template_info['voucher_order_type'];
                $insert_arr['voucher_active_date'] = time();
                $insert_arr['voucher_owner_id'] = $val['member_id'];
                $insert_arr['voucher_owner_name'] = $val['member_name'];;
                $insert_arr['voucher_gc_id'] = $template_info['voucher_t_gc_id'];
                $addall_arr[] = $insert_arr;
              }
             $result = $this->addVoucherBatch($addall_arr);
            }
            if ($result){
                $result1 = $this->editVoucherTemplate(array('voucher_t_id'=>$template_info['voucher_t_id']), array('voucher_t_giveout'=>array('exp','voucher_t_giveout + '.$count)));
                if (!$result1){
                    return array('state'=>false,'msg'=>'发放失败');
                    break;
                }
    ...
    public function addvoucherLog($data) {
        ...
        $this->table('donate_log')->insert($data);
    }
    ...
    */
    public function get_user_info($condition){
        $on='voucher.voucher_owner_id=member.member_id';
        // $where="voucher_t_id='$t_id'";
        //20190516
        $t_id = $condition['t_id'];
        $member_name = $condition['member_name'];
        if(!empty($member_name)){
        $where=array(
            'voucher_t_id'=>$t_id,'voucher_owner_name'=>$member_name);
          }else{
             $where=array('voucher_t_id'=>$t_id);
        }
        //$field='voucher.voucher_owner_id,voucher.voucher_owner_name,voucher.voucher_start_date,member.member_mobile';
//        $user_info=$this->table('voucher,member')->join('inner')->on($on)->where($where)->page(20)->order('member_id asc')->select();
     ...
     * 订单满赠增加代金券
     */
        public function mangzeng_voucher($member_id,$store_id,$order_amount){
            $model_member = Model('member');
            $condition=array();
            $condition['store_id']=$store_id;
            $condition['order_limit'] = array('elt',$order_amount);
            $order='order_limit desc';
            $p_voucher = Model('p_voucher');
            $p_voucher_rule = Model('p_voucher_rule');
            $mansong_voucher_list = $p_voucher->getMansongList($condition, null, $order, '*');
            //print_r( $mansong_voucher_list);//die;
            $member_list = $model_member->getMemberInfoByID($member_id);
            //print_r($member_list);die;
            foreach ($mansong_voucher_list as $key => $value) {
              if($order_amount>=$value['order_limit']){
                    $rule_list = $p_voucher_rule->getMansongRuleListByID($value['p_voucher_id']);
                    //print_r($rule_list);die;
                    foreach ($rule_list  as $k => $v) {
                        $template_info=$this->table('voucher_template')->where(array('voucher_t_id' =>$v['voucher_t_id']))->find();
                        for($i=0;$i<=$v['count']-1;$i++){
                                //验证是否可以领取代金券
                             $voucher_t_id=$template_info['voucher_t_id'];
                             $data = $this->getCanChangeTemplateInfo3($voucher_t_id,intval($member_id));
                           // print_r($data);die;
                            if ($data['state'] ){              
                                $insert_arr = array();
                                $insert_arr['voucher_code'] = $this->get_voucher_code($member_list['member_id']);
                                $insert_arr['voucher_t_id'] = $template_info['voucher_t_id'];
                                $insert_arr['voucher_title'] = $template_info['voucher_t_title'];
                                $insert_arr['voucher_desc'] = $template_info['voucher_t_desc'];
                                //所发放代金券时用户得到的代金券有效起止日期
                        ...
                        //var_dump($result);die;
                        if ($result){
                            $result3 = $this->editVoucherTemplate(array('voucher_t_id'=>$template_info['voucher_t_id']), array('voucher_t_giveout'=>array('exp','voucher_t_giveout + '.$v['count'])));
                            if (!$result3){
                                return array('state'=>false,'msg'=>'发放失败2');
                                break;
                            }
                        } else {
                            return array('state'=>false,'msg'=>'发放失败');
                     ...
                     return array('state'=>true,'msg'=>'发放成功','voucher'=> $voucher);break;
                }
            }
       }
    /**
  分享裂变
  */
    public function liebian_voucher($member_id,$store_id){
        $buyer_id = $member_id;
        $member_info = Model('member')->where(array('member_id' =>trim($member_id)))->find();
        if($member_info['is_liebian'] == 0 || empty($member_info['share_id'])){
            return 1;
        }else{
            //店铺ID
            $store_id = $store_id;
            //发放人ID
            $member_id = $member_info['share_id'];
            ...
            foreach ($rule_list  as $k => $v) {
                ...
                if ($result){
                    ...
                    if (!$result1){
                      return array('state'=>false,'msg'=>'发放失败2');break;
                    }
                } else {
                    return array('state'=>false,'msg'=>'发放失败');break;
                }
                $voucher_msg[]=$template_info['voucher_t_title'];
            }
            $update_array['is_liebian']   = 0;
            $result =$model_member->table('member')->where(array('member_id'=>$buyer_id))->update($update_array);
            return array('state'=>true,'msg'=>'发放成功','voucher'=> $$voucher_msg);
        }        
    } 

    ///批量发放代金券slk20200731
    public function batch_sendVoucherAdmin($rs,$member_id_array,$number){
        // $model_member = Model('member');
        // $condition=array();
        // $condition['member_id']  = array('in',$member_id);
        // $member_list = $model_member->getMemberList($condition);
        $count=count($member_id_array);
        ...
        for($t = 0; $t < $num; $t++){
            ...
            // $member_list = $model_member->getMemberList($condition);
            $addall_arr=array();
            foreach ($member_id_array as $val){
                $insert_arr = array();
                $insert_arr['voucher_code'] = $this->get_voucher_code($val);
                $insert_arr['voucher_t_id'] = $template_info['voucher_t_id'];
                $insert_arr['voucher_title'] = $template_info['voucher_t_title'];
                $insert_arr['voucher_desc'] = $template_info['voucher_t_desc'];
                //所发放代金券时用户得到的代金券有效起止日期
                ...
                $insert_arr['voucher_order_type'] = $template_info['voucher_order_type'];
                $insert_arr['voucher_active_date'] = time();
                $insert_arr['voucher_owner_id'] = $val;
                $insert_arr['voucher_owner_name'] = Model()->table('member')->getfby_member_id($val,'member_name');
                $insert_arr['voucher_gc_id'] = $template_info['voucher_t_gc_id'];
                $addall_arr[] = $insert_arr;
            }
            $result = $this->addVoucherBatch($addall_arr);
