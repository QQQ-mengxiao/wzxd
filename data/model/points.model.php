...
class pointsModel extends Model {
	...
	function savePointsLog($stage,$insertarr,$if_repeat = true){
		...
		$member_name = Model('member')->getfby_member_id($insertarr['pl_memberid'],'member_name');
		//记录原因文字
		switch ($stage){
			case 'regist':
			...
			case 'order':
				...
				if ($insertarr['orderprice']){
					...
					$points_grade = $member_model->getpoints_grade($insertarr['pl_memberid']);
		            $insertarr['pl_points'] = @intval($insertarr['orderprice']*$points_grade);
