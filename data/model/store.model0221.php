...
/**
    ...
    private function getGoodsListBySales($store_array) {
    	...
    	foreach ($store_array as $value) {
    		$store_array[$value['store_id']]['search_list_goods'] = $model->table('goods')->field($field)->where(array('store_id'=>$value['store_id'],'goods_state'=>1,'goods_commend'=>1))->order('goods_salenum desc')->limit(8)->select();
    	}
    	return $store_array;
    }
}
