...
class p_xianshi_goodsModel extends Model{
    ...
    public function getXianshiGoodsOnList($condition, $page=null, $order='', $field='*', $limit = 0,$goods_serial) {
        $where = array();
        if($condition['goods_name']){
            $where['p_xianshi_goods.goods_name'] = $condition['goods_name'];
        }
        $where['p_xianshi_goods.xianshi_id'] = $condition['xianshi_id'];
        $where['goods.goods_serial'] = $goods_serial;
        $on = 'goods.goods_id = p_xianshi_goods.goods_id';
        return $xianshi_goods_list = $this->table('p_xianshi_goods,goods')->join('left')->field($field)->where($where)->on($on)->page($page)->order($order)->limit($limit)->select();
    }
	 ...
	 *
	 */
	public function getXianshiGoodsExtendList($condition, $page=null, $order='', $field='*', $limit = 0,$goods_serial = '') {
	    if($goods_serial) {
            $xianshi_goods_list = $this->getXianshiGoodsOnList($condition, $page, $order, $field, $limit,$goods_serial);
        }else{
    ...
    public function editXianshiGoods($update, $condition){
        ...
        if ($result) {
            $xianshiGoods_list = $this->getXianshiGoodsList($condition);
             
        foreach ($xianshiGoods_list as $key => $value) {
           $model_goodspromotion=Model('goods_promotion'); 
            $arr_goodspromotion=array();
            $arr_goodspromotion['promotion_type_id']=$value['xianshi_goods_id'];
            $info=$model_goodspromotion->getgoods_promotionInfo($arr_goodspromotion);
            if(!empty($info)){
                $model_goodspromotion=Model('goods_promotion'); 
                $array_goodspromotion=array();
                $array_goodspromotion['promotion_type_id']=$value['xianshi_goods_id'];
                $update_goodspromotion=array();
                if(!empty($update['commis_rate'])){
                    $update_goodspromotion['commis_rate']=$update['commis_rate'];
                }
                 if(!empty($update['xianshi_price'])){
                    $update_goodspromotion['price']=$update['xianshi_price'];
                }
                 if(!empty($update['upper_limit'])){
                    $update_goodspromotion['upper_limit']=$update['upper_limit'];
                }
                if(!empty($update['end_time'])){
                    $update_goodspromotion['end_time']=$update['end_time'];
                }
                $model_goodspromotion->editgoods_promotion($update_goodspromotion,$array_goodspromotion);
            }else{
                 $where=array();
                $where['xianshi_goods_id']=$value['xianshi_goods_id'];
                $this->where($where)->update(array('is_tongbu'=>0));
                // $this->editXianshiGoods(array('is_tongbu'=>0), $where);
            }
        }
    ...
    public function delXianshiGoods($condition){
        $xianshi_goods_list = $this->getXianshiGoodsList($condition, null, '', 'goods_id,xianshi_goods_id');
        ...
        if ($result) {
            ...
            if (!empty($xianshi_goods_list)) {
                foreach ($xianshi_goods_list as $val) {
                    ...
                    // $array_goodspromotion['promotion_type']=10;
                    $array_goodspromotion['promotion_type_id']=$val['xianshi_goods_id'];
    ...
    public function getXianshiGoodsExtendInfo($xianshi_info) {
		...
		$xianshi_info['xianshi_app_price'] = ncPriceFormat($xianshi_info['xianshi_app_price']);
