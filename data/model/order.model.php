...
class orderModel extends Model {
    ...
    */
    ...
    public function apiGetOrderListStatePage($member_id,$condition)
    {
        ...
        foreach ($order_common_list as $value) {
            ...
            if($order_list[$value['order_id']]["order_state"] == 30 && $order_list[$value['order_id']]["wms_sn"] != null){
                $order_list[$value['order_id']]["order_amount"] .=  " 取件码：".$order_list[$value['order_id']]["wms_sn"];
        }
        }
        //取商品列表
        $order_goods_list = $this->getOrderGoodsList(array('order_id' => array('in', array_keys($order_list))));
 ...
 /**
    ...
    public function getOrderList2($consignee_name = '',$condition, $pagesize = '', $field = '*', $order = 'order.order_id desc', $limit = '', $extend = array(), $master = false,$type=''){
        $model = Model();
        $field = '*';
        $on = 'order.order_id=order_common.order_id,order.order_id=order_goods.order_id';
        $model->table('order,order_common,order_goods')->field($field);
 ...
 public function getOrderList3($consignee_name = '',$condition, $pagesize = '', $field = '*', $order = 'order.order_id desc', $limit = '', $extend = array(), $master = false){
	...
	public function getOrderGoodsExportList($condition,$limit,$goods_serial=''){
        ...
        $field = 'order.order_id,order.z_order_sn,order.order_sn,order.add_time,order.payment_time,order.finnshed_time,order.goods_amount,order.order_amount,order.pd_amount,order.rcb_amount,order.shipping_fee,order.store_tax_total,order.buyer_name,order.buyer_id,order_common.reciver_info,order_common.reciver_name,order_common.deliver_explain,order_common.order_message,order_common.daddress_id,order.store_name,order_common.voucher_price,order.payment_code,order_common.shipping_time,order_common.reciver_ziti_id,order_common.order_message,order_common.deliver_explain,order_common.mall_info,order.refund_state,order.refund_amount,order.shipping_code,order.order_type,order_common.voucher_code,order.by_post,order.order_state,order.is_zorder,order_common.ziti_ladder_time';
        ...
        foreach ($list as $key => $value) {
            ...
            $order_goods_list = $this->getOrderGoodsList(array('order_id'=>$value['order_id']),'goods_id,goods_name,goods_num,goods_price,goods_pay_price,goods_cost_price,voucher_price,is_cw,goods_type,goods_serial as order_goods_serial,goods_barcode as order_goods_barcode,deliverer_id as order_goods_deliverer_id,gc_id as order_goods_gc_id');//,goods_spec,gc_id,goods_storage
            if($order_goods_list){
    ...
    public function getOrderGoodsList($condition = array(), $fields = '*', $limit = null, $page = null, $order = 'goods_type asc , rec_id desc', $group = null, $key = null)
   {
             ...
             public function tzGetOrderListPage($ziti_id, $num_page,$condition)
    {
        ...
        $sql = "SELECT $fields1 FROM 718shop_order WHERE order_id IN (SELECT order_id FROM 718shop_order_common WHERE reciver_ziti_id = " . $ziti_id . " )AND delete_state = 0 AND by_post = 1". $condition." ORDER BY payment_time DESC,  order_id DESC LIMIT 10 offset " . ($num_page - 1)*10;
        $list = Model()->query($sql);
        // print_r($sql);die;
        if (empty($list)) return array();
        $order_list = array();
   ...
   public function tzGetOrderListStatePage($ziti_id,$condition)
    { 
        $field ='order_id';
        $sql = "SELECT $field FROM 718shop_order WHERE order_id IN (SELECT order_id FROM 718shop_order_common WHERE reciver_ziti_id = " . $ziti_id . "  ) AND by_post = 1".$condition;
        $list = Model()->query($sql);
        // var_dump($list);die;
        return $list;
    }
