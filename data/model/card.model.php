...
class cardModel extends Model {
    ...
    * return 支付成功:true，失败:false/null
    */
    public function updateHCConsumeExchange($data_card,$type,$data_consume){
        ...
        if($dbh){
            ...
            switch($type){
                case 'zihpay':
                    $isPaidSql = "select count(*) from HCConsumeExchange where OrderSno='$orderSno' and Amount>0";
                    //判断是否有记录
                    $stmt = $dbh->query($isPaidSql);
                    if ($stmt) {
                        $count = $stmt->fetchColumn();
            ...
            switch($type){
                case 'zihpay':
                    $amount = $data_card['amount'];
                    $lastData = $data_card['LastDate'];
                    $amountSql="update HCCardBaseExchange set Balance16=Balance16-$amount where PersonalID='$gonghao'";
                break;
                case 'refund':
                    $amountSql="update HCCardBaseExchange set Balance16=Balance16+".$data_card['amount']." where PersonalID='$gonghao'";
                break;
            }
            ...
            $ConsumeTime = $data_consume['ConsumeTime'];
            $ConsumeAmount = $data_consume['Amount'];
            $consumeSql="insert into HCConsumeExchange select $gonghao,16,'新零售','$ConsumeDate','$ConsumeTime',$ConsumeAmount,Balance16,$ConsumeType,'$orderSno' FROM HCCardBaseExchange WHERE PersonalID='$gonghao'";
    ...
    *获取一卡通新零售余额根据工号
    */
    public function getBalancebygh($gonghao){
        // $conn=$this->dbconnect('waterrun');
        $conn=$this->dbconnect("waterrun");
        if($conn){
            try{
                $sql="select * from HCCardBaseExchange where PersonalID in ($gonghao)";
                // $exec=odbc_exec($conn,$sql);
                $exec=$conn->query($sql);
                // $card_info=odbc_fetch_array($exec);
                $card_info=$exec->fetch();
