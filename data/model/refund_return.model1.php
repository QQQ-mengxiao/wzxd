...
class refund_returnModel extends Model{
    ...
    public function editOrderRefund_jp($refund) {
        ...
        if ($refund_id > 0) {
            Language::read('model_lang_index');
            $order_id = $refund['order_id'];//订单编号
            $field = 'order_id,buyer_id,buyer_name,store_id,order_sn,pay_sn,order_amount,payment_code,order_state,refund_amount,rcb_amount,pd_amount,card_amount';//mx 多获取预存款支付金额一列
            $model_order = Model('order');
            $order = $model_order->getOrderInfo(array('order_id'=> $order_id),array('order_common'),$field);
            ...
            //获取商品金额，判断
            $goods_pay_price = Model('order_goods')->getfby_rec_id($refund['refund_id'], 'goods_pay_price');
            try {
                $this->beginTransaction();
                $rcb_amount = $order['rcb_amount'];//充值卡支付金额
                $card_amount=$order['card_amount'];//一卡通支付金额
                $predeposit_amount = $refund['refund_amount']-$order['refund_amount']-$rcb_amount;//可退金额 可能包括预存款和三方支付金额
                //$order['refund_amount']未退款是0，有过退款则为前次退款金额和
                //减去一卡通支付金额
                $api_amount = $predeposit_amount-$order['pd_amount']-$card_amount;

                //存在充值卡支付的情况 优先退充值卡
                //判断存在充值卡支付的金额  并且充值卡支付的金额大于要退的金额 退款金额全部退到充值卡里
                if ($rcb_amount > 0) {
                    if ($rcb_amount > $refund['refund_amount']) {//充值卡金额大于申请的退款金额
                        $log_array = array();
                        $log_array['member_id'] = $order['buyer_id'];
                        $log_array['member_name'] = $order['buyer_name'];
                        $log_array['order_sn'] = $order['order_sn'];
                        $log_array['amount'] = $refund['refund_amount'];//充值卡退得到的金额为申请的退款金额
                        $state = $model_predeposit->changeRcb('refund', $log_array);//增加买家可用充值卡金额
                    } elseif ($rcb_amount <= $refund['refund_amount']) {//如果充值卡支付的金额小于或等于要退的金额 退款金额等于充值卡支付的金额
                        $log_array = array();
                        $log_array['member_id'] = $order['buyer_id'];
                        $log_array['member_name'] = $order['buyer_name'];
                        $log_array['order_sn'] = $order['order_sn'];
                        $log_array['amount'] = $rcb_amount;
                        $state = $model_predeposit->changeRcb('refund', $log_array);//增加买家可用充值卡金额
                    }
                }
                //判断预存款支付金额是否大于零
                if ($order['pd_amount'] > 0) {
                    //如果预存款支付金额小于等于剩余的可退金额 退款金额等于预存款支付的金额
                    if ($order['pd_amount'] <= $predeposit_amount) {
                        $log_array = array();
                        $log_array['member_id'] = $order['buyer_id'];
                        $log_array['member_name'] = $order['buyer_name'];
                        $log_array['order_sn'] = $order['order_sn'];
                        $log_array['amount'] = $order['pd_amount'];
                        $state = $model_predeposit->changePd('refund', $log_array);//增加买家可用预存款金额
                    } elseif ($order['pd_amount'] > $predeposit_amount && $predeposit_amount > 0) {//如果预存款支付金额大于剩余的可退金额 退款金额等于剩余的可退金额
                        //接上 可能存在可退剩余金额小于0的情况，即申请的退款金额小于充值卡支付的金额，此时不需要更改预存款数据
                        $log_array = array();
                        $log_array['member_id'] = $order['buyer_id'];
                        $log_array['member_name'] = $order['buyer_name'];
                        $log_array['order_sn'] = $order['order_sn'];
                ...
                } elseif ($api_amount == 0) {//商品金额为0 或者存在一卡通支付的情况
                    if ($goods_pay_price == 0 || $card_amount >= 0) {
                        $pdc_sn = $model_predeposit->makeSn();
                        $data = array();
                        $data['pdc_sn'] = $pdc_sn;
                        $data['pdc_member_id'] = $member_info['member_id'];
