...
class countryModel extends Model{
	...
	public function getCodeRow($code_id,$web_id){
		...
		$param['web_id']	= $web_id;
		$result	= $this->table('country_code')->where($param)->find();
		return $result;
	}
	...
	public function getCodeList($condition = array()){
		$result = $this->table('country_code')->where($condition)->order('web_id')->select();
		return $result;
	}
	...
	public function updateCode($condition,$data){
		...
		if (is_array($data)){
			$result = $this->table('country_code')->where($condition)->update($data);
			return $result;
		} else {
			return false;
		}
	...
	public function getWebList($condition = array('web_page' => 'index'),$page = ''){
		$result = $this->table('country')->where($condition)->order('web_sort')->page($page)->select();
		return $result;
	}
	...
	public function updateWeb($condition,$data){
		...
		if (is_array($data)){
			$result = $this->table('country')->where($condition)->update($data);
			return $result;
		} else {
			return false;
		}
	...
	public function updateWebHtml($web_id = 1,$style_name = 'orange'){
		...
		if(!empty($code_list) && is_array($code_list)) {
    		...
    		switch ($web_id) {
        	    case 101:
        	        $style_file = BASE_DATA_PATH.DS.'resource'.DS.'country'.DS.'country_focus.php';
        	    	break;
        	    case 102:
        	        $style_file = BASE_DATA_PATH.DS.'resource'.DS.'country'.DS.'country_focus2.php';
        	    	break;
        	    case 121:
        	    	$style_file = BASE_DATA_PATH.DS.'resource'.DS.'country'.DS.'country_sale_goods.php';
        	    	break;
        	    default:
        	    	$style_file = BASE_DATA_PATH.DS.'resource'.DS.'country'.DS.'country_default.php';
        	    	break;
    		}
	 ...
	 *
	 */
	public function updateWebCountryHtml($web_id = 1,$style_name = 'orange'){
		$web_html = '';
		$code_list = $this->getCodeList(array('web_id'=>"$web_id"));
		if(!empty($code_list) && is_array($code_list)) {
			Language::read('web_config,home_index_index');
    		...
    		switch ($web_id) {
        	    ...
        	    case 121:
        	    	...
        	    	break;
        	    default:
        	    	$style_file = BASE_DATA_PATH.DS.'resource'.DS.'country'.DS.'country_detail.php';
        	    	break;
    		}
			if (file_exists($style_file)) {
				ob_start();
			...
			$web_array = array();
			$web_array['country_html'] = addslashes($web_html);
			$web_array['update_time'] = time();
			$this->updateWeb(array('web_id'=>$web_id),$web_array);
		}
		return $web_html;
	 ...
	 *
	 */
	public function getWebCountryHtml($web_id = 1,$web_page = 'index',$update_all = 0){
		$web_array = array();
		$web_list = $this->getWebList(array('web_show'=>1,'web_id'=>$web_id,'web_page'=> array('like',$web_page.'%')));
		if(!empty($web_list) && is_array($web_list)) {
			foreach($web_list as $k => $v){
			    $key = $v['web_page'];
				if ($update_all == 1 || empty($v['web_html'])) {//强制更新或内容为空时查询数据库
...
/**
	 ...
	 *
	 */
	public function getWebName($web_page = 'index'){
		$web_array = array();
		$web_list = $this->getWebList(array('web_id'=>array('lt','10'),'web_show'=>1,'web_page'=> array('like',$web_page.'%')));
		if(!empty($web_list) && is_array($web_list)) {
			foreach($web_list as $k => $v){
			    //$key = $v['web_page'];	
				$web_array[$k]['name'] = $v['web_name'];
				$web_array[$k]['id'] = $v['web_id'];
	 ...
	 *
	 */
	public function getCountryStyleList($style_id = 'index'){
		$style_data = array(
			'rose' => 玫瑰红,
			'cherry' => 樱桃红,
			'olive' => 橄榄绿,
