...
class goods_classModel extends Model
{
    ...
    public function getGoodsClassList2($condition, $field = '*') {
        $result = $this->table('goods_class')->field($field)->where($condition)->page(10)->limit(false)->select();
        return $result;
    }
    /**
     * 从缓存获取全部分类
     ...
     * 根据一级分类id取得所有三级分类
     */
    public function getChildClassByFirstId_vouchertype($id) {
        $data = $this->getCache();
        $result = array();
        if (!empty($data['children2'][$id])) {
            foreach ($data['children2'][$id] as $val) {
                $child = $data['data'][$val];
                $result['class'][$child['gc_id']] = $child['gc_name'];
            }
        }
        return $result;
    }
		...
		if (is_array($class_list)){
			foreach($class_list as $v) {
				if($v['gc_parent_id'] == 0) {
					...
					if(isset($d[$v['gc_parent_id']])) $d[$v['gc_parent_id']]['child'] = array_values($this->getZChildClassByFirstId($v['gc_parent_id']))[0]['class'];//防止出现父类不显示时子类被调出
				}
			}
		}
		return $d;
    ...
    public function getZChildClassByFirstId($id) {
        $data = $this->getCache();
        $result = array();
        if (!empty($data['children2'][$id])) {
            foreach ($data['children2'][$id] as $k=>$val) {
                $child = $data['data'][$val];
                $result[$child['gc_parent_id']]['class'][$k]['gc_id'] = $child['gc_id'];
                $result[$child['gc_parent_id']]['class'][$k]['gc_name'] = $child['gc_name'];
            }
        }
        return $result;
    }
    ...
    public function getParentIdById($id){
        if($id){
            $parent_id = Model()->table('goods_class')->getfby_gc_id($id,'gc_parent_id');
            if($parent_id!=0){
                $parent_id = Model()->table('goods_class')->getfby_gc_id($parent_id,'gc_parent_id');
            }
            return $parent_id;
        }
    }
