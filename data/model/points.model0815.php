...
class pointsModel extends Model {
	...
	function savePointsLog($stage,$insertarr,$if_repeat = true){
		...
		$member_name = Model('member')->getfby_member_id($insertarr['member_id'],'member_name');
		//记录原因文字
		switch ($stage){
			case 'regist':
				if(!$insertarr['pl_desc'] && $insertarr['pl_inviterid']){
					$insertarr['pl_desc']='注册会员[邀请人'.$insertarr['pl_inviterid'].']';
				}
				else{
			...
			case 'order':
				...
				if ($insertarr['orderprice']){
                    ...
                    $data_array = Array(18);
                    //得到当前几号
                    $ordersn=$insertarr['order_sn'];
                    $time = Model()->query("SELECT add_time  FROM `718shop_order` where order_sn=\"$ordersn\" ");
                    $addtime=$time[0]['add_time'];
                    $add_time=date("d",$addtime);
                     if(in_array($add_time, $data_array))
                       {
                       	$insertarr['pl_points'] = @intval(($insertarr['orderprice']/C('points_orderrate'))*2);
			...
			case 'rebate':
				if (!$insertarr['pl_desc']){
					$insertarr['pl_desc'] = '被邀请人['.$member_name.']消费';
				}
