...
class groupbuyModel extends Model{
	 ...
	 *
	 */
	public function getGroupbuyExtendList($condition, $page = null, $order = 'end_time desc,groupbuy_id asc', $field = '*', $limit = 0) {
    ...
    public function getGroupbuyInfoByGoodsID($goods_id) {
        $info = $this->_rGoodsGroupbuyCache($goods_commonid);
        if (empty($info)) {
            $condition = array();
            $condition['state'] = self::GROUPBUY_STATE_NORMAL;
