...
class buyLogic
{
    ...
    public function getGoodsOnlineList($buy_items, $member_id)
    {
            ...
            foreach ($goods_list as $k => $v) {
                ...
                switch ($v['promotion_type']) {
                    ...
                    $order_info = Model()->table('order,order_goods')->field('goods_num')->join('inner right')->on('order.order_id = order_goods.order_id')->where(array('order_goods.goods_id' => $v['goods_id'], 'order_goods.buyer_id' => $member_id, 'order_goods.promotions_id' => $v['promotion_type_id'], 'order.order_state' => array('gt', 0)))->select();
                        ...
                        $goods_list[$k]['is_group_ladder'] = $v['promotion_type']==10?4:10;
        ...
        if($sql_vstr){
            $sql_voucher = "SELECT sv.voucher_id,sv.voucher_code,sv.voucher_t_id,sv.voucher_title,sv.voucher_price,sv.voucher_limit,sv.voucher_order_type,svt.is_use,svt.goods_id,svt.goodsclass_id,svt.type FROM 718shop_voucher sv LEFT JOIN 718shop_voucher_type svt ON sv.voucher_t_id=svt.voucher_tid LEFT JOIN 718shop_voucher_template svt1 ON sv.voucher_t_id = svt1.voucher_t_id WHERE UNIX_TIMESTAMP() BETWEEN sv.voucher_start_date AND sv.voucher_end_date AND sv.voucher_owner_id=" . $member_id . " AND sv.voucher_state=1 AND svt1.voucher_t_state=1 " . $sql_vstr." GROUP BY sv.voucher_t_id";
    ...
    public function getGoodsOnlineInfo($goods_id, $quantity, $member_id)
    {
        ...
        switch ($goods_info['promotion_type']) {
            ...
            case 20: //10秒杀20折扣
             ...
             // slkedit已买，order_state>0
                ...
                $goods_info['is_group_ladder'] = $goods_info['promotion_type']==10?4:10;
        ...
        if (($goods_info['xianshi_type']) != 1) { //非限时秒杀商品
            ...
            $sql_voucher = "SELECT sv.voucher_id,sv.voucher_code,sv.voucher_t_id,sv.voucher_title,sv.voucher_price,sv.voucher_limit,sv.voucher_order_type,svt.is_use,svt.goods_id,svt.goodsclass_id,svt.type FROM 718shop_voucher sv LEFT JOIN 718shop_voucher_type svt ON sv.voucher_t_id=svt.voucher_tid LEFT JOIN 718shop_voucher_template svt1 ON sv.voucher_t_id = svt1.voucher_t_id WHERE UNIX_TIMESTAMP() BETWEEN sv.voucher_start_date AND sv.voucher_end_date AND sv.voucher_limit<=".$goods_total_price." AND sv.voucher_owner_id=" . $member_id . " AND sv.voucher_state=1 AND svt1.voucher_t_state=1 AND (svt.type IS NULL OR svt.type=1 OR svt.type=2 OR svt.type = 0) GROUP BY sv.voucher_t_id";
    ...
    public function buyStep2($data, $member_id)
    {
        ...
        if ($data['ifcart']) { //来自购物车
            ...
            $sql_voucher = "SELECT sv.voucher_id,sv.voucher_code,sv.voucher_t_id,sv.voucher_title,sv.voucher_price,sv.voucher_limit,sv.voucher_order_type,svt.is_use,svt.goods_id,svt.goodsclass_id,svt.type FROM 718shop_voucher sv LEFT JOIN 718shop_voucher_type svt ON sv.voucher_t_id=svt.voucher_tid LEFT JOIN 718shop_voucher_template svt1 ON sv.voucher_t_id = svt1.voucher_t_id WHERE UNIX_TIMESTAMP() BETWEEN sv.voucher_start_date AND sv.voucher_end_date AND sv.voucher_owner_id=" . $member_id . " AND sv.voucher_state=1 AND svt1.voucher_t_state=1 AND sv.voucher_t_id = " . $data['voucher_t_id'] . " " . $sql_vstr . "LIMIT 1";
            ...
            if ($mansong_info[0]['goods_storage'] >= 1) { //赠品库存充足
                ...
                $mansong['goods_num'] = $mansong_info[0]['goods_num'];
            ...
            if ($goods_info['promotion_type'] != 10) {
                ...
                $sql_voucher = "SELECT sv.voucher_id,sv.voucher_code,sv.voucher_t_id,sv.voucher_price,sv.voucher_limit,svt.is_use,svt.goods_id,svt.goodsclass_id,svt.type FROM 718shop_voucher sv LEFT JOIN 718shop_voucher_type svt ON sv.voucher_t_id=svt.voucher_tid LEFT JOIN 718shop_voucher_template svt1 ON sv.voucher_t_id = svt1.voucher_t_id WHERE sv.voucher_t_id=" . $voucher_t_id . " AND sv.voucher_owner_id=" . $member_id . " AND sv.voucher_state=1 AND svt1.voucher_t_state=1 AND UNIX_TIMESTAMP() BETWEEN sv.voucher_start_date AND sv.voucher_end_date and sv.voucher_limit <=" . $goods_list['goods_total_price'] . " LIMIT 1";
                $voucher_info = Model()->query($sql_voucher)[0];
                if ($voucher_info) {
