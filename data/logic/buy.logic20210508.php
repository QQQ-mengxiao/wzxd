...
class buyLogic {
    ...
    public function getCartList($cart_id, $member_id) {
        ...
        //购物车列表 [得到最新商品属性及促销信息]
        $cart_list = $this->_logic_buy_1->getGoodsCartList($cart_list,'',$member_id);
    ...
    public function getGoodsList($cart_id, $member_id, $store_id) {
        ...
        //商品信息[得到最新商品属性及促销信息]
        $goods_info = $this->_logic_buy_1->getGoodsOnlineInfo($goods_id,intval($quantity), $member_id);
        if(empty($goods_info)) {
...
//使用满减后，代金券是否能使用
    ...
    private function _createOrderStep2() {
        ...
        if ($post['ifcart']) {
            ...
            //购物车列表 [得到最新商品属性及促销信息]
            $cart_list = $this->_logic_buy_1->getGoodsCartList($cart_list,$this->_order_data['rule_id'],$this->_member_info['member_id']);
            ...
            //商品信息[得到最新商品属性及促销信息]
            $goods_info = $this->_logic_buy_1->getGoodsOnlineInfo($goods_id,intval($quantity),$this->_member_info['member_id'],$this->_order_data['rule_id']);
            if(empty($goods_info)) {
...
/**		
 ...
 // var_dump($this->_post_data['password']);die;
        ...
        //使用一卡通支付
        if(!empty($this->_post_data['card_pay'])){
            // $card_info=Model('member')->getMemberCardInfo($buyer_info['cardno'],'waterrun','HCCardBase');
