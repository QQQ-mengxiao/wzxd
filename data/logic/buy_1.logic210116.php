...
class buy_1Logic {
     ...
     */
    public function getGoodsCartList($cart_list,$rule_id) {
        ...
        //折扣
        $this->getCartDiscount($cart_list);
         //阶梯折扣
     ...
     * @return array
     */
    public function getGoodsOnlineInfo($goods_id,$quantity,$rule_id) {
    ...
    public function cardPay($order_list,$input,$buyer_info){
        ...
        foreach($order_list as $order_info){
            ...
            if($available_card_amount>=$order_amount){
                $model_pd->changeCard('order_pay',$data_card);
...
//			$goods_info=null;
    ...
    private function getCartDiscount(& $cart_list){
    ...
    public function getDiscount(& $goods_info = array()){
        $member_model = Model('member');
        $goods_model = Model('goods');
        $discount = $member_model->getDiscount($_SESSION['member_id']);
        //20181120 隐藏会员折扣价
         $is_vip_price = $goods_model->getGoodsCommonList(array('goods_commonid' => $goods_info['goods_commonid']))[0]['is_vip_price'];
         if ($is_vip_price == 1) {
             $goods_info['goods_price'] = ncPriceFormat($discount * $goods_info['goods_price']);
            $goods_info['goods_tax'] = ncPriceFormat($goods_info['goods_price'] * $goods_info['goods_tax_rate']);
         }
    }
    //阶梯价
        ...
        $goods_model = Model('goods');
        foreach ($cart_list as $k => $v) {
             $is_ladder = $goods_model->getGoodsCommonList(array('goods_commonid' => $v['goods_commonid']))[0]['is_ladder'];
            if ($is_ladder == 1) {
    ...
    public function getladder_price(& $goods_info = array(),$rule_id){
        $goods_model = Model('goods');
         $is_ladder = $goods_model->getGoodsCommonList(array('goods_commonid' => $goods_info['goods_commonid']))[0]['is_ladder'];
         if ($is_ladder == 1) {
              $model_mansong_rule = Model('p_ladder_rule');
              $discount = $model_mansong_rule->findladderByRuleID($rule_id);
