...
class orderLogic {
     ...
     * @return array
     */
    public function changeCrossborderOrderState($order_info, $role, $user = '', $msg = '', $if_update_account = true, $if_quque = true) {
        try {
            $model_order = Model('order');
            ...
            //更新订单信息
                            //更新订单中的“需求”
            $update_crossborder_order = array('crossborder_pay_state' => 1);
            $update = $model_order->editOrder($update_crossborder_order,array('order_id'=>$order_id));
            if (!$update) {
                throw new Exception('保存失败');
            }
     ...
     * @return array
     *
    public function changeCrossborderOrderHandState($order_info, $role, $user = '', $msg = '') {
        try {
            //
            ...
            $update_order['crossborder_pay_state'] = 1 ;

            $update = $model_order->editOrder($update_order,array('order_id'=>$order_id));

            //jinp07040905
            //$update = $model_order->editOrder(array（'crossborder_pay_state'=>1,'finnshed_time'=TIMESTAMP),array('order_id'=>$order_id));
            ...
            return callback(true,'操作成功jinp07021447');
        } catch (Exception $e) {
            return callback(false,'操作失败');
        }
    }
    ...
    /**
     * jinp07041034推单手动,上面的错误（copy 收到货款）
     * @param array $order_info
     * @param string $role 操作角色 buyer、seller、admin、system 分别代表买家、商家、管理员、系统
     * @param string $user 操作人
     * @return array
     */
    public function changeCrossborderOrderHandState($order_list, $role, $user = '', $post = array()) {
        $model_order = Model('order');
        ...
        try {
            ...
            $update = $model_order->editOrder($update_order,array('pay_sn'=>$order_info['pay_sn'],'crossborder_pay_state'=>0));
            if (!$update) {
        ...
        return callback(true,'操作成功07041041');
    }
    ...
    public function changeOrderStateReceive($order_info, $role, $user = '', $msg = '') {
        try {
            ...
            return callback(true,'操作成功jinp07021446');
        } catch (Exception $e) {
            return callback(false,'操作失败');
        }
    }
