...
class buyLogic {
     ...
     */
    public function buyStep1($cart_id, $ifcart, $member_id, $store_id, $time_lag) {

        //得到购买商品信息
        if ($ifcart) { //获取购物车中的商品信息，需要在此处加上gc_id_1商品一级分类id,以便于进行商品分类计算。
            $result = $this->getCartList($cart_id, $member_id, $time_lag);
        } else { //获取立即购买的商品信息，同上
            $result = $this->getGoodsList($cart_id, $member_id, $store_id, $time_lag);
        }
     ...
     */
    public function getCartList($cart_id,$member_id,$time_lag) {
        ...
        //购物车列表 [得到最新商品属性及促销信息]
        $cart_list = $this->_logic_buy_1->getGoodsCartList($cart_list,$time_lag,$member_id);
     ...
     */
    public function getGoodsList($cart_id, $member_id, $store_id, $time_lag) {
        ...
        //商品信息[得到最新商品属性及促销信息]
        $goods_info = $this->_logic_buy_1->getGoodsOnlineInfo($goods_id,intval($quantity), $member_id,$time_lag);
        if(empty($goods_info)) {
    ...
    public function getBuyStep1Data($member_id, $data) {
        ...
        // echo "<pre>";
        // print_r(current(array_keys($store_cart_list)));
        // echo "</pre>";
 ...
 //商品金额计算(分别对每类商品/优惠套装小计、每个店铺小计)  //xinzeng
        list($store_agc_total) = $this->_logic_buy_1->calcCartGCList($store_cart_list);
        $result['store_agc_total'] = $store_agc_total;

        //商品税金计算(分别对每个商品/优惠套装小计、每个店铺小计)
        list($store_cart_list,$store_goods_tax_total,$store_is_mode) = $this->_logic_buy_1->calcStoreTax($store_cart_list);
        $result['store_cart_list'] = $store_cart_list;
        $result['store_goods_tax_total'] = $store_goods_tax_total;
        $result['store_is_mode']    =    $store_is_mode;

        //取得店铺id
        $result['store_id'] = current(array_keys($store_cart_list));

        //取得店铺优惠 - 满即送(赠品列表，店铺满送规则列表)
             list($store_premiums_list,$store_mansong_rule_list) = $this->_logic_buy_1->getMansongRuleCartgcListByTotal($store_agc_total); //gai
        ...
        //重新计算优惠后(满即送)的店铺实际商品总金额
            $store_goods_total = $this->_logic_buy_1->reCalcGoodsTotal($store_goods_total,$store_mansong_rule_list,'mansong');//gai

        //计算商品总价格和分类总价格,单品、分类价格
        //list($store_class_total)=$this->_logic_buy_1->calcCartList1($store_cart_list);
	    list($store_class_total)=$this->_logic_buy_1->calcCartList2($store_cart_list);
        //返回店铺可用的代金券
        //使用满减后，代金券是否能使用
        $store_class_total=$this->_logic_buy_1->cartList($store_mansong_rule_list,$store_class_total);
        $store_voucher_list = $this->_logic_buy_1->getStoreAvailableVoucherListPro($store_class_total, $member_id);
        //$store_voucher_list = $this->_logic_buy_1->getStoreAvailableVoucherList($store_class_total, $member_id);
        ...
        //echo "<pre>";
        //print_r($freight_list);//jinp170606
        //echo "</pre>";
        //print_r($freight_list['store_goods_all_weight'][2]['store_goods_all_weight']); //jinp170606
        ...
        //一卡通支付
        // $card_info=Model('member')->getMemberCardInfo($buyer_info['cardno'],'waterrun','HCCardBase');
        ...
        //         $card_info['Balance'] = ncPriceFormat($card_info['Balance']);
        //         $card_info['cardopdate']=strtotime($card_info['cardopdate']);
        //         $result['cardInfo']=$card_info;
        //     }
        // }
    ...
    private function _createOrderStep1() {
        ...
        //自提点编号
        $input_ziti_id = intval($input_address_info['ziti_id']);
        //是否开增值税发票
        ...
        if (!empty($post['voucher']) && is_array($post['voucher'])) {
            foreach ($post['voucher'] as $store_id => $voucher) {
                $input_list=array();
                foreach($voucher as $key =>$voucher_list){
                    if (preg_match_all('/^(\d+)\|-?(\d+)\|([\d.]+)$/',$voucher_list,$matchs)) {
                        if (floatval($matchs[3][0]) > 0) {
                            $input_list[$key]['voucher_t_id'] = $matchs[1][0];
                            $input_list[$key]['voucher_price'] = $matchs[3][0];
                        }
                    }
                }
                $input_voucher_list[$store_id]=$input_list;
            }
        }
        ...
        $this->_order_data['order_from'] = $post['order_from'] == 2 ? 2 : 1;
         $this->_order_data['rule_id'] = $post['rule_id'];//折扣id
         $this->_order_data['input_ziti_id'] = $input_ziti_id;
          $this->_order_data['ziti_ladder_time'] = $post['ziti_ladder_time'];//折扣id
    ...
    private function _createOrderStep2() {
        ...
        if ($post['ifcart']) {
            ...
            //购物车列表 [得到最新商品属性及促销信息]
            $cart_list = $this->_logic_buy_1->getGoodsCartList($cart_list,$post['time_lag'],$this->_member_info['member_id']);
            //print_r($cart_list);
            //商品列表 [优惠套装子商品与普通商品同级罗列]
            $goods_list = $this->_getGoodsList($cart_list);
            ...
            foreach ($goods_list as $key => $value) {
               if($value['is_group_ladder']==5){
                    $jimai_ziti_id=$this->_order_data['input_address_info']['ziti_id'];
                    $buy_deliver_goods_id = intval($value['goods_id']);
                    $model_buy_deliver_goods = Model('buy_deliver_goods');
                    //根据goods_id+ziti_id查找即买即送商品
                    $buy_deliver_goods_info = $model_buy_deliver_goods->getBuyDeliverGoodsInfo(array('goods_id' => $buy_deliver_goods_id,'ziti_id'=>$jimai_ziti_id));
                    if (empty($buy_deliver_goods_info)) {
                        throw new Exception($value['goods_name'].'不在此自提点即买即送');
                    }
                    // $buy_deliver_goods_info = $model_buy_deliver_goods->getBuyDeliverGoodsInfoBygoodsID($buy_deliver_goods_id,$value['store_id']);
                    // if($jimai_ziti_id!=$buy_deliver_goods_info['ziti_id']){
                    //       throw new Exception($value['goods_name'].'不在此自提点即买即送');
                    //     // echo '2222';die;
            ...
            //商品信息[得到最新商品属性及促销信息]
            $goods_info = $this->_logic_buy_1->getGoodsOnlineInfo($goods_id,intval($quantity),$this->_member_info['member_id'],$post['time_lag']);
            if($goods_info['is_group_ladder']==5){
                    $jimai_ziti_id=$this->_order_data['input_address_info']['ziti_id'];
                    $buy_deliver_goods_id = intval($goods_info['goods_id']); 
                    $model_buy_deliver_goods = Model('buy_deliver_goods');
                    //根据goods_id+ziti_id查找即买即送商品
                    $buy_deliver_goods_info = $model_buy_deliver_goods->getBuyDeliverGoodsInfo(array('goods_id' => $buy_deliver_goods_id,'ziti_id'=>$jimai_ziti_id));
                    if (empty($buy_deliver_goods_info)) {
                    ...
                    // $buy_deliver_goods_info = $model_buy_deliver_goods->getBuyDeliverGoodsInfoBygoodsID($buy_deliver_goods_id,$goods_info['store_id']);
                    // if($jimai_ziti_id!=$buy_deliver_goods_info['ziti_id']){

                    //       throw new Exception($goods_info['goods_name'].'不在此自提点即买即送');
                    //        // return false;
                    // }
                }
            if(empty($goods_info)) {
    ...
    private function _createOrderStep3() {
        ...
        $input_ziti_id=$this->_order_data['input_ziti_id'] ;
        $rule_id=$this->_order_data['rule_id'] ;
         $ziti_ladder_time=$this->_order_data['ziti_ladder_time'];

        //商品金额计算(分别对每个商品/优惠套装小计、每个店铺小计)
        list($store_cart_list,$store_goods_total) = $this->_logic_buy_1->calcCartList($store_cart_list);
	list($store_agc_total) = $this->_logic_buy_1->calcCartGCList($store_cart_list);//xinzeng

       //取得店铺优惠 - 满即送(赠品列表，店铺满送规则列表)   //gai
        //list($store_premiums_list,$store_mansong_rule_list) = $this->_logic_buy_1->getMansongRuleCartListByTotal($store_goods_total);
        list($store_premiums_list,$store_mansong_rule_list) = $this->_logic_buy_1->getMansongRuleCartgcListByTotal($store_agc_total);
        ...
        //计算商品总价格和分类总价格,增加三级分类及商品价格
        //list($store_class_total)=$this->_logic_buy_1->calcCartList1($store_cart_list);
        list($store_class_total)=$this->_logic_buy_1->calcCartList2($store_cart_list);
        //得到有效的代金券
	 $store_class_total=$this->_logic_buy_1->cartList($store_mansong_rule_list,$store_class_total);
        $input_voucher_list = $this->_logic_buy_1->reParseVoucherList($input_voucher_list,$store_class_total,$this->_member_info['member_id']);
        ...
        //计算每个店铺税金
        list($store_cart_list,$store_tax_total) = $this->_logic_buy_1->calcStoreTax($store_cart_list);

        //echo $store_tax_total[2];
        //break;
        //计算店铺最终订单实际支付金额(加上运费)
        $store_final_order_total = $this->_logic_buy_1->reCalcGoodsTotal($store_final_goods_total,$store_freight_total,'freight');

        //计算店铺最终订单实际支付金额（加上税金）
        $store_final_order_total = $this->_logic_buy_1->reCalcGoodsTotal($store_final_order_total,$store_tax_total,'tax');
        //echo $store_final_order_total[2];
       // break;
        // print_r($store_tax_total);
        // break;
        ...
        //计算店铺分类佣金[改由任务计划]
         ...
         //新增-订单级满赠商品 - 满即送(赠品列表，店铺满送规则列表)  
        //list($store_premiums_list,$store_mansong_rule_list) = $this->_logic_buy_1->getMansongRuleCartListByTotal($store_goods_total);
        //list($store_premiums_list,$store_mansong_rule_list) = $this->_logic_buy_1->getMansongRuleCartgcListByTotal($store_agc_total);
        //print_r($store_agc_total);die;
        list($store_premiums_list,$store_mansong_rule_list) = $this->_logic_buy_1->getMansongRuleCartgcListByTotal1($store_final_order_total);
        ...
        // print_r($goods_buy_quantity);
        // print_r($input_voucher_list);die;
        // var_dump($store_goods_total);
        // var_dump($store_final_order_total);
        // var_dump($store_tax_total);
        // var_dump($store_freight_total);
        // var_dump($store_promotion_total);die;
        //保存数据
        $this->_order_data['store_goods_total'] = $store_goods_total;
        $this->_order_data['store_final_order_total'] = $store_final_order_total;
        $this->_order_data['store_tax_total'] = $store_tax_total;
    ...
    private function _createOrderStep4() {
        ...
        foreach ($store_cart_list as $store_id => $goods_list) {
                  ...
                  // add判断是否需要拆单
            foreach ($goods_list as $goods) {
               $array_type[]= $goods['storage_id'];
            } 
            $array_type=array_unique($array_type);//剔除重复
            ...
            // $order['pay_sn'] = $pay_sn;
            //add分单
            if($is_zongdan==1){//是否为分单1是
            $order['is_zorder'] =2;
             $order['z_order_sn'] =$order_sn;
             $order['storage_id'] = $goods_list[0]['storage_id'];
            ...
            //增加分享公司id、分享人id
            $order['share_id'] = $this->_post_data['share_id'];
            $order['company_id'] = $this->_post_data['company_id'];
            ...
            $order['store_tax_total'] = $store_tax_total[$store_id];
            $order['goods_amount'] = $order['order_amount'] - $order['shipping_fee'] - $store_tax_total[$store_id];
            $order['order_from'] = $order_from;
            $order['is_mode'] =  $goods_list[0]['is_mode'];
			//mx存入order_type
            $order['order_type'] =  $goods_list[0]['is_group_ladder'];
            if (isset($this->_post_data['freight'])) {
                $freight = $this->_post_data['freight'];
                $order['shipping_fee'] = $freight;
                $order['order_amount'] += $freight;
                $order['order_type'] = 5;
            }
			//如果支持方式为空时，默认为货到付款
...
/**		
  ...
  //更改用户是否为新用户状态
            ...
            if(!$member_info){
                throw new Exception('本系统无此用户!');
            }else{
                if($member_info['is_xinren'] == 1){
                    //若为新用户1，改为老用户2
                    $update_array=array();
                    $update_array['is_xinren'] = 2;
                     //echo $this->returnMsg(10001, $member_id, $update_array);exit;
            ...
            if (isset($input_voucher_list[$store_id])){
                list($receive_code) = $this->_logic_buy_1->getReciverAddr1($input_voucher_list[$store_id]);
                $order_common['voucher_code']=$receive_code;
            }
            $order_common['voucher_id'] = array_values($input_voucher_list[$store_id])[0]['voucher_id'];
            //记录使用的代金券的总价格
            $price=0;
            foreach($input_voucher_list[$store_id] as $key =>$voucherp)
            {
                $price+=$voucherp['voucher_price'];
            }
            $order_common['voucher_price']=$price;

            $order_common['reciver_info']= $reciver_info;
            $order_common['reciver_name'] = $reciver_name;
            $order_common['reciver_city_id'] = $input_city_id;
            //自提点id
            $order_common['reciver_ziti_id'] = $input_ziti_id;
            //即买即送，详细地址
            $order_common['mall_info'] = $input_address_info['mall_info'];
            //享受的折扣
             if ($rule_id>0) {
              $model_mansong_rule = Model('p_ladder_rule');
              $discount = $model_mansong_rule->findladderByRuleID($rule_id);
             $order_common['ladder_discount'] =$discount['discount'];
            }
           $order_common['ziti_ladder_time'] = $ziti_ladder_time;//阶梯折扣商品ziti时间
            //发票信息
            ...
            //生成order_goods订单商品数据
            $i = 0;
            //add拆单计算分摊优惠券金额，剔除秒杀和新人商品
            $voucher_t_info = Model()->table('voucher_template')->field('voucher_t_state,voucher_t_type')->where(array('voucher_t_id'=>$input_voucher_list[$store_id][1]['voucher_t_id']))->find();
            $voucher_type_info = Model()->table('voucher_type')->field('is_use,goods_id,goodsClass_id')->where(array('voucher_tid'=>$input_voucher_list[$store_id][1]['voucher_t_id']))->find();
            $voucher_t_info['is_use'] = $voucher_type_info['is_use'];
            $voucher_t_info['goods_id'] = $voucher_type_info['goods_id'];
            $voucher_t_info['goodsClass_id'] = $voucher_type_info['goodsClass_id'];
            //根据代金券类型分配
            if ($voucher_t_info['voucher_t_type'] == '0') {//是否判断全场不可用或可用？
                foreach ($goods_list as $goodsall) {
                    // if($goodsall['is_group_ladder']<3 || $goodsall['is_group_ladder']>4){
                     $model_xianshi = Model('p_xianshi');
                $xianshi_type = $model_xianshi->getXianshitype2($goodsall['goods_id'],$goodsall['is_group_ladder']);
                if($xianshi_type>1){
                        $p[]=$goodsall['goods_num']*$goodsall['goods_price'];
                        $fentan_goods[]=$goodsall;
                    }
                }
            }
            elseif ($voucher_t_info['voucher_t_type'] == '1') {//品类
                $class_total = 0;
                //字符集转数组
                $class_ids = explode(",",$voucher_t_info['goodsClass_id']);
                //1.可用
                if ($voucher_t_info['is_use'] == '1') {
                    foreach ($goods_list as $goodsall) {
                        // if($goodsall['is_group_ladder']<3 || $goodsall['is_group_ladder']>4){
                         $model_xianshi = Model('p_xianshi');
                $xianshi_type = $model_xianshi->getXianshitype2($goodsall['goods_id'],$goodsall['is_group_ladder']);
                if($xianshi_type>1){
                            if (in_array($goodsall['gc_id'],$class_ids)) {
                                $p[]=$goodsall['goods_num']*$goodsall['goods_price'];
                                $fentan_goods[]=$goodsall;
                            }
                        }
                ...
                if($voucher_t_info['is_use'] == '0') {//2.不可用
                    foreach ($goods_list as $goodsall) {
                        // if($goodsall['is_group_ladder']<3 || $goodsall['is_group_ladder']>4){
                           $model_xianshi = Model('p_xianshi');
                $xianshi_type = $model_xianshi->getXianshitype2($goodsall['goods_id'],$goodsall['is_group_ladder']);
            ...
            elseif ($voucher_t_info['voucher_t_type'] == '2') {
                $goods_total1 = 0;
                //字符集转数组
                $goods_ids = explode(",",$voucher_t_info['goods_id']);
                //var_dump('da');die;
                //1.可用
                if ($voucher_t_info['is_use'] == 1) {
                    foreach ($goods_list as $goodsall) {
                ...
                $xianshi_type = $model_xianshi->getXianshitype2($goodsall['goods_id'],$goodsall['is_group_ladder']);
                if($xianshi_type>1){
                            if (in_array($goodsall['goods_id'],$goods_ids)) {
                                $p[]=$goodsall['goods_num']*$goodsall['goods_price'];
                                $fentan_goods[]=$goodsall;
                            }
                        }
                    }
                }elseif ($voucher_t_info['is_use'] == 0) {//2.不可用
                    foreach ($goods_list as $goodsall) {
                        // if($goodsall['is_group_ladder']<3 || $goodsall['is_group_ladder']>4){
                         $model_xianshi = Model('p_xianshi');
                        $xianshi_type = $model_xianshi->getXianshitype2($goodsall['goods_id'],$goodsall['is_group_ladder']);
            ...
            $count=count($fentan_goods)-1; 
            $voucher_price=$price;
            $a1=0;
            foreach ($fentan_goods as $key => $value) {
               //优惠券优惠金额
                 //计算商品金额
                    $goods_total = $value['goods_price'] * $value['goods_num'];
                    
                    $rate=$goods_total/$all_price;
                    // $rate=floor($rate*100)/100;
                    if($a1==$count){
                      $fentan_goods[$key]['voucher_price']= $voucher_price-$all_voucher_price;
                     }else{
                         $fentan_goods[$key]['voucher_price']= floor($voucher_price*$rate*100)/100;
                     }
                 $all_voucher_price+= $fentan_goods[$key]['voucher_price'];
                 $a1++;
            }
            foreach ($goods_list as $goods_info) {
                ...
                if (!intval($goods_info['bl_id'])) {
                    ...
                    //增加额外送积分在order—goods表
                     $goodsid=$goods_info['goods_id'];
                     $points_send = Model()->query("SELECT points_send  FROM `718shop_goods` where goods_id=\"$goodsid\" ");
                      $order_goods[$i]['points_send'] =  $points_send[0]['points_send'];
                    $order_goods[$i]['goods_price'] = $goods_info['goods_price'];
                    $order_goods[$i]['goods_num'] = $goods_info['goods_num'];
                    $order_goods[$i]['goods_serial'] = Model()->table('goods')->getfby_goods_id($goods_info['goods_id'],'goods_serial');
                    $order_goods[$i]['goods_barcode'] = Model()->table('goods')->getfby_goods_id($goods_info['goods_id'],'goods_barcode');
                    $order_goods[$i]['deliverer_id'] = Model()->table('goods')->getfby_goods_id($goods_info['goods_id'],'deliverer_id');
                    $order_goods[$i]['is_cw'] = Model()->table('goods')->getfby_goods_id($goodsid,'is_cw');
                    $order_goods[$i]['commis_rate'] = Model()->table('goods')->getfby_goods_id($goodsid,'commis_rate');
                    ...
                    // }
                    $order_goods[$i]['goods_type'] = $goods_info['is_group_ladder'];
                    $order_goods[$i]['promotions_id'] = $goods_info['promotions_id'] ? $goods_info['promotions_id'] : 0;
					//商品成本价
                    // if($goods_info['is_group_ladder']==3) {
                    //     $order_goods[$i]['goods_cost_price'] = $goods_info['xinren_info']['goods_price'];
                    // }elseif ($goods_info['is_group_ladder']==4){
                    //     $order_goods[$i]['goods_cost_price'] = $goods_info['xianshi_info']['goods_price'];
                    // }elseif ($goods_info['is_group_ladder']==1){
                    //     $order_goods[$i]['goods_cost_price'] = Model()->table('goods')->getfby_goods_id($goods_info['goods_id'],'goods_price');
                    // }else{
                        $order_goods[$i]['goods_cost_price'] = $goods_info['goods_costprice'];
                    // }
                 ...
                 //    if($i==$count){
                 //         if($i==0){
                 //            $order_goods[$i]['voucher_price']= $voucher_price;
                 //         }else{
                 //          $order_goods[$i]['voucher_price']= $voucher_price-$all_voucher_price;
                 //         }
                 //     }else{
                 //          $order_goods[$i]['voucher_price']= floor($voucher_price*$rate*100)/100;
                 //     }
                 // $all_voucher_price+= $order_goods[$i]['voucher_price'];
                     //add分单计算
                   // if($goods_info['is_group_ladder']<3 || $goods_info['is_group_ladder']>4){
                      $model_xianshi = Model('p_xianshi');
                    $xianshi_type = $model_xianshi->getXianshitype2($goods_info['goods_id'],$goods_info['is_group_ladder']);
                   if($xianshi_type>1){
                            foreach ($fentan_goods as $fentan) {
                                if($fentan['goods_id']==$goods_info['goods_id']){
                                     $order_goods[$i]['voucher_price']=$fentan['voucher_price'];
                                }
                            }
                    }else{
                           $order_goods[$i]['voucher_price']=0;
                    }

                  $order_goods[$i]['goods_pay_price'] = $goods_total - $order_goods[$i]['voucher_price'];
                 //放入跨境信息，修复商家删除商品后，跨境信息丢失bug
                 $order_goods[$i]['kuajing_info'] = $this->_get_kuajing_info($goods_info['goods_id']);
                ...
                //end不是优惠套装    
                    ...
                    foreach ($goods_info['bl_goods_list'] as $bl_goods_info) {
                        ...
                        //增加额外送积分在order—goods表
                     $goodsid=$bl_goods_info['goods_id'];
                     $points_send = Model()->query("SELECT points_send  FROM `718shop_goods` where goods_id=\"$goodsid\" ");
                      $order_goods[$i]['points_send'] =  $points_send[0]['points_send'];
                        $order_goods[$i]['store_id'] = $store_id;
			...
			$order_goods[$i]['commis_rate'] = floatval($store_gc_id_commis_rate_list[$store_id][$goods_info['gc_id']]);
                    ...
                    $rate=$goods_total/$all_price;
                   //add分单计算
                   if($bl_goods_info['is_group_ladder']<3 || $bl_goods_info['is_group_ladder']>4){
                            foreach ($fentan_goods as $fentan) {
                                if($fentan['goods_id']==$goods_info['goods_id']){
                                     $order_goods[$i]['voucher_price']=$fentan['voucher_price'];
                                }
     ...
     * 跨境信息组合kuajing_info
     */
    private function _get_kuajing_info($goods_id) {

        //取跨境id 和shipper id
        $sql = 'SELECT goods_kuajingD_id,goods_shipper_id,goods_tax_rate,is_mode FROM `718shop_goods` where goods_id='.$goods_id.' LIMIT 1';
        ...
        $goods_tax_rate = $goods_kuajingD_idArr[0]['goods_tax_rate'];
        $is_mode = $goods_kuajingD_idArr[0]['is_mode'];
        ...
        //计量单位名称    unitname
        $unitname = $kuajingArr[0]['unit_name'];
        ...
        //海关商品备案编号    goodidguan
        $goodidguan = $kuajingArr[0]['record_no_guan'];

        //毛重    weigth   非必填
        $weight = $kuajingArr[0]['gross_weight'];

        //重量单位代码    weightunit   非必填
        $weightunit = $kuajingArr[0]['weight_unit'];

        //净重  --自定义
        $net_weight = $kuajingArr[0]['net_weight'];
        ...
        $kuajing_info = array(
                ...
                'ciqbarcode'=>$ciqbarcode,
                'goodidinsp'=>$goodidinsp,
                'goodidguan'=>$goodidguan,
                'weight'=>$weight,
                'weightunit'=>$weightunit,
                'net_weight'=>$net_weight,
                'packcategoryinsp'=>$packcategoryinsp,
                'senderusername'=>$senderusername,
                'senderuseraddress'=>$senderuseraddress,
                'senderusertelephone'=>$senderusertelephone,
                'goods_shipper_id'=>$goods_shipper_id,
                'country_origin_ID'=>$country_origin_ID,
                'country_trade_ID'=>$country_trade_ID
 ...
 // var_dump($this->_post_data['password']);die;
        //使用充值卡支付
        ...
        //使用一卡通支付
        // if(!empty($this->_post_data['card_pay']) && $this->_post_data['card_pay'] == 1){
        //     // $card_info=Model('member')->getMemberCardInfo($buyer_info['cardno'],'waterrun','HCCardBase');
        //     $cardno=Model()->table('member_card')->where(array('member_id'=>$this->_member_info['member_id']))->limit(1)->find();
        ...
        //     $this->_logic_buy_1->cardPay($order_list?$order_list:$this->_order_data['order_list'],$this->_post_data,$buyer_info);
        // }
    }
    ...
    private function _getGoodsList($cart_list) {
        ...
        foreach ($cart_list as $key => $cart) {
            ...
            if (!intval($cart['bl_id'])) {
                ...
                $goods_list[$i]['is_group_ladder'] =$cart['is_group_ladder'];
                //jinp170606
                $goods_list[$i]['goods_all_weight'] = $cart['goods_all_weight'];
                $i++;
            } else {
                //如果是优惠套装商品
