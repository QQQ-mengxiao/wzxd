...
class orderLogic {
    ...
    public function changeOrderStateCancel($order_info, $role, $user = '', $msg = '', $if_update_account = true, $if_quque = true) {
        try {
            ...
            if (!$update) {
             ...
             //181119 新添加 将待入账分销佣金退回
            $order_id = $order_info['order_id'];
            $field = 'order_id,buyer_id,order_sn,order_state';
            $model_order = Model('order');
    ...
    public function changeOrderStateReceive($order_info, $role, $user = '', $msg = '') {
        try {
            ...
            //获取表单
            $data_fenxiao = $model_fenxiao->getfenxiaoList();
            //成为分销商条件1
            if($data_fenxiao['fenxiao_con'] == 1){
                $data = array();
                $data['fenxiao_con_val'] = $data_fenxiao['fenxiao_con_val'];
                if ($order_info['order_amount'] >= $data['fenxiao_con_val']) {
                    $model_member->is_fenxiao($member_id);//例如这个类有个方法叫php()，调用这个方法    
                }
             }else{
                //成为分销商条件2
                //反序列化商品commonID
                $condition1 = $data_fenxiao['fenxiao_con_val'];
                $temp = unserialize($condition1);
                $data_goods = $model_order->getOrderGoodsInfo($order_id);
                $condition = $data_goods['goods_id'];
                $data_goods_com = $model_goods->getGoodsInfo($condition);
                $data_goods_commonid = array();
                $data_goods_commonid = $data_goods_com['goods_commonid'];
                $isin = in_array($data_goods_commonid,$temp);
                //var_dump($isin);die;
                if($isin){
                   $model_member->is_fenxiao($member_id);//
                }
            }
            //将分销佣金给各分销商账户
            ...
            $order_state = $order['order_state'];
            if($order_state == 40){
                    $sql = "SELECT a.id,a.member_id,a.commission,a.real_pay,a.relation,b.fx_before_account,b.fx_in_account,b.fx_settled_account from 718shop_distribute_commission as a inner join 718shop_member as b on a.member_id=b.member_id where a.order_sn=".$order['order_sn']." order by a.relation asc";
                    $tree_info = Model()->query($sql);
                    if($tree_info){
                        foreach($tree_info as $v){
                            $update = [];
                            $update['fx_before_account'] = $v['fx_before_account'] - $v['commission'];
                            $update['fx_in_account'] = $v['fx_in_account'] + $v['commission'];
                            $update['fx_settled_account'] = $v['fx_settled_account'] + $v['commission'];
                            Model()->table('member')->where(['member_id' => $v['member_id']])->update($update);
                            Model()->table('distribute_commission')->where(['id' => $v['id']])->update(['commission_state' => 2, 'update_time' => time()]);
                        }
                    }
                }else{
                    return callback(false,'操作失败');
