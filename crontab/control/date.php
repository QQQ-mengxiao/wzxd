...
class dateControl extends BaseCronControl {
    ...
    /**
     * 订单结束后可评论时间，20天，60*60*24*20
     * @var int
     */
    const ORDER_EVALUATE_TIME = 1728000;

    /**
     * 订单结束后自动好评时间，20天，60*60*24*20
     * @var int
     */
    const ORDER_EVALUATE_AUTO = 1728000;
...
//         $this->_add_points();
        ...
        //**********发货订单7天自动确认收货,25->40**********//
        $this->_order_auto_receive();
        //**********发货订单7天自动确认收货**********//
        
        sleep(10);

        //**********发货订单7天自动确认收货数据发送云仓**********//
        $this->_cwOrderCompleteNewPost();
        //**********发货订单7天自动确认收货数据发送云仓**********//
    }
     ...
     * 实物订单超期自动好评
     */
    private function _order_eval_auto() {
        //实物订单超期自动好评，每次最多更新1000个订单
        $model_order = Model('order');
        ...
        $condition['finnshed_time'] = array('lt',TIMESTAMP - self::ORDER_EVALUATE_AUTO);
        //----------------------------
        //获取订单信息
        $order_list_auto = $model_order->getOrderList($condition);        
        foreach ($order_list_auto as $order_value){
            //获取订单商品
            $order_goods = $model_order->getOrderGoodsList(array('order_id'=>$order_value['order_id']));
            ...
            foreach ($order_goods as $value){
                ...
                $evaluate_goods_info['geval_orderid'] = $order_value['order_id'];
                $evaluate_goods_info['geval_orderno'] = $order_value['order_sn'];
                ...
                $evaluate_goods_info['geval_isanonymous'] = 1;  //1表示匿名评价
                $evaluate_goods_info['geval_addtime'] = TIMESTAMP;
                $evaluate_goods_info['geval_storeid'] = $order_value['store_id'];
                $evaluate_goods_info['geval_storename'] = $order_value['store_name'];
                $evaluate_goods_info['geval_frommemberid'] = $order_value['buyer_id'];
                $evaluate_goods_info['geval_frommembername'] = $order_value['buyer_name'];
            ...
            //更新 evaluate_store表  start
            $store_desccredit= 5;
            $store_servicecredit = 5;
            $store_deliverycredit = 5;
            //添加店铺评价
            $store_info = $model_store->getStoreInfoByID($order_value['store_id']);
            if (!$store_info['is_own_shop']) {
                $evaluate_store_info = array();
                $evaluate_store_info['seval_orderid'] = $order_value['order_id'];
                $evaluate_store_info['seval_orderno'] = $order_value['order_sn'];
                $evaluate_store_info['seval_addtime'] = time();
                $evaluate_store_info['seval_storeid'] = $order_value['store_id'];
                $evaluate_store_info['seval_storename'] = $order_value['store_name'];
                $evaluate_store_info['seval_memberid'] = $order_value['buyer_id'];
                $evaluate_store_info['seval_membername'] = $order_value['buyer_name'];
            ...
            //更新 evaluate_store表  end

        }//end order_list_auto foreach
        

        //写到上面循环中
        $update = array();
        $update['evaluation_state'] = 3;  //自动好评
	...
	private function _cwOrderCompleteNew(){
		$model_cw = Model('cw');
        $order_list = Model('order')->getOrderList(array('order_id'=>array('gt',0),'order_state'=>40,'by_post'=>1,'finnshed_time'=>array('between',[TIMESTAMP-432000,TIMESTAMP-259200]),'is_cw_completed'=>0,'is_zorder'=>array('neq',0)),'','order_id,order_sn,order_amount,refund_amount');
        // $order_list = Model('order')->getOrderList(array('order_id'=>454714),'','order_id,order_sn,order_amount,refund_amount');
    ...
    /***
     * 邮寄订单7天自动确认收货（完成）25->40
     */
    private function _order_auto_receive()
    {
        $_break = false;
        $model_order = Model('order');
        $model_cw = Model('cw');
        ...
        for ($i = 0; $i < 500; $i++) {
            ...
            if (empty($order_list)) break;
            foreach ($order_list as $order_info) {
                $result = $logic_order->changeOrderStateReceive($order_info, 'system', '系统', '超期未收货系统自动完成订单(邮寄)');
                if (!$result['state']) {
                    $this->log('实物订单超期未收货自动完成订单失败(邮寄)SN:' . $order_info['order_sn']);
                    $_break = true;
                    break;
                }
                /**********小店收货同步云仓**********/
     ...
     * 邮寄订单，直接推40状态的订单
     */
	private function _cwOrderCompleteNewPost(){
		$model_cw = Model('cw');
        $order_list = Model('order')->getOrderList(array('order_id'=>array('gt',0),'order_state'=>40,'is_cw_completed'=>0,'by_post'=>2,'is_zorder'=>array('neq',0)),'','order_id,order_sn,order_amount,refund_amount');
        if($order_list){
            foreach ($order_list as $k=>$v){
                $order_info = Model('order')->getOrderInfo(array('order_id' => $v['order_id']), [], 'order_sn,order_amount,refund_amount,ruku_time');
                //查询是否有未处理的退款单
