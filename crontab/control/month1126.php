...
class monthControl extends BaseCronControl {
    ...
    private function _real_order() {
        ...
        while (($time = strtotime('-'.$i.' month',$current_time)) >= $start_unixtime) {
            ...
            $fileds = 'sum(ob_order_totals) as ob_order_totals,sum(ob_shipping_totals) as ob_shipping_totals,
                    ...
                    sum(ob_store_cost_totals) as ob_store_cost_totals,sum(ob_result_totals) as ob_result_totals,sum(ob_commission_totals) as ob_commission_totals';//181119新添加总分销佣金ob_commission_totals
    ...
    //181119新添加月出账单中总分销金额ob_commission_totals
            $data[$key]['os_commission_totals'] = floatval($order_bill_info['ob_commission_totals']);
    ...
    private function _calc_real_order_bill($data_bill){
        ...
        $model_bill = Model('bill');
        $model_store = Model('store');
        $model_commission = Model('commission');//181119新添加计算分销佣金的模型
        ...
        $order_condition['store_id'] = $data_bill['ob_store_id'];//181119新添加店铺id
        ...
        //181119 新添加 分销佣金金额
        $order_condition1 = array();
        $order_condition1['order_state'] = ORDER_STATE_SUCCESS;
        $order_condition1['store_id'] = $data_bill['ob_store_id'];
        $order_condition1['finnshed_time'] = array('between',"{$data_bill['ob_start_date']},{$data_bill['ob_end_date']}");

        $order_info = $model_order->getOrderInfo($order_condition1,array(),'count(DISTINCT order_id) as count');
        $order_count = $order_info['count'];
        //分批计算分销佣金，最后取总和
        for ($i = 0; $i <= $order_count; $i = $i + 300){
            $order_list = $model_commission->getPdRechargeList(array('update_time'=>array('between',"{$data_bill['ob_start_date']},{$data_bill['ob_end_date']}"),'store_id'=>$order_condition1['store_id']),'','order_sn','',"{$i},300");
            $order_sn_array = array();
            foreach ($order_list as $order_info) {
                //5$result[$n]['属性']=$child->where('条件')->select(); 
                $order_sn_array[] = $order_info['order_sn'];
            }
            if (!empty($order_sn_array)){
                $distribute_commission_condition = array();
                $distribute_commission_condition['order_sn'] = array('in',$order_sn_array);
                $distribute_commission_condition['commission_state'] == 2;
                // if ($distribute_commission_condition['commission_state'] == 2) {
                    // $field = 'SUM(ROUND(commission,2)) as commission_amount';
                $order_goods_info = $model_commission->getWdCashInfo($distribute_commission_condition);
                $commission_totals += $order_goods_info['commission'];
                // }
            }else{
                $commission_totals = 0;
            }
        ...
        $update['ob_commission_totals'] = $commission_totals;//floatval(array_sum($commission_totals_array));
        ...
        $update['ob_commission_totals'];//181119 新添加分销佣金总金额ob_commission_totals
...
/*        public function calc_commission(){
        ...
        $con = array();
        $con['store_id'] = 16;
        $orderlist = $model_order->getOrderList($con);
        foreach ($orderlist as $k => $v) {
            $order_id = $v['order_id'];
            $ordergoods_list = $model_order->getOrderGoodsList(array('order_id'=>$order_id));
            $commission = 0;
            foreach ($ordergoods_list as $key=>$value) {
                $goods_id = $value['goods_id'];
                $goods_commonid = $model_goods->getfby_goods_id($goods_id,'goods_commonid');
                $goods_fx_percent = unserialize($model_goods->table('goods_common')->getfby_goods_commonid($goods_commonid,'goods_fx_percent'));
                $commission += $value['goods_price'] * $goods_fx_percent[$relation-1];
            }
            $condition = array();
            $condition['member_id'] = $v['buyer_id'];
            $condition['order_sn'] = $v['order_sn'];
            $condition['store_id'] = $v['store_id'];
            $condition['real_pay'] = $v['order_amount'];
            $condition['relation'] = $relation;
            $condition['commission'] = $commission;
            $condition['subline_name'] = Model('member')->getfby_member_id($v['buyer_id'],'member_name');
            $condition['create_time'] = time();
            $condition1[$k] = $condition;
        }
var_dump($condition1);
        $reslut = Model()->table('distribute_commission')->insertAll($condition1);
        die;
    }*/
}